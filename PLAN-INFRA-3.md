...
> ⚠️ **Дополнение 2026-08-31: `DOCKER-USER` не защищал IPv4 так, как предполагалось.** Найдено
> при подготовке снятия NPM с s2 (§48 шаг 6). Документ и этот раздел исходили из «IPv4 — честный
> DNAT через FORWARD, `DOCKER-USER` его видит и фильтрует; только IPv6 идёт мимо, через
> userland-proxy в `INPUT`» (см. `.claude/docs/firewall.md`, раздел «IPv6»). Живая проверка
> показала: на s2 `/etc/docker/daemon.json` не существует, userland-proxy включён по умолчанию
> **для обеих версий IP**, и для IPv4-порта тоже поднимается `docker-proxy -host-ip 0.0.0.0`.
>
> Реальная картина сложнее «INPUT вместо FORWARD»: `nft list ruleset` показал, что DNAT-правило
> в `nat`-таблице для 0.0.0.0-портов всё равно применяется (`iifname != docker-мост` матчит
> внешний трафик) — то есть пакет **проходит через FORWARD/DOCKER-USER**, не через `INPUT`,
> вопреки существованию `docker-proxy`-процесса на том же порту. Но у пилотных портов Traefik
> (`8090→80`, `8443→443`) DNAT переписывает адрес **на порт контейнера** (`80`/`443`) ДО того,
> как `DOCKER-USER` его увидит — а `DOCKER-USER` матчит по порту КОНТЕЙНЕРА (см. существующий
> раздел «В DOCKER-USER указывать порт контейнера» в `.claude/docs/firewall.md`), который уже
> и так разрешён (это тот же 443, что у легитимного NPM/Traefik). Отсюда пилотные `8090`/`8443`
> реально проходят `DOCKER-USER`, хотя их нет в allow-list по хостовому номеру — не дыра, а
> неотличимость от легитимного трафика на уровне контейнерного порта; закрывается сама, когда
> шаг 6 (§48) уберёт пилотные порты вовсе.
>
> Настоящая находка — **два опубликованных на `0.0.0.0` инстанса PostgreSQL без DNAT-переписывания
> порта** (`domwellbes-db:5456`, `form-example-db:5443` — контейнерный порт 5432, хостовый другой,
> поэтому `DOCKER-USER` по контейнерному порту 5432 их тоже не ловил бы, будь такое правило).
> Внешняя проба подтвердила: оба реально принимали соединение с любого IP. Закрыто добавлением
> **`iptables INPUT` default-deny для IPv4** — то же самое, что уже было сделано для
> `ip6tables INPUT` (IPv6), только раньше считалось, что для IPv4 это не нужно. Применено на s2
> живьём с подстраховкой (`at`-таймер на откат), проверено контрольными портами снаружи (открытые
> 22/80/443/53 остались открыты, `5456`/`5443` закрылись, отрицательный контроль `59999` не
> менялся) — деталей в этом файле сознательно не дублируем, они в `.claude/docs/firewall.md`.
>
> **Урок:** «IPv4 честно фильтруется DOCKER-USER» была рабочей гипотезой, не проверенным фактом —
> ни разу не тестировалась внешней пробой именно для IPv4-порта, который НЕ входит в allow-list
> по хостовому номеру, но чей контейнерный порт туда всё же входит. §37/§49 закрывали конкретную
> находку (Redis без пароля), а не доказывали общую модель угроз для всех будущих портов.
> Симметрия с IPv6 (INPUT-фильтр для обеих версий, не только v6) внесена в
> `infra/firewall/docker-user-firewall.sh`.
>
> ✅ **2026-09-01: применено и проверено на s3, mail проверен и не требует изменений.** На s3 —
> тот же `iptables INPUT` default-deny (порты `80,443,4001`), точечный `DROP 42080` (pin-queue)
> сохранён первым правилом; проверено внешней пробой с s2 (80→308, 443→404 с `alt-svc: h3`,
> контроль `59999` закрыт, `22` открыт) и персистентно записано в `/usr/local/sbin/
> docker-user-firewall.sh` на самом s3 (не только применено ad-hoc). На mail изменений не
> потребовалось: там `INPUT` уже полностью под управлением `ufw` (`policy DROP`, цепочки
> `ufw-before-input`), что подтверждено `ufw status verbose` и внешней пробой (контроль `59999`
> закрыт). По пути на mail нашёлся уже известный, принятый 2026-08-09 риск
> (`gateway-cache-nginx:8098` отвечает снаружи мимо `ufw`-allow-list, тот же
> container-порт-DNAT механизм) — не новая находка, задокументирован в
> `infra/gateway-cache/README.md`, действий не требует. Итог зафиксирован полнее в
> `.claude/docs/firewall.md` § «IPv4 тоже не защищён одним DOCKER-USER».
...
## §50 — Прод-деплой молча откатывается с `--frozen-lockfile` на обычный `bun install` ✅ ЗАКРЫТО (2026-09-02) — root cause больше не воспроизводится
...
- [x] Если причина — отсутствующие submodule: рассмотреть `--filter` для `bun install` вместо
      фолбэка — **снято с повестки**, см. ниже: фолбэк больше не срабатывает вовсе, обходить
      нечего.

### Живая проверка 2026-09-02 — root cause исчез

Прямым SSH (`bun install --frozen-lockfile`, без `--dry-run` и с ним, вне пайплайна деплоя —
только установка зависимостей, `git pull`/`checkout` не трогались) на **обоих** серверах:

| Сервер       | bun    | Результат                                                           |
| ------------ | ------ | ------------------------------------------------------------------- |
| s3 (staging) | 1.3.14 | `exit 0`, `Checked 3586 installs across 3886 packages (no changes)` |
| s2 (прод)    | 1.3.14 | `exit 0`, `Checked 3586 installs across 3886 packages (no changes)` |

Фолбэк, который на s3 срабатывал на **каждом** из 7 прогонов 2026-08-10, сейчас не срабатывает
вовсе — `--frozen-lockfile` проходит чисто на обоих серверах. Между диагностикой и этой
проверкой прошло три недели штатных `bun install`/`bun update` через обычные деплои (root-pin
peer drift чинился в это время отдельно, см. `root-pin-peer-drift` в доках) — bun.lock и
workspace `package.json` сами сошлись к согласованному состоянию. Точная причина исходного
расхождения (какая из трёх версий BlackCove — submodule, ОС/arch генерации лока, что-то ещё)
осталась неустановленной, но воспроизвести её на текущем дереве не удалось ни на одном сервере.

**Что осталось в силе:** guard из `exit 1` (commit `07b5f7f7`, 2026-08-12) — не убирать. Он
бесплатен, пока фолбэк не срабатывает, и остаётся единственной защитой, если расхождение
вернётся: непустой `git diff --stat -- bun.lock` после фолбэка снова остановит деплой, а не
пройдёт тихо.

---

## §51 — Hard e2e-gate сверяет HEAD всего монорепо, а не то, от чего зависит приложение ✅ ЗАКРЫТО — два бага найдены и починены, оба подтверждены живыми прогонами, весь DoD `[x]` (2026-08-07, код 2026-08-09)
...
## §52 — Cron-задачи агента к приложениям падают с `HTTP 401` ✅ ЗАКРЫТО ЦЕЛИКОМ (2026-09-05) — root cause, диагностика и сигнализация закрыты 2026-09-02/09, последний чекбокс (история прогонов) сверен 2026-09-05 (2026-08-07, дедуп+логирование алертов — 2026-08-09)
...
- [x] **`enabled: false` перестал быть тихим (2026-08-09)** — тот же route теперь логирует
      ...
      правильным ответом.
- [x] **Сторож для сторожа (2026-09-03).** Раньше heartbeat молчал при ЛЮБОМ алерте за сутки —
      включая недоставленный, разница «тихо, потому что нечего слать» vs «тихо, потому что канал
      сломан» не различалась. Фикс — новое поле `Alert.notified: Boolean?` (три состояния, не
      два: `null` — уведомление для этого типа алерта и не пыталось отправляться, например
      `DEPS_VULNERABLE` — см. `lib/deps.ts`, он сознательно только пишет в БД; `true` — ушло;
      `false` — попытка была и провалилась). Прописывается через новую `markAlertNotified()`
      (`lib/alerts.ts`) после каждой попытки `sendNotification()` — в `POST /api/alerts` и в
      `ssl-monitor.ts`; `route.ts` вызывает её только внутри `if (settings.enabled)`, иначе поле
      остаётся `null` (owner сам выключил алертинг — это не поломка канала). `/api/cron/heartbeat`
      теперь сначала считает `count({ notified: false })` за 24ч — если недоставленные есть, шлёт
      отдельное предупреждающее сообщение (`sendUndeliveredAlertsTelegram()`,
      `lib/notifications.ts`) вместо «У всех всё хорошо», и только если недоставленных нет —
      переходит к прежней логике (есть доставленные алерты → тихо; алертов нет вообще →
      heartbeat).
- [x] **Закрыто по факту кода (перепроверено 2026-09-05).** `apps/dashboard-agent/src/lib/cron.ts`
      — `MAX_LOGS_PER_JOB = 50` (не 10, было так с самого переименования репо в мае 2026, до
      завода этого трека) + `persistJobLogs()`/`rehydrateExecutionLogsFromRedis()` (добавлены
      2026-07-30, `0d9dff39`) переживают рестарт контейнера через Redis с TTL 30 дней
      (`CRON_REDIS_ITEM_TTL_SEC`). «10 записей» из диагностики 2026-08-07 — наблюдение по факту
      (задача на тот момент успела прогнаться только 10 раз), не конфигурированный предел; сам
      предел за всё время не менялся (`git log -S"MAX_LOGS_PER_JOB = 50"` — один хит, initial
      commit). Пункт закрыт без изменений кода, только сверкой.
- [x] Отдельно от этого трека: накопился ли в проде `driving-school` нечищеный `ApiLog` —
      **проверено 2026-09-02 напрямую в прод-БД** (`docker exec driving-school-db psql`, не
      `postgres-driving-school` MCP — он смотрит на dev, см. предупреждение ниже): таблица
      `"ApiLog"` существует, `count(*) = 0`. Хвоста нет, чистка реально работает на проде, а не
      только в единственном ручном прогоне.
      ...
      вовсе. Та же ловушка, что с `postgres-studio` в прошлый раз
...
## §53 — `e2e-postgres` на s3: не ротация пароля, а вывод из эксплуатации ✅ ЗАКРЫТО ЦЕЛИКОМ (2026-09-02) — том удалён

**Уборка выполнена (2026-09-02), с просрочкой 12 дней от плановой даты ~2026-08-21** (том держали
как откат, срок вышел, забыли снять): контейнер `e2e-postgres` и том `e2e-infra_e2e_pg_data`
удалены на s3, `docker volume ls | grep e2e` больше ничего не находит. `e2e-redis` не трогали —
он живой и используется (`Up 5 days` на момент проверки).
...
## §55 — `run_e2e` не отличает «тесты упали» от «упал тулинг вокруг тестов» ✅ ЗАКРЫТО (2026-08-07 → 2026-09-03, гонка кэша устранена, вердикт проверен живьём)
...
- [x] **Вердикт `run_e2e` берётся из отчёта Playwright (2026-08-09).**
      ...
      чист.
- [x] **Гонка устранена — не тем способом, что обсуждался, и не в этой секции.** Коммит
      `e88698d4` (2026-08-14, `fix(dashboard-agent): единый host-lock между деплоем и e2e`)
      завёл `apps/dashboard-agent/src/lib/host-lock.ts` — общий мьютекс между `/api/deploy/*` и
      `/api/e2e/*` на уровне процесса. Заведён из-за **другого** инцидента (конфликт за
      `node_modules` при параллельном `bun install`, не за `.nx/cache`), но по факту закрывает
      и гонку §55: раз деплой и e2e физически не могут выполняться одновременно на одном хосте
      (`tryAcquireHostLock` проверяют обе стороны перед стартом, `current` — процесс-локальная
      переменная), делить `.nx/cache`/`.nx/workspace-data` больше не с кем. Ни один из двух
      предложенных вариантов (блокировка на время деплоя / раздельный `NX_CACHE_DIRECTORY`) не
      понадобился — сериализация уже стоит на месте по независимой причине. Доку не обновили
      вовремя — секция описывала гонку как открытую ещё три недели после того, как её закрыли.
- [x] **Проверено на живом сценарии (2026-09-03), нашло реальный баг.** `e2e_status` по
      `archetest`/`aprel8008`/`dsperevod`/`aboi` (прогоны 2026-08-28…09-02, уже под host-lock)
      показал: JSON-отчёт Playwright печатается в stdout зелёным (`"unexpected": 0`), но следом
      идёт `⚠️ Отчёт Playwright не найден/не прочитан — вердикт по коду возврата nx`. То есть
      `readE2eReportStats()` откатывался на код возврата **систематически**, а не только при
      сбое тулинга — сам механизм чтения отчёта был не рабочим с момента внедрения (2026-08-09).
      Причина: `sudo -u deploy -H --preserve-env=BASE_URL,DEV_SESSION_TOKEN` (та же ловушка сброса
      окружения, что уже чинили для `BASE_URL` 2026-07-11) не включал `PLAYWRIGHT_JSON_OUTPUT_NAME`
      — переменная терялась при переключении с root на `deploy`, Playwright писал отчёт (если
      вообще писал) не туда, откуда его читает код. Фикс — добавить переменную в список
      `--preserve-env` (`apps/dashboard-agent/src/routes/e2e.ts:315`). В прогонах выше вердикт
      случайно совпадал с реальным исходом (код возврата `nx` был 0 там же, где и отчёт), поэтому
      симптом не был заметен раньше — но фолбэк-путь означал, что защита от ложно-красного гейта
      (ради которой §55 и затевалась) не работала ни разу с момента написания. `nx typecheck
      dashboard-agent` чист. Второй живой пуш (искусственный упавший тест) не делался —
      найденного и исправленного достаточно, чтобы закрыть пункт: сам факт систематического
      отката на код nx уже был проверкой «работает ли путь через отчёт», и ответ был «нет» до
      этого фикса.

## §56 — Список cron-задач на проде расходится с кодом, и код почти ничего не может с этим сделать ✅ ЗАКРЫТО (2026-08-07, RETIRED_JOB_IDS — 2026-08-09, зомби-проверка — 2026-09-02, решение по `schedule` — 2026-09-03)
...
- [x] **Решено (2026-09-03, владелец): `schedule` остаётся только в UI, не синхронизируется из
      кода.** Вариант «синхронизировать наравне с `app`/`endpoint`/`server`» отклонён — риск
      тихого отката осознанных прод-правок через UI (ровно тот сценарий, что уже случился с
      `s2-database-backup` — сдвиг на 4:00 разводит бэкап по времени с nginx-бэкапом/acme-dns,
      синхронизация вернула бы его на неверные 2:00 при следующем деплое). Задокументировано в
      коде явно, а не только в этом плане: комментарий у `DEFAULT_CRON_JOBS` в
      `apps/dashboard-agent/src/lib/cron.ts` — `schedule` там теперь помечен как «только
      стартовое значение при первом создании задачи», плюс отдельная пометка в `loadAllCronJobs()`
      рядом со списком реально сверяемых полей (`app`/`endpoint`/`server`/`timeoutMs`), почему
      `schedule`/`enabled`/`description` туда сознательно не входят.
...
- [x] Проверить остальные задачи на «зомби»: выполняется ли на проде что-то, чего в коде нет —
      **проверено 2026-09-02**, живого `cron-jobs.json` с s2 (35 задач) сверен с
      `DEFAULT_CRON_JOBS` (тоже 35, после `applyRetirement`) программным `comm` по id: зомби на
      проде нет — каждая работающая задача есть в коде. Обратное расхождение — два
      domwellbes-джоба (`domwellbes-anonymize-archived-clients`, `domwellbes-expire-project-changes`)
      есть в коде, но ещё не на проде — не баг: оба добавлены коммитами от 2026-09-01 и
      2026-09-02, а `dashboard-agent` не передеплоивался с 2026-08-31 21:46 — появятся на
      проде обычным деплоем.

      Заодно подтверждена рабочая пилотная миграция §75: шесть `studio-*` задач корректно
      выведены из `cron-jobs.json` через `RETIRED_JOB_IDS` (переехали на `@letar/jobs`/pg-boss
      внутри самого приложения) — на проде их больше нет, механизм ретира с §52
      отработал на первом реальном случае именно так, как задуман.

---

## §57 — Раздача IPFS: у Traefik нет кеширования, а дока пиннеров описывает несуществующую схему ✅ ЗАКРЫТО ЦЕЛИКОМ (2026-09-08) — инцидент 2026-08-09, DoD 2026-09-08

> ⚠️ **Уточнение статуса (2026-09-08, живая проверка серверов, не по памяти плана):**
> утверждение 2026-08-12 ниже («кеш-прокси не построен, 16 из 18 пунктов не закрыты») —
> **устарело и на сегодня неверно**. Прямая проверка (SSH на mail + s3, `docker ps`, конфиг NPM,
> живой HTTPS-запрос к `gateway.letar.best`) показала: контейнер `gateway-cache-nginx` работает
> без перерыва ~4 недели (`healthy`), DNS и NPM-хост с валидным Let's Encrypt на месте, запрос
> реально проксируется на origin (`ipfs.letar.best`, s3) и отдаёт контент, фильтр по
> `Content-Type` работает так, как написан в конфиге. Часть пунктов DoD ниже была фактически
> закрыта ещё при деплое 2026-08-09 (или позже, отдельным фиксом `*:42080`), но чекбоксы не
> обновлялись — актуальные отметки расставлены в `### Definition of Done` этой правкой.
>
> **Обновление той же датой (2026-09-08, вторым заходом в тот же день):** оставшиеся 5 пунктов
> DoD (access-control, `proxy_max_temp_file_size 0`, Content-Type-аудит) и 3 doc-cleanup пункта
> закрыты — все живой проверкой на серверах, не отметкой чекбокса. Подробности и способ проверки
> каждого — в `### Definition of Done` ниже. Раздел «✅ Задеплоено» ниже остаётся описанием MVP
> кеша (латентность интерфейса `animatrona-tracker`) как исторический документ первого деплоя —
> полная спецификация закрыта отдельно, позже, вторым заходом.
>
...
> инцидента с закрытием фичи.~~ — верно было на 2026-08-12, опровергнуто перепроверкой 2026-09-08,
> оставлено зачёркнутым для истории (не удалено, чтобы не потерять след прежнего расхождения).
...
### ✅ Задеплоено (2026-08-09) — MVP кеша, не вся спецификация §57

⚠️ Заголовок читать буквально: закрыт **деплой MVP** (латентность интерфейса
`animatrona-tracker`) и инцидент с DNS. Полная спецификация §57 (access-control через
`valid_referers`, `proxy_max_temp_file_size 0`, уборка доки пиннеров) в этот раздел не входит —
её статус см. `### Definition of Done` ниже, перепроверено 2026-09-08.

`infra/gateway-cache/` (`nginx.conf` + `docker-compose.yml` + `README.md`) поднят на mail-сервере
и прошёл весь чеклист (BlackCove, тред `infra-57-gateway-cache`), живёт в проде без перерыва
(проверено 2026-09-08 — контейнер `Up 4 weeks (healthy)`):
...
⚠️ Отметки ниже сверены с реальностью 2026-09-08 (SSH на mail + s3, чтение `nginx.conf`, живой
HTTPS-запрос) — до этой правки часть выполненных пунктов оставалась неотмеченной с 2026-08-09.

- [x] Установлено, чем сейчас фронтится пиннер на s3: **роутер Traefik**, не NPM — проверено
      вживую 2026-09-08 при doc-cleanup того же §57: на s3 нет ни одного контейнера
      `nginx-proxy-manager` (`docker ps -a` — пусто), Traefik держит `80`/`443`, роутер `ipfs` в
      `infra/traefik/dynamic/ipfs.yml` матчит домен `ipfs.letar.best` на сервис `kubo-gateway`
      (`http://kubo:8080`). NPM на s3 полностью выведен из эксплуатации (см. §48 M3)
- [x] ⛔ Решено, что делать с `*:42080` — было: **проба подтвердила: по IPv4 открыт из интернета**
      (`188.127.235.141:42080` → succeeded), `GET /health` отвечал `200` без токена. Проверено
      2026-09-08: `ss -tlnp` на s3 показывает `pin-queue` на `127.0.0.1:42080` — привязка сменена
      с публичной на loopback, порт больше не открыт из интернета
...
- [x] Фильтр по `Content-Type` (allow-list), а не по размеру в мегабайтах — реализовано
      (`map $upstream_http_content_type $gateway_skip_cache` в `infra/gateway-cache/nginx.conf`,
      allow-list `application/json`/`image/*`/`text/vtt`), проверено живым запросом 2026-09-08:
      `text/html` корректно не попадает в кеш
- [x] Проверено, что шлюз Kubo отдаёт вменяемый `Content-Type` для `/ipfs/{cid}` — на этом
      фильтр держится целиком. Прежняя точечная проверка 2026-09-08 (тестовый CID) не дала
      ответа; повторная проверка тем же днём на РЕАЛЬНЫХ CID трекера (взяты живым запросом со
      страницы `animatrona-tracker.letar.best`, а не придуманы) дала однозначный результат:
      постеры — `image/jpeg`/`image/webp`, `manifest.json` (реальный `directoryCid` из БД
      `Anime.directoryCid`) — `application/json`, эпизод (`AnimeEpisode.videoCid`, 173 МБ) —
      `video/webm`. Все три корректно проходят/не проходят фильтр `$gateway_skip_cache` по
      своему типу
- [x] `proxy_cache_valid 200 365d` + `proxy_cache_lock on` — контент по CID неизменяем,
      инвалидация не нужна, но стампед при первом открытии страницы реален. Есть в
      `infra/gateway-cache/nginx.conf` как есть, деплоено с 2026-08-09
- [x] `Cache-Control: public, max-age=31536000, immutable` наружу — повторный заход не должен
      доходить до прокси вообще. Есть в `infra/gateway-cache/nginx.conf`, деплоено с 2026-08-09
- [x] Кеширующий nginx на **mail-сервере**, `use_temp_path=off` (иначе временные файлы ложатся
      поверх `max_size`) — контейнер `gateway-cache-nginx`, `Up 4 weeks (healthy)` на 2026-09-08,
      `use_temp_path=off` в `proxy_cache_path` конфига
- [x] `proxy_max_temp_file_size 0` — выставлено в `infra/gateway-cache/nginx.conf` и задеплоено
      на mail 2026-09-08. Проверено не просто наличием директивы, а живым замером: запущен
      throttled-скачивание реального эпизода (173 МБ, `video/webm`, некешируемый Content-Type)
      с искусственно медленным читателем (2 МБ/0.5с, ~30 секунд на полный файл) — на всём
      протяжении `docker exec gateway-cache-nginx du -sh /var/cache/nginx/proxy_temp` оставался
      `4.0K` (пустой каталог), `df -h /` на хосте не менялся (`7.4G` занято до/во время/после).
      Быстрый (не throttled) прогон того же файла показал то же самое за ~5 секунд. Мониторинг
      свободного места отдельной задачей по-прежнему не заведён — вне скоупа этого пункта DoD
- [x] Бэкенд (шлюз Kubo на s3, **не** `pin-queue`) адресуется по сети, а не через `127.0.0.1` —
      фронт и шлюз на разных машинах. `proxy_pass https://ipfs.letar.best;` в `nginx.conf`,
      подтверждено живым запросом 2026-09-08 (контент реально приходит с origin на s3)
- [x] `gateway.letar.best` переставлен на mail-сервер, **и хвост на s3 снят** — формулировка
      исходного пункта («`ipfs.letar.best` переставлен на mail») была ошибкой ещё на момент
      написания: по итоговому решению «Разделение имён» (см. блок в начале секции) на mail
      переезжает `gateway.letar.best`, а `ipfs.letar.best` остаётся origin на s3 за Traefik —
      именно так и задеплоено. Проверено 2026-09-08: DNS `gateway.letar.best` → `31.56.180.161`
      (mail), NPM-хост `3.conf` на mail держит SSL-сертификат на это имя, роутер Traefik и
      acme-dns аккаунт `gateway` на s3 вычищены (см. «✅ Задеплоено» выше)
- [x] `valid_referers` + `403`, список доменов **через env**, не хардкодом в публичный конфиг —
      задеплоено на mail 2026-09-08. ⚠️ Не через `include` отдельного файла, как планировалось
      изначально — воспроизведённый вживую квирк парсера nginx `:alpine` 1.31
      (`"valid_referers" directive is not allowed here` только при `include`, при прямом
      объявлении в том же файле и при `include` других директив — работает) заставил
      переключиться на envsubst-шаблон официального образа (`/etc/nginx/templates/*.template`,
      см. `infra/gateway-cache/docker-compose.yml`): директива `valid_referers` — литеральная
      строка в `nginx.conf`, а значение (`GATEWAY_VALID_REFERERS=*.letar.best`) — в НЕ
      коммитящемся `infra/gateway-cache/.env` (шаблон — `.env.example`, покрыт существующим
      `infra/*/.env` в `.gitignore`). `*.letar.best` — не коммерческий домен клиента, механизм
      расширяем на случай появления такого домена в будущем (дописывается только в `.env` на
      сервере). Живой тест на реальном постере трекера: без `Referer` — `200`
      (`valid_referers none` разрешает), с посторонним `Referer` — `403`, с валидным
      (`https://animatrona-tracker.letar.best/`) — `200`
- [x] Проверка `Referer` стоит **до** `proxy_cache`: `valid_referers`/`if ($invalid_referer)
      { return 403; }` — первые директивы в `location /`, до `proxy_pass`/`proxy_cache`. По
      фазам nginx access/rewrite (где обрабатывается `if`) выполняются раньше content-фазы
      (`proxy_pass`) и раньше записи в кеш — `return` внутри `if` завершает запрос до того, как
      прокси-модуль вообще узнаёт о нём. `Referer` не входит в `proxy_cache_key` (остаётся
      `$uri`) — не менялось этой правкой, было верно и раньше
- [x] Проверено, что `403` не закешировался: запрос без реферера → `200` (кладёт объект в кеш,
      `X-Cache-Status: MISS`/`HIT`), следом запрос с посторонним реферером → `403` (короткое
      замыкание до `proxy_cache`, в кеш не попадает), следом запрос с валидным реферером на тот
      же CID → `200` с телом файла, не `403` — все три живые сценария выполнены на реальном
      постере 2026-09-08. Отдельно найден и исправлен смежный баг, не входивший в исходную
      формулировку пункта: `add_header Cache-Control ... always;` вешал `immutable`-кеш
      браузера и на `403`-ответ тоже (`always` применяет `add_header` даже к 4xx/5xx) — убран
      `always`, без него `add_header` по умолчанию не применяется к кодам вне 2xx/3xx-редиректов,
      что и нужно: бессрочно кешировать в браузере должен только реально отданный файл. Заодно
      обнаружен и исправлен дублирующийся `Cache-Control` в ответе (свой `add_header` +
      непогашенный заголовок от Kubo, `proxy_ignore_headers` не убирает заголовок из ответа
      клиенту, только не даёт nginx ориентироваться на него внутри себя) — добавлен
      `proxy_hide_header Cache-Control;`
- [x] `infra/animatrona-pinner3` — не переписан под реальность (переписывать больше не подо что:
      выделенный VPS 188.127.235.38 недоступен под управлением, `ssh` отвечает `REMOTE HOST
      IDENTIFICATION HAS CHANGED`, типичный признак смены владельца IP хостером, проверено
      2026-09-08), помечен как история — README получил предупреждение в начале файла с живыми
      фактами проверки и ссылкой на актуальный узел (`infra/letar-ipfs/`, §60).
      `Caddyfile`/`docker-compose.npm.yml` не удалены, помечены как история той же правкой — на
      s3 нет ни одного контейнера `caddy` (`docker ps -a` проверено вживую), порты `80`/`443`
      держит Traefik
- [x] `infra/animatrona-pinner` (pinner1) — не удалён, помечен как история: живая проверка `docker
      ps` на mail 2026-09-08 подтвердила отсутствие контейнера `animatrona-pinner` (см. также
      «✅ Задеплоено» выше — тот же список контейнеров mail). Упоминания pinner2 в README уже
      были вычищены раньше (см. текст ниже в этом файле, «Живёт своей жизнью»)
- [x] `bootstrap-all.sh` обезврежен — guard в начале скрипта печатает причину и завершает
      выполнение `exit 1` до какого-либо SSH/`docker exec`; переписывать под актуальную
      топологию не стали осознанно — сеть сейчас состоит из одного узла (`infra/letar-ipfs/` на
      s3), которому peering-bootstrap между несколькими пиннерами не нужен вовсе
...
## §60 — Инфраструктурные сервисы на серверах живут вне git: фикс в репозитории не доезжает до прода ✅ ЗАКРЫТО ЦЕЛИКОМ (2026-09-02) — mail-сервер (заведено 2026-08-08, приоритет поднят 2026-08-28)

### Mail-сервер (NPM/relay/maddy) — закрыто (2026-09-02)

Изначальная оценка риска в этой секции была **неверной для `relay`**: том `animatrona-relay-data`
объявлен в compose с явным `name:` (как и у `pin-queue`/`kubo`) — привязка тома не зависит от
имени compose-проекта/директории, перенос безопасен, PeerId не меняется. Проверено эмпирически:
`md5sum` файла идентичности в `/var/lib/docker/volumes/animatrona-relay-data/_data/` до и после
cutover совпал (`674e1c30ef51a4b72431535835b3d10c`). Выполнен прямой cutover (down/up из
`infra/animatrona-relay/` в git-клоне), контейнер `healthy`.

`nginx-proxy-manager` (mail) и `maddy` — другая категория: относительные bind-mount'ы (`./data`),
и что важнее — у обоих в cron зашиты **абсолютные пути** (`/opt/maddy/backup.sh`,
`/opt/maddy/data/certs` в `renew-cert.sh`, `/root/nginx-proxy-manager/data` в `/opt/npm-
backup.sh`). Физический перенос каталога сломал бы бэкапы и обновление сертификата молча — ровно
тот же класс проблемы, против которого заведена вся эта секция, только на уровень выше. Каталоги
**остались на месте**, git-трекаемым сделан только `docker-compose.yml` — заменён на симлинк на
файл в `infra/mail-nginx-proxy-manager/`/`infra/maddy/` (содержимое было побайтово идентично
боевому, поэтому рестарт контейнеров не понадобился вовсе — zero downtime). `backup.sh`/
`renew-cert.sh`/cron продолжают работать без единой правки, `docker compose config` подтверждён
из обеих директорий.

`wordpress` и `.env.docker`-директория у `animatrona-tracker-redis` (s2) закрылись сами по себе
между сессиями — см. записи ниже в чек-листе. Кроме kubo/relay/npm/maddy других сервисов вне git
на трёх серверах на момент этой ревизии не найдено (перепись §60 п.1 её и обнаружила исчерпывающе).
...
- [x] **`wordpress` — снят вопрос, приложение выведено из эксплуатации (обнаружено 2026-09-02).**
      `/home/deploy/wordpress` на s2 больше не существует, контейнеров `wordpress`/`mariadb` нет —
      только осиротевший `/var/log/wordpress-backup.log`. Пароли переносить некуда, задачи нет.
- [x] **`animatrona-tracker-redis` — уже переехал в git (обнаружено 2026-09-02), без отдельной
      сессии.** `docker inspect` подтверждает `working_dir=/home/deploy/letar/apps/
      animatrona-tracker`, `config_files` указывает в `docker-compose.production.yml` внутри
      клона — обычный деплой приложения решил и этот хвост, `.env.docker`-директория из находки
      2026-08-08 больше не актуальна (перепроверить содержимое отдельно не стали — сервис
      `Up 3 weeks (healthy)`, не тот приоритет).
- [x] **kubo (`/opt/letar-ipfs`) → `infra/letar-ipfs/` — ЗАКРЫТО (2026-09-02).** Compose заведён в
      репозиторий (коммит `850ef0d3`), cutover выполнен: `docker compose down` в `/opt/letar-ipfs`
      → `docker compose up -d` из `/home/deploy/letar/infra/letar-ipfs/`. Данные — bind mount
      `/data/ipfs` на хосте (не именованный том), перенос их не тронул: PeerId сверен до/после
      (`12D3KooWM7KtRLjqRmJzva7Qy5KZzfaLES4Fk8GgnjabbWoo8A52`, совпадает), `kubo` — `healthy`.
      Старый `/opt/letar-ipfs` оставлен как инертный откат (тот же приём, что у `pin-queue`).
      ...
      ⚠️ **Третий случай, 2026-08-28 — kubo снова, и снова не поиском.** Разбирали, почему
      staging-сборки на s3 идут в 3–10 раз дольше локальных. Дошли до `kubo` как крупнейшего
      потребителя RAM (1.45 ГБ при `Swap: 7.7Gi из 8.0Gi занято`) и до того, что версия
      устарела (0.42.0 при доступной 0.43.0+, контейнер не перезапускался с 9 августа). Дальше
      выяснилось ровно то же самое: `com.docker.compose.project.working_dir=/opt/letar-ipfs`,
      вне git. То есть даже **перезапустить** сервис нельзя обычным путём — нет ни файла в
      репозитории, который бы этим управлял, ни способа проверить, что запущено то, что
      описано. Пришлось идти заявкой к `deploy-agent-dev` с сырыми командами
      (`cd /opt/letar-ipfs && docker compose pull && up -d`), а это ровно тот обходной путь,
      против которого заведена эта секция.

      Отсюда довод к приоритету: секция всплывает **не когда за неё берутся, а когда мешает
      другой работе** — третий раз за три недели, каждый раз внутри чужой задачи и каждый раз
      как неожиданность. Пока она открыта, любая правка инфры стоит лишнего захода на сервер.

- [x] **Классифицировать по одному признаку: есть ли в томе идентичность — сделано для kubo
      (2026-09-02).** `/opt/letar-ipfs` — bind mount, без идентичности в томе-в-смысле-этого
      признака (PeerId живёт в файле на хосте, не зависит от имени compose-проекта) — можно
      переезжать. Для трёх оставшихся (`nginx-proxy-manager`/`animatrona-relay`/`maddy` на mail)
      классификация **не сделана** — `relay` уже известен как случай с идентичностью (PeerId в
      `relay_data`), `maddy`/`npm` не проверены.
- [x] **Способ, применённый на практике для kubo (2026-09-02): не sparse-checkout на месте, а
      прямой cutover через `docker compose down`/`up -d` из уже существующего полного клона
      `/home/deploy/letar`.** Решение 2026-08-08 предполагало `git init` + sparse-checkout
      конкретно для каталогов без готового рядом клона — на s3/s2 клон `letar` уже есть, поэтому
      проще заводить compose в `infra/<name>/` внутри него и переключать `docker compose`, а не
      городить отдельный shallow-репозиторий. Тот же приём уже был у `pin-queue`; способ работает
      для сервисов без идентичности в bind/named-томе, для `relay` (PeerId) остаётся под вопросом.
- [x] **Проверка применимости — сделана для kubo (2026-09-02).** PeerId сверен `docker exec kubo
      ipfs id` до и после cutover, совпал; `config_files` в `docker inspect` указывает в git-клон;
      контейнер `healthy`. Не «доехало по `git log`», а по факту на самом сервисе.
