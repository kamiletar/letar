...
## §57 — Раздача IPFS: у Traefik нет кеширования, а дока пиннеров описывает несуществующую схему 🟡 ИНЦИДЕНТ ЗАКРЫТ (2026-08-09), КЕШ-ПРОКСИ ЖИВ В ПРОДЕ, DoD ЗАКРЫТ ЧАСТИЧНО
...
>
> **Что действительно ещё открыто:** `valid_referers`/`403` (access-control) в `nginx.conf`
> отсутствует целиком; `proxy_max_temp_file_size` стоит `1024m`, а не `0` — осознанный
> компромисс, задокументированный как открытый риск в `infra/gateway-cache/README.md`, не
> ошибка учёта; плюс doc-cleanup пункты (`infra/animatrona-pinner*`, `bootstrap-all.sh`) не
> тронуты вовсе. Раздел «✅ Задеплоено» ниже — это MVP кеша (латентность интерфейса
> `animatrona-tracker`), не вся спецификация §57 целиком; название раздела оставлено, но читать
> «закрыто» нужно как «закрыт инцидент и ядро кеша», не «закрыт весь §57».
>
...
- [ ] Проверено, что шлюз Kubo отдаёт вменяемый `Content-Type` для `/ipfs/{cid}` — на этом
      фильтр держится целиком. Точечная проверка 2026-09-08 не дала однозначного ответа
      (заголовок `content-type` не пришёл в тестовом запросе к origin) — требует отдельного
      прогона на реальном постере/JSON трекера, не общем тестовом CID
...
- [ ] ⛔ `proxy_max_temp_file_size 0` — **некешируемое видео тоже пишется на диск** через
      ...
      На 2026-09-08 в `nginx.conf` стоит `proxy_max_temp_file_size 1024m`, не `0` — осознанный
      промежуточный компромисс (ограничивает рост, но не убирает риск целиком), задокументирован
      как открытый нерешённый риск в `infra/gateway-cache/README.md` § «Мониторинг диска».
      Мониторинг свободного места отдельной задачей не заведён
...
- [ ] `valid_referers` + `403`, список доменов **через env/`include`**, не хардкодом в публичный
      ...
      проверено 2026-09-08: в `infra/gateway-cache/nginx.conf` этого блока нет вообще, гейт
      открыт кому угодно, кто узнает домен
- [ ] Проверка `Referer` стоит **до** `proxy_cache`, и `Referer` не входит в ключ кеша — не
      применимо, пока не реализован пункт выше
- [ ] Проверено, что `403` не закешировался: запрос без реферера, следом запрос с реферером —
      второй должен отдать файл, а не `403` из кеша — не применимо, пока не реализован
      `valid_referers`
