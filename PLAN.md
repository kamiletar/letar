...
- ✅ **Конфиги Maddy** (2026-06-04): `/opt/maddy/backup.sh` тарует `maddy.conf` + `docker-compose.yml` +
  ...
  ⚠️ **Урок:** этот бэкап не был покрыт canary/алертингом Этапа 0.7 (тот проверяет доставку email,
  не целостность самого бэкап-пайплайна) — стоило бы алертить на «нет нового `maddy_*.tar.gz` за N
  дней», задача не заведена отдельно, т.к. вне скоупа этой сессии.
