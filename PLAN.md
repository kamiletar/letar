...
**Дальше по убыванию срочности:**

1. **§66 п.2 — `deploy-affected.sh` не знает про `docker-compose.s3.yml`** и молча берёт продовый
   дефолт. Повторится у любого, кто запустит штатную команду. До починки `dashboard-agent` на s3
   деплоится только вручную.
2. **Хвост M2:** `dashboard-agent` 0.15.1 (бэкап секретов Traefik на s3) — ждёт повторного
   деплоя после хотфикса.
3. **Уборка путей отката** — после недели без инцидентов, порядок в §48.
4. **M3 (s2)** — не начат, блокируется §18.7.

**Заведено попутно:** §65 (на s2 не было swap вообще — сборка могла убить чужой продакшен;
