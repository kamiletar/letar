...
> опровергнута живым прогоном, найдена и починена настоящая причина (2026-07-19, aboi-dev, commit
...
> Статус **⏳**, не **✅** — этот фикс тоже не подтверждён живым staging-прогоном (два предыдущих
> раунда правок про этот же баг уже давали ложное ощущение уверенности).
>
...
> aboi). **➡️ Следующий шаг:** запросить у BlackCove восьмой прогон `aboi` на новом коммите для
> живого подтверждения фикса; если снова идентичный отказ — гипотеза №2 тоже неверна, нужна живая
> WebKit-отладка (недоступна из этой сессии), не третья гипотеза вслепую.
>
...
- [ ] Прогнан `workflow:archive-completed`, закрытые секции в архиве
- [ ] Принято решение по `PLAN-INFRA.md` (вынесено или обосновано, почему нет)
- [ ] Ссылки на секции проверены после переноса
