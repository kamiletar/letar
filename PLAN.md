...
## СТАТУС (обновлено: 2026-08-06)

### ▶ Следующий старт — `PLAN-INFRA.md` §48, милестон **M1b**

Активный фронт сессии 2026-08-06 (вечер) — инфраструктурный трек **§48: замена Nginx Proxy Manager
на Traefik**. Auth-план (этот файл) в этой сессии не двигался.

**Закрыто:** §48 **M1a** — собственный `acme-dns` на s2, wildcard-сертификат `*.letar.best`
выпущен и проверен вживую. С этого момента выпуск и продление сертификатов **не обращаются к
регистратору вообще** — ни при выпуске, ни каждые 90 дней, ни при добавлении нового поддомена.

**Дальше — M1b: Traefik на s3.** Конфигурация написана и закоммичена (`infra/traefik/`), на
серверах ещё ничего не разворачивалось. Порядок и чек-листы — в §48; первым делом четыре шага
подготовки на серверах (сеть `traefik-network`, proxy host `acme-api.letar.best` с Access List,
копия аккаунта acme-dns на s3, файл basicAuth дашборда), они уходят BlackCove одним запросом.

Хвосты M1a, не блокирующие M1b: acme-dns в бэкапы и мониторинг (от него зависит продление всех
сертификатов зоны — если он тихо умрёт, выяснится через 90 дней в худший момент), опциональная
`CAA`-запись, проверка доставки на `kami@letar.best`.

**Заведено попутно:** §18.8.1 (секреты инфра-сервисов вне конвейера `.enc` — у `infra/` нет пути
деплоя, куда встроить расшифровку), §50 (прод-деплой молча откатывается с `--frozen-lockfile` на
обычный `bun install`, и улику стирает `git checkout` в начале следующего деплоя), §49 (firewall на
s2 неактивен — отдельная сессия).

**Ждёт ответа BlackCove:** `git submodule status` с s2/s3 — он разводит две версии причины §50,
безобидную и серьёзную.
