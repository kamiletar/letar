...
> **`dsperevod` — compose смигрирован (2026-07-13, submodule commit `a8491ca` +
> bump SHA `25d3c9c` в letar):** первый submodule-кандидат тиража §18.6 Сессии J (приватный
> `letar-private-dsperevod`) и первый с БД со времён `time` — только сервис `app` мигрирован,
> `db`/bind-mount `uploads`/`depends_on` не трогались. `doctor` 8/8 READY. Запрос пилота отправлен
> BlackCove (thread `deploy-dsperevod-rollout-J`) — **ждёт выполнения**.
...
| **J** | ⏳ начат досрочно (сессия №69, 2026-07-12, независимо от I — rollout не требует hard gate) | Тираж на остальные приложения пачками 3–5 через doctor-чек-лист; проверка, что host-порты нигде больше не используются (мониторинг!); blue-green fallback задокументирован | 6/~19 SERVER_APPS на rollout (`time`, `form-docs`, `pravda`, `kami-key-the-landing`, `letar-landing`, `animatrona-landing` — все ✅ чистые пилоты, подробности выше в шапке файла). Найден и закрыт баг детектора label в `deploy-affected.sh` (`4fbc414`), важен для всего тиража. `form-example` и `mandala` — обычный (не-rollout) деплой закрыт, `letar.rollout` пока выключен (mandala — период стабильности после прод-инцидента сессии №70; form-example — найден отдельный незаблокированный баг `/products` ECONNREFUSED, сессия №72, закрыт сессией №73). `dsperevod` — compose смигрирован (submodule commit `a8491ca` + bump `25d3c9c`), `doctor` 8/8 READY, запрос пилота отправлен BlackCove (thread `deploy-dsperevod-rollout-J`) — **ждёт выполнения**. Осталось пройти тиражом: `aboi`, `umami`, `kami`, затем `archetest`/`grandslamcup`, затем `auth-hub`/`driving-school` последними (риск по возрастанию) |
