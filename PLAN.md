...
1. **Уведомление в РКН** — подать через pd.rkn.gov.ru для каждого оператора. Получить PDF с номером → занести в README/PLAN приложения.
   - ✅ **aboi (neyroaboi.ru):** подано 16.05.2026 оператором-владельцем (ИП), рег. № 100286690. PDF у владельца.
     ⚠️ Проверить при аудите aboi: (а) в уведомлении «трансграничная передача не осуществляется» — сверить с реальными
     способами входа/интеграциями aboi; (б) заявленный ЦОД (ООО «Цифровые решения», Москва) — сверить с фактическим
     хостингом s2.
   - ✅ **letar (`*.letar.best`: auth-hub, grandslamcup и пр.):** подано 02.06.2026 оператором-владельцем (ИП),
     ...
     ⚠️ Проверить при аудите: в уведомлении «трансграничная передача не осуществляется», но auth-hub предлагает
     Google/GitHub/Facebook OAuth и Telegram-вход → де-факто передача за рубеж; снять противоречие
     (Этап 6.7 гео-скрытие + оговорка в /privacy, либо уточнить уведомление).
   - ❌ Остальные операторы (premium-rosstil, imot, dsperevod, driving-school) — не подано.
