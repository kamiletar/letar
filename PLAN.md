...
> **Архив:** сессии старше ~1 месяца (до 2026-06-21) перенесены в [PLAN_COMPLETED.md](./PLAN_COMPLETED.md).
...
- [ ] Принято решение по `PLAN-INFRA.md` (вынесено или обосновано, почему нет) — не решено в
      этом проходе, оставлено открытым
- [x] Ссылки на секции проверены после переноса — `grep` по `.claude/docs/*.md`/`CLAUDE.md` на
      ...
      не по строке, перенос не ломает существующие ссылки
