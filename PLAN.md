...
> **🔴 `archetest` деплой прерван ДО rollout — обнаружена failed-миграция на проде (не связано с
> compose-миграцией §18.6):** BlackCove (msg #395/#397, thread `deploy-archetest-rollout-J`)
> запустил деплой compose-изменений (`f6fb9ca`), но применение миграций упало с Prisma `P3009` —
> `20260321000000_baseline` (с 23 марта) помечена в проде как **failed**, новые миграции не
> применяются поверх failed-состояния. `deploy-affected.sh` защита сработала штатно: pre-migrate
> dump сделан, деплой прерван **до** докер-билда/rollout — старый контейнер `archetest` не тронут,
> даунтайма нет. Резолвить (`prisma migrate resolve --applied`/`--rolled-back`) не стали, не
> убедившись в реальном состоянии схемы — приложение хранит психоданные, цена ошибки высока.
> **Пользователь подтвердил: сначала диагностика.** Запрос BlackCove — сравнить фактические
> таблицы/колонки БД `archetest` с тем, что должна создать baseline-миграция, прислать результат,
> резолвить только после этого. `grandslamcup` (compose тоже готов, `f6fb9ca`, `doctor` 8/8) —
> **на паузе**, не трогаем, пока не закроем archetest.
>
...
> `deploy-archetest-rollout-J` — ждём диагностику BlackCove по failed-миграции
> `20260321000000_baseline` (сравнение фактических таблиц БД с baseline); резолвить
> (`prisma migrate resolve`) только после того, как диагностика подтвердит applied/rolled-back —
> не вслепую; (2) после закрытия archetest — отправить запрос пилота для `grandslamcup` (compose
> уже готов, commit `f6fb9ca`); (3) затем `auth-hub`/`driving-school` последними, риск выше — не
> мигрировать с ходу без дополнительного анализа, сверяться с пользователем; (4) когда все ~20
...
- **🔴 `archetest` — failed-миграция на проде (обнаружено 2026-07-13, §18.6 Сессия J):**
  `20260321000000_baseline` (с 23 марта) помечена в БД как failed, блокирует Prisma `P3009` любые
  новые миграции. Обнаружено случайно при обычном компос-деплое (не связано с ним) —
  `deploy-affected.sh` защита сработала (pre-migrate dump, прервал до rollout, даунтайма нет), но
  вопрос **не закрыт с марта** ни разу до этой сессии. Резолвить `--applied`/`--rolled-back`
  вслепую не стали — приложение хранит психоданные, сначала сверка фактической схемы БД с
  ожиданиями миграции (в работе, thread `deploy-archetest-rollout-J`).
