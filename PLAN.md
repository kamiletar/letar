...
## СТАТУС (обновлено: 2026-08-12, сессия продолжена)

### ▶ Следующий старт — ждать ответ BlackCove (form-example e2e повтор, kami staging+seed) и чинить реальные баги, найденные e2e (mandala/pravda)

Сессия 2026-08-12 (продолжение) — инфраструктурные треки `PLAN-INFRA.md` §18.8.1/§18.8 закрыты
(см. архив ниже), **§70 (GlitchTip) закрыт по деплою на все 17 некоммерческих приложений** —
клиентские+серверные ошибки живьём подтверждены везде, где проверялось. §18.7 (e2e-гейт):
...
- **M1 (`mandala`, `pravda`)** задеплоены, e2e прогнан — **реальные красные, не флейк**
  ...
  webkit не рендерятся на pravda). `E2E_GATED_APPS` не обновлён, чинить — отдельная задача.
- **M2 (`form-example`, `kami`)** — staging заведён впервые (не было вообще). Два реальных
  ...
  Ждём от BlackCove: повторный `run_e2e` для `form-example`, `deploy_app(staging, seed:true)` +
  `run_e2e` для `kami`.
- Обнаружен системный гэп — ещё 4 e2e-сьюта без `project.json` (не тронуты, заметка на будущее).
...
второй — ошибка классификации), оба заведены. Auth-план (этот файл) в этой сессии не двигался.
