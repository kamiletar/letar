...
> одной пачкой). Свободные концы: (1) e2e auth-hub прогонялся на старом коммите `6ee1751` и
> старше 24ч (предупреждение гейта) — прогнать на актуальном; (2) реальное исполнение Tier
> 1-перехода когда появится первый запрос (Этап 8); (3) 🔴 `svoichuzhie` прод-баг (запись
> ниже) — если ещё актуален.
