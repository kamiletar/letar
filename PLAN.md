...
## СТАТУС (обновлено: 2026-08-11)

### ▶ Следующий старт — GlitchTip на staging (ждёт BlackCove) и тираж на остальные приложения

Сессия 2026-08-11 — инфраструктурные треки `PLAN-INFRA.md`: закрыты §61 (секрет в desktop-
приложении — клиент удалён, аудит остальных distributable-приложений чист), §65 (swap на s2
подтверждён живым прогоном), §71 частично (pg_stat_statements на dev studio, `hypopg` и
driving-school — открытые решения), `.gitignore` для `.env.staging` в 5 приватных submodule.
**§70 (GlitchTip) — основной объём:** развёрнут на s3, создан `libs/glitchtip` (SDK-обёртка
`@sentry/node`/`@sentry/browser`, не `@sentry/nextjs`), подключён и задеплоен на прод `studio` —
первое приложение монорепо с трекингом ошибок. По ходу найден и исправлен баг (клиентский DSN не
инлайнился в бандл, задокументирован отдельно как общий класс —
[nextjs-public-env-build-time-inlining.md](/.claude/docs/nextjs-public-env-build-time-inlining.md)),
подтверждён живой проверкой на проде. Открыто: тот же фикс для staging studio (запрос BlackCove
отправлен, ждёт), тираж на остальные приложения (список кандидатов — `PLAN-INFRA.md` §70 п.7).
Auth-план (этот файл) в этой сессии не двигался.
