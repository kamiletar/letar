...
> **🟡 `archetest` — диагностика failed-миграции завершена, резолв санкционирован (не связано с
> compose-миграцией §18.6):** BlackCove (msg #398, thread `deploy-archetest-rollout-J`)
> прогнал read-only диагностику: в `_prisma_migrations` **три** записи, не одна — настоящий baseline
> `20260321075436_baseline` (2026-03-21) применён успешно, следом `..._add_psychologist_cabinet`
> тоже успешна, а `20260321000000_baseline` (2026-03-23, более старое имя по алфавиту) упала на
> первом же `CREATE TYPE` (`42710 type "UserRole" already exists`) — **0 применённых шагов**,
> дубликат уже применённого baseline, схема БД полностью соответствует состоянию до её запуска.
> Данные пользователей не читал/не менял. **Пользователь подтвердил резолв** —
> `prisma migrate resolve --rolled-back 20260321000000_baseline`, затем повтор деплоя. `deploy-
> affected.sh` защита сработала штатно при первой попытке: pre-migrate dump, прервал до
> докер-билда/rollout, даунтайма не было. `grandslamcup` (compose готов, `f6fb9ca`, `doctor` 8/8) —
> **на паузе**, ждёт закрытия archetest.
>
