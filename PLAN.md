...
> **➡️ Следующий старт:** **Этап 2 п.3 ремедиация застрявших** (bэклог неверифицированных в aboi/dsperevod) или **Этап 4** (premium-rosstil миграция на Better Auth).
> Открытые риски 0.3: offsite для Nginx NPM не проверен; IgnoreList Resilio не сужен; локальные kreды не bэкапятся.
...
- ⏳ **Форвард на gmail** режется (DKIM/SPF) — чинится DKIM/SPF/DMARC Этапа 0; DKIM DNS-записи для
  `letar.best`, `neyroaboi.ru`, `premium.rosstil.ru` уже есть; `направа.рф` — DKIM ключ не сгенерирован.
...
### Этап 0.3 — Ревизия системы бэкапов (прод + локальные) ⏳ ЧАСТИЧНО (2026-06-04)

> **Проблема:** сейчас бэкапится много лишнего, а часть критичного — нет. Нужна единая продуманная стратегия.

- **Сузить scope синхронизации (Resilio Sync).** ⏳ Пересмотреть `.sync/IgnoreList`.
