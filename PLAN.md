...
технический.
**Следующий шаг:** ✅ 4 коммита сессии (группы A/B/C/D) запушены в `origin/main` (2026-07-28). ✅ Чеклист
Этапа 0.8 перепроверен по факту кода и обновлён (таблица §7 + DoD) — 13 приложений подтверждены Grep/Glob,
`consentAccepted: true` как default нигде не найден. Осталось: решить с владельцем svoichuzhie вопрос
отдельной РКН-регистрации → после подачи дозаполнить плейсхолдеры на `svoichuzhie.ru/privacy`.
