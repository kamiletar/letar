...
> Запрос на ротацию `DB_PASSWORD` для `aprel8008` (реальная утечка, см. Сессию №71 ниже) фактически
> **никогда не отправлялся** — предыдущая запись «ответ ещё не пришёл» не подтвердилась ни одним
> сообщением в Agent Mail (`search_messages`/`fetch_inbox`/`fetch_summary` — 0 результатов). Отправлен
> заново (msg id 367, thread `deploy-aprel8008-db-password-rotation`).
> **Результат по отчёту BlackCove (очередь разобрана):** `aprel8008` — пароль Postgres ротирован,
> задеплоено, HTTP 200; `form-example` — пароль ротирован **и** зафиксирован фикс `/products`
> (`ECONNREFUSED`, открытый с Сессии №72), задеплоено, HTTP 200. ⚠️ Подтверждено только по отчёту
> BlackCove (скриншот пользователю) — ack-сообщение в Agent Mail на момент записи не получено,
> независимая проверка через `curl` недоступна из этого окружения (нет сетевого доступа наружу).
> Если появятся сомнения — проверить `deploy_status`/`git_status` (deploy-mcp) или зайти на сайты напрямую.
>
...
| **J** | ⏳ начат досрочно (сессия №69, 2026-07-12, независимо от I — rollout не требует hard gate) | Тираж на остальные приложения пачками 3–5 через doctor-чек-лист; проверка, что host-порты нигде больше не используются (мониторинг!); blue-green fallback задокументирован | 2/~19 SERVER_APPS на rollout (`time`, `form-docs` — оба ✅ чистые пилоты). Найден и закрыт баг детектора label в `deploy-affected.sh` (`4fbc414`), важен для всего тиража. `form-example` и `mandala` — обычный (не-rollout) деплой закрыт, `letar.rollout` пока выключен (mandala — период стабильности после прод-инцидента сессии №70; form-example — найден отдельный незаблокированный баг `/products` ECONNREFUSED, сессия №72, закрыт сессией №73). ✅ `pravda` — compose смигрирован (commit `bf79514`, сессия №73), `doctor --app pravda` 8/8 READY, запрос супервизируемого пилота отправлен BlackCove (thread `deploy-pravda-rollout-J`) — **ждёт выполнения**. Осталось пройти тиражом: `kami-key-the-landing`, `letar-landing`, `animatrona-landing`, `dsperevod`, `aboi`, `umami`, `kami`, затем `archetest`/`grandslamcup`, затем `auth-hub`/`driving-school` последними (риск по возрастанию) |
