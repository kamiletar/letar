...
> даунтайма не было. **3/~19 SERVER_APPS на rollout** (`time`, `form-docs`, `pravda`).
...
| **J** | ⏳ начат досрочно (сессия №69, 2026-07-12, независимо от I — rollout не требует hard gate) | Тираж на остальные приложения пачками 3–5 через doctor-чек-лист; проверка, что host-порты нигде больше не используются (мониторинг!); blue-green fallback задокументирован | 3/~19 SERVER_APPS на rollout (`time`, `form-docs`, `pravda` — все ✅ чистые пилоты). Найден и закрыт баг детектора label в `deploy-affected.sh` (`4fbc414`), важен для всего тиража. `form-example` и `mandala` — обычный (не-rollout) деплой закрыт, `letar.rollout` пока выключен (mandala — период стабильности после прод-инцидента сессии №70; form-example — найден отдельный незаблокированный баг `/products` ECONNREFUSED, сессия №72, закрыт сессией №73). ✅ `kami-key-the-landing` — compose смигрирован (commit `fa50fa9`, сессия №73), `doctor` 8/8 READY, запрос пилота отправлен BlackCove (thread `deploy-kami-key-the-landing-rollout-J`) — **ждёт выполнения**; ⚠️ нестандартная сеть `nginx-proxy-manager_default` вместо `premium-network`, попросил BlackCove проверить NPM-резолвинг вживую. Осталось пройти тиражом: `letar-landing`, `animatrona-landing`, `dsperevod`, `aboi`, `umami`, `kami`, затем `archetest`/`grandslamcup`, затем `auth-hub`/`driving-school` последними (риск по возрастанию) |
