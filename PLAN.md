...
## СТАТУС (обновлено: 2026-09-06, инфра-сессия: аудит и переработка §157)

### ▶ Следующий старт (обновлено 2026-09-21) — [§157](/PLAN-INFRA-6.md): сборка на s1, s2 только релиз

Железо больше не блокер: 2026-09-19 старый s3 разнесён на **s1** (сборка/staging/e2e/registry,
12 ядер, 31 ГБ RAM) и s3 (хранилище). **Код разреза конвейера написан 2026-09-21** (`deploy-affected.sh
--remote-release` + `scripts/deploy-release.sh` на s2, ограниченный SSH-ключ s1→s2, переход по
приложениям через `BUILD_ON_S1_APPS`) — см. «Реализацию разреза» в §157. **Дальше:** запушить, владелец
запускает `scripts/setup-release-channel.sh`, выкат `dashboard-agent` на s1, пилот 1 (`letar-landing`),
затем пилоты 2–3 и тираж. Ниже — прежняя шапка (с «s3» в значении сервера сборки), оставлена как история.

### Предыдущий старт (2026-09-08) — §157: переезд сборки на s3, s2 только релиз

Направление выбрано владельцем 2026-09-06: **перенос всей сборки на s3, на s2 только релиз**
([§157](/PLAN-INFRA-6.md), переработан после аудита; постановка §80 вместо remote buildx). План
проработан целиком, открытых вопросов нет — registry живёт на s3, аварийный однохостовый путь
сознательно не поддерживается.

Обе задачи, не зависящие от железа, — **гейт изоляции Nx-кеша** (`NX_CACHE_DIRECTORY` +
`NX_WORKSPACE_DATA_DIRECTORY` по окружению) и **`registry:2` на s3 + cron ретеншна тегов** —
✅ закрыты 2026-09-06 (детали в конце §157). **Дальше в §157 работы нет до апгрейда железа**:
три пилота (лендинг без БД → приложение с БД → полный пререндерный путь), разрез
`deploy-affected.sh`/`deploy-release.sh` и тираж на остальные приложения ждут смены сервера s3
на машину с большим RAM (~2026-09-20). До этой даты §157 не даёт исполняемых задач.

Ниже — предыдущая шапка, оставлена как история.

### ▶ Предыдущий старт — не зафиксирован. M3 (перевод s2 на Traefik) фактически уже выполнен
...
(mandala добавлена в `E2E_GATED_APPS`). **Обновление 2026-09-22:** pravda тоже добавлена в
`E2E_GATED_APPS` — владелец снял прежнее исключение («мы же договорились, что у нас теперь всё
`HARD_GATED_APPS` без исключений»), заодно `E2E_GATED_APPS`/`HARD_GATED_APPS` схлопнуты в один
всегда fail-closed список (`libs/infra-config/src/index.ts`, разбор — `.claude/docs/deployment.md`
и `libs/deploy-mcp/README.md`). Список теперь без исключений покрывает все 16 гейтуемых приложений.
**Тираж TS7 (§19 в PLAN-INFRA-1.md) —
...
| ⚠️ Открытый вопрос (2026-09-04): старый `registration_token` агента `forms-coordinator-dev` остался в git-истории публичного репо (найден захардкоженным в `forms-coordinator.md`, ротирован, файл почищен — `PLAN-INFRA-6.md` §152). Переписывать историю (`git filter-repo`/force-push) или оставить — риск ограничен agent-mail messaging от этого имени, не инфраструктурным доступом                                                                                                                                                                                                                                                   | —    | Решение владельца — рискованная операция на публичном multi-agent репо             |
| ⚠️ Открытый вопрос (2026-09-04): актуальна ли ещё задача `nginx-backup-s2` в cron/`backup-audit.md` после перехода s2 на Traefik (2026-08-31) — NPM снят, возможно бэкапить уже нечего                                                                                                                                                                                                                                                                                                                                                                                                                                                      | —    | Нужна проверка реальной конфигурации cron на сервере                               |
| Идея (2026-09-08, только обсуждение): вместо хардкода условий доступа в `@@allow`/`@@deny` schema.zmodel — RBAC + Policy layer, где конкретные права редактируются админкой и хранятся как данные (таблица вида `RolePermission`: роль × тип ресурса × действие × тристейт allow/deny/unset, deny всегда побеждает allow). Референс — `auth_access` из TorrentPier/phpBB. Кандидат на `domwellbes` и потенциальный дефолт для новых проектов монорепо                                                                                                                                                                                      | —    | Реализация не начата — ждёт решения о первом приложении-пилоте                     |
| ✅ (2026-09-14) `assertAuthOk` (`@letar/auth/client`, v0.14.0) — throw-bridge между `authClient.*` (`{data,error}`, никогда не бросает) и throw-based `onSubmit` у `@letar/forms`. Заведён заранее — `aboi`/`dsperevod`/`studio`/`svoichuzhie` вот-вот получат задачу мигрировать auth-страницы на `@letar/forms`, паттерн (включая ветку `EMAIL_NOT_VERIFIED`) — `libs/forms/docs/server-errors.md` § «Better Auth — throw-bridge», разослан всем четырём заранее, чтобы не изобретали свой inline `if (result.error) throw` независимо                                                                                                   | —    | Готово к использованию — не блокирован                                             |
| ⚠️ Открытый вопрос (2026-09-21): `/admin/materials/new` в domwellbes — после `--reload-after-fill --accept-restore` (`.claude/scripts/dev-session-console-check.mjs`) черновик восстановил поля 2 и 3, а первое видимое текстовое поле осталось пустым, хотя `form-persistence:material-form:new` сохранён (372 симв.). Не выяснено: поле намеренно вне черновика или баг восстановления в `@letar/forms`                                                                                                                                                                                                                                   | —    | Не разбирали — вне задачи сессии; проверить состав полей формы и `FormPersistence` |
...
- **`@letar/pin-auth`** — верификация email: **коды + ссылки в одном письме**, **синхронизация вкладок**
  (SSE), **resend с cooldown**, авто-логин. Уже существует и зрелая. С v0.3.0 (2026-09-14) —
  опциональный `PinValidatorAdapter.reserveAttempt` (атомарная compare-and-swap резервация
  попытки ДО сравнения PIN) закрывает check-then-act гонку параллельного перебора в
  `maxAttempts`; легаси-путь `incrementAttempts` сохранён, race-prone, задокументирован.
  driving-school мигрирован на новый метод. mandala/другие приложения со своим обходным путём —
  кандидаты на тот же перенос (детали — `libs/pin-auth/CHANGELOG.md`). С v0.7.0 (2026-09-15) —
  клиентский `usePinVerification`/`PinVerificationForm` (`./client`) тоже консолидирован из
  единственного реального потребителя (driving-school); библиотека раньше держала более старую
  версию хука без потребителей в монорепо. Форма не зависит от `@letar/forms` — поле кода
  рендерит приложение через render-prop `renderCodeForm`, тот же паттерн, что у `EmailCodePanel`.
