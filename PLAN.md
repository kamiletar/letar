...
> merge-скрипт, см. записи выше). Свободные концы: (1) деплой auth-hub v0.6.4 через BlackCove
> (linked-email вход пока только локально); (2) реальное исполнение Tier 1-перехода когда
> появится первый запрос (Этап 8); (3) 🔴 `svoichuzhie` прод-баг (запись ниже) — если ещё
> актуален.
