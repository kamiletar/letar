...
## СТАТУС (обновлено: 2026-07-28, root-weaver)

**Активный фронт:** Этап 0.8 (152-ФЗ аудит) — переаудит охвата **и техническая починка** (root-weaver,
2026-07-28). Полная таблица по факту кода — §7 Этап 0.8 (было 9 приложений в старой таблице, реально ~28).
Технически закрыто в этой сессии (весь приоритетный список):

- ✅ **auth-hub, grandslamcup** — добавлена страница `/privacy`, битая ссылка `CookieBanner → /privacy` починена.
- ✅ **mandala, time, animatrona-tracker** — добавлен полный комплект с нуля: `ConsentLog` в `schema.zmodel`,
  `/api/consent`, `CookieBanner`/`CookieSettingsButton`, страница `/privacy`. Pre-existing дрейф локальных
  dev-БД устранён (сброс — с явного разрешения владельца в чате 2026-07-28): `mandala`/`animatrona-tracker`
  — `prisma migrate reset --force`; `time` — у приложения вообще не было папки миграций (жило на `db:push`),
  создан baseline (`init`), включающий существующие модели + `ConsentLog`. Миграция `add_consent_log`
  создана и применена на всех трёх, `nx zenstack:generate`+lint+typecheck зелёные.
- ✅ **kami, archetest** — добавлен `CookieBanner`+`ConsentLog`+`/api/consent`, аналитика (Yandex/Umami)
  переведена на consent-aware обёртку. Миграции созданы и применены.
- ✅ **aira-web, animatrona-landing, kami-key-the-landing, letar-landing, pravda, synth** — минимальное
  cookie-уведомление (localStorage-only, без `ConsentLog`/`/api/consent` — **осознанно**, приложения без БД
  вообще). Consent-aware Umami, страница `/privacy`. `nx build` зелёный на всех 6.
- ✅ (отдельная фоновая сессия, вне этого плана) `UmamiScript` (`@letar/analytics`) получил consent-гейтинг
  (`hasConsent` проп) — закрывает системный пробел, что аналитика грузилась безусловно почти во всех
  приложениях монорепо.
- ✅ `.gitignore` — добавлен `apps/grandslamcup/public/swe-worker-*.js` (был только у archetest, хотя
  grandslamcup генерирует такие же хэшированные PWA service-worker файлы при сборке).

**Блокеры:** 🔴 РКН-регистрация (см. §10) остаётся блокером публичного запуска для `svoichuzhie` — свой
домен (`svoichuzhie.ru`), не покрыт ни одной поданной регистрацией, юридический вопрос владельцу, не
технический.
**Следующий шаг:** **11 приложений с изменениями не закоммичены** (сознательно — для ревью перед коммитом):
auth-hub, grandslamcup, mandala, time, animatrona-tracker, kami, archetest, aira-web, animatrona-landing,
kami-key-the-landing, letar-landing, pravda, synth + корневой `.gitignore`/`PLAN.md`. Ревью → коммит(ы) по
группам (можно одним коммитом на группу — A/B/C/D из этой сессии) → решить с владельцем svoichuzhie вопрос
отдельной РКН-регистрации → проставить галочки чеклиста Этапа 0.8 по каждому приложению.
...
| ------------------------------------------------------------------------------------- | ---- | ------------------------------------------------------------ |
| `X-Cron-Secret` продублирован в 5 местах (studio×2, dashboard×2, driving-school)      | —    | Кандидат на `verifyCronSecret()` в `libs/*`, никто не начал  |
...
| Чеклист Этапа 0.8 (152-ФЗ) — техника готова на 11 приложениях, галочки не проставлены | 0.8  | Ревью+коммит несделанных изменений, потом построчный чеклист |
| `svoichuzhie` (`svoichuzhie.ru`) не входит ни в одну поданную РКН-регистрацию         | 0.8  | Юридическое решение владельца — отдельная подача или нет     |
| 11 приложений с готовыми правками Этапа 0.8 не закоммичены (см. блок СТАТУС)          | 0.8  | Ждёт ревью владельца перед коммитом                          |
...
| **auth-hub**                                                                     | да (email, IP, OAuth)                                          | ❌ **битая ссылка**         | ✅ (`@letar/ui`, дефолт `/privacy`) | ✅           | 🔴 `CookieBanner` ссылается на несуществующий `/privacy` |
| **grandslamcup**                                                                 | да (email игроков/судей)                                       | ❌ **битая ссылка**         | ✅ (`privacyUrl="/privacy"`)        | ✅           | 🔴 та же проблема; регистрация судей без чекбокса        |
| **archetest**                                                                    | да (email, психопрофили)                                       | ✅                          | ❌                                  | ❌           | 🟠 нет cookie-баннера/ConsentLog                         |
| **kami**                                                                         | да (email, Umami+Yandex)                                       | ✅                          | ❌                                  | ❌           | 🟠 нет cookie-баннера/ConsentLog                         |
| **mandala**                                                                      | да (email, Yandex Metrika)                                     | ❌                          | ❌                                  | ❌           | 🔴 полный пробел — ни одного пункта                      |
| **time**                                                                         | да (email, hub-client)                                         | ❌                          | ❌                                  | ❌           | 🔴 полный пробел                                         |
| **animatrona-tracker**                                                           | да (email, hub-client)                                         | ❌                          | ❌                                  | ❌           | 🔴 полный пробел                                         |
| aira-web, animatrona-landing, kami-key-the-landing, letar-landing, pravda, synth | только аналитика (Umami)                                       | ❌                          | ❌                                  | ❌           | 🟡 минимум — cookie-уведомление про аналитику            |
...
1. **Уведомление в РКН** — подать через pd.rkn.gov.ru для каждого оператора. Реквизиты (даты подачи, номера
   ...
   - 🔴 **svoichuzhie — не в реестре поданных уведомлений (найдено 2026-07-28).** Собственный домен
     `svoichuzhie.ru`, `mode: 'standalone'` (не hub-client) → identity и ПД (fanclub-подписки, билеты) хранятся
     в собственной БД, letar-регистрация (домены `*.letar.best`) его не покрывает. Технический комплект уже
     полный (см. таблицу выше) — не хватает только самого уведомления в РКН. Требует решения владельца
     svoichuzhie (аналогично aboi/dsperevod — отдельный оператор или тот же ИП, что и letar/driving-school).
...
- [x] Техническая инфраструктура (`/privacy`+`CookieBanner`+`ConsentLog`/localStorage) — на всех
      приложениях из полной таблицы, кроме N/A (Electron/desktop, `umami`, внутренние)
- [ ] Изменения 11 приложений закоммичены (см. §0 — ждёт ревью владельца)
- [ ] Уведомление в РКН подано для каждого отдельного оператора (включая нерешённый вопрос svoichuzhie)
- [ ] Нет `consentAccepted: true` как default нигде в кодовой базе (Grep-проверка — не перепроверялось
      в этой сессии, аудит был про наличие/отсутствие инфраструктуры, не про defaultValue форм)
