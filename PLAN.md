...
### ▶ Следующий старт (2026-09-06) — [§157](/PLAN-INFRA-6.md) задача №1: гейт изоляции Nx-кеша по окружению
...
Ближайшая задача, готовая к исполнению и не зависящая от железа, — **гейт: изоляция Nx-кеша**
(`NX_CACHE_DIRECTORY` по окружению). Nx не учитывает переменные окружения в ключе кеша, поэтому
после переезда прод-сборка получит cache hit на staging-артефакт с чужими `NEXT_PUBLIC_*` —
затронуто 16 приложений. Детальная постановка с номерами строк и критериями приёмки — в конце
§157. Пилоты переезда ждут смены сервера s3 на машину с большим RAM (~2026-09-20).
