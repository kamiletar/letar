...
> BlackCove (`docker stats`, поиск OOM-событий в `dmesg`/`journalctl`) — **ждёт ответа**. Фиксы
> таймаутов НЕ задеплоены — ждём подтверждения гипотезы, чтобы не маскировать реальную причину
> преждевременным деплоем.
