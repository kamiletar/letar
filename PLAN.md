...
**Активный фронт:** §27 **закрыт полностью.** Часть 2 добита в этой сессии (Шаг 2.1: построчный
аудит §7 нашёл ровно один ещё не вынесенный узкий факт — driving-school DKIM/домен письма,
перенесён в `apps/driving-school/PLAN.md`; остальные ~50 упоминаний aboi/driving-school/dsperevod
в §7 — метки этапов/статус-таблицы без деталей, сознательно оставлены). Часть 3 (пункты 3.1–3.6)
переверифицирована той же сессией по факту в файле — все 6 подтвердились: блок СТАТУС ✅,
журнал §18.7 вынесен ✅, три противоречия из 3.3 устранены ✅ (P3009-риск снят, бэкапы Этап 0.3
`~~зачёркнуты~~`, `trustedClients` в §3.1 актуален), §3 помечен снимком ✅, §0 Открытые хвосты ✅,
3.6-мелочи все закрыты ✅ (пустой Этап 9 удалён, 0.5/0.6 переставлены, легенда статусов есть,
РКН в §10 Рисков есть, §21-коллизия разрешена перенумерацией в `PLAN-INFRA.md`).
**Блокеры:** —
**Следующий шаг:** §27 закрыт, свободный трек. Единственный оставшийся пункт DoD §27 не про
приватность, а про объём файла: `PLAN.md` — 1671 строка при ориентире ~1200 (блок СТАТУС + «§0
Открытые хвосты» компенсировали вычищенное). Дальнейшее сокращение — не приватность, а читаемость;
не блокирует, можно не делать.
...
| Чеклист Этапа 0.8 (152-ФЗ) не отмечен ни по одному приложению                      | 0.8  | Не начат построчный аудит                                    |
