...
- [ ] Сквозная e2e-проверка фикса регрессии на mandala — juнит-тесты зелёные (115/115), но
      живой прогон после фикса не переделан: параллельно в той же рабочей директории шла чужая
      сборка mandala (конкуренция за порт 3004 и `.next`), результаты были ненадёжны. Смотри
      фоновую задачу «Fix 6 pre-existing mandala e2e failures» и/или `apps/mandala/PLAN_COMPLETED.md`
      (запись v0.41.0) для актуального статуса.
- [ ] `libs/image-upload/README.md` — как и в §29, документация клиентского API догоняет отдельно
      (агент `image-upload-docs`, тред `image-upload-server-helper`).
