...
> **ответ ещё не пришёл** на момент записи.
>
> **➡️ Следующий старт:** (1) проверить статус выполнения rename у BlackCove — если сеть ещё не
> пересоздана, дождаться/повторно запросить; (2) продолжить тираж §18.6 Сессии J (кандидаты:
> `pravda` ✅, `kami-key-the-landing` ✅, `letar-landing` (ждёт пилота), `animatrona-landing`,
> `dsperevod`, `aboi`, `umami`, `kami`); (3) проверить статус ротации `aprel8008`/`form-example`,
> если не закрыто предыдущей сессией.
...
| **J** | ⏳ начат досрочно (сессия №69, 2026-07-12, независимо от I — rollout не требует hard gate) | Тираж на остальные приложения пачками 3–5 через doctor-чек-лист; проверка, что host-порты нигде больше не используются (мониторинг!); blue-green fallback задокументирован | 5/~19 SERVER_APPS на rollout (`time`, `form-docs`, `pravda`, `kami-key-the-landing`, `letar-landing` — все ✅ чистые пилоты, подробности выше в шапке файла). Найден и закрыт баг детектора label в `deploy-affected.sh` (`4fbc414`), важен для всего тиража. `form-example` и `mandala` — обычный (не-rollout) деплой закрыт, `letar.rollout` пока выключен (mandala — период стабильности после прод-инцидента сессии №70; form-example — найден отдельный незаблокированный баг `/products` ECONNREFUSED, сессия №72, закрыт сессией №73). `animatrona-landing` — compose смигрирован (commit `986d8da`), `doctor` 8/8 READY, запрос пилота отправлен BlackCove (thread `deploy-animatrona-landing-rollout-J`) — **ждёт выполнения**. Осталось пройти тиражом: `dsperevod`, `aboi`, `umami`, `kami`, затем `archetest`/`grandslamcup`, затем `auth-hub`/`driving-school` последними (риск по возрастанию) |
