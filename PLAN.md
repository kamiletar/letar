...
- [ ] `apps/aboi/tsconfig.json` — по репорту одного из агентов, уже красный на чистом HEAD
      (`TS6059` на `libs/forms`) независимо от этого фикса. Требует отдельного разбора.
