...
1. **Уведомление в РКН** — вероятно, не подано ни для одного приложения. Подать через pd.rkn.gov.ru для каждого домена
   (или по оператору, если все под одним ИП/ЮЛ). Получить PDF с номером оператора → занести в README/PLAN приложения.
