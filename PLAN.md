...
**Часть 3 (защита от повторения) и часть 4 (доки) — не начаты**, вынесены отдельными задачами
(см. spawn_task ниже): скрипт-гейт по схемам, алерт в dashboard, синтетическая проверка входа,
переписать `better-auth-1.7-account-issuer-field.md` (сейчас там неверные цифры — 4 приложения
вместо 14, «только sign-up» вместо «обычный вход тоже»).
