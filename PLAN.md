...
> (3) 🔴 находка сессии v0.6.4 — логин-формы без `method="post"` до hydration сабмитятся
> нативным GET, пароль утекает в URL (воспроизведено на auth-hub `/sign-in`); нужен аудит
> логин-форм всего монорепо (aboi/dsperevod/driving-school — тот же паттерн вероятен),
> детали в apps/auth-hub/PLAN.md «Бэклог».
