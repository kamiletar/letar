...
## СТАТУС (обновлено: 2026-08-12)

### ▶ Следующий старт — деплой-запросы BlackCove (traefik-секреты, dashboard/time GlitchTip) и продолжение тиража

Сессия 2026-08-12 — инфраструктурные треки `PLAN-INFRA.md`: **§18.8.1 закрыт по существу**
(конвейер SOPS-секретов для `infra/*` — `scripts/deploy-infra.sh`, ручка `deploy_infra` в
`deploy-mcp`, первый потребитель `infra/traefik` мигрирован живьём с s3), **§18.8 тираж закрыт**
(`.env.staging.enc` заведён на всех 11 оставшихся приложениях, включая попутный фикс дыры в
корневом `.gitignore`), **§70 продолжен** — `dashboard`/`time` получили DSN (проекты созданы в
GlitchTip через Django-shell на s3, тем же способом, что `studio`), тираж на остальные ~15
кандидатов (`PLAN-INFRA.md` §70 п.7) не начат. Ни один из трёх треков не подтверждён живым
деплоем на сервере — везде готовы секреты/код, но `deploy_infra`/`deploy_app` ещё не
прогонялись, запросы BlackCove отправлены. Auth-план (этот файл) в этой сессии не двигался.
