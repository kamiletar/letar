...
> **🟡 `archetest` — вторая попытка резолва failed-миграции, `--rolled-back` не сработал, перешли
> на `--applied`:** BlackCove (msg #398/#400, thread `deploy-archetest-rollout-J`) диагностировал:
> `20260321000000_baseline` (2026-03-23) — дубликат уже применённого `20260321075436_baseline`
> (2026-03-21). Первая попытка резолва (`--rolled-back`, санкционирован пользователем) при
> повторном деплое заставила Prisma **заново применить DDL** — упала на той же строке (`P3018`,
> `type "UserRole" already exists`), деплой снова прерван до rollout, даунтайма не было (2-я
> неудачная попытка). BlackCove сам скорректировался: `applied_steps_count: 0` — не «ничего не
> сделано», а следствие самого сбоя; правильный резолв — `--applied` (пометить выполненной без
> повторного запуска SQL, целевое состояние схемы уже достигнуто дубликатом-предшественником).
> **Пользователь подтвердил `--applied`** — исполнение в работе. Договорённость с BlackCove: если
> и третья попытка деплоя упадёт на миграциях — остановиться, не пробовать дальше без более
> глубокого разбора. `grandslamcup` (compose готов, `f6fb9ca`, `doctor` 8/8) — на паузе.
>
...
> `deploy-archetest-rollout-J` — ждём `resolve --rolled-back` + повторный деплой archetest от
> BlackCove; если завершён, зафиксировать в PLAN.md; (2) после закрытия archetest — отправить
> запрос пилота для `grandslamcup` (compose уже готов, commit `f6fb9ca`); (3) затем
> `auth-hub`/`driving-school` последними, риск выше — не мигрировать с ходу без дополнительного
> анализа, сверяться с пользователем; (4) когда все ~20 приложений подтверждённо переехали на
