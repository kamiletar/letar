...
| ⚠️ Открытый вопрос: аудит `proxy.ts` на баг класса studio (2026-08-20 — matcher без dot-исключения 404-ил metadata-роуты типа `icon.tsx` через next-intl redirect) не завершён. `domwellbes`/`svoichuzhie` проверены 2026-08-21 — ложная тревога (нет next-intl вовсе). Не проверены: `aira-web`, `time`, `archetest`, `mandala`, `kami`, `aboi`, `dashboard`, `aprel8008`, `dsperevod`, `driving-school` — бага стоит ждать только там, где реально подключён next-intl | —    | Не блокирован; требует времени на прогон по списку       |
