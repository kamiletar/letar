...
>
> **Найдено, отдельная задача (НЕ решать в рамках §18):** `01-public.spec.ts` содержит тесты на
> `page.goto('/')`, ожидающие контент city-дашборда («Ближайшие матчи», nav-ссылки «Расписание»
> и т.д.) — структурно невозможно, `/` сейчас city-selector, не дашборд конкретного города.
> Похоже на тест-сьют, не обновлённый под мультигород-редизайн приложения — нужно решение
> владельца фичи: обновить тесты (навигация на конкретный город перед проверкой) или это сигнал,
> что `/` должен что-то делать иначе при одном активном городе.
>
...
> в стале-структуру `01-public.spec.ts` (отдельная задача выше) и, возможно, `/teams`-пустоты.
