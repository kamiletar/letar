...
### ▶ Следующий старт — тираж e2e-гейта (§18.7): точечные UI-баги, найденные им (mandala/pravda/form-example/kami), либо план M3 (перевод s2 на Traefik). **После закрытия тиража e2e-гейта** — переходить к тиражу TS7 (§19 в PLAN-INFRA-1.md): §19.1 (гейт типов в деплое) намеренно поставлен ниже по приоритету владельца, пока тираж e2e не закрыт.
