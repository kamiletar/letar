...
> **Статус:** ✅ план утверждён, реализация идёт. **Сделано:** Этап 1 + код-часть Этапа 0 (сессия №1); Этап 2 эталон aboi (сессия №5) + тираж на dsperevod (сессия №6); реестр hub-клиентов → БД (сессия №7); **Этап 0.1 ✅ ПОЛНОСТЬЮ** (сессия №8); **Этап 1.5 ⏳ В РАБОТЕ** — фабрика `createAuth()` реализована + эталоны dsperevod/time (сессия №9).
> **➡️ Следующий старт:** **Этап 1.5 — завершение DoD** (README + E2E behavior-parity dsperevod) → затем **Этап 2 ремедиация застрявших** или **Этап 3 admin «Пользователи»**.
