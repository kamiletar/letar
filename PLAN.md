...
>
> **➡️ Следующий старт: вход по любому linked-email (Этап 8.5).** Тираж §2.3/Этап 8 на aboi и
> driving-school завершён, Этап 8.5 self-service email + merge-скрипт завершены (2026-07-16, см.
> записи выше). Осталось: перехват резолва sign-in в Better Auth, чтобы пользователь мог войти по
> любому подтверждённому `UserEmail`, не только по основному `User.email` — риск для core
> auth-flow ~10 downstream-приложений (hub-provider), нужен spike перед реализацией. Второй
> свободный конец — реальное исполнение Tier 1-перехода когда появится первый запрос (Этап 8),
> ниже приоритетом.
...
- **Фича:** управление своими email в профиле (как GitHub) — привязка/подтверждение нескольких адресов к
  ...
  **разных** адресов нужна кастомная merge-логика (прообраз — `mergeAnonymousAccount` в aboi).
  ...
  `cookieCache`, см. запись в шапке плана). **Не сделано:** вход по любому linked-email — требует перехвата
  резолва sign-in в Better Auth (риск для core auth-flow ~10 downstream-приложений), отдельная задача.
