...
> **Статус:** ✅ план утверждён, реализация идёт. **Сделано:** Этап 1 + код-часть Этапа 0 (сессия №1); Этап 2 эталон aboi (сессия №5) + тираж на dsperevod (сессия №6); реестр hub-клиентов → БД (сессия №7).
> **➡️ Следующий старт:** **Этап 1.5 — серверная абстракция `createAuth()`** (новый трек, основная цель ревизии №3) —
> спроектировать и внедрить фабрику режимов (standalone / hub-client / hub-provider) ДО тиража (Этапы 3–7).
> Параллельно: **Этап 2 → ремедиация застрявших** (п.3) или **Этап 3 → admin «Пользователи»**. Инфра-часть 0.1 — s2 + deploy.
...
>   ⏳ Деплой на s2: push + `nx run auth-hub:db:seed` на сервере + перезапуск контейнера.
...
### Этап 1.5 — Серверная абстракция `createAuth(profile)` ⭐ ОСНОВНАЯ ЦЕЛЬ (ревизия №3)
...
1. **Spike (1–2 дня) — финализация контракта §4.** Проверить по докам Better Auth: единообразная сборка
   `socialProviders`/`plugins` из конфига; подключение `genericOAuth` на OIDC-discovery Ключницы (`hub-client`);
   совместимость с **enhanced Prisma** (dsperevod/premium) как adapter; примирение **3 моделей ролей**
   (`role` / `roles: string[]` / `roles: UserRole[]`). Зафиксировать финальную сигнатуру.
2. **Реализация `createAuth()` в `@letar/auth/server`** — режимы `standalone` / `hub-client` / `hub-provider`;
   источник соц-секретов (`env` / `db`); подключение верификации (базовая Better Auth или pin-auth). Тесты Vitest.
3. **Эталон-миграция: 2 приложения, behavior-parity (E2E зелёный до/после).**
   - **`standalone`** → **dsperevod** (уже на `@letar/email` + resend — чистый кандидат): `auth.ts` → декларация
     `createAuth({ mode: 'standalone', ... })`.
   - **`hub-client`** → **archetest** или **time** (чистый OIDC-клиент Ключницы, без локального — проще kami).
     Доказать оба режима на фабрике. ⚠️ **kami НЕ берём в 1.5** — это сложный гибрид (local + Ключница), его полный
     объём — Этап 6 (уже на готовой фабрике).
   - **`hub-provider`** (auth-hub) — фабрика лишь **поддерживает** режим; реальная миграция auth-hub — в Этапе 8
     (вместе с управлением секретами), чтобы не трогать рабочий OIDC-провайдер раньше времени.
4. **Под-вопросы (решить в spike, влияют на §10):**
   - Реестр hub-клиентов (`trustedClients`) — оставить хардкод-массив или вынести список в БД/env (лёгкая динамика
     **списка клиентов**, НЕ провайдеров — это не D8). От этого зависит, как «коммерс становится hub-клиентом».
   - Внешний коммерс как hub-клиент — `trusted` (skip consent) или обычный (показывать consent другому владельцу)?
   - Подключение pin-auth хуков на уровне `libs/` (cross-lib резолв — отложено в Этапе 1).
