...
- **Регистрация hub-клиента** — `trustedClients` сейчас хардкод-массив + redeploy. «Коммерс становится hub-клиентом»
  через админку требует реестра клиентов в БД/env (под-вопрос Этапа 1.5 п.4) — иначе self-service невозможен.
