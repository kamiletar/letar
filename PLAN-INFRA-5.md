...
## §77 — Новый паттерн-документ: гидратационный мисматч от чтения localStorage в инициализаторе `useState` ✅ ЗАКРЫТО (2026-08-13)
...
## §82 — Приватный bind-mount вне `uploads/` не покрыт Resilio-синхронизацией ✅ ЗАКРЫТО (2026-09-08) — ложная тревога, deny-list синхронизирует его по умолчанию
...
### Что выяснилось (2026-09-08, `repo-dev`)

Открытый вопрос был основан на неверной модели. `Resilio` синхронизирует **весь** `dir`
(корень репозитория на сервере) за вычетом `.sync/IgnoreList` — это **deny-list**, не allow-list.
«Синхронизируются только uploads/backups» было приблизительной формулировкой стратегии
(«что мы туда специально кладём»), а не описанием механизма («что физически летит по сети»).

Проверено эмпирически по обоим концам:

- На s2: `apps/aboi/print-sources/` физически существует, 0 файлов (владелец `root:root` —
  каталог создан вручную при первом запуске bind-mount, `deploy` внутрь ещё ничего не писал).
- В `.sync/IgnoreList` на s2 нет ни одной записи `print-sources` (только ложное текстовое
  совпадение с `dprint.json`).
- В оффсайт-копии `C:\BackupSync\letar\s2\apps\aboi\print-sources\` — тот же пустой каталог,
  то есть Resilio его уже видит и синхронизирует, просто синхронизировать пока нечего.

### Решение

Ничего регистрировать не нужно — deny-list модель уже закрывает произвольные новые bind-mount
«бесплатно», пока их имя не совпадает с одним из паттернов `IgnoreList` (`src`, `data`, `content`
и т.п. — см. список в доке). Единственное реальное действие — привести формулировку
`.claude/docs/backup-architecture.md` в соответствие с механизмом, чтобы не создавать то же
ложное предположение снова (**сделано**, добавлено уточнение 2026-09-08 в § «Исключения из
синхронизации»).

### DoD

- [x] Решено: точечная регистрация новых bind-mount не нужна — deny-list покрывает их по
      умолчанию; следить нужно только за коллизией имени с паттернами `IgnoreList`
- [x] `.claude/docs/backup-architecture.md` — уточнён механизм (deny-list, не allow-list)
- [x] `apps/aboi/print-sources/` — проверено напрямую (SSH на s2 + локальная Windows-копия):
      уже в Resilio scope, синхронизация работает
...
## §111 — обход блокировки Claude Browser tool на dev-session bypass ✅ ЗАКРЫТО (2026-08-26, сессия `aboi`)
