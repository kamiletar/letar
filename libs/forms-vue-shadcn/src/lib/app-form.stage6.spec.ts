...
describe('Этап 6 (часть 1) — Likert/MatrixChoice', () => {
  ...
  it('FieldMatrixChoice (radio): таблица содержит строку на каждый row и колонку на каждый column', () => {
    ...
    expect(wrapper.find('[data-field-name="matrixRadio"]').exists()).toBe(true)
    expect(wrapper.findAll('[data-field-name="matrixRadio"] tbody tr')).toHaveLength(rows.length)
    expect(wrapper.findAll('[data-field-name="matrixRadio"] thead th')).toHaveLength(columns.length + 1)
  })

  it('FieldMatrixChoice (radio): клик по ячейке выставляет значение строки, только одно на строку', async () => {
    const wrapper = mount(Stage6TestForm())
    const firstRowCells = wrapper.findAll('[data-field-name="matrixRadio"] tbody tr')[0]!.findAll('[role="radio"]')
  ...
  it('FieldMatrixChoice (checkbox): позволяет выбрать несколько колонок в одной строке', async () => {
    const wrapper = mount(Stage6TestForm())
    const firstRowCells = wrapper.findAll('[data-field-name="matrixCheckbox"] tbody tr')[0]!.findAll(
      'input[type="checkbox"]',
    )
  ...
  it('FieldMatrixChoice (rating): клик по звезде отмечает её выбранной', async () => {
    const wrapper = mount(Stage6TestForm())
    const firstRowCells = wrapper.findAll('[data-field-name="matrixRating"] tbody tr')[0]!.findAll('button')
