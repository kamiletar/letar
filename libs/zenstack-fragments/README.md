...
> Ключи реестра `createForm` (`@meta("form.fieldType", "Select.<Имя>")`) во фрагменты намеренно не кладём: фрагмент
> общий для приложений, а компонент справочника есть только в `createForm` конкретного приложения. Ключ ставится в
> схеме самого приложения.
