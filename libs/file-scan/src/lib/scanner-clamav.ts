...
/**
 * По умолчанию — 100 МБ, ровно `StreamMaxLength` из конфигурации `clamd` на сервере
 * (`infra/clamav/docker-compose.yml`, `CLAMD_CONF_StreamMaxLength`). Файл больше лимита clamd
 * не сканирует, а отвечает ошибкой, поэтому режем на своей стороне: так в audit попадает
 * внятный `SIZE_LIMIT`, а не разбор чужого текста ошибки.
 *
 * ⚠️ Поднято с 25 МБ (стоковый дефолт `clamd.conf.sample`) 2026-09-11 под загрузку чертежа
 * архитектора целиком (DXF-раздел проекта дома — десятки МБ, apps/domwellbes/PLAN.md §
 * «загрузка чертежа архитектора как есть»). Меняешь один — меняй и второй, иначе один из двух
 * пределов остаётся старым независимо от того, что реально настроено на другой стороне.
 */
