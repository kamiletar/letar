...
const KNOWN_DIRECTIVES: DirectiveInfo[] = [
  ...
  },
  {
    ...
    description:
      ...
      + 'С forms-react v0.7.0 UI-пропсы резолвятся и в явных типизированных тегах (<AppForm.Field.Currency '
      + 'name="x" />), не только через Form.Field.Auto — useResolvedFieldProps подтягивает meta.fieldProps, '
      + 'createField мержит с приоритетом props > meta. До v0.7.0 произвольный fieldProps (minorUnitScale, '
      + 'currency, showValue, layout, count...) работал только через Auto — см. историю разрыва в '
      + '.claude/docs/letar-forms-fieldprops-typed-tags-not-resolved.md.',
