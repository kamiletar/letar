...
- **Промис-путь у `Field.Combobox`** — паритет с `@letar/forms` 2.23.0: `loadOptions`, `loadSelected`, `onLoadError`,
  `debounce`, `getLabel`/`getValue`; спиннер в поле, прошлая выдача при новом запросе, «Не удалось загрузить» и
  «Повторить». `loading` у статичных `options`. Ровно один источник опций — в типах.
- Выбор опции подставляет её подпись в поле ввода; подпись выбранного значения показывается при монтировании
  (`initialLabel`, статичные `options` или `loadSelected`), список при этом не фильтруется по ней.

## [0.43.0] - 2026-09-26

### Added

- **`loading` у `Field.Select`** — паритет с `@letar/forms` 2.22.0: спиннер, «Загрузка...» в пустом списке и в кнопке
  вместо placeholder, пока у значения нет опции.
...
- **`onCreate`/`createLabel` у `Field.Select` и `Field.Combobox`** (паритет с `@letar/forms`
  2.17.0): пункт «+ Добавить…» в конце списка, `{ label, value }` — опция создаётся и
  выбирается, `null` — без изменений.

### Fixed

- **`Field.Select` с опцией `value: ''` («Все категории»)**: Radix трактует `''` как «сброс», и
  триггер показывал placeholder вместо подписи опции — значение маппится в служебный токен внутри
  поля и возвращается в `''` наружу.
...
- **Мок Canvas 2D API в `vitest.setup.ts`** — `FieldSignature` вызывает `canvas.getContext('2d')`
  уже при монтировании (`initCanvas` в `useEffect`), и все 5 тестов `field-signature.spec.tsx`
  грузили настоящий нативный `.node`-аддон пакета `canvas` (peer dependency jsdom) без мока. Тот
  же класс флаки, что был найден и исправлен в `@letar/forms` (2.14.9): при test isolation аддон
  переинициализируется в каждом spec-файле заново, под нагрузкой первая загрузка иногда превышает
