 ...
 * Портировано из `@letar/forms` (Chakra) без изменений — сами SVG не Chakra-специфичны.
 */
const BRAND_ICONS: Partial<Record<CardBrand, (s: number) => ReactElement>> = {
  visa: (s) => (
  ...
  mir: (s) => (
    <svg width={s} height={s * 0.3} viewBox="0 0 400 120" fill="none" aria-label="МИР">
      <linearGradient id="mir-g-shadcn" x1="370" x2="290" gradientUnits="userSpaceOnUse">
      ...
      <path
        ...
        fill="#0F754E"
      />
      <path d="m382 53c4-18-8-40-34-40h-68c2 21 20 40 39 40" fill="url(#mir-g-shadcn)" />
    </svg>
  ),
}

/** Заглушка для неизвестного бренда — generic card icon */
function GenericCardIcon({ size }: { size: number }): ReactElement {
  return (
...
/**
 * Иконка бренда карты (shadcn-скин).
 * Рендерит inline SVG (CC0 лицензия, Simple Icons + Wikimedia).
 */
export function CardBrandIcon({ brand, size = 32 }: CardBrandIconProps): ReactElement {
