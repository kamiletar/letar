 ...
 * `form-registry-keys` пишется всегда (этап Е), поэтому экспортируется безусловно.
 */
function generateIndexCode(enumNames: string[], modelNames: string[]): string {
  ...
  exports.push(`export * from './form-registry-keys'`)
...
export async function generate(context: CliGeneratorContext): Promise<void> {
  ...
  // Этап Е: список ключей реестра createForm (пишется всегда, пустые пространства — [])
  const registryData = collectRegistryKeys(modelInfos)
  await writeFileWithDir(join(outputDir, 'form-registry-keys.ts'), generateRegistryKeysCode(registryData))

  // Предупреждения по form.relation.* (модель/поля не найдены) и по ключу вместе с relation
  const schemaFields = new Map(models.map((m) => [m.name, new Set(collectAllFields(m).map((f) => f.name))]))
  for (const warning of findRegistryWarnings(modelInfos, schemaFields)) {
