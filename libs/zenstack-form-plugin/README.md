...
### Массив объектов (`options: [{ value, label }]`) — только legacy comment-синтаксис
...
индексируется по массиву объектов (`form.props.options.0.value` не работает). Для select-полей с
опциями вида `{ value, label }[]` единственный рабочий путь — legacy comment-директива:
...
Это **постоянный, легитимный escape-кейс**, не временный след недомигрированного кода — кодмод
(`scripts/codemod-form-directives.mjs`) намеренно его не трогает, конвертировать в `@meta` для
объектных массивов некуда. Живой прецедент — `Content.category`/`Content.quality`/`Report.reason`
в `animatrona-tracker` (2026-09-04): попытка написать `@form.options([...])` вместо
`@form.props({ options: [...] })` — несуществующая директива, молча игнорировалась (см. warning
ниже, добавленный именно из-за этого случая).

### Warning на неизвестную директиву (v3.2.0)

И comment-синтаксис (`@form.<key>`), и `@meta("form.<key>", …)` молча игнорируют любой `<key>`,
не входящий в распознаваемый набор (`title`/`placeholder`/`description`/`fieldType`/`props`/
`relation`/`exclude`) — опечатка или несуществующая директива (`@form.options`, `@form.widget`)
не даёт ошибки ни на этапе `zenstack generate`, ни при типизации: поле просто остаётся без нужных
метаданных. С v3.2.0 `nx zenstack:generate` печатает `console.warn` для каждого такого случая —
с именем модели/поля, самим неизвестным ключом и списком поддерживаемых. Не ломает сборку, только
предупреждает — как и существующий deprecation-warning на legacy-синтаксис.
