...
- **Экранирование `/` в regex-паттерне.** `constraints.pattern` вставлялся как `.regex(/${pattern}/)`:
  паттерн с `/` (например `^https?://…`) завершал литерал раньше времени и давал невалидный TS.
  Новый `quoteRegexLiteral` (`ts-literal.ts`) экранирует `/` и переводы строки, а уже
  экранированные автором пары с обратным слэшем не трогает — обычный паттерн не меняется ни на байт.

## [4.1.2] - 2026-09-19

### Fixed

- **Экранирование строк в constraints и `fieldType`.** Продолжение 4.1.1: значения
  `constraints.startsWith`/`endsWith`/`contains` (`.startsWith('…')`, `.endsWith('…')`,
  `.includes('…')`) и `fieldType` вставлялись в одинарные кавычки как есть —
  `@meta("form.props.startsWith", "d'or")` давал невалидный TS. Теперь они идут через
  `quoteTsString`. Обычный текст не меняется ни на байт. Остальные вставки `'${…}'` в плагине
  (имена моделей, полей, enum-значений, ключи i18n, имена атрибутов) — идентификаторы, не
  пользовательский текст, и оставлены как есть.

## [4.1.1] - 2026-09-19

### Fixed

- **Экранирование строк в сгенерированном коде.** `title`/`placeholder`/`description` поля,
  `label` значения enum (`EnumLabels` и `options`) и строковый `@default(...)` вставлялись в
  одинарных кавычках как есть: `@meta("form.title", "Prix d'or")` давал невалидный TS. Теперь
  новый `quoteTsString` (`ts-literal.ts`) экранирует `\`, `'`, переводы строк и U+2028/2029.
  Обычный текст не меняется ни на байт — `zenstack generate` на `form-example` и
  `form-develop-app` дифа не даёт.
...
- **`@meta("form.tooltip.<title|description|impact|example>", "…")` — подсказка поля.**
  ...
  - Значение сериализуется `JSON.stringify`: тултип — проза, кавычки и апострофы не ломают литерал.
...
- **README — почему `extractEnumLabel` (`///` на значении enum) остался на doc-comment, а не
  переехал на `@meta`.** Вопрос от studio-dev/владельца: единственный синтаксис директив с
  Фазы 4 — `@meta("form.*", value)`, legacy `/// @form.*`-парсер убран целиком, почему тогда
  label enum-значений всё ещё через комментарий. Ответ — не тот же механизм и не входил в
  миграцию: `extractEnumLabel` существовала всегда отдельно, синтаксического запрета на `@meta`
  на значении enum нет (`EnumField.attributes` в AST это допускает), но смысла нет — `@meta` не
  умеет объектный литерал, а `///`-комментарий уже и так конвенция для документирования enum.
  Явно задокументировано, чтобы вопрос не повторялся.
