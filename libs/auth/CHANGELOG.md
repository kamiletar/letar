...
- **Регрессионный тест на TODO из `create-dev-session-route.ts`**: сессия, созданная фабрикой,
  теперь прогоняется через настоящий `betterAuth().api.getSession()` (не мок) поверх
  `memoryAdapter` из самого better-auth — если формат подписи cookie или её имя разойдутся с
  реальным Better Auth в будущей версии, тест упадёт первым, а не бэкдор молча перестанет
  работать в проде/на staging незамеченным. Поводом послужила проверка domwellbes: сообщённый
  «сломанный dev-session bypass» на деле оказался тем, что `/admin/` (с завершающим слэшем)
  безусловно отдаёт 308 на канонический `/admin` — обычная нормализация Next.js, не связанная с
  аутентификацией; `fetch(..., {redirect:'manual'})` в браузере намеренно скрывает статус/Location
  таких ответов (`type: 'opaqueredirect'`), что и было принято за редирект на `/sign-in`.
