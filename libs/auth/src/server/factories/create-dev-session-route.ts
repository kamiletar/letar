 ...
 *
 * ⚠️ **Архитектурный компромисс:** cookie подписывается вручную по формату `better-call`
 ...
 * уже трижды ловили здесь). Регрессия на этот случай — `create-dev-session-route.spec.ts` (describe
 * «cookie распознаётся реальным betterAuth()»): создаёт сессию через эту фабрику и гоняет её через
 * настоящий `betterAuth().api.getSession()` (не мок) поверх `memoryAdapter` из самого better-auth —
 * упадёт первым, если формат разойдётся, вместо того чтобы бэкдор молча переставал работать.
 *
 * @example
 * ```typescript
