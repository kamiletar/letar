...
Модель `SocialProvider` остаётся per-app (ZenStack не шарит модели между независимыми БД), но admin
UI (список + создание/редактирование + server actions CRUD) — общий компонент, см.
[§ SocialProvidersSettings](#socialprovidersettings--self-service-oauth-ключи-tier-2) ниже.
...
рантайм-флаг): см. [§ AuthModeSettings](#authmodesettings--tier-1tier-2-informed-consent-этап-8-корневого-planmd).
