 ...
 *
 * `isPathAllowed` передаётся вызывающим кодом — whitelist путей специфичен для
 * конкретного приложения (см. `allowed-paths.ts`).
 */
...
/**
 * Создаёт обработчик запросов к media:// протоколу
 *
 * @param isPathAllowed Проверка whitelist — специфична для приложения (см. `allowed-paths.ts`)
 */
export function createMediaProtocolHandler(isPathAllowed: (filePath: string) => boolean) {
 ...
 */
export function registerMediaProtocol(scheme = 'media'): void {
 ...
 *
 * @param isPathAllowed Проверка whitelist — специфична для приложения
 */
export function setupMediaProtocolHandler(isPathAllowed: (filePath: string) => boolean, scheme = 'media'): void {
  protocol.handle(scheme, createMediaProtocolHandler(isPathAllowed))
}
