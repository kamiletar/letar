 ...
 * - внешние файлы (сканер внешних субтитров) — имя файла и имя папки;
 * - встроенные дорожки контейнера (ffprobe) — `title` из `stream_tags`
 *   и `disposition` из ffprobe.
 *
 * Модуль намеренно без Node-зависимостей (`path` не импортируется) — используется и в
 * main-процессе, и в renderer.
 */
