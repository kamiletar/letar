...
describe('Этап 8 — недостающие документные поля (BirthCertificate/ForeignPassport/DepartmentCode)', () => {
  ...
  it('FieldBirthCertificate: неверный формат показывает ошибку', async () => {
    const wrapper = mount(Stage8TestForm(), { attachTo: document.body })
    const input = wrapper.find('input[data-field-name="birthCertificate"]')

    await input.setValue('неверно')
    await nextTick()

    expect(wrapper.find('[data-field-name="birthCertificate"] .letar-field__error').exists()).toBe(true)
    wrapper.unmount()
  })

  it('FieldForeignPassport: маска "99 9999999", ошибка не на 9 цифрах', async () => {
    const wrapper = mount(Stage8TestForm(), { attachTo: document.body })
