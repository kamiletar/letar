...
/**
 * Резолвит колонки таблицы из schema и/или пользовательских определений.
 *
 * Портировано из `libs/forms-shadcn/src/lib/table/use-table-columns.ts` — там это React-хук
 * (`useMemo`), здесь обычная функция: Vue-версия вызывает её прямо внутри render-замыкания
 * `field-table-editor.ts`, которое и так пересчитывается только при реальном изменении реактивных
 * зависимостей (`schema`/`arrayPath`/`userColumns` в этом пакете не меняются после монтирования
 * формы) — отдельного мемо-слоя не потребовалось.
 */
