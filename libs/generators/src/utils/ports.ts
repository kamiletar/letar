 ...
 * **При правке регулярок меняй оба файла** — расхождение регулярок ловит
 * `libs/infra-config/src/port-parser.guard.spec.ts` (`nx test infra-config`). Дрейф самого
 * `extractPorts` он не поймает: сверяются только литералы регулярок.
 *
