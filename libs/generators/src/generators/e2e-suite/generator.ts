...
export default async function e2eSuiteGenerator(tree: Tree, options: E2eSuiteGeneratorSchema): Promise<void> {
  ...
  logger.info(
    `Прогон: подними dev-сервер вручную (nx run ${app}:dev) и вызови "bunx playwright test" из apps/${app}-e2e — ` +
      `"nx e2e ${app}-e2e" виснет в dev-режиме, см. .claude/docs/e2e-testing.md`
  )
}
