...
### Куда пойдёт production-деплой: `BUILD_ON_S1_APPS` перечитывается каждый раз

`deploy_app` с `target: "production"` перед выбором сервера читает `libs/infra-config/src/index.ts`
**текстом** и берёт `BUILD_ON_S1_APPS` из файла (`src/build-on-s1.ts`), а не из значения, вычисленного при
старте процесса. Приложение, добавленное в список (или убранное — откат пилота) после запуска MCP, действует
со следующего вызова, **перезапуск не нужен**. Если маршрут по актуальному списку отличается от того, что
процесс помнил при старте, ответ начинается со строки «BUILD_ON_S1_APPS перечитан из файла…».

- ⛔ Файл не читается или массив записан не литералом строк (spread, константа, вычисление) — `deploy_app`
  отказывает с причиной и **не обращается к агенту**. Молчаливого отката на s2 нет: сборка на прод-хосте
  вместо s1 — именно тот дефект, который это чинит. Формат записи — контракт, он назван комментарием у
  `BUILD_ON_S1_APPS`; охранный тест `build-on-s1.spec.ts` сверяет разбор реального файла с импортированной
  константой.
- `target: "staging"` список не читает (всегда s1) и битым файлом не блокируется.
...
- Перечитывается **только** `BUILD_ON_S1_APPS`; остальные константы `infra-config` (в том числе
  `HARD_GATED_APPS`) фиксируются при старте процесса. Сам код `deploy-mcp` подхватывается перезапуском.
