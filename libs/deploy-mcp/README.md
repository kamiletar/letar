...
статус записан фильтрованным прогоном (`grep`/`project`, поле `filtered: true`); или сам запрос
статуса не удался (сеть/туннель).

⚠️ Засчитывается только **полный** `run_e2e` — без `grep` и `project` (`workers` можно).
dashboard-agent (с 0.18.4) зелёный итог фильтрованного прогона в файл не пишет вовсе, красный
пишет как `passed: false` — точечная проверка может статус гейта только ухудшить. Статусы без поля
`filtered` (записанные раньше) читаются как полный прогон. Инцидент — domwellbes, 2026-09-23:
прогон одного теста записал `passed: true`.
