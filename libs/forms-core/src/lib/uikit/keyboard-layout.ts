/**
 * Перенесено из `@letar/fuzzy-search` (Select/Combobox `@letar/forms` ищут с учётом раскладки, а
 * `forms-core` — framework-free ядро, зависеть от чужой библиотеки ему нельзя). `@letar/fuzzy-search`
 * реэкспортирует функции отсюда.
 *
