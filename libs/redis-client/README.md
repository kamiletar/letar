...
### Лог ошибок схлопывается: первая — сразу, повторы — молча

`retryStrategy` переподключается бесконечно, поэтому при лежащем Redis событие `error` приходит
вечно: сначала раз в секунду, дальше раз в 30 секунд. Печать каждого события забивает
`docker logs` (у долгоживущих процессов вроде `dashboard-agent` это основной инструмент разбора
инцидентов) и маскирует настоящие ошибки.

Правила вывода:

- **первая ошибка печатается сразу** — момент падения Redis виден без задержки;
- её повторы (совпадает `err.message`) копятся молча;
- **ошибка с другим текстом** (`ECONNREFUSED` → `WRONGPASS`) — новая информация, печатается сразу
  и дублем не считается; в этой же строке отчёт о проглоченных повторах предыдущей;
- при восстановлении (`connect`) — одна строка `Соединение восстановлено, подавлено повторов: N`,
  после которой счётчик сбрасывается: следующее падение снова печатается сразу;
- первое подключение, если ошибок не было, проходит молча;
- `silent: true` подавляет всё, включая сообщение о восстановлении.

```
[redis] Ошибка: connect ECONNREFUSED 127.0.0.1:6379
... Redis лежит 20 минут, в логе тишина ...
[redis] Соединение восстановлено, подавлено повторов: 43
```

Состояние счётчика живёт внутри инстанса `createRedisClient()` — два разных вызывающих модуля
глушат ошибки независимо друг от друга.
