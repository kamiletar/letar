...
```tsx
import { Form, FormI18nProvider } from '@letar/forms'
...
### Влияет на десятичный разделитель числовых полей

`Form.Field.Number`, `NumberInput`, `Currency`, `Percentage` берут `locale` из `FormI18nProvider`
для парсинга и форматирования (`@internationalized/number` под капотом у Chakra `NumberInput`).
Без провайдера (или без `locale="ru"`) действует `en-US` — десятичный разделитель `.`, запятая в
поле ввода не распознаётся как дробная часть числа (обрезается). С `<FormI18nProvider locale="ru">`
в дереве провайдеров формы запятая работает как десятичный разделитель.
...
```tsx
import { getLocalizedValue } from '@letar/forms'
...
```tsx
import { ZOD_ERROR_CODES } from '@letar/forms'
...
```tsx
import { SIZE_ORIGINS } from '@letar/forms'
...
```tsx
import { STRING_FORMATS } from '@letar/forms'
