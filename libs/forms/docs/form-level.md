...
## Form.DirtyGuard — защита от потери данных

Пока форма изменена (`isDirty`), предупреждает об уходе. Ставится вручную (`<Form.DirtyGuard />`) или
включается автоматически на всех формах инстанса — см. «Автоматическая защита» ниже:

- закрытие вкладки и обновление страницы — нативный диалог браузера (`beforeunload`);
- клик по внутренней ссылке (`<a href>`, `next/link`) — своё окно подтверждения.
...
| Prop                | Тип                     | Описание                                                             |
| ------------------- | ----------------------- | -------------------------------------------------------------------- |
| `dialogTitle`       | `string`                | Заголовок окна подтверждения                                         |
| `dialogDescription` | `string`                | Текст окна подтверждения                                             |
| `confirmText`       | `string`                | Кнопка «уйти»                                                        |
| `cancelText`        | `string`                | Кнопка «остаться»                                                    |
| `message`           | `string`                | Текст `beforeunload` (современные браузеры показывают свой)          |
| `enabled`           | `boolean`               | `false` — отключить защиту (по умолчанию `true`)                     |
| `onBlock`           | `() => boolean \| void` | Вызывается перед блокировкой; `false` — пропустить без подтверждения |

**Тексты и i18n (v2.17.1+).** Порядок резолва каждого текста: проп → перевод приложения по ключу
`formDirtyGuard.<имя пропа>` (`FormI18nProvider` с `t`) → встроенный словарь ru/en по `locale` →
английский, если провайдера нет. Свою обёртку с русскими пропсами писать не нужно:

```tsx
<FormI18nProvider locale="ru">
  <Form.DirtyGuard /> {/* «Несохранённые изменения» / «Остаться» / «Уйти» */}
</FormI18nProvider>
```

**Автоматическая защита: `dirtyGuard` (v2.18.0+).** Не нужно ставить `<Form.DirtyGuard />` в каждую форму:

```tsx
// на инстансе — умолчание для всех форм; true или объект с текстами
const AppForm = createForm({ dirtyGuard: true })

<AppForm ...>              {/* защита включена */}
<AppForm dirtyGuard={false} ...>  {/* выключено: логин, фильтры, одноклик-действия */}
<AppForm dirtyGuard={{ dialogTitle: 'Уйти?' }} ...>  {/* свои тексты этой формы */}
```

- Тип пропа — `boolean | DirtyGuardOptions` (`dialogTitle`, `dialogDescription`, `confirmText`,
  `cancelText`, `message`, `onBlock`). Проп формы перебивает опцию инстанса: `false` выключает, `true`
  оставляет тексты инстанса, объект дополняет объект инстанса (тексты формы важнее; `undefined`
  значение не затирает).
- **По умолчанию выключено** — включает приложение в своём `createForm`. Проп работает и на базовом
  `<Form dirtyGuard>`.
- **Ручной `<Form.DirtyGuard />` внутри формы с автозащитой не даёт дубля**: пока он смонтирован,
  автоматическая уступает ему (одно окно, один `beforeunload`, тексты — ручного).
- **`dirtyGuard={false}` не отключает вручную поставленный `<Form.DirtyGuard />`** — явный компонент
  важнее выключателя; `false` гасит только автоматическую защиту.
- Тексты — по той же лестнице i18n (`formDirtyGuard.*`).
- Только React-скины на `@letar/forms`: у `forms-shadcn`, `forms-vue*`, `forms-angular` своего
  `createForm`/`Form.DirtyGuard` нет — паритет не требуется.

**Что не ловится (ограничения, не баги):**

| Способ ухода                                     | Ловится | Почему                                                                                                                                      |
| ------------------------------------------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| Закрытие вкладки, обновление                     | да      | `beforeunload`                                                                                                                              |
| Клик по внутренней ссылке (`<a>`, `next/link`)   | да      | перехват `click` в capture-фазе                                                                                                             |
| Программный `router.push()` / `router.replace()` | нет     | это вызов функции, а не событие; перехватить снаружи нельзя без подмены роутера                                                             |
| «Назад»/«Вперёд» браузера                        | нет     | `popstate` приходит уже после смены адреса, отменить его нельзя. Обход через фиктивную запись в `history` конфликтует с историей App Router |

Для программной навигации проверяйте вручную: `if (form.state.isDirty && !confirm(...)) return`
перед `router.push`. Отдельный хук (`useDirtyGuardNavigate`) в плане, но пока без потребителя.
...
Автоматическое сохранение данных формы с диалогом восстановления. Принцип —
[Ководство §188](https://www.artlebedev.ru/kovodstvo/sections/188/): пользовательский ввод
священен, закрытие вкладки/краш/перезагрузка не должны его стирать. Для любой нетривиальной
формы (создание/редактирование сущности, длинная форма — не одноразовый auth-экран) подключай
`persistence` по умолчанию, не только по напоминанию.
...
### ⛔ `excludeFields` — чувствительные поля никогда не попадают в снимок (v2.4.0+)

Пароль, номер карты, CVV, срок действия и другие auth/платёжные данные **не должны** сохраняться
в localStorage, даже если остальная форма персистится. `excludeFields` вычищает их из объекта
перед сериализацией — исключённые поля просто отсутствуют в `savedData` при восстановлении,
форма их не перезаписывает и оставляет как есть в `initialValue`:
...
Тот же параметр доступен и на низкоуровневом хуке `useFormPersistence` напрямую (если форма не
использует компонент `<Form>`, а собирает persistence вручную).
...
```tsx
import { useFormAnalytics, AnalyticsPanel } from '@letar/forms/analytics'
