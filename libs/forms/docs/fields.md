...
| Компонент                    | Описание                                     |
| ---------------------------- | -------------------------------------------- |
| `Form.Field.Select`          | Стилизованный Select, поиск с 10-й опции     |
...
### Поиск в `Form.Field.Select`: `searchable` (v2.21.0+)

В открытом списке `Select` с **10 и более опциями** появляется поле поиска с автофокусом — само, без пропа.
Chakra-скин; в shadcn-скине поля поиска нет (`searchable: true`/объект пишут одно предупреждение в консоль,
`'auto'`/`false` — молча), нужен поиск — `Form.Field.Combobox`.

| Значение `searchable`                                  | Что делает                                                              |
| ------------------------------------------------------ | ----------------------------------------------------------------------- |
| `'auto'` (по умолчанию)                                | Поле поиска, когда опций больше 9                                       |
| `false`                                                | Выключить                                                               |
| `true`                                                 | Всегда, даже при трёх опциях (`{ threshold: 0 }`)                       |
| `{ threshold?, placeholder?, emptyMessage?, filter? }` | `'auto'` со своими настройками; `filter(option, query)` — свой предикат |

Ещё пропы: `renderEmpty({ search }) => ReactNode` — своё содержимое «ничего не найдено».

- **Как ищет.** Подстрока в тексте опции (`getOptionText`) без регистра и диакритики, `ё` ≡ `е`. Запрос, набранный
  не в той раскладке, тоже находит: «ghbdtn» → «Привет», «,fyb» → «бани». Латинская опция находится по латинскому запросу.
  Название группы в поиске не участвует; группы, где ничего не осталось, исчезают.
- **Выбранное остаётся в кнопке**, пока список отфильтрован. Опция «Все категории» со значением `''` не сбрасывает выбор.
- **Клавиатура.** Печать фильтрует (typeahead закрытой кнопки не меняется); первая подходящая опция подсвечена —
  Enter выбирает её, стрелки ходят по списку, Escape закрывает и сбрасывает поиск. Пробел, Home и End работают как в
  обычном поле ввода.
- **Порог считается по полному списку** (с созданными опциями и наложенными правками, без служебного «+ Добавить»).
  Пока запрос непустой, поле поиска не пропадает, даже если опций стало меньше порога.
- **С `onCreate`.** Поиск пуст — пункт «+ Добавить…» → `onCreate('')`; в поиске есть текст и точного совпадения нет —
  «+ Добавить "текст"» → `onCreate('текст')`. Пункт идёт **после** фильтра. При пустом результате под сообщением стоит
  пункт создания, он подсвечен. С `createItem={false}` кнопку ставят сами: `renderEmpty` +
  `<Form.Field.Select.CreateButton />`.
- ⚠️ **Изменение поведения для существующих потребителей:** у Select с 10+ опциями печать после открытия теперь
  фильтрует список, а не прыгает typeahead-ом; перебор повторным нажатием одной буквы не работает. Выключается
