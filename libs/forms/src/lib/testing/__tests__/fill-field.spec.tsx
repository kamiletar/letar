...
const TestSchema = z.object({
  name: z.string().meta({ ui: { title: 'Имя', placeholder: 'Введите имя' } }),
  age: z.number().meta({ ui: { title: 'Возраст' } }),
})

function TextForm() {
  return (
    <Form schema={TestSchema} initialValue={{ name: '', age: 0 }} onSubmit={() => {}}>
...
describe('fillField', () => {
  ...
  it('выбрасывает ошибку если поле не найдено', async () => {
    ...
    await expect(fillField('nonexistent', 'test')).rejects.toThrow('fillField: поле "nonexistent" не найдено')
  })
