...
- **Правила:** по умолчанию карандаш только у выбранного значения, рядом с триггером в `Control` (триггер —
  `<button>`, вложенная кнопка недопустима); в пунктах списка — только в своём `renderOption` (клик не должен
  выбирать пункт, оговорки по клавиатуре — в доке). Перед окном закрывать выпадашку. Правило дублей обратное
