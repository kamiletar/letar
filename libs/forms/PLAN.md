...
- **Решение координатора:** наследуем шесть частых атрибутов (`@email`, `@length(min,max)`,
  `@gte`/`@gt`/`@lte`/`@lt`, `@regex`) — не весь список ZModel. `@form.props` остаётся источником
  истины при конфликте, миграция существующих директив не требуется.
- **Статус:** делегировано `forms-dev` (msg id 1104, `forms-task`), ждём реализации.
