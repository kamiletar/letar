...
- **Приоритет:** high
- **Статус:** Фаза 0 (spike) делегирована `forms-dev` 2026-09-04 — agent-mail msg `1109`,
  тред `forms-native-migration`, `topic: forms-task`. Фазы 1-4 не начаты и до итогов spike не
  начинаются. Узкая часть (наследование 7 атрибутов) закрыта отдельно в v2.3.0 — см. следующий
  пункт, он остаётся в силе и ничему тут не мешает.
...
**Приоритет 4 — публичные приложения экосистемы (мои, каскад по правилам координатора).**

- `apps/form-docs` — 6 MDX-гайдов в двух языках: `zenstack-plugin.{mdx,ru.mdx}` (по 19 вхождений
  каждый — переписывается целиком), `relation-fields.{mdx,ru.mdx}` (по 3), `mcp.{mdx,ru.mdx}`
  (по 4), плюс `src/app/llms.txt/route.ts`. **Обе локали синхронно**, иначе RU и EN разъедутся
  по рекомендованному синтаксису. После — деплой через `deploy-agent-dev`.
- `apps/form-example` — `schema.zmodel` (кодмод), `README.md`, `CHANGELOG.md`. Витрина для внешних
  пользователей: то, что здесь написано, копируют себе.
- `apps/form-develop-app` — демо-страницы; сюда же добавляются демо новых возможностей
  (`@@validate`, `@trim`/`@lower`, `message`) по мере фаз 1-2, а не только в фазе 3.

**Приоритет 5 — статьи (`libs/forms/articles/`, 14 штук).**

⚠️ **Планировочная сцепка, которую легко пропустить: статьи ещё не опубликованы**
(`ARTICLE.md` — «Часть 3 → публикация по графику — НЕ НАЧАТО»), а материал готов. Опубликовать
их до миграции — значит выпустить 14 статей, обучающих синтаксису, который мы сами депрекейтим,
и потом либо жить с расхождением, либо править опубликованное. **Решение: публикация ждёт
фазу 3 либо стартует только теми статьями, где `@form.*` не упоминается.**

- `articles/08-zenstack-pipeline.md` — 31 вхождение, статья ровно про этот конвейер, переписывается
  целиком.
- `articles/11-mcp-ai.md` (5), `12-open-source.md` (2), `benchmarks.md` (2), `visuals-needed.md` (1).
- `ARTICLE.md` (3) — план публикации; там же обновить статус-таблицу.
- `articles/images/08-zenstack-pipeline.svg` — **`@form.` нарисован внутри диаграммы**, картинку
  нужно перерисовать, а не только текст. В `ARTICLE.md` она числится `done` — статус вернётся
  в «нужна переделка».
- Остальные 10 статей `@form.*` не упоминают — их публикацию миграция не блокирует.
