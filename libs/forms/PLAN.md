...
- **Приоритет:** normal
- **Описание:** `extractAllErrors` (`libs/forms/src/lib/declarative/form-errors.tsx:88`) собирает
  сообщение как `` `${fieldPath}: ${issue.message}` `` — пользователь видит технический ключ
  (`consentAccepted`), а не подпись поля.
- **Осложнение (найдено при триаже):** в `libs/forms` сейчас нет реестра field label,
  доступного `<Form.Errors>` — `useDeclarativeForm`/`form-context.tsx` его не ведут. Значит
  фикс не точечный: либо поля регистрируют свою подпись в контексте формы при монтировании
  (снять на анмаунте), либо `FormErrors` резолвит подпись из Zod `.meta({ ui: { title } })` по
  `fieldPath`, если схема доступна в контексте. Оцени, что дешевле и не потянет ли реестр
  лишний ре-рендер формы при каждом монтировании поля.
- **Если подписи нет** (динамическое поле без label, path не резолвится в схему): не показывать
  сырой ключ вообще, только `issue.message`.
- **Тесты:** `form-errors.spec.tsx` — оба случая (с подписью / без). Перед мержем грепни e2e на
  ассерты, завязанные на текущий формат `<field>: <message>` (например `consentAccepted:`) —
  иначе фикс тихо сломает существующие тесты.
- **Статус:** ожидание
