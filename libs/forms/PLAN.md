...
- **⚠️ Формат работы — НЕ обычная задача форм-дева с ходу.** Ками прямо сказал: нужна отдельная
  **исследовательская сессия**, а не сразу реализация. Библиотечное решение, продуманное, с
  ...
  4. **Ками лично контролирует качество и возможности** — не автономная реализация как остальные
     ...
     в отчёте.
- **Статус:** ✅ **исследование проведено 2026-08-12** (Ками инициировал сессию лично). Результат —
  [MASK_ENGINE.md](./MASK_ENGINE.md): боли существующих библиотек с цитатами мейнтейнеров,
  разбор архитектуры современных решений, UX/a11y-доказательная база, предложенная архитектура.
  Реализация **не начата** — ждёт решения по порядку работ.
...
### Этап 1. Ядро: чистые функции без DOM

📍 `libs/forms-core/src/lib/mask/` → новый subpath `@letar/forms-core/mask`

- [ ] **Модель токенов.** Встроенные `9` (цифра), `a` (буква), `*` (буква или цифра),
      экранирование литералов. Пользовательские токены с двумя свойствами:
      `pattern` (алфавит — набор символов или предикат) и `transform` (замена символа:
      латиница→кириллица, верхний регистр, гомоглифы). Опциональные позиции для переменной
      длины хвоста.
- [ ] `format(raw, opts)` / `unformat(value, opts)` — прямое и обратное преобразование.
- [ ] `formatToParts(value, opts)` — разметка каждого символа (`input` / `literal` /
      `placeholder`), на ней строится вычисление каретки.
- [ ] `caretBoundary(value, opts): boolean[]` — карта допустимых позиций каретки.
      Переопределяема скином (нужно числовым форматам).
- [ ] **`applyChange({ previousValue, inputType, addedValue, changeStart, changeEnd, ... })
      → { value, selectionStart, selectionEnd }`** — центральная функция. Вся семантика маски,
      включая позицию каретки, выражена здесь и без DOM.
  - [ ] Три **раздельные** ветки `insert` / `deleteBackward` / `deleteForward` — при вставке
        каретка после последнего вставленного, при удалении назад — после последнего уцелевшего
        слева, при удалении вперёд — перед первым уцелевшим справа. Дельта длины эту разницу не
        выражает, эту ошибку не повторять.
  - [ ] Позиционирование считать по **значащим символам слева**, не по смещению длины.
- [ ] Ноль зависимостей, ноль импортов фреймворков (жёсткое правило Фазы 7).
...
### Этап 2. DOM-контроллер

📍 `libs/forms-core/src/lib/mask/controller.ts` (DOM — но всё ещё без React)

- [ ] `MaskController(element, options)` с `attach()` / `detach()`.
- [ ] **Модель событий** — `keydown` НЕ перехватывает символы:
  - [ ] `input` с двойным guard `composing || e.isComposing` — основной путь;
  - [ ] `compositionend` — единственная точка применения маски при IME;
  - [ ] `beforeinput` — **только** `historyUndo` / `historyRedo`;
  - [ ] `keydown` — **только** запасные undo-хоткеи и трекинг выделения.
- [ ] **IME-guard.** Никогда не звать `preventDefault()` на композиции (по спецификации
      `insertCompositionText` не отменяем) и не писать `value` во время неё.
- [ ] **Детект автозаполнения** — три механизма вместе: `inputType === undefined` трактовать как
      «заменили всё значение»; CSS `:-webkit-autofill` + `animationstart` (Safari может не прислать
      `input`); сверка значения в `queueMicrotask` при инициализации.
- [ ] **Каретка**: установка + повторная через `setTimeout` (мобильный Chrome сбрасывает);
      не трогать, если выделено всё значение.
- [ ] **Свой undo-стек** (~100 записей `{ value, selection }`).
- [ ] Запись значения: `setRangeText` для локальных правок (сохраняет нативный стек),
      прямое присвоение — только при полном переформатировании.

**Критерий готовности:** тесты в jsdom + живая проверка в браузере (`form-develop-app`) по
сценариям: посимвольный ввод, вставка из буфера, правка в середине, `Backspace` через литерал,
выделить всё и заменить, Ctrl+Z, автозаполнение.

⚠️ **Ловушка проверки:** `computer{key: ctrl+a}` в Browser pane не бьёт настоящий `KeyboardEvent` —
выделение не срабатывает, и тест молча «проходит». Выделять через `setSelectionRange`. На этом
уже один раз построили ложный вывод (2026-08-12).
...
### Этап 4. Миграция существующих полей и снятие зависимости

- [ ] 8 документных полей (`createDocumentField`) → новый движок.
- [ ] **ИНН — снять маску** (длина 10 или 12, разделителей нет): `maxLength` + валидация
      контрольной суммы. Критерий уместности — MASK_ENGINE.md §5.3.
- [ ] `FieldPhone` → движок; **консолидировать с `@letar/format-utils/phone`** (сейчас телефон
      реализован в монорепо трижды). Забрать препроцессоры вставки и автозаполнения — это
      закроет хвост фикса 2.0.4 (посимвольный ввод с `8`).
- [ ] `PHONE_MASKS` — вынести в `forms-core`, убрать дубль между Chakra- и shadcn-скином.
- [ ] `FieldCreditCard` → движок (сейчас на собственном форматтере, работает — мигрировать
      без спешки).
- [ ] **Удалить `use-mask-input`** из `package.json` корня, `libs/forms/package.json`,
      `package.publish.json`, `tsup.config.ts`.
- [ ] Починить **фантомные тесты** документных полей (`document-fields.spec.ts` не импортирует
      ни одного компонента — там локальные копии `validate`, расходящиеся с продовым кодом).
- [ ] `ui.mask` в Zod-мете: либо реализовать, либо убрать из `form-docs` (сейчас документация
      обещает несуществующую фичу).

**Критерий готовности:** `use-mask-input` не резолвится ни в одном графе сборки; замер bundle
size до/после в CHANGELOG.

---

### Этап 5. Новые документы (запрос 2026-08-12)

Форматы проверены, детали и предупреждения — MASK_ENGINE.md §7.1.

- [ ] **Загранпаспорт** `99 9999999` + `zRu.foreignPassport()`.
- [ ] **Код подразделения** `999-999` + `zRu.departmentCode()`. Опционально — подсказка «кем
      выдан» по открытому справочнику ФМС.
- [ ] **Свидетельство о рождении** — **без маски**: составное поле (римская часть / две буквы /
      номер) либо свободный ввод с нормализацией. Нужна нормализация гомоглифов по позиции
      (`|||` → `III`, латинская `X` ↔ кириллическая `Х`).
- [ ] Валидаторы в `zRu`, тесты на каждый.

⚠️ **Не валидировать:** серию загранпаспорта по типу бланка, третью цифру кода подразделения по
списку 0–3, регион по первым цифрам серии паспорта (новые регионы не совпадают с ОКАТО). Каждый
такой «улучшайзинг» даёт ложные отказы реальным людям.
...
### Задачи вне `libs/forms` (делегировать после готовности движка)

- `driving-school`: проверить, сводится ли `InputPlateNumber` (230 строк) к декларативной маске;
  перевести валидацию СНИЛС/ИНН с локальных регулярок на `zRu` (сейчас там проверка формата
