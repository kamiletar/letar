...
- [ ] `FieldCreditCard` → движок — **отложено**, как и предполагал сам пункт («работает —
      мигрировать без спешки»): собственный форматтер рабочий, не блокирует остальной Этап 4.
...
- ⚠️ **Не входит в сделанное:** дубль `PHONE_MASKS` между `forms`/`forms-shadcn` устранён в
  прошлой сессии (см. запись ниже), но перевод самого `FieldPhone` на движок (§4-й пункт
  исходного плана «перевод FieldPhone на движок») — по-прежнему НЕ сделан в этой сессии,
  несмотря на заголовок «Этап 4»: фокус сессии был на документных полях и очистке
  зависимости. `FieldPhone` остаётся на чистом JS-форматтере (`@letar/forms-core/phone`),
  препроцессоры вставки/автозаполнения — тоже открыты. Оставлено как следующий подпункт.
