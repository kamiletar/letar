...
- **Ж.** Автоподбор компонента реестра по имени модели или enum (§17.9).
- **З.** Зависимые поля: `dependsOn` у обычных `Field.Select`/`Field.Combobox` (страна → город, компания →
  сотрудник), значения родителей `deps` — в загрузчиках, хуках, `onCreate`/`onUpdate` и догрузке выбранного;
  очистка только по правке родителя, не при восстановлении и гидратации; директива `form.dependsOn` (§18). После Ж.

**Не-цели.** Мультивыбор. `onDelete`/`DeleteButton` (место оставлено, §4.6). Скины vue/vue-shadcn/angular
(потребителей нет; в этапе З тоже). `renderValue` у Combobox (значение там — текст инпута, §9). Окно
редактирования — его делает приложение, как у `onCreate`. `Field.NativeSelect` (deprecated), `Listbox`,
`Autocomplete`. `CascadingSelect` — не развиваем; с этапа З — `@deprecated` (§18.9).
...
**Этап З — зависимые поля (§18).** DS — unit в `libs/forms` (оба скина, где есть UI), `forms-react` и `forms-core`;
PD — плагин; QD — `@letar/forms-query`; MD — `form-mcp`. Номера сценариев — таблица §18.3.

- DS1 — `forms-core`: `serializeDeps` (порядок `dependsOn`, числа и строки различаются, `undefined`/`null`),
  `isEmptyDepValue` (`undefined`, `null`, `''`, `[]` — пусто; `0`, `false` — нет), разбор путей (`countryId` в группе
  `items.3` → `items.3.countryId`; `/countryId` → `countryId`; `/a` и `a` в одном списке — dev-ошибка).
- DS2 — реестр зависимостей (`forms-core`): регистрация и снятие; `suppress` вложенный и после исключения внутри `fn`
  (флаг снимается в `finally`); цикл регистраций → dev-ошибка.
- DS3 — сценарий 1: родитель выбран → ребёнок разблокирован, загрузчик вызван с `deps`, значение ребёнка не тронуто.
- DS4 — сценарии 2–3: смена родителя пользователем → ребёнок пуст, `isTouched` ребёнка `false`, ошибки валидации нет,
  live-область объявила очистку; пустой ребёнок — `setFieldValue` не вызван.
- DS5 — сценарий 4: очистка родителя → ребёнок пуст, заблокирован, подсказка и `aria-describedby` на триггере.
- DS6 — сценарий 5: цепочка из трёх полей → две очистки в одном батче (один коммит React), запрос городов не уходит,
  одно объявление на цепочку.
- DS7 — сценарии 6 и 14: `initialValue` с обоими → очистки нет; Combobox — подпись из `useSelected`/`loadSelected` с
  `deps`; Select — «Загрузка…», затем подпись; значение при пустом родителе показано и не стёрто.
- DS8 — сценарии 7–8: `initialValue` меняется `''` → запись (перерисовка корня с новым пропом) и `reset(values)` →
  очистки нет.
- DS9 — сценарии 9–10: черновик `useFormPersistence` с ребёнком **до** родителя в порядке ключей и с обоими вместе →
  после восстановления оба на месте; то же через `UrlSync` и `useUrlPrefill`.
- DS10 — сценарий 11: `form.setFieldValue(parent)` → ребёнок очищен; внутри `dependents.suppress` → не очищен.
- DS11 — сценарий 12: `Form.Field.Array` — перестановка, удаление, вставка строк с `dependsOn` на соседнее поле
  строки и на `/countryId` → значения строк не стираются, загрузчик получает `deps` своей строки.
- DS12 — сценарии 15 и 18: A → B → C быстро (фейковые таймеры, загрузчик с задержкой) → `signal` A и B отменены,
  на экране опции C; загрузчик, игнорирующий `signal`, — ответ B после C не применяется; размонтирование → отмена и
  снятие регистрации (очистка родителя после размонтирования ребёнка — без ошибок).
- DS13 — сценарий 16: `onCreate` с `ctx.optimistic` и `ctx.deps` = A; смена родителя на B до подтверждения →
  ожидающий выбор снят, `pending`-реестр ждёт, подтверждённая опция не появилась в списке B и появилась при возврате к
  A (если кэш A жив), `onSettleError` при отказе получает `deps` A.
- DS14 — сценарий 17: отправка во время загрузки опций ребёнка → ошибка обязательного поля, `onSubmit` не вызван.
- DS15 — `deps` в действиях: `onCreate(search, ctx)`/`onUpdate(option, ctx)` получают `deps` на момент начала;
  наложение правок и `createdOptions` сбрасываются при смене `depsKey`.
- DS16 — Select-источники: `options` функцией от `deps`; `loadOptions` (разово на `depsKey`, `search = ''`, прежние
  опции скрыты при смене родителя, видны при `reload`); `useOptions(deps)`; «ровно один источник» — compile-only
  `@ts-expect-error`.
- DS17 — `isPlaceholderData`: хук-путь с `keepPreviousData` — при смене `depsKey` старые опции не видны, при печати в
  поиске внутри того же родителя — видны.
- DS18 — регрессия: поля без `dependsOn` ведут себя как раньше (все существующие spec Select/Combobox без правок);
  `CascadingSelect` (Chakra и shadcn) — существующие spec без изменений, JSDoc `@deprecated`.
- PD1 — плагин: `@meta("form.dependsOn", "countryId")` → `fieldProps.dependsOn` в том же литерале `fieldProps`, что и
  `form.props.*`/`form.relation.*`; массив → массив (или строка через запятую, если массив не проходит generate, §18.8).
- PD2 — плагин: ссылка на несуществующее поле, на себя, цикл → ошибка generate с `Модель.поле`; родитель с
  `form.exclude` → предупреждение.
- PD3 — плагин: у целевых моделей нет связи (`City` без связи с `Country`) → предупреждение; `form.dependsOn` без ключа
  реестра на поле с `form.relation` → предупреждение «список не фильтруется».
- PD4 — живой `nx zenstack:generate form-develop-app` с массивом в `form.dependsOn` — проходит или нет (фиксирует
  решение вопроса 57), затем `nx typecheck:tsgo form-develop-app`.
- QD1 — `fromSearchQuery`/`fromSelectedQuery`: хук получает `deps`, `enabled: false` при неготовых `deps`.
- QD2 — `useLoaderQuery`: ключ `[...key, deps, search]`, загрузчик получает `{ signal, deps }`; смена `deps` — новый
  запрос, возврат к прежним `deps` — из кэша.
- QD3 — `useInvalidateAfter((ctx) => keys)`: инвалидирует только ключ родителя из `ctx.deps`.
- QD4 — совместимость (compile-only): старые хуки `(search, options)` без третьего аргумента компилируются.
- MD1 — `form-mcp`: `get_directives` содержит `form.dependsOn` с проверками; паттерн `dependent-select` есть в
  `get_form_pattern`.
...
Этап З: `/dependent-select-demo` — цепочка страна → регион → город (смена страны очищает оба, в сети нет запроса
городов при пустом регионе); быстрая смена страны при медленном ответе (`page.route` с задержкой) — на экране города
последней страны; форма редактирования — после `page.reload()` с черновиком город на месте; Combobox сотрудника —
создание из пустого поиска, окно получило компанию; строки массива с `/countryId`; axe на заблокированном поле
(подсказка связана `aria-describedby`); мобильный вьюпорт. Ассерты скоупить на своё поле.
...
**Этап З — зависимые поля** (§18) — после `Form.Inline.*` и Ж; версии в таблице ниже при допущении §18.14. Все
пакеты — minor:

1. Живой `zenstack generate` с массивом в `@meta("form.dependsOn", [...])` на демо-схеме (тест PD4) — решает вопрос 57
   до кода плагина.
2. `forms-core`: `FieldDeps`, `DependentFieldProps`, `serializeDeps`, `isEmptyDepValue`, разбор путей,
   `createDependentsRegistry`; `deps` в `LoadContext`, `SelectionActionContext`, `SettleErrorInfo`;
   `resolveEmptyValue` — если принят вопрос 50; тесты DS1, DS2.
3. `forms-react`: `useFieldDeps`, реестр в `DeclarativeFormContextValue`, `depsKey` в ключе `usePromiseSearch`,
   `keepPrevious` у `useOptionsLoader`, сброс наложения и `createdOptions` по `depsKey`.
4. `libs/forms`: корни форм — `listeners.onChange` реестра, `suppress` вокруг `restoreFormData`; Select и Combobox —
   `dependsOn` и общие пропсы, блокировка, подсказка, live-область, `deps` во все источники и действия; Select —
   `options(deps)`, `loadOptions`, `useOptions`, «ровно один источник»; учёт `isPlaceholderData`; i18n-ключи
   `formSelection.dependsOnHint`/`dependentCleared`; `CascadingSelect` — `@deprecated`; тесты DS3–DS18. Перед релизом —
   `nx typecheck:tsgo` потребителей Select (объединение источников у Select новое, как Р11 для Combobox).
5. shadcn — то же на общих хуках; `CascadingSelect` shadcn — `@deprecated`.
6. `forms-query`: `deps` в `fromSearchQuery`/`fromSelectedQuery`/`useLoaderQuery`, функция в `useInvalidateAfter`;
   тесты QD1–QD4.
7. `zenstack-form-plugin`: директива `form.dependsOn`, проверки; тесты PD1–PD3; README.
8. `form-mcp`: директива, пропсы, паттерн `dependent-select`; тест MD1.
...
| З — зависимые поля ¹    | 2.28.0  | 0.24.0       | 0.17.0        | 0.46.0         | 0.3.0         | 4.4.0                  | 2.5.0      |

¹ При допущении §18.14: `Form.Inline.*` — `forms` 2.26.0, Ж — `forms` 2.27.0, `forms-core` 0.23.0, плагин 4.3.0,
`form-mcp` 2.4.0 (номера Ж ещё не назначены, §17.9). Если Inline или Ж поднимут другие пакеты — З берёт следующий
свободный minor каждого.
...
- **Р18. Очистка зависит от внутреннего поведения TanStack Form:** что `setFieldValue` зовёт form-level листенеры, а
  `reset`/`update` — нет (§18.1). Обновление `@tanstack/form-core` может это сдвинуть — и очистка начнёт срабатывать
  при гидратации (стирание данных) или перестанет при правке. Смягчение: тесты DS4, DS8, DS9 обязательны при каждом
  обновлении TanStack Form; ссылки на строки в §18.1.
- **Р19. Листенер не срабатывает, если родитель не смонтирован** (`getFieldInfo(field).instance?`, `FormApi.js:670–672`):
  скрытый `Form.When` родитель или родитель вне формы — ребёнок не очистится. Смягчение: dev-предупреждение реестра
  «родитель `countryId` не найден среди полей формы»; строка в доке.
- **Р20. Текстовый родитель** (индекс → город): каждое нажатие — правка, ребёнок очищается на первом символе, загрузка
  перезапускается на каждом (отменяются, но нагрузка есть). Смягчение: `depsReady` (например «6 цифр»), совет в доке;
  отдельного дебаунса `deps` в З нет.
- **Р21. Программная запись родителя приложением очищает ребёнка** (§18.3, сценарий 11) — неожиданно для кода,
  который выставляет пару значений. Смягчение: порядок «родитель, потом ребёнок» и `dependents.suppress` в доке,
  тест DS10.
- **Р22. Новое объединение источников у Select** (`options` | `loadOptions` | `useOptions`) может сломать typecheck
  приложений, где в Select распыляют `Partial<…>`, — как Р11 у Combobox. Смягчение: прогон `typecheck:tsgo`
  потребителей до релиза З (§13, шаг 4).
...
🆕 **2026-09-26 — этап З, зависимые поля (§18).** ✅ Вопросы 46–58 приняты владельцем с рекомендациями в тот же
день. Открытых вопросов нет.
...
Вопросы 46–58 — по зависимым полям (§18, этап З); ✅ приняты владельцем с рекомендациями:

46. **Делаем ли этап З:** `dependsOn` у обычных Select и Combobox, `deps` в загрузчиках, хуках, `onCreate`/`onUpdate` и
    догрузке выбранного, вместо развития `CascadingSelect`? — _Рекомендую да, после Ж_: `CascadingSelect` собран на
    сыром `Select.Root` и не получает ничего из этапов А–Д (§18.1); зависимость как свойство поля даёт их все.
47. **Директива `form.dependsOn` в схеме** — брать в этап? — _Рекомендую да, строкой (массивом для нескольких), без
    автовывода по FK_ (§18.8): отдельный ключ даёт проверки на generate; автовывод неоднозначен (несколько путей и
    полей с одной целью, связь ≠ фильтр).
48. **`CascadingSelect` — `@deprecated` после этапа?** — _Рекомендую да, оставить как есть, не переписывать обёрткой_
    (§18.9): обёртка молча изменила бы поведение устаревшего компонента; реальных потребителей в монорепо нет (только
    демо), для npm — JSDoc с примером нового API; semgrep WARNING, миграция не форсируется. Chakra и shadcn; порты
    vue/angular не трогаем.
49. **Источник очистки:** form-level `listeners.onChange` TanStack + `suppress` на восстановлении черновика, а не
    эффект по смене значения и не «нет среди опций»? — _Рекомендую да_ (§18.3): отличает правку от гидратации без
    флагов в каждом месте записи; `reset`/`update`/`initialValue` листенеры не зовут, черновик подавлен явно.
50. **Пустое значение при очистке nullable-поля:** `null` вместо `''` — общий `resolveEmptyValue(schemaField)` и для
    собственной кнопки очистки Select/Combobox, и для зависимой? — _Рекомендую да, в этапе З_: внешний ключ `''` не
    проходит на сервере, а `0` у числового Select — неверное значение. Это изменение поведения — строка в CHANGELOG.
51. **Путь к полю вне группы** (из строки массива на поле формы) — ведущий `/`: `dependsOn="/countryId"`? —
    _Рекомендую да_: короче объектной формы, похоже на абсолютный путь; относительный путь по умолчанию — как у `name`.
52. **`loadOptions` у Select** (разовая загрузка на каждое значение `deps`, тот же тип `LoadOptionsFn`, `search = ''`) —
    пересмотр вопроса 38? — _Рекомендую да_: без него зависимый Select на server action требует от приложения самому
    читать родителя; поиск по строке на сервере по-прежнему — задача Combobox.
53. **Хук-путь у Select** — `useOptions(deps) => OptionsSourceProps` (та же форма, что `useQueryOptions(...).fieldProps`),
    хук на уровне модуля? — _Рекомендую да_: единственный способ дать Select на ZenStack родителя без чтения формы в
    приложении.
54. **Опции прежнего родителя при смене:** скрывать (пустой список с загрузкой), несмотря на `keepPreviousData`; внутри
    одного родителя прежняя выдача при печати остаётся? — _Рекомендую да_ (§18.7): города чужой страны на экране —
    путь к неверному выбору.
55. **Оптимистичное создание в ребёнке, пока меняется родитель:** не отменять серверное действие, реестр отправки
    ждёт подтверждения, подтверждённая опция — только в кэш прежнего родителя, выбор снят очисткой? — _Рекомендую
    да_ (§18.6).
56. **a11y:** видимая подсказка «Сначала выберите «Страна»» с `aria-describedby` и объявление очистки в live-области,
    обновление списка не объявлять, фокус не переносить? — _Рекомендую да_ (§18.10).
57. **Несколько родителей в схеме:** массив `@meta("form.dependsOn", ["countryId", "typeId"])` или строка через
    запятую? — _Рекомендую массив_, если его пропускает `zenstack generate` (проверить первым шагом, тест PD4); иначе —
    строка через запятую.
58. **`RelationFieldProvider` + `form.dependsOn`** (автоформа без ключа реестра): фильтр
    `RelationConfig.filter(record, deps)` в этапе З? — _Рекомендую не в З_: провайдер грузит всю модель, фильтр на
    клиенте не масштабируется; путь — ключ реестра, плагин предупреждает (PD3). Добавить по первому запросу.
...
#### 17.9. Этап Ж — автоподбор компонента реестра по соглашению (после `Form.Inline.*`)

- **Запросил:** владелец 2026-09-26 («зачем ключ руками, если он выводится из имени модели»). **Решено владельцем:**
  «маппер ищет по имени, плагин подставляет модель из `@relation` и пишет список кандидатов».
- **Как работает.** Поле — FK со связью `@relation` на модель `X` (или поле типа enum `E`): плагин берёт имя целевой
  модели из схемы и кладёт его в поле (то же, что сегодня даёт `form.relation.model`, но без ручной директивы).
  Маппер автоформ ищет в реестре `Select.X` (для enum — `Select.E`); есть — рисует его, нет — стандартный
  Select/поле без ошибки. Явный ключ (`form.fieldType = "Select.Имя"`, этап Е) остаётся: другое имя компонента,
  принудительный `Combobox.*`; для него отсутствие в реестре по-прежнему ошибка.
- **Умолчания координатора (владелец не возражал, но явно не подтверждал — сверить в архитектурной заметке):**
  по соглашению работают только `Select.*` (и enum); `Combobox.*` — только по явному ключу, иначе при двух
  зарегистрированных компонентах под одним именем нужен приоритет. **Набор полей в форме не меняется:** плагин
  по-прежнему включает FK-поле, только если оно уже включено (`form.relation`, явный ключ и т.п.) — автоподбор
  не втягивает в автоформы все внешние ключи.
- **Плагин:** список кандидатов (имена моделей/enum, для которых выведен ключ) — в `form-registry-keys.ts`,
  отдельным union от явных ключей; чтобы приложение видело, для каких кандидатов компонента нет (warn в dev, не
  ошибка и не typecheck: отсутствие компонента законно).
- **Не проверено:** как плагин определяет целевую модель у FK (`@relation`, `DataFieldAttribute`), поведение на
  слитых фрагментах `libs/*.zmodel`, конфликт с явным `form.relation.model`.
- **Версии/тесты:** назначить в архитектурной заметке перед кодом (`forms`, `forms-core`, `zenstack-form-plugin`,
  `form-mcp`). Тесты: выбор по соглашению, отсутствие компонента, явный ключ побеждает, enum, Combobox не
  подбирается, набор полей формы не изменился.

### 18. Зависимые (каскадные) селекты (этап З)

**Запрос владельца 2026-09-26:** страна → город, компания → сотрудник, категория → подкатегория. Замысел
координатора, одобренный владельцем в общих чертах («отправляй»): отдельный компонент не нужен, зависимость —
свойство обычных `Field.Select` и `Field.Combobox`. Формально не приняты вопросы 46–48 (§14). Этап З идёт **после Ж**.
...
<Form.Field.Combobox
  ...
  label="Сотрудник"
  dependsOn="companyId"
  loadOptions={(search, { signal, deps }) => searchEmployees({ companyId: deps.companyId, search }, signal)}
  getLabel={(e) => e.name}
  getValue={(e) => e.id}
  onCreate={(search, { deps }) => openEmployeeDialog({ companyId: deps.companyId, name: search })}
/>
```

#### 18.1. Что есть сейчас (проверено по коду)

`Form.Field.CascadingSelect` (`libs/forms/src/lib/declarative/form-fields/selection/field-cascading-select.tsx`):

- **пропсы** (`:22–67`): `dependsOn` (одна строка), `loadOptions(parentValue)` → массив или `{ options }`,
  `initialOptions`, `clearOnParentChange` (=`true`), `disableWhenParentEmpty` (=`true`), `placeholderWhenDisabled`,
  `clearable`, `size`, `variant`;
- **собран на сыром `Select.Root`** Chakra (`:187`), не на `Field.Select`. Поэтому нет поиска (В), `renderOption`/
  `renderValue`/`data` (А), `onCreate`/`onUpdate`/слотов (Б), `loading` в триггере, промис/хук-источников (Г),
  `pending` и оптимизма (Д), общего конвейера `forms-react/selection`;
- **гонки нет защиты:** загрузка в эффекте без `AbortSignal` и без номера запроса (`:118–141`), ответ устаревшего
  запроса перезаписывает новый (`:128–131`); ошибка — только `console.error` и пустой список (`:132–134`), «Повторить»
  нет;
- **лишние загрузки (по коду, запуском не проверено):** умолчание `initialOptions = []` (`:280`) создаёт новый массив
  на каждом рендере `FieldCascadingSelect`, а он стоит в зависимостях эффекта загрузки (`:141`) — каждая перерисовка
  внешнего компонента перезапускает запрос;
- **очистка — эффектом по сравнению с прошлым рендером** (`:144–152`): любая смена родителя, кроме первой с
  `undefined`, стирает значение и пишет `''` (`:148`); выбор тоже пишет `''` при очистке (`:194`);
- **путь родителя** — префикс группы (`:308`), разбор по `split('.')` (`:311–323`); сослаться на поле вне группы (из
  строки массива на поле формы) нельзя; подписка — `form.Subscribe` (`:327`);
- **при редактировании** значение есть, опции ещё грузятся — триггер пустой (нет `initialLabel`/догрузки выбранного).

**Поправка к исходной постановке:** shadcn-скин у `CascadingSelect` **есть** — порт той же логики
(`libs/forms-shadcn/src/lib/fields/field-cascading-select.tsx:24–150`, очистка в `''` на `:146`), со своим spec. Свои
порты есть и в `forms-vue`, `forms-vue-shadcn`, `forms-angular` (`field-cascading-select.ts`/`.component.ts`).

**Использование** (`grep CascadingSelect` по `apps/` и `libs/`): только демо и документация — `form-develop-app`
(`select-demo` + e2e `select-demo.spec.ts:96–239`), `form-develop-app-shadcn`, `form-example` (`all-fields`),
`form-docs`, README и статьи. В приватных submodule (15 шт.) — **0** использований. Настоящих потребителей в
монорепо нет; `@letar/forms` публикуется в npm, внешние — возможны.

**Механизмы, от которых зависит решение:**

- **Когда TanStack зовёт листенеры.** `FormApi.setFieldValue` вызывает `triggerOnChangeListener` поля, если не передан
  `dontRunListeners` (`@tanstack/form-core@1.33.5 dist/esm/FormApi.js:646–672`); тот зовёт **form-level**
  `listeners.onChange({ formApi, fieldApi })` (`FieldApi.js:598–615`). `field.handleChange` идёт тем же путём
  (`FieldApi.js:527–529` → `setValue`). `reset()` (`FormApi.js:127`) и `update()` при новых `defaultValues`
  (`FormApi.js:90–111`: значения заменяются целиком, если форма не тронута) листенеров **не зовут**. Операции массива
  (`insert/remove/swap/moveFieldValues`, `FormApi.js:701–800`) пишут через `setFieldValue` по пути **массива** —
  листенер поля строки не срабатывает.
- **Восстановление черновика** — цикл `form.setFieldValue(key, value)` по ключам верхнего уровня
  (`form-root/use-form-features.ts:255–273`, вызов `form-simple.tsx:200`, `form-with-api.tsx:203`) — листенеры
  срабатывают.
- **`UrlSync` и `useUrlPrefill`** в форму пишут только через `initialValue` (`use-form-url-sync.ts:72–102`,
  `use-url-prefill.ts:54`; `setFieldValue` в них нет) — то есть через `update()`, без листенеров.
- **`useFieldChangeListeners`** (`form-root/use-field-change-listeners.ts`) подписан на `form.store` и срабатывает на
  любое изменение, включая `reset` — для решения «правка пользователя или нет» не годится.
- **Источники:** `LoadContext` = только `{ signal }` (`forms-core/src/lib/uikit/load-options.ts:7–9`);
  `SelectionActionContext` = только `optimistic` (`creatable-options.ts:21–29`); `ComboboxSource` — ровно один источник
  (`field-combobox.tsx:251–342`), `useQuery(search)`, `useSelected(value)` (`:305`). Промис-поиск Combobox держит ключ
  `nonce + search` (`forms-react/src/lib/field/use-promise-search.ts:55`) и читает загрузчик через ref — если
  загрузчик замкнут на значение родителя, смена родителя **не перезапрашивает** список. `useOptionsLoader(load, deps)`
  при смене `deps` показывает прежние опции, пока идёт запрос (`use-options-loader.ts:25–31`).
- **`@letar/forms-query`:** `fromSearchQuery` ставит `placeholderData: keepPreviousData` (`from-search-query.ts:48–53`),
  `useLoaderQuery` — ключ `[...key, search]` (`use-loader-query.ts:28–40`).
- **Очистка своего `Field.Select`** пишет `''`, у числового поля — `0` (`field-select.tsx:352–354`).
- **Пути групп:** `FormGroup` склеивает имя с родительской группой (`forms-react/src/lib/context/form-group.tsx:37–45`),
  строка массива — `FormGroup name={String(index)}` (`form-group/form-group-list-declarative.tsx:122`).

#### 18.2. Контракт

**Общие пропсы** Select и Combobox (`forms-core/uikit/dependent-fields.ts` — типы и чистые функции; в `libs/forms` —
через `SelectFieldProps`/`ComboboxFieldBaseProps`):
...
Имена `clearOnParentChange`/`disableWhenParentEmpty`/`placeholderWhenDisabled` — те же, что у `CascadingSelect`:
перенос кода — удаление строки `Cascading`. «Пустое» = `undefined`, `null`, `''`, пустой массив
(`isEmptyDepValue` в `forms-core`). Ключ зависимостей `depsKey` — `JSON.stringify` значений в порядке `dependsOn`
(`serializeDeps` в `forms-core`).

**`deps` доходит во все источники и действия:**

| Где                                   | Было                        | Станет                                                                                                                     |
| ------------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `LoadContext` (`load-options.ts`)     | `{ signal }`                | `{ signal; deps: TDeps }` — `deps` всегда передаётся, без `dependsOn` — `{}`; старые загрузчики его не читают              |
| Combobox `loadOptions`/`loadSelected` | `(search, { signal })`      | `(search, { signal, deps })` / `(value, { signal, deps })`                                                                 |
| Combobox `useQuery`                   | `(search)`                  | `(search, deps)`; `AsyncQueryResult` получает необязательный `isPlaceholderData` (у `UseQueryResult` он есть)              |
| Combobox `useSelected`                | `(value)`                   | `(value, deps)`                                                                                                            |
| Select `options`                      | массив                      | массив или `(deps) => массив` — фильтр уже загруженного полного списка (категория → подкатегория)                          |
| Select `loadOptions` (**новое**)      | нет (вопрос 38)             | тот же `LoadOptionsFn`, `search` всегда `''`: разовая загрузка на каждый `depsKey` (`useOptionsLoader` внутри) — вопрос 52 |
| Select `useOptions` (**новое**)       | нет                         | хук `(deps) => OptionsSourceProps` — та же форма, что `useQueryOptions(...).fieldProps` (вопрос 53)                        |
| `SelectionActionContext`              | `{ optimistic }`            | `{ optimistic; deps }` — снимок на момент начала действия (окно создания получает страну)                                  |
| `SettleErrorInfo`                     | `kind`, `preview`, `reason` | + `deps` — для текста «город не сохранён для страны X»                                                                     |

У Select тоже «ровно один источник»: `options` | `loadOptions` | `useOptions` (объединение с `?: never`, как
`ComboboxSource`). Типы `deps`: `TDeps` — второй generic поля (`<Form.Field.Combobox<Employee, { companyId: string }>`)
или вывод из аннотации параметра загрузчика; без аннотации — `FieldDeps` (`unknown` по ключам). Проверить на
компиляции, что `dependsOn` и ключи `TDeps` совпадают, нельзя без типизации имён полей формы (её у полей нет) — это
dev-предупреждение, не typecheck.

**`forms-react`** (общий для Chakra и shadcn):

- `useFieldDeps(dependsOn)` → `{ deps, depsKey, ready, parentPaths, parentLabels }` — подписка на значения родителей
  через `useStore(form.store, selector)`; путь — с префиксом группы или от корня (`/`); метки родителей — из
  `ui.title` схемы (для подсказки), иначе имя поля. Экспортируется: нужен компонентам реестра, которые не оборачивают
  `Field.*` (см. «Реестр» ниже);
- реестр зависимостей формы `createDependentsRegistry` (`forms-core`, как `createPendingRegistry` этапа Д) и
  `dependents` в `DeclarativeFormContextValue`: `register(childPath, parentPaths, clear)`, `handleFieldChange` (для
  form-level `listeners.onChange`), `suppress(fn)` — выполнить запись без очистки зависимых;
- `usePromiseSearch`: ключ `nonce + depsKey + search`; `useOptionsLoader`: параметр `keepPrevious` (при смене `depsKey`
  прежние опции **не** показываются, при `reload` — показываются).

#### 18.3. Когда очищать дочернее поле (ловушка 1)

**Правило: очищаем только по «правке» родителя — `field.handleChange` или `form.setFieldValue` без подавления.
Гидратация, сброс и восстановление поле не трогают.**

Механизм: корни форм (`form-simple.tsx:122`, `form-with-api.tsx:134`) передают в `useAppForm` form-level
`listeners.onChange: dependents.handleFieldChange`. Реестр знает, какие поля зависят от `fieldApi.name`; прошлое
значение родителя берёт из своей подписки на `form.store` (её обновляют и `reset`, и `update`). Если значение
изменилось, реестр не в режиме `suppress`, у ребёнка `clearOnParentChange` и значение непустое — пишет пустое
значение: `form.setFieldValue(child, empty, { dontUpdateMeta: true, dontValidate: true })`. Листенеры этой записи
**не** подавляются — так очищается вся цепочка страна → регион → город синхронно, в одном батче React.

Почему не другие варианты:

- **эффект по смене значения** (как сейчас) не отличает правку от гидратации: `update()` с новым `initialValue`
  (`''` → `'RU'` после загрузки записи) и восстановление черновика меняют родителя вместе с ребёнком в одном батче,
  эффект видит «родитель изменился» и стирает только что выставленный город — **независимо от порядка ключей**;
- **«очищать, если значения нет среди опций нового родителя»** не работает у Combobox с серверным поиском (опции не
  загружены целиком), даёт ложную очистку, пока опции ещё грузятся, и тянет лишнюю загрузку ради проверки. Как
  дополнительная проверка — не нужна: несогласованную пару ловит сервер (§18.5).

Подавление (`dependents.suppress`) оборачивает библиотечные массовые записи: `restoreFormData`
(`use-form-features.ts:255–273`) — обязательно; `useFieldChangeListeners` не трогаем (пишет приложение по своему
смыслу). Приложению `suppress` доступен через `useDeclarativeForm().dependents` — для «выставить страну и город
разом» (например, выбор адреса на карте). Без него программная запись родителя — это правка: ребёнок очищается,
поэтому порядок «сначала родитель, потом ребёнок» в доке.

| №  | Сценарий                                                                | Что происходит                                                                                                      | Тест |
| -- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---- |
| 1  | пользователь выбрал родителя (был пуст)                                 | ребёнок разблокирован, опции грузятся; значение ребёнка не меняется                                                 | DS3  |
| 2  | сменил A → B, у ребёнка значение                                        | ребёнок очищен без ошибки валидации и без `isTouched`; объявление для скринридера; опции A скрыты, грузятся опции B | DS4  |
| 3  | сменил A → B, ребёнок пуст                                              | записи нет                                                                                                          | DS4  |
| 4  | очистил родителя                                                        | ребёнок очищен и заблокирован, текст `placeholderWhenDisabled`                                                      | DS5  |
| 5  | цепочка страна → регион → город, сменил страну                          | регион и город очищены в одном батче; запрос городов не уходит (регион пуст)                                        | DS6  |
| 6  | форма редактирования: `initialValue` с обоими                           | очистки нет; в триггере подпись из `initialLabel`/`useSelected`/`loadSelected` или «Загрузка…», затем подпись       | DS7  |
| 7  | `initialValue` пришёл позже (`''` → запись), `update()`                 | очистки нет — `update()` листенеры не зовёт                                                                         | DS8  |
| 8  | `reset(values)` после отправки (`use-post-submit-reset-guard.ts:52–58`) | очистки нет                                                                                                         | DS8  |
| 9  | восстановление черновика, любой порядок ключей                          | очистки нет (`suppress`)                                                                                            | DS9  |
| 10 | `UrlSync`/`useUrlPrefill`                                               | очистки нет (через `initialValue`)                                                                                  | DS9  |
| 11 | приложение: `setFieldValue(parent)`                                     | ребёнок очищен, как при правке; `suppress` — не очищен                                                              | DS10 |
| 12 | строка массива: перестановка, удаление, вставка строк                   | очистки нет (листенер пути массива, не поля строки); опции строки перезапрашиваются по новому `depsKey`             | DS11 |
| 13 | родитель `readOnly`/`disabled`                                          | правки нет — ничего                                                                                                 | —    |
| 14 | ребёнок со значением при пустом родителе (несогласованные данные)       | поле заблокировано, значение показывается, **не** стирается (это не правка)                                         | DS7  |
| 15 | быстрая смена родителя A → B → C                                        | запросы A и B отменены (`signal`), применяется только C                                                             | DS12 |
| 16 | создание в ребёнке с `ctx.optimistic`, пока ждём — смена родителя       | см. §18.6                                                                                                           | DS13 |
| 17 | отправка, пока опции ребёнка грузятся                                   | обычная валидация (обязательный пустой ребёнок — ошибка); очередь `submitWhenSettled` не участвует                  | DS14 |
| 18 | размонтирование                                                         | запрос отменён, регистрация в реестре снята                                                                         | DS12 |

**Пустое значение при очистке** — то же, что пишет собственная кнопка очистки поля; сейчас это `''` (и `0` у числа,
`field-select.tsx:352–354`). Для nullable-схемы правильнее `null` (внешний ключ `''` не проходит на сервере) — вопрос
50: общий `resolveEmptyValue(schemaField)` в `forms-core` для собственной очистки и для зависимой.

**`DirtyGuard`** (ловушка 3): автоочистка — следствие правки родителя, форма уже изменена. Отдельного учёта нет;
`dontUpdateMeta` оставляет ребёнка «нетронутым», чтобы сразу не загоралась ошибка «обязательное поле» — она
появится при отправке или при касании.

#### 18.4. Предзагрузка и редактирование (ловушка 6)

- Очистки при гидратации нет по §18.3 (строки 6–10 таблицы).
- Подпись выбранного до загрузки опций: Combobox — `initialLabel`, `useSelected(value, deps)`, `loadSelected(value,
  { signal, deps })` (как в Г); Select — `loading` (В) показывает «Загрузка…» в триггере при непустом значении, после
  загрузки — подпись. Значения нет среди загруженных опций (данные несогласованы) — Select показывает сырое значение и
  dev-предупреждение, **не** стирает.
- Кэш `loadSelected` — по `value` (идентификаторы уникальны между родителями); при смене `depsKey` не сбрасывается.

#### 18.5. Серверная согласованность (ловушка 2)

Город принадлежит стране — проверяет сервер, не UI: запрос можно отправить в обход формы, а список мог устареть.
Библиотека **ничего не добавляет** в рантайм, только правило в доке:

- server action / процедура проверяет пару (`city.countryId === countryId`) и возвращает ошибку **поля ребёнка**
  через `errorMap.onServer`
  ([letar-forms-server-errors-errormap-onserver](/.claude/docs/letar-forms-server-errors-errormap-onserver.md)) —
  сообщение встанет под «Город»;
- ZenStack-политикой: выразимо ли это через `@@validate` с обращением к полю связи в ZenStack v3 — **не проверено**;
  в доке — пример на server action, политика — «если поддерживается».

#### 18.6. Гонки и жизненный цикл (ловушка 4)

- **Смена родителя** меняет `depsKey`: промис-путь отменяет запрос (`signal`) и применяет только результат
  последнего ключа (как поиск Г, ключ `nonce + depsKey + search`); Select — `useOptionsLoader` с `keepPrevious: false`.
- **Состояние поля по `depsKey`:** наложение правок и `createdOptions` этапа Б относятся к списку родителя — при смене
  `depsKey` сбрасываются (иначе созданный город страны A появится в списке страны B).
- **Оптимистичное создание, пока родитель меняется** (вопрос 55): серверное действие не отменяем (не можем — оно уже
  ушло); ожидающий выбор снимается очисткой (правило O5 этапа Д: внешняя запись снимает ожидающий выбор); реестр
  отправки продолжает ждать подтверждения — запись могла создаться; подтверждённая опция попадает только в кэш
  прежнего `depsKey`; `onSettleError` получает `deps` снимка. Отправка после подтверждения идёт с пустым ребёнком —
  валидация обязательного поля остановит её с понятной ошибкой.
- **Размонтирование:** отмена запросов, снятие регистрации в реестре зависимостей (иначе очистка писала бы в поле,
  которого нет).

#### 18.7. TanStack Query и ZenStack (`@letar/forms-query`, ловушка 5)

Новых экспортов нет, расширяются существующие:

- `fromSearchQuery(useHook)` — хук получает `(search, options, deps)`; `options.enabled` = `minChars` пройден **и**
  `depsReady`; `fromSelectedQuery` — `(value, options, deps)`;
- `useLoaderQuery(key, load)` — ключ `[...key, deps, search]`, в загрузчик — `{ signal, deps }`, `enabled` учитывает
  готовность `deps`;
- `useInvalidateAfter(keys)` — дополнительно форма-функция `(ctx) => keys`: инвалидировать только список нужного
  родителя (`(ctx) => [['employees', ctx.deps.companyId]]`); хуки ZenStack инвалидируют модель целиком сами —
  активен только запрос текущего родителя, лишнего сетевого трафика нет;
- **`keepPreviousData` при смене родителя.** Внутри одного родителя (печать в поиске) прежняя выдача остаётся — как
  сейчас; при смене `depsKey` опции чужого родителя показывать нельзя. Решение на стороне поля, а не пакета: пока
  `isPlaceholderData === true` и `depsKey` сменился после последнего настоящего ответа, поле показывает пустой список
  с загрузкой (вопрос 54). Работает и для хуков ZenStack — их результат это `UseQueryResult`.

Версия пакета — minor (сигнатуры расширены аддитивно; `useLoaderQuery` меняет ключ запроса — строка в CHANGELOG).

#### 18.8. Схема: директива `form.dependsOn` и плагин

- `@meta("form.dependsOn", "countryId")` → `fieldProps.dependsOn` в сгенерированной схеме (через одну точку сборки
  `fieldProps` — баг двойного ключа исправлен в Е). Несколько родителей — массивом:
  `@meta("form.dependsOn", ["countryId", "typeId"])`. Плагин массив разбирает (`parser.ts:140–141`); пропускает ли
  его upstream-генерация TS-схемы ZenStack, как не пропускает `ObjectExpr`, — **не проверено**, первый шаг этапа —
  живой `zenstack generate` на демо-схеме; не пройдёт — строка через запятую (вопрос 57).
- Уже сегодня работает `@meta("form.props.dependsOn", "countryId")` (любой `form.props.*` уходит в `fieldProps`).
  Отдельный ключ нужен ради **проверок на generate**: поле есть в модели, не ссылается само на себя, нет цикла, родитель
  не исключён `form.exclude` (иначе ребёнок навсегда заблокирован) — ошибка/предупреждение с `Модель.поле`; если у
  обоих полей есть связи — у целевой модели ребёнка есть связь с целевой моделью родителя (`City` → `Country`), иначе
  предупреждение «похоже на ошибку». `KNOWN_FORM_DIRECTIVE_KEYS` (`parser.ts:100–109`) + `'dependsOn'`.
- **Автовывод зависимости по FK — не делаем** (подтверждаю позицию координатора). Проверено рассуждением по схеме:
  путей бывает несколько (`City → Region → Country`), полей с одной целью — несколько (`homeCountryId`,
  `workCountryId`), связь в схеме не значит «фильтровать список» (сотрудник любой компании при поле «компания-клиент»),
  а фильтр в загрузчике всё равно пишет приложение. Автовывод давал бы ложные блокировки и очистки. Плагин только
  проверяет явную директиву (выше).
- **С ключом реестра (Е) и автоподбором (Ж):** `RegistryField` распыляет `fieldProps` в компонент — `dependsOn` доходит
  до внутреннего `Field.Select`/`Field.Combobox`, а загрузчики компонента получают `ctx.deps`. Отдельный проп `deps`
  у компонента реестра не вводим: одна подписка и одна регистрация в реестре, внутри поля. Компонент, который не
  оборачивает `Field.*`, берёт `useFieldDeps(dependsOn)`. Контракт «компонент реестра распыляет пропсы» (§17.3) это
  уже покрывает.
- **`RelationFieldProvider` (автоформы без ключа):** провайдер грузит все записи модели, фильтра по родителю нет —
  поле будет блокироваться и очищаться, но список не отфильтрован. В З не решаем: плагин предупреждает «`form.dependsOn`
  без ключа реестра — список не фильтруется», путь — ключ реестра (вопрос 58).
- `form-mcp`: `get_directives` — `form.dependsOn` с проверками; `get_field_props` — `dependsOn`, `depsReady`,
  `clearOnParentChange`, `disableWhenParentEmpty`, `placeholderWhenDisabled`, `deps` в `ctx`; `get_form_pattern` —
  паттерн `dependent-select` (Country → City на промисе, Company → Employee на ZenStack с `onCreate`).

#### 18.9. Старый `CascadingSelect`

**Решение: `@deprecated` и оставить как есть, обёрткой над новым API не делать** (вопрос 48).

- Обёртка изменила бы поведение устаревшего компонента молча: очистка по правке вместо эффекта, другое пустое
  значение, другой набор пропсов у `loadOptions(parentValue)`. Для компонента, который не развивается, тихое изменение
  поведения хуже, чем известные ограничения.
- Реальных потребителей в монорепо нет (§18.1) — миграция не нужна; внешним пользователям npm — JSDoc `@deprecated`
  с примером нового API и перечнем известных проблем (гонка, стирание при восстановлении черновика).
- Демо в `form-develop-app`, `form-develop-app-shadcn`, `form-example` и e2e `select-demo.spec.ts` переводятся на
  новый API; `libs/forms/docs/fields.md` раздел `CascadingSelect` (`:549–584`) — пометка «устарел» и ссылка на
  «Зависимые поля».
- semgrep `letar-forms-cascading-select-deprecated`, WARNING, по образцу `letar-forms-native-select-deprecated`
  (`.semgrep/letar-rules.yml:209`) — вне `libs/forms`, за координатором.
- Chakra и shadcn помечаются оба; порты vue/vue-shadcn/angular — вне охвата, не трогаем.

#### 18.10. a11y (ловушка 7)

- **Заблокированное поле:** нативный `disabled` триггера (как у любого `disabled`-поля) плюс видимая подсказка под
  полем «Сначала выберите «Страна»» (i18n `formSelection.dependsOnHint`), связанная с триггером `aria-describedby`.
  Подсказка видна, а не только в placeholder: заблокированный триггер не получает фокус, зрячий пользователь
  клавиатуры должен понять, почему поле пропущено.
- **Очистка объявляется:** вежливая live-область «Поле «Город» очищено: изменилось поле «Страна»» (i18n
  `formSelection.dependentCleared`) — иначе значение исчезает молча. Обновление списка **не** объявляем (шум при
  каждой смене).
- **Фокус** после смены родителя остаётся на родителе; никуда не переносим.
- Цепочка из двух очисток — одно объявление с перечислением полей.

#### 18.11. Скины и охват (ловушка 8)

Chakra (`libs/forms`) и shadcn (`libs/forms-shadcn`): Select и Combobox; общая логика — в `forms-core` (типы,
`serializeDeps`, `isEmptyDepValue`, реестр) и `forms-react` (`useFieldDeps`, реестр в контексте, источники). У shadcn
нет своего корня формы — реестр подключается в тех же корнях, что и реестр ожидания этапа Д. **vue, vue-shadcn,
angular — вне охвата** (потребителей нет); их `CascadingSelect` не меняются. Примеры — нейтральные: Country/City,
Company/Employee, Category/Subcategory.

#### 18.12. Документация и демо

- `libs/forms/docs/fields.md` — раздел «Зависимые поля»: `dependsOn`, пути (`/`), `deps` в загрузчиках и `ctx`,
  правило очистки и таблица §18.3 (коротко), `suppress`, сервер проверяет пару (§18.5), TanStack Query/ZenStack
  (§18.7), директива `form.dependsOn`; README — строка в таблице пропсов Select/Combobox.
- `apps/form-develop-app` — `/dependent-select-demo`: Country → Region → City (промис с задержкой), Company →
  Employee (Combobox на ZenStack с `onCreate`, окно получает компанию), форма редактирования с черновиком
  (`useFormPersistence`), строки массива с `dependsOn="/countryId"`; модели Country/Region/City/Company/Employee — в
  демо-схему.
- `apps/form-docs` — страница «Зависимые поля» (ru/en); `apps/form-example` — пример страна → город.
- `form-mcp` — §18.8.

#### 18.13. Тесты

Перечень — в §12, блок «Этап З» (DS1–DS18, PD1–PD4, QD1–QD4, MD1, e2e).

#### 18.14. Версии и порядок

Текущие версии (`package.json`): `forms` 2.25.0, `forms-core` 0.22.0, `forms-react` 0.16.0, `forms-shadcn` 0.45.0,
`forms-query` 0.2.0, `zenstack-form-plugin` 4.2.0, `form-mcp` 2.3.0. До З в очереди `Form.Inline.*` (Backlog) и Ж —
их версии ещё не назначены. **Допущение:** Inline — `forms` 2.26.0, Ж — `forms` 2.27.0, `forms-core` 0.23.0, плагин
4.3.0, `form-mcp` 2.4.0; тогда З — строка в таблице §13. Если Inline/Ж займут другие номера (например поднимут
`forms-react`) — З сдвигается на следующий свободный minor каждого пакета. `forms-query` 0.3.0 раньше предполагался
для отложенного `useZenStackRelations` (вопрос 43) — берёт тот, кто придёт первым.

Порядок шагов — §13, «Этап З».
