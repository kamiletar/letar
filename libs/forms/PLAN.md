...
- [ ] **7.3 `@letar/forms-shadcn` beta** — 15–20 ходовых полей (Input/Textarea/Number/Select/Checkbox/Radio/Date).
      ...
      **уже Chakra-free**, вопреки опасению, что «composition завязан на Chakra целиком».
      ...
      - **Дальше:** RichText/FileUpload/Address — оставшиеся ~1-6 полей до 15-20, самые тяжёлые
      (RichText — Tiptap, FileUpload — своя инфраструктура загрузки, Address — DaData-провайдер).
      14 полей — уже представительный набор для дев-харнесса визуальной проверки; решение
      координатора (сообщение 2026-08-10) — продолжать поля, харнесс пока не заводить. Порядок
      дальнейших полей на усмотрение forms-dev, ничего не блокирует.
