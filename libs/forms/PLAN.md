...
### 📋 [2026-09-12] Field.Password/Field.MaskedInput не принимают `size` (найдено при аудите, не фикшено)

- **Найдено:** попутно с фиксом `Field.String`/`Field.Textarea`/`Field.Number` (см.
  `PLAN_COMPLETED.md`, та же дата) — оба поля импортируют Chakra `Input` напрямую (не через
  `chakraUIKit`) и не пробрасывают `size`, тот же класс пробела.
- **Приоритет:** low — не нашлось живого случая рассинхрона высоты с этими двумя полями
  (в отличие от String/Select), фиксился превентивно только явно запрошенные поля.
- **Статус:** не сделано, вне скоупа сессии 2026-09-12. Подобрать при следующей работе над
  `text/field-password.tsx`/`text/field-masked-input.tsx`, если всплывёт живой кейс, либо
  докинуть вместе с очередным аудитом size-пропсов.

### 🔄 [2026-09-12] Маска NumberInput с автоформатированием сбрасывается при удалении цифры (в работе → forms-dev, от domwellbes-dev)
