...
### [2026-09-24] Persistence: устаревший черновик не удаляется, когда значения вернулись к исходным (от domwellbes-dev)
...
- **Статус:** делегировано `forms-dev` (2026-09-24, тред `forms-domwellbes-2026-09-24`, п.1 из 3). Баг подтверждён по коду координатором.
...
- **Статус:** делегировано `forms-dev` (п.2, после п.1): проп `persistence.restoreUi: 'dialog' | 'banner'`,
  дефолт в релизе `'dialog'`. ❓ Открыт вопрос владельцу: менять ли дефолт на `'banner'` для всех потребителей.
