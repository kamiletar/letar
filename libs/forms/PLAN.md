...
- **Предложение (не реализовано):** опциональное наследование нативных атрибутов, когда
  `@form.props` их явно не переопределяет (обратная совместимость для намеренных расхождений).
  Нужно решение координатора по объёму (весь список атрибутов или только частые) и по миграции
  существующих `@form.props`, которые могут стать избыточными.
- **Статус:** отправлено `forms-coordinator-dev`, тред `form-feature-request` (msg id 1102),
  ждём ответа.
