...
### [ ] ⚠️ Открытый вопрос [2026-09-21]: сколько обратных слэшей реально стоит в `@regex` схем приложений
...
- **Что сделать:** grep `@regex(` по `apps/*/schema.zmodel` (включая submodule) и сверить
  сгенерированные `.regex(/…/)` в `src/generated/form-schemas/`; заодно дописать замер слэшей в
  README плагина и `.claude/skills/zenstack-helper/reference/form-directives.md`.
