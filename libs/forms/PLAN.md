...
- **Приоритет:** high
- **Статус:** Фаза 0 (spike) закрыта `forms-dev` 2026-09-04 — решение A3 (кодоген + `ZodUtils.*`
  с типизированной обёрткой, критическая находка по стиранию типов зафиксирована и закрыта) и
  плоский синтаксис `@meta` (объектный литерал не проходит компилятор ZModel). Подробности —
  секция «Фаза 0 — результаты spike» ниже. Фазы 1-4 не начаты, могут стартовать. Узкая часть
  (наследование 7 атрибутов) закрыта отдельно в v2.3.0 — см. следующий пункт, он остаётся в силе
  и ничему тут не мешает.
