...
- **Из документационного чек-листа ниже фактически закрыты:** Приоритет 0 (`form-mcp`
  ...
  **НЕ закрыты** (осознанно, вне периметра «код + core-доки» этой сессии): `CLAUDE.md`/
  `AGENTS.md`-зеркало, остальные скиллы/агенты из Приоритета 1-2 со старыми упоминаниями
  `@form.*` в промптах, весь Приоритет 4 (`apps/form-docs` MDX RU+EN, `apps/form-example`) и
  Приоритет 5 (14 статей, включая SVG-диаграмму с нарисованным `@form.`). Это реальный остаток
  задачи, не забытьё — фиксирую явно, чтобы не потерялось.
