...
закрыты 2026-08-13. Дальше: Этап 2 (включение оси Framework React↔Vue) — ждёт решения Ками
"когда наберётся осмысленный минимум Vue-полей", не блокирует Фазу 9.
...
**Статус:** Фаза 9 **не закрыта**. Решение принято Ками напрямую (в этой же сессии, без
координатора) — **доделывать Vue-паритет до реального 61/61 Этапом 8**, не замораживать на
неполном охвате. Angular-порт остаётся ждать закрытия Этапа 8.
...
Осталось от Этапа 8: select-семейство (9 полей: Combobox/Autocomplete/Listbox/CascadingSelect/
CheckboxCard/RadioCard/SegmentedGroup/ImageChoice/Tags) и специализированные (5 полей: Auto/
Calculated/Editable/PasswordStrength/Schedule) — следующие заходы.
...
- **Почему не сейчас:** Angular-разработчики по умолчанию ожидают Reactive Forms/сигналы, а не
  ...
  Фазы 9.
- **Статус:** backlog, не назначено. **Уточнение 2026-08-13 (по факту):** Фаза 9 на момент этой
  записи ещё не закрыта — forms-dev перед стартом Angular сверила реестр и нашла, что «61/61»
  было неверным отчётом, реально 44/61 (см. запись Фазы 9 выше, Этап 8). Angular блокирован не
  формально «после Фазы 9», а именно до зелёного Этапа 8 с механической сверкой по реестру, не
  на слово — та же ошибка на числах полей повторилась дважды подряд. Начинать после того, как
  Фаза 9 (Vue-паритет) закроется и API `forms-core`/`createField`-контракт устаканится — тогда
  Angular-адаптер пишется по готовому
  шаблону, а не с нуля.
