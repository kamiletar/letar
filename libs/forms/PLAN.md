...
**Этап Г — источники данных** (§16.8, §16.9) — `forms` 2.22.0, `forms-core` 0.19.0, `forms-react` 0.14.0,
...
`@letar/forms-query`). Вопросы 23–27 остаются открытыми — владелец по ним ещё не высказался; связанные с ними
шаги (`loading` у Select, `useSelected`, `RelationConfig.fieldProps`, relation + combobox) `forms-dev` не берёт,
пока нет ответа.
