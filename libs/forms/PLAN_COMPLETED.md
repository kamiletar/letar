...
`forms-domwellbes-2026-09-24`, п.1). Недозакрытая часть 2.16.8: там починили запись
«черновика» на шуме стора, но не удаление уже записанного.

**Механизм:** `subscribeToFormChanges` при `snapshot === baseline` делал `return`, не трогая
`localStorage`; `useFormPersistence` при монтировании открывал диалог по самому факту наличия
записи, не сравнивая её со значениями формы.

**Решение (`use-form-features.ts`):** единая `toSnapshot` (без `sensitivePaths` и `excludeFields`)
для baseline, живых значений и черновика; `clearSavedData()` при возврате к baseline (флаги
черновика читаются из `draftStateRef`, чтобы не перезапускать подписку); эффект тихо удаляет и
закрывает диалог, если загруженный черновик равен baseline. Пока диалог открыт, черновик не
принят — шум стора его не удаляет (отдельный тест). Паритет: персистенции нет в shadcn/vue/angular
скинах, долга нет.

**Проверка:** `use-form-features-stale-draft.spec.tsx` — 6 тестов, 4 красные до фикса;
`nx test forms` 866/866, lint, `typecheck:tsgo` зелёные.
...
ключу/бросило исключение — откатиться на встроенный словарь по `locale`, если и там пусто — на
жёстко заданный дефолт» в трёх независимых местах: `combinedT` и `getLocalizedValue`
(`libs/forms-react/src/lib/i18n/form-i18n-provider.tsx`) и новая
`resolveDefaultErrorsTitle` (`libs/forms/src/lib/declarative/form-errors.tsx`, добавлена
предыдущей сессией при локализации заголовка `Form.Errors`). Попросил оценить, стоит ли
унифицировать, или разница между местами делает механическое объединение бессмысленным.

**Решение:** извлечён минимальный общий примитив
`resolveTranslation(t, key, params?)` в `libs/forms-core/src/lib/i18n/resolve-translation.ts` —
только общая часть (try `t()`, сравнение с пустой строкой/ключом, try/catch). Источник
встроенного словаря (`createBuiltinTranslateFunction` в одном случае, локальный
`Record<string, string>` в другом) и итоговый жёстко заданный дефолт остались на месте вызова
в каждом из трёх мест — их объединение усложнило бы код без реальной выгоды, поскольку сами
словари и порядок фолбэков (например, проверка `i18n.enabled` до вызова `t` в
`resolveDefaultErrorsTitle`) у каждого места свои. Приватный `tryTranslate` внутри
`create-form-error-map.ts` (четвёртая похожая, но не идентичная реализация — с дополнительной
проверкой `result.startsWith(key)`) намеренно не тронут — не входил в область задачи и
поведенчески отличается.

**Версии:** `@letar/forms-core` 0.12.4 → 0.12.5, `@letar/forms-react` 0.9.0 → 0.9.1,
`@letar/forms` 2.14.15 → 2.14.16.

**Проверка:** `nx test`/`typecheck:tsgo`/`lint`/`format` на `forms-core`, `forms-react`, `forms` —
все зелёные, существующие тесты (`form-errors.spec.tsx`, `use-resolved-field-props.spec.ts` и
др.) прошли без изменений — поведение подтверждённо не изменилось. Коммит `e1e9bcbee`.
...
английском и не резолвился через `FormI18nProvider` — русскоязычные приложения (найдено на
`aboi` sign-in) показывали смешение языков в блоке ошибок формы.

**Фикс:** `resolveDefaultErrorsTitle()` в `form-errors.tsx` — без явного `title`-пропа пробует
`i18n.t('formErrors.title')` (если приложение передало `t` в `FormI18nProvider`), иначе
встроенный словарь `BUILTIN_ERRORS_TITLE` по `locale` (ru/en), иначе прежний английский текст
(если провайдера в дереве нет вовсе — backward-compat с приложениями без `FormI18nProvider`).
Явный `title`-проп по-прежнему приоритетен. `@letar/forms` 2.14.13 → 2.14.14.

**Проверка:** живым браузером на `apps/aboi/src/app/[locale]/(auth)/sign-in` с неверными
email/паролем — заголовок стал «Пожалуйста, исправьте следующие ошибки:». `nx test`/
`typecheck:tsgo`/`lint`/`format` зелёные (801 тест). Правка в приложениях-потребителях не
требуется — дефолт локализуется сам без миграции существующих `<AppForm.Errors />`.
...
799 — `field-signature.spec.tsx`, `getContext(2d) возвращает контекст`, таймаут 5000ms.

**Root cause:** jsdom не рендерит canvas сам — контекст берётся из нативного пакета `canvas`
(peer dependency, `.node`-аддон + DLL cairo/pango/...). Стандартно установлен и работает быстро
(70-200ms) при изолированном запуске. Но у `@letar/forms` включена test isolation (по умолчанию
vitest), значит модульный реестр — и вместе с ним нативный аддон — переинициализируется заново в
каждом из 109 spec-файлов. Под нагрузкой (параллельные воркеры, общая машина) первая загрузка
иногда превышает 5с — падает не рисование подписи, а сама загрузка нативного модуля. Полностью
изолированный прогон (`-- field-signature`) стабильно проходил за ~24s (в основном за счёт этого
самого импорта), а нестабильность проявлялась только при полном прогоне под нагрузкой — то есть
это флаки окружения, не регрессия компонента `FieldSignature`.

**Фикс:** детерминированный no-op мок Canvas 2D API в `vitest.setup.ts` (`HTMLCanvasElement.
prototype.getContext`/`toDataURL`). Тесты подписи проверяют только отсутствие исключений и форму
data URI (`data:image/png...`), реальный пиксельный рендеринг не нужен — мок убирает зависимость
от нативного аддона целиком, а не просто увеличивает таймаут.

**Результат:**

- `nx test @letar/forms -- field-signature`: 24s → 6s (изолированный прогон).
- `nx test @letar/forms` (полный): 799/799 passed, ~100-104s — без роста относительно прежних
  прогонов (90-104s).
- `typecheck:tsgo`/`lint` — чисто.
...
(коммит `f0303382`) и оставался захардкожен на английском ("Clear draft").

`form-persistence.tsx`: дефолт переведён на русский ("Очистить черновик"), переопределение
резолвится через уже существующий `localizeOrFallback(i18n, ...)` по новому ключу
`formPersistence.clearDraftButton` — тем же паттерном, что и четыре строки диалога восстановления.

`nx lint forms` / `nx typecheck:tsgo forms` — чистые.
