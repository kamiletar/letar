...
| Пакет                                            | Что даёт                                                                                                                                                                                            |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`@letar/forms-query`](../forms-query/README.md) | TanStack Query для Select/Combobox: `fromSearchQuery`, `fromSelectedQuery`, `useQueryOptions`, `useLoaderQuery`, `useInvalidateAfter`; ZenStack — подпуть `/zenstack`. Ядро форм от него не зависит |
...
```tsx
import { generatePrefillUrl, useUrlPrefill } from '@letar/forms'
...
> **SSR:** при чтении из `window.location` первый рендер (сервер и гидратация) возвращает `{}`,
> значения из URL появляются после маунта — иначе клиентское дерево расходится с серверной
> разметкой (2.14.21). Внутри `<Form initialValue={{ ...defaults, ...prefilled }}>` поле получит
> значение сразу после эффекта, пока пользователь его не трогал. С явным `searchParams` значения
> считаются синхронно.
...
```bash
nx build @letar/forms    # Сборка
nx lint @letar/forms     # Линтинг
nx test @letar/forms     # Тесты
```
...
`@letar/forms` — Chakra-**скин**. Под ним два пакета, ни один из которых не знает про Chakra:

```
@letar/forms-core    ноль фреймворков — Zod-мета, валидаторы, i18n-словари, серверные ошибки,
                     security, offline, UIKit-контракт
        ↑
...
абстракций нижних слоёв.

Связывание одной строкой в `form-fields/base/primitives.ts`:

```tsx
export const { createField, FieldErrorBoundary, FieldWrapper } = createFieldPrimitives(chakraUIKit)
```

Смысл расслоения: альтернативному скину (`forms-shadcn`) не нужно ни менять поля, ни повторять
сборку формы — он даёт свою реализацию `UIKit` и переиспользует тот же композиционный слой.
