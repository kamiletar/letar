 ...
 *
 * `deploy_app(production)` в `libs/deploy-mcp` (`evaluateE2eGate`) ОТКАЗЫВАЕТ в деплое любого
 * приложения из этого списка, если e2e на staging не прошёл/не прогонялся/устарел/не на том
 * коммите. Для каждого: `deploy_app(staging)` → `run_e2e` → зелёный прогон против реального
 * staging-контейнера (не placeholder/локальный dev-сервер) — только тогда имя попадает сюда.
 *
 * ⚠️ **2026-09-22: было два списка — `E2E_GATED_APPS` (warn-only, только предупреждает) и
 * `HARD_GATED_APPS` (fail-closed, строгое подмножество первого, инвариант закреплялся тестом).
 * Схлопнуты в этот один** — владелец сначала уравнял оба списка буквально («всё, что деплоится
 * на прод, должно перед этим проходить e2e», warn-only как постоянное состояние для готовой
 * staging-инфры больше не нужен), затем прямо попросил снять саму эту раздвоенность: «мы же
 * договорились, что у нас теперь всё HARD_GATED_APPS без исключений». Раз ни одно приложение не
 * должно годами сидеть в warn-only — держать для этого отдельный список смысла не было, только
 * риск рассинхрона (`studio` уже расходился на три недели, см. историю ниже). Владелец решил
 * применить fail-closed сразу ко всем пяти активным коммерческим приложениям, без warn-only
 * периода — реакция на прод-инцидент archetest v0.25.5 (сломанный рендер, не пойманный
 * HTTP-проверками деплоя). `studio` добавлен позже (2026-08-06) — тоже коммерческое приложение
 * (тайм-трекинг и инвойсы владельца), изначально пропущено; расхождение со старым warn-only
 * списком (тогда назывался `E2E_GATED_APPS`) держалось три недели, потому что константу не
 * читал никакой код — с единым списком такой класс рассинхрона больше не воспроизводим.
 *
 ...
 * списка, хотя выручки за ним нет. К моменту включения уже имел 18 тестов в четырёх
 * spec-файлах, свой `docker-compose.staging.yml` и зелёный последний прогон.
 *
 * `grandslamcup`, `time`, `aira-web`, `domwellbes`, `kami`, `form-example`, `driving-school`,
 * `mandala` добавлены 2026-09-22 — у каждого уже есть `docker-compose.staging.yml` и как минимум
 * один зелёный прогон на s1 (проверено перед правкой через `e2e_status`, часть прогонов на
 * 2026-09-19 и раньше, `aboi` — 2026-09-21).
 *
 * `pravda` добавлен 2026-09-22 (тем же днём) — владелец: «мы же договорились, что у нас теперь
 * всё HARD_GATED_APPS без исключений». Прогон v1.9.13 (runId `83efaee4`) зелёный: 225 passed /
 * 0 flaky, остаток — 8 skipped (кластер прогресс-бара + Firefox RSC-навигация, апстрим-баг
 * vercel/next.js#85374, не наш баг) — см. apps/pravda/PLAN.md.
 *
 * Приложения БЕЗ staging-инфры/e2e-suite (пока не могут попасть в список) — `dashboard-agent`,
 * `dashboard` (нет staging-compose и не может быть: перезапускают сами себя), `umami` (сторонний
 * продукт). Приложения с e2e-suite и docker-compose.staging.yml (заведены 2026-09-22, коммит
 * `11c63e014`), но БЕЗ подтверждённого зелёного прогона на s1 — `form-docs`, `animatrona-landing`,
 * `kami-key-the-landing`, `letar-landing`. `animatrona-tracker` — есть e2e-suite, staging-compose
 * не заведён (БД+Redis+Better Auth OIDC через Ключницу, отдельная проработка). Новое приложение
 * попадает сюда сразу после первого подтверждённого зелёного прогона на staging — без
 * промежуточного warn-only периода.
 */
 ...
 * `E2E_GATED_APPS`, typecheck чист у каждого, проверено перед добавлением). Тест
 ...
 * не прошли явное решение о переносе, а не для списка целиком (см. коммент у `E2E_GATED_APPS`
