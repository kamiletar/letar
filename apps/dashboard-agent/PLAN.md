...
При добавлении cron-задач для `studio` выяснилось: приложение вообще отсутствовало в реестре —
`APP_PORTS`/`APP_HOSTS` (`lib/app-registry.ts`) и `SERVER_APPS` (`lib/server-config.ts`) нужно
трогать отдельно друг от друга, плюс третье место — `cron-jobs.json` (рантайм-файл, не в git).
Любое из трёх мест забыть — и `executeJob` падает с «Неизвестное приложение» без явной подсказки
куда смотреть. `studio` был живым приложением на проде несколько недель прежде чем это вскрылось.

**Предложение:** либо генератор `new-app` (`libs/generators`) сам добавляет запись в
`app-registry.ts`/`server-config.ts` при скаффолдинге, либо слить `APP_PORTS`+`APP_HOSTS`+
...
### Устаревшее описание cron-задачи `s2-database-backup` (найдено MagentaGlen, 2026-07-28)

`DEFAULT_CRON_JOBS` в `src/lib/cron.ts:115` — описание «Автоматический бэкап всех БД на s2
(driving-school)» вводит в заблуждение: задача бэкапит **все** БД из `APP_CONFIG` (сейчас 15
приложений, включая studio с 2026-07-28), driving-school упомянут как исторический пример.
Мелкая правка формулировки.
