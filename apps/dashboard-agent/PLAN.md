...
| Плановая остановка простаивающих staging-контейнеров (s1, 24ч idle)      | ✅ Готово (2026-08-28, `0.15.18`) | P1        |
| Плановая чистка `.next/cache` в host-чекауте (s1+s2, 2 дня idle)         | ✅ Готово (2026-08-28, `0.15.19`) | P1        |
...
| Плановая чистка `.nx/cache` в host-чекауте (s1+s2, 2 дня idle)           | ✅ Готово (2026-09-06, `0.16.2`)  | P1        |
| Плановый ретеншн тегов Docker Registry на s1 (TS-порт registry-gc.sh)    | ✅ Готово (2026-09-06, `0.16.5`)  | P2        |
| Таблица маршрутов s1-деплоя отдельно от капнутого лога (§157)            | ✅ Готово (2026-09-22, `0.18.1`)  | P2        |
| Cron-эндпоинт `anonymize-retail-customers` (domwellbes) не был в реестре | ✅ Готово (2026-09-23, `0.18.2`)  | P1        |
| Фильтрованный `run_e2e` (grep/project) писал зелёный статус гейта        | ✅ Готово (2026-09-23, `0.18.4`)  | P1        |

**Плановый ретеншн тегов Docker Registry (2026-09-06, `0.16.5`):** `infra/registry/README.md`
документировал ретеншн (`scripts/registry-gc.sh`) как «пока только вручную» — TS-порт
(`lib/registry-gc.ts`, job `registry-gc-s1` (до 2026-09-19 — `registry-gc-s3`), `50 4 * * *`, server: `s1`) закрывает разрыв.
Отличия от bash-версии продиктованы средой контейнера: `fetch` вместо `curl`/`jq` (их нет в
`Dockerfile.production`), `garbage-collect` через уже используемый в приложении `dockerode`
(`container.exec()`, тот же паттерн, что `database.ts` — `pg_dump`), а не через `docker-cli`.
Секреты `REGISTRY_USER`/`REGISTRY_PASS` — в `.env.docker.enc` (только s1 их использует, но
файл общий с s2 — `env_file:` передаёт лишние переменные безвредно). Bash-скрипт не удалён,
остаётся для ручных `DRY_RUN`-прогонов. Подробности — `PLAN-INFRA-6.md` §157.
...
`nx-cache-cleanup-s2`/`-s1` (`lib/nx-cache-cleanup.ts`, `40 4 * * *`) удаляет записи
...
✅ **Подтверждено на проде в тот же день** (s2, образ `80b3c8c45`): при старте нового контейнера
восстановлено 188 записей логов cron по 41 задаче и запись текущего деплоя, строк
`Stream isn't writeable` нет вовсе. Наблюдаемость self-deploy восстановлена — `deploy_wait`
впервые досмотрел деплой dashboard-agent до финального статуса, без добивания через SSH.
⚠️ Прогноз «эффект проверится только на следующем деплое» оказался пессимистичным: старый
контейнер успевает записать свой деплой в Redis до смены, поэтому новый читает его сразу.
...
`e2e-gate-filtered-run`). Локальный MCP `letar` подхватит deploy-mcp 0.7.3 после перезапуска.

**[ ] ⚠️ Открытый вопрос: выносить ли `getRedisWhenReady()` в `@letar/redis-client`.** Хелпер
ожидания готовности клиента (`lib/redis.ts`, 2026-09-06) сейчас живёт локально в dashboard-agent,
хотя причина гонки — в самой библиотеке (`lazyConnect` + `connect()` без await). Не вынесен сразу
осознанно: два других потребителя (`animatrona-tracker`, `svoichuzhie`) читают Redis только в
обработчике запроса и гонке не подвержены, то есть выгоды от переноса сегодня нет, а правка общей
библиотеки задевает три приложения. Пересмотреть, когда появится четвёртый потребитель с чтением
на пути старта процесса — до тех пор класс бага закрыт докой
`.claude/docs/redis-client-not-ready-at-startup.md`, а не кодом.
