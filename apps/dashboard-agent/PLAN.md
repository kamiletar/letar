...
| Плановый ретеншн тегов Docker Registry на s3 (TS-порт registry-gc.sh)   | ✅ Готово (2026-09-06, `0.16.5`)  | P2        |
...
(`lib/registry-gc.ts`, job `registry-gc-s3`, `50 4 * * *`, server: `s3`) закрывает разрыв.
...
Секреты `REGISTRY_USER`/`REGISTRY_PASS` — в `.env.docker.enc` (только s3 их использует, но
