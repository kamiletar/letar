...
// 30, а не 15 минут: полный набор domwellbes (~245 тестов × 5 браузеров) при workers=4 шёл
// ~15 минут и убивался на 91% — гейт не мог стать зелёным в принципе (2026-09-23). Поднимать
// workers вместо лимита бесполезно: узкое место — однопоточный Node staging-контейнера.
const E2E_RUN_TIMEOUT_MS = 30 * 60 * 1000
