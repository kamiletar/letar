/**
 * Бэкап секретов Traefik на s1 (PLAN-INFRA.md §48 M2).
 *
 * Заведён 2026-08-08, сразу после переезда staging-сервера (тогда он назывался s3) с NPM на
 * Traefik. До этого утверждение «на staging-сервере бэкапить нечего — там нет прод-БД» было верным и записано в
 ...
 * - `infra/traefik/acme/acme.json` — приватные ключи всех сертификатов s1. Восстановимо
 ...
 * (`infra/acme-dns/data/`), а сам acme-dns живёт на **s2**. На s1 такой базы нет и не будет,
...
/**
 * Файл аккаунтов lego на s1. Лежит ВНЕ workspace, поэтому монтируется в контейнер агента
 * отдельной строкой (`docker-compose.s1.yml`). Забыть про монтирование — получать
    ...
    sources: [
      {
        ...
        hint: 'проверить, смонтирован ли /home/deploy/lego в контейнер агента на s1',
      },
      {
