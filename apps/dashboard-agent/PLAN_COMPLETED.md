...
## Фильтрованный `run_e2e` писал зелёный статус гейта (2026-09-23, `0.18.4`)

Найдено на domwellbes: точечный `run_e2e` (`grep: sales-funnel-won`, `project: chromium`) записал
`{"commitSha":"d5dbfe99…","passed":true}` в `.last-e2e-status/domwellbes.json` — тот же файл,
который читает прод-гейт `evaluateE2eGate` в `libs/deploy-mcp`. Гейт не отличал один прошедший
тест от всего набора и пропустил бы прод-деплой на совпавшем коммите.

Решение — «частичный прогон может статус только ухудшить», с защитой с двух сторон:

- **dashboard-agent** — `lib/e2e-last-status.ts` (`buildLastStatusUpdate`, `isFilteredRun`,
  `describeRunScope`, тип `LastE2eStatus` переехал сюда из роута). Полный прогон пишет
  `filtered: false`; частичный зелёный возвращает `null` — файл не трогается, в лог прогона
  строка `ℹ️ Фильтрованный прогон … не обновлён`; частичный красный пишет `passed: false,
  filtered: true` + `project`/`grep`. `workers` фильтром не считается. `routes/e2e.ts` зовёт
  `recordLastStatus(run, …)` в обоих обработчиках (`close` и `error`), фильтры берутся из `run`.
- **deploy-mcp 0.7.3** — гейт отказывает на `filtered: true` при `passed: true` (вторая линия на
  случай старого агента), в причине «УПАЛ» называет фильтры. Файлы без поля `filtered` читаются
  как полный прогон — `E2E_GATED_APPS` не ломаются.

Почему не «гейт отказывает на любом filtered»: тогда точечный зелёный прогон затирал бы
предыдущий полный зелёный статус, и после любой отладочной проверки пришлось бы заново гонять
весь набор. Вариант «не перезаписывать зелёным» сохраняет полный статус.

Тесты: `e2e-last-status.spec.ts` (9), `server.spec.ts` в deploy-mcp (+4). Доки —
`.claude/docs/run-e2e-staging-pitfalls.md` §4, README deploy-mcp, описание `run_e2e`.
...
~2200 строк (kami) — первые ~170 вытеснены, и `deploy_status({ routeTable: true })` не находит
таблицу «Route (app)», если она среди них.

**Где таблица в логе.** Её печатает шаг `nx build <app> --verbose` в `deploy-affected.sh` — внутри
фазы `build` (`::phase:build:start`), после подготовки (git, сборка/типизация либ, туннель БД) и
**до** typecheck приложения, загрузки sourcemaps, `docker build`, `docker push` и release-фазы на
s2. Всё, что растёт после неё, вытесняет именно её первой.

**Решение — не поднимать лимит, а сохранять блок.** `lib/deploy-route-table.ts`:
`captureRouteTableLine` — чистый автомат без состояния вне массива блоков (открыт, пока нет
легенды и лимита строк; хвост легенды принимает подряд идущие строки). Вызывается из `appendOutput`
с сквозным номером строки; результат — `DeployStatus.routeTables`. Цена: килобайты на деплой (до 4
блоков × 600 строк в худшем случае), Redis-снимок почти не растёт.

**Почему не лимит ×N.** `schedulePersist` (debounce 1 с) пишет в Redis весь деплой одним `SET` —
при 2000 строках это уже ~260 КБ раз в секунду во время сборки; лимит ×3 утроил бы трафик и память
20 записей истории, а начало лога всё равно терялось бы на достаточно длинном деплое. Лимит ×N
нужен был бы, чтобы искать `grep`-ом в вытесненном — отдельная задача, не решённая здесь.

**Потребитель.** `libs/deploy-mcp/src/log-tools.ts` (`parseAgentRouteTables`): блоки агента в
приоритете, разбор `output` — запасной путь для агента < 0.18.1; `routeTables` из JSON-вида ответа
`deploy_status` убирается.
...
состояния писали в лог `Stream isn't writeable and enableOfflineQueue options is false` —
`rehydrateFromRedis` (история деплоев) и `rehydrateExecutionLogsFromRedis` (логи cron). Через доли
секунды Redis отвечал нормально, поэтому снаружи агент выглядел здоровым.

### Причина — не Redis, а окно между созданием клиента и его готовностью

`createRedisClient` из `@letar/redis-client` работает с `lazyConnect: true` и вызывает
`client.connect()` **не дожидаясь результата**, возвращая клиент немедленно. Замерено на живом
Redis: сразу после `getRedis()` статус клиента — `connecting`. Дальше складываются две вещи, каждая
из которых по отдельности правильная:

1. `getRedis()` возвращает **не-null** клиент, поэтому проверка `if (!r) return` его пропускает;
2. `enableOfflineQueue: false` не даёт команде подождать в очереди — она отклоняется сразу.

Обе функции восстановления вызываются на старте процесса, то есть ровно в этом окне.

### Почему `try/catch` не спас, хотя ошибка ловилась

Ловилась и логировалась честно. Не было **повторной попытки** — а восстановление бывает ровно один
раз за жизнь процесса. Дальше в памяти оставалось пустое состояние, и первый же новый персист
перезаписывал индекс в Redis: прежние данные исчезали безвозвратно.

⚠️ Отказ был тихим и выглядел как здоровый сервис. Всплыло только вторичным симптомом: агент
деплоит сам себя, и после self-deploy `deploy_status` через deploy-mcp отвечал
`Deploy <id> not found in history`, затем `No deploys yet` — статус собственного деплоя
приходилось добивать через SSH (`docker ps`, `docker logs`).

### Решение

`getRedisWhenReady(label, timeoutMs)` в `lib/redis.ts` поверх `waitForRedisReady(client, timeoutMs)`
— ждёт событие `ready`, досрочно выходит по `end`, ограничен `REDIS_READY_TIMEOUT_MS = 3000`.
Хелпер ожидания вынесен отдельно от геттера ради тестируемости: в него можно передать любой объект
с `status` и семантикой событий ioredis, не поднимая настоящий Redis.

⛔ **Офлайн-очередь ioredis намеренно НЕ возвращена.** Соблазн понятный — очередь как раз и
дождалась бы готовности. Но именно она 2026-08-08 уронила агента на s3 в crash loop: при
недоступном Redis команда не падает и не завершается **никогда**, Fastify убивает плагин по
`AVV_ERR_PLUGIN_EXEC_TIMEOUT` (разбор — `lib/with-timeout.ts`). Размен «зависание → быстрый отказ»
верен; недостающей была не очередь, а порядок — не отправлять команду до готовности.

Смежные правки:

- внешние границы `withTimeout` подняты до `REDIS_READY_TIMEOUT_MS + 2000`. При **равных**
  значениях внешний таймаут срабатывал бы ровно в момент готовности клиента и снова терял
  состояние — тот же симптом, другая причина;
- `rehydrateExecutionLogsFromRedis()` в `index.ts` получила границу по времени, которой не было
  вовсе: вызов стоит внутри `try/catch`, выполняющего `process.exit(1)`, — то есть повисший Redis
  имел право не пустить агента подняться.

### Проверка

Юнит-тестов было мало — они проверяют хелпер, но не саму гонку. Кроме 7 тестов `redis.spec.ts`
(готовность сразу, ожидание `ready`, отказ по таймауту, досрочный выход по `end` статусом и
событием, снятие слушателей в обоих исходах) прогнаны две живые проверки:

- **Redis недоступен:** старт 3023мс, дальше не блокируется, предупреждение печатается **один раз**
  (второй вызов уходит за 0мс — в `@letar/redis-client` уже взведён флаг `connectionFailed`);
- **Redis живой, один процесс, два пути:** старый — `status = "connecting"` и ровно прод-ошибка;
  новый — за 13мс восстановил 2 записи истории (включая `interrupted=true` у застигнутой в
  `running`) и логи cron.

✅ **Подтверждено на проде в тот же день** (s2, образ `80b3c8c45`, деплой 86с, `exitCode: 0`): при
старте нового контейнера восстановлено 188 записей логов cron по 41 задаче и запись текущего
деплоя, строк `Stream isn't writeable` нет вовсе. Со стороны deploy-mcp `deploy_wait` впервые
досмотрел self-deploy до финального статуса с полным списком фаз — наблюдаемость собственного
деплоя восстановлена.

⚠️ Прогноз «эффект проверится только на следующем деплое» оказался пессимистичным: старый контейнер
успевает персистить запись о своём деплое **до** смены, поэтому новый читает её сразу.

Класс бага вынесен в общую доку — `.claude/docs/redis-client-not-ready-at-startup.md` (механизм
живёт в общей библиотеке, а не в одном приложении). Остальные два потребителя
`@letar/redis-client` проверены и гонке не подвержены: `animatrona-tracker` (cache-aside) и
`svoichuzhie` (rate-limit) читают Redis в обработчике запроса, когда клиент давно `ready`, а первый
запрос после старта деградирует штатно — fallback на БД и fail-open соответственно.
...
пина на диапазон `^5.0.1`, но `apps/dashboard-agent/package.json` держал свой отдельный точный пин
`5.0.1` — root-манифест его не перекрывает. Проверка `grep -n '"dockerode@' bun.lock` подтвердила
единственную запись в дереве (риска дублирования физических копий не было), пин заменён на
`^5.0.1`, `bun install` и `nx typecheck dashboard-agent` — без изменений/ошибок.
...
- [x] **`imapflow` выровнен на корневую версию (dashboard-agent-dev, 2026-08-22, `0.15.13 →
      0.15.14`)** — держал собственный пин `^1.4.7`, расходящийся с корневым `package.json` и
      `apps/domwellbes` (оба `^1.7.1`); bun хостил рядом несколько версий пакета в
      `node_modules/.bun`. `bun.lock` теперь резолвит единственную `imapflow@1.7.1` во всех
      местах. Класс проблемы — см. память `feedback_root_only_dependency_versions`. Остальные
      расходящиеся с корнем зависимости этого приложения (`fastify`, `dockerode`,
      `cron-parser`, `node-cron`, `systeminformation`) — вне скоупа этой правки, заведена
      отдельная задача.
- [x] **Остальные 5 зависимостей выровнены на корневые версии (dashboard-agent-dev, 2026-08-22,
      `0.15.14 → 0.15.15`)** — `cron-parser` `^5.0.0→^5.10.0`, `dockerode` `^4.0.6→5.0.1`
      (мажорный апгрейд), `fastify` `^5.2.3→^5.12.0`, `node-cron` `^3.0.3→^4.6.0` (мажорный
      апгрейд), `systeminformation` `^5.25.11→^5.33.1`. После `bun install` `bun.lock` больше не
      хостит рядом дубли `dockerode@4.0.12`/`node-cron@3.0.3` — резолвится единственная версия
      каждого пакета. `typecheck` и полный `test` (111/111, 15 файлов, включая
      `email-canary`/`cron-retirement`/`docker-prune`) зелёные — используемый в коде API
      `dockerode`/`node-cron` не изменился при мажорном апгрейде. context7 MCP на момент правки
      был недоступен (invalid API key), верификация breaking changes сделана через
      typecheck+test, не чтением CHANGELOG пакетов.
      ⚠️ **Не убирать эти зависимости из `package.json` вслед за «версии только в корне»** —
      `Dockerfile.production` собирает изолированный мини-workspace для прод-образа: копирует
      туда ТОЛЬКО `apps/dashboard-agent/package.json` (+ 3 внутренних либы) и гоняет
      `bun install` в этом контексте, корневой `package.json` монорепо туда не попадает вообще.
      Правило «версия только в корне» — про то, что пин не должен _расходиться_ с корневым, не
      про то, что зависимость можно не объявлять в package.json приложения. Убрать её отсюда —
      сломать `bun install` в builder-стейдже (проверено на вопрос владельца в той же сессии).
- [x] **`pg` и `@fastify/cors` выровнены на корневые версии (dashboard-agent-dev, 2026-08-22,
      `0.15.15 → 0.15.16`)** — `pg` `^8.14.0→^8.23.0`, `@fastify/cors` `^11.0.0→^11.3.0`.
      `typecheck` и полный `test` (111/111, 15 файлов) зелёные — используемый в коде API
      `pg.Client` (`lib/database.ts`) и регистрация `@fastify/cors` (`src/index.ts`) не
