...
- **Фильтрованный `run_e2e` больше не пишет зелёный статус гейта** (`routes/e2e.ts`,
  `lib/e2e-last-status.ts`). Прогон с `grep`/`project` проверяет не весь набор, но записывал
  `passed: true` в `.last-e2e-status/<app>.json` — прод-гейт принимал точечную проверку одного теста
  за полный зелёный прогон (domwellbes, 2026-09-23). Теперь зелёный фильтрованный прогон файл не
  трогает, красный пишет `passed: false, filtered: true` с фильтрами. Полные прогоны пишут
  `filtered: false`. `workers` фильтром не считается.

## [0.18.3] — 2026-09-23

### Changed

- **Лимит e2e-прогона — 30 минут вместо 15** (`routes/e2e.ts`, `E2E_RUN_TIMEOUT_MS`). Полный набор
  domwellbes при workers=4 шёл ~15 минут и убивался на 91%: гейт в `deploy_app(production)` не мог
  стать зелёным в принципе. Смысл таймаута прежний — зависший прогон помечается failed.
...
- **Production-деплой на s1: «сборка здесь, релиз на s2»** (`PLAN-INFRA-6.md` §157). `POST
  /api/deploy/app` на s1 больше не отвергает `staging: false`, а запускает `deploy-affected.sh` с
  `--remote-release`. Допустимость приложения решает сам скрипт по `BUILD_ON_S1_APPS`
  (`libs/infra-config`): список здесь не дублируется, агент собран изолированно от монорепо.
  Пока список пуст, любой production-запрос на s1 заканчивается отказом скрипта, а не деплоем.

### ⚠️ Порядок выката

- До включения первого приложения в `BUILD_ON_S1_APPS` должен быть настроен канал s1→s2
  (`scripts/setup-release-channel.sh`, запускает владелец) — иначе скрипт остановится на проверке
  ключа `~/.ssh/s1_release_ed25519`.
