...
- `Card.Body` вокруг `Table.Root` на странице заказа
  (`src/app/(admin)/admin/orders/[id]/page.tsx`, таблица товаров заказа) без `overflowX="auto"` —
  на узких экранах широкая таблица вызывала горизонтальный скролл всей страницы вместо локального
  скролла внутри карточки. Найдено точечным аудитом по образцу фикса в domwellbes (61 место,
  `schedule-gantt.tsx`). Остальные два `Table.Root` в mandala (таблица заказов, таблица страниц
  контента) не обёрнуты в `Card.Body` — паттерн бага к ним не применим.
...
Продолжение в [./CHANGELOG_2026_08_09.md](./CHANGELOG_2026_08_09.md) (версии 0.22.1 — 0.28.0)
