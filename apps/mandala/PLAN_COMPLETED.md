...
- [x] `admin-orders` — `04-admin-orders.spec.ts` использовал рыхлый CSS-локатор `tr a` (первая
      ...
      Коммит `61f13bb8`
- [ ] **`09-admin-order-status.spec.ts:38/83` остаются под вопросом** — уже используют точный
      `getByRole`-локатор, но по трейсу BlackCove клик не приводит к навигации на стейдже (URL
      остаётся `/admin/orders`). Локально (prod-сборка, тот же порядок тестов) не воспроизводится.
      Гипотезы не подтверждены: перехват pointer-events на стейдже или гонка с `router.refresh()`.
      Ждём от BlackCove network-лог из трейса — был ли вообще отправлен запрос на
      `/admin/orders/[id]` в момент клика
