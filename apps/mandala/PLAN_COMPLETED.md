...
`@letar/forms` форма + `Dropzone` из `@letar/image-upload`, но сам `Dropzone` из этого не
виноват) — проверил гипотезу «это деградация конкретного dev-сервера» против «это системный
баг». Прогнал `07-full-mandala-crud.admin.spec.ts` на полностью свежем, никогда ранее не
затронутом `mandala` dev-сервере (свежий seed) — воспроизвелось 3/3 раз, деградация сервера
исключена.

**Механизм:** в первые ~1-2с после первого интерактивного пейнта форма (`useFormPersistence`)
ещё донагружает часть JS. Если `setInputFiles` на скрытом `<input type="file">` внутри
`Dropzone` попадает в это окно — файл молча теряется: `onChange` не приводит ни к вызову
`/api/upload`, ни к обновлению локального `file`-state, ни к единой ошибке в консоли. Точный
React-механизм (кандидат — hydration-mismatch recovery от какого-то `next/dynamic`-компонента)
не идентифицирован до конца — не потребовалось для практического фикса.

**Фикс:** `await page.waitForLoadState('networkidle')` сразу после навигации, до первого
взаимодействия с формой/dropzone. Подтверждено эмпирически: 3/3 падения без ожидания, 3/3
успеха с ним. Заодно опровергнута устаревшая запись в `.claude/docs/e2e-testing.md` от
2026-07-18 («`networkidle` виснет на весь таймаут в dev-режиме Next.js») — прямой замер на
текущем стеке (Next.js 16.3.5, `next dev --webpack`, Playwright 1.63) не подтвердил зависание
(0–3.3с на резолв). Доку поправил на месте.
...
страницы — воркер прошлой сессии браузера в ref не попадал, выключатель не делал ничего до
перезагрузки. Заменён на общий `ServiceWorkerRegistration` (`@letar/ui`), эталон — фикс на
`studio` 2026-09-03. Проверено на прод-сборке Playwright-сценарием: `regs: 0, caches: 0` после
выключения. ⚠️ Отдельно найдена (не фиксилась) остаточная особенность: если снимаемый воркер
ещё контролирует текущую страницу, его собственный `fetch`-обработчик (`cache.put` на каждый
ответ, `public/sw.js`) может пересоздать кеш уже после нашей чистки — снимается следующей
загрузкой, когда воркер контроль уже потерял. Не регрессия, ограничение самого Service Worker
API, тот же паттерн — у `pravda`.
...
`driving-school`. Сведены к общей библиотеке без изменения видимого поведения.
Кросс-приложенческий контекст — корневой [PLAN.md](/PLAN.md) §65.
...
### Стейджинг e2e — раунд 7: root cause найден и починен (2026-09-01)

Продолжение раунда 6 (`/repo` → «Начни с mandala»). Расхождение с рабочей гипотезой раунда 6
(«нестабильность самого staging-окружения, не баг компонента») — root cause оказался конкретным
и детерминированным, просто его не было видно на предыдущих прогонах.

**Главная причина — `Account.accountId` в `prisma/seed.ts` создавался как email, а не
`user.id`.** Better Auth 1.7+ ищет credential-аккаунт при входе строгим совпадением
`providerId+issuer+accountId`, где `accountId` обязан равняться `user.id` (не email) — это
задокументировано в `libs/auth/src/server/factories/create-dev-session-route.ts` и
`.claude/docs/better-auth-1.7-account-issuer-field.md`, но касается только кода, читающего
готовую БД. Миграция `20260827000100_backfill_account_issuer` бэкафилила поле `issuer` у старых
строк, но не трогала `accountId` — оставленный старым upsert-ключом (`accountId: admin.email`)
seed продолжал молча чинить не ту запись при каждом повторном запуске.

**Следствие:** на свежепосеянной staging-БД `/api/auth/sign-in/email` для админа отвечал
«Неверный email или пароль» — не 500, не таймаут, обычная бизнес-ошибка входа. Все три ранее
известных «навигационных» падения (`04-admin-orders.admin.spec.ts:32`,
`09-admin-order-status.admin.spec.ts:38,83`) на самом деле были не проблемой навигации, а
следствием того, что `auth.setup.ts` не мог залогинить админа — `storageState` сохранялся с
неавторизованной сессией, и первый же admin-тест, пытающийся кликнуть по ссылке за auth-гейтом,
у которой в DOM нет `href` без сессии (или редиректящей на `/login`), упирался в `waitForURL`
таймаут. `.first().click()` по «Подробнее» и precise-локатор `getByRole('link', {name:
/подробнее/i})` из раунда 6 были стабильны и корректны с самого начала — гипотеза «нестабильный
селектор» не подтвердилась ни разу за оба раунда.

**Диагностика:**

1. Rate-limit (`/sign-in/*`, in-memory, `max: 5/900s`) — рассмотрен и ОТКЛОНЁН: рестарт
   `mandala-staging-app` (обнуляет in-memory состояние) не исправил изолированный login-тест,
   в Postgres нет таблицы `rateLimit` (нет `secondaryStorage`).
2. Сверка prod: `Account.accountId` у боевого админа mandala уже верный (`user.id`) — баг
   специфичен для staging-seed'а, продакшен не затронут, отдельная миграция не нужна.
3. Живой фикс на staging (реверсивный, только для проверки гипотезы):
   `UPDATE "Account" SET "accountId" = "userId" WHERE "providerId"='credential' AND
   "accountId"='admin@elfafeya.art';` — изолированный ре-тест `можно открыть заказ`
   (`09-admin-order-status`) до фикса падал с «Неверный email или пароль», после — прошёл за
   2.8с.
4. Полный прогон (все 3 проекта) после фикса: **`unexpected` 5 → 2**. Оставшиеся два — другой
   класс, не auth (`10-integration-full-flow.admin.spec.ts:86`, `05-full-checkout.guest.spec.ts:97`).

**Фикс в коде** (`apps/mandala/prisma/seed.ts`): ключ upsert Account сменён на `accountId:
admin.id`, плюс `deleteMany` legacy-строки по старому ключу (`accountId: admin.email`) — на
случай если seed уже когда-то создал её на этой БД.

**Второстепенная причина (1 из 2 оставшихся failure) — гонка клика с `OfflineConsentBanner`.**
`05-full-checkout.guest.spec.ts:97` падал на `page.waitForURL(/\/shop\/.+/)` после клика по
первой карточке товара. Скриншот падения показал: страница осталась на `/shop`, клик не долетел
— `OfflineConsentBanner` (`position: fixed`, появляется через `delayMs=2000` после гидратации)
перехватил pointer-events поверх статичного (не `position`-контента) продукт-карточки в её
области. Раунд 6 уже координировал `OfflineConsentBanner`↔`CookieBanner` **друг с другом**
(bottom-offset через CSS-переменную) — но не рассматривал их интерференцию со **статичным
контентом страницы под ними**, что и оказалось настоящей причиной. Гонка недетерминирована по
построению (зависит от того, успевает ли тест кликнуть до истечения `delayMs`) — тот же клик в
первом тесте того же файла (строка 21) в этом конкретном прогоне прошёл, во втором — нет.
Задокументировано как новый класс (`.claude/docs/sticky-actionbar-cookiebanner-zindex-race.md`
описывает похожий, но другой конфликт — между двумя bottom-anchored компонентами, не между
баннером и обычным контентом).

Фикс — новый `apps/mandala-e2e/src/fixtures/offline-consent.ts` (`seedOfflineConsent`, зеркало
уже существующего `seedCookieConsent` из `cookie-consent.ts`) — проставляет `localStorage`
согласие заранее, баннер не рендерится вовсе. Применено в обоих тестах
`05-full-checkout.guest.spec.ts`, использующих один и тот же клик.

**Второй из двух оставшихся (`10-integration-full-flow.admin.spec.ts:86`) — не баг.**
Изолированный ре-запуск всего интеграционного flow (6 тестов, serial) прошёл чисто за 26.3с
(шаг создания товара — 2.8с при таймауте 15с). Transient-нагрузка на общий staging-контейнер
под полным ~5-минутным прогоном, тот же класс, что уже описан в комментариях самого теста
(`nextjs-server-action-redirect-race.md`) — не требует отдельного фикса.

### Стейджинг e2e — раунд 8: настоящая причина `05-full-checkout` — не баннер, а гонка гидратации (2026-09-01)

Диагноз раунда 7 про `OfflineConsentBanner` оказался неполным. `seedOfflineConsent`/
`seedCookieConsent` через `page.evaluate()` **после** `page.goto()` не срабатывали надёжно:
запись в `localStorage` из того же документа не порождает `storage`-событие — `useSyncExternalStore`
у баннера подписан именно на него и не перечитывает значение на уже смонтированном компоненте.
Промежуточный фикс — `page.reload()` сразу после сидирования — баннер действительно гасил (сверено
скриншотами), но создавал **новую** гонку: физический клик по карточке товара мог попасть в узкое
окно между `reload()` и повторным навешиванием React-обработчика на `<Link>` — событие клика
проглатывалось без единой ошибки и без fallback-навигации браузера (`preventDefault()` уже
сработал внутри обработчика Next.js, но переход к `router.push()` не долетал).

Итоговый фикс — двухчастный:

1. **`seedOfflineConsentBeforeNavigation`/`seedCookieConsentBeforeNavigation`**
   (`apps/mandala-e2e/src/fixtures/offline-consent.ts`, `cookie-consent.ts`) через
   `page.addInitScript()` — сидирует `localStorage` ДО первой загрузки страницы, баннер вообще не
   рендерится, `reload()` не нужен. Вынесено в общую фикстуру
   `apps/mandala-e2e/src/fixtures/guest.fixture.ts` (по образцу уже существующей
   `auth.fixture.ts`) и подключено во ВСЕХ 4 гостевых spec-файлах, использующих
   `page.locator('a[href^="/shop/"]').first()`: `01-public-pages`, `03-cart`, `04-checkout`,
   `05-full-checkout` — раньше защиту от баннера имел только последний.
2. **`page.goto(href)` вместо `productCard.click()`** — реальная причина, вскрывшаяся уже ПОСЛЕ
   фикса баннера: полный прогон с гашением баннера всё равно ловил `05-full-checkout` на
   `waitForURL` таймауте, скриншот подтвердил легитимную первую карточку
   (`/shop/magnit-mandala-om`) без единого баннера в кадре — страница просто не перешла никуда.
   Причина — SSR-разметка `<a href>` видна и кликабельна ДО того, как React успел навесить
   client-side обработчик `<Link>`; клик в этом окне проглатывается молча. Изолированный прогон
   ОДНОГО этого файла (без прогрева, без соседей) воспроизводил падение детерминированно на
   обоих тестах — это исключило и гипотезу «нагрузка стейджинга», и «баннер». Заменено на чтение
   атрибута `href` продукт-карточки и прямой `page.goto(href)`, не зависящий от готовности
   клиентского JS. Тем же классом оказалась и кнопка «Добавить в корзину» во втором тесте файла —
   там уже была `expect(...).toBeVisible()` перед кликом в первом тесте (даёт гидратации время
   через поллинг), но не было во втором; выровнено.

После обоих правок полный прогон (все 3 проекта, 123 теста) — **123 passed, 0 failed**. Раунд 7
зафиксировал реальный и отдельный баг (`Account.accountId`) и реальный, но не единственный
