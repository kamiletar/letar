...
> **Версия:** 0.40.5 (SSRF-фикс в `/api/og-image`)
...
## 🔴 07/08/10: форма мандалы/товара сабмитится с пустыми name/slug (2026-08-04, найдено при доверификации §30)
...
**Воспроизведено на чистой production-сборке** (`nx build` + `nx start`, без `next dev`/HMR,
без конкурентных сборок других агентов) — то есть это НЕ вариант уже разобранного и опровергнутого
в «Третьем раунде» (см. ниже) HMR-артефакта. Дважды подряд, детерминированно.
...
до Server Action, но `waitForURL` не дожидается редиректа. Debug-лог, оставленный в коде именно для
этого случая (`[DEBUG create-action-factory]`, коммит `9e2cb9ba`, 2026-07-23, «убрать после
диагностики» — не убран, диагностика тогда не была доведена до конца), показывает точную причину:

```
data: {"slug":"","name":"", ..., "imageId":"cmsf0gkg60000s8mx5a3cgd0u", ...}
```

`imageId` заполнен верно, `name`/`slug` — пустые строки, хотя `nameInput.fill(testMandalaName)`
отработал и `SlugField` их визуально показывал. Второй прогон падает уже на
`Product_slug_key`/`Mandala_slug_key` — первая попытка с пустым slug тихо создаёт мусорную запись
с `slug: ''`, вторая на неё натыкается.

**Гипотеза (не подтверждена экспериментально до конца — не удалось проверить localStorage вживую,
сервер уже был остановлен):** `createFormPersistence('mandala', undefined)` →
`useFormPersistence` (`libs/forms/src/lib/declarative/form-persistence.tsx`) при монтировании формы
читает `localStorage['form-persistence:mandala-form-new']` и, если там есть черновик от
предыдущего (в т.ч. неудачного, недокоммиченного до `clearSavedData()`) прогона, показывает
модальный Dialog «Восстановить данные?». Обе тестовые формы («создание») используют один и тот же
ключ `<entity>-form-new` для ЛЮБОГО прогона — черновики от разных попыток создания перезаписывают
и накапливаются без TTL (`ttl` не передан в `createFormPersistence`). Модалка поверх формы могла
не дать реальному вводу/сабмиту дойти до `form.state.values`, либо тест кликнул мимо (Dialog через
`Portal`).

**Что нужно, чтобы закрыть:** воспроизвести с открытым `localStorage` (DevTools/`javascript_tool`)
в момент падения — подтвердить или опровергнуть наличие ключа `form-persistence:mandala-form-new`
и открытого диалога. Если гипотеза верна — это баг `@letar/forms` (`useFormPersistence` без TTL +
без обработки диалога в e2e-тестах), делегируется через `.claude/rules/form-delegation.md`, не
чинится точечно в mandala.

**Что НЕ является причиной:** миграция на `@letar/image-upload` (§30) — `imageId` в пейлоаде
корректен на обоих прогонах, UI поля изображения (превью, кнопка удаления) отрабатывает штатно.
