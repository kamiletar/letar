...
3. **Реальная причина `01`/`05`:** локальная БД (и, по всей видимости, staging) захламлена
   ...
   ⚠️ Не до конца проверено локально: в этом рабочем дереве отсутствуют исходные файлы
   `uploads/mandala/*` (сама мандальная часть сида тоже локально даёт 0 — не хватает файлов), хотя
   на staging они точно есть (BlackCove ранее подтвердил успешный сидинг 31 мандалы) — логика
   идентична уже рабочему паттерну сидинга мандал, но end-to-end подтверждение только на staging.

**Не мой фикс, но нужно сделать на staging перед следующим прогоном:** очистить/пересидить
staging БД от накопленного мусора прошлых неудачных e2e-прогонов (тестовые Product/Mandala без
`test.afterAll`-очистки) — `prisma/seed.ts` использует `upsert`, поэтому сам по себе не удаляет
посторонние записи. Без этого `01`/`05` могут продолжить падать даже после фикса изображений, если
мусорная запись всё ещё сортируется первой.
...
**41/41 passed** локально (production-сборка, с параллельной нагрузкой). Запрошен финальный
деплой+прогон на staging для официального подтверждения.
