...
- [ ] ⚠️ Открытый вопрос: фикс входа/регистрации по email+паролю (см. `PLAN_COMPLETED.md`,
      2026-08-28) закоммичен (`49de3021`), но не запушен и не задеплоен — `git push` летар
      заблокирован pre-push хуком из-за **несвязанного** незапушенного коммита в submodule
      `apps/aprel8008` ("зафиксировать первый биографический экран", SHA `0cddd5fb`). Нужно
      решение владельца: либо запушить aprel8008 (не моя задача/контекст), либо явно разрешить
      обход (`GIT_ALLOW_UNPUSHED_SUBMODULES=1 git push`) и оставить aprel8008 непушенным до его
      собственной сессии. До push — деплой mandala с этим фиксом невозможен через deploy-agent-dev.
...
- [x] ~~Нестабильность shop→product/cart/checkout навигации в полном e2e-прогоне на staging~~ —
      найдено 2026-08-25 (Раунд 6), главный root-cause найден и починен 2026-09-01, разбор в
      [PLAN_COMPLETED.md § Раунд 7](./PLAN_COMPLETED.md#стейджинг-e2e--раунд-7-root-cause-найден-и-починен-2026-09-01).
      Главная причина — `Account.accountId` в seed-скрипте создавался как email вместо
      `user.id`, несовместимо с Better Auth 1.7+ (админ не мог залогиниться на свежепосеянной
      staging-БД) — это объясняло 3 из 5 unexpected (все admin-orders навигационные тесты).
      Проверено живым SQL-фиксом на staging + изолированным ре-тестом: 5 unexpected → 2.
      Из оставшихся двух: один — реальная гонка клика с `OfflineConsentBanner` (починена
      seed'ом localStorage-согласия в тесте, код готов, ⚠️ **ещё не задеплоен** — нужен
      деплой + повторный полный прогон для финального подтверждения 0); второй —
      `10-integration-full-flow.admin.spec.ts` прошёл чисто в изолированном ре-запуске
      (2.8с), это transient-нагрузка на staging под полным ~5-мин прогоном, не баг.
      Готово к добавлению `mandala` в `E2E_GATED_APPS` после деплоя + чистого полного прогона.
