...
- [x] ~~Нестабильность shop→product/cart/checkout навигации в полном e2e-прогоне на staging~~ —
      ...
      и [§ Раунд 8](./PLAN_COMPLETED.md#стейджинг-e2e--раунд-8-настоящая-причина-05-full-checkout--не-баннер-а-гонка-гидратации-2026-09-01).
      Три независимых причины: (1) `Account.accountId` в seed-скрипте создавался как email
      вместо `user.id` — админ не мог залогиниться на свежепосеянной staging-БД; (2)
      `OfflineConsentBanner`/`CookieBanner` не гасились надёжно старым `page.evaluate()`-сидом
      согласия (same-document `storage`-событие не срабатывает); (3) главная — клик по SSR-
      разметке `<Link>` мог попасть в окно до навешивания React-обработчика (гонка
      гидратации), событие проглатывалось без единой ошибки. Фикс (3) — `page.goto(href)`
      вместо клика, применено во всех 4 гостевых spec-файлах через общую фикстуру
      `guest.fixture.ts`. Финальный полный прогон (все 3 проекта, 123 теста) — **123 passed,
      0 failed**. `mandala` готов к добавлению в `E2E_GATED_APPS`.
