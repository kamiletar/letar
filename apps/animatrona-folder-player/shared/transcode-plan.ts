...
export function buildCodecArgs(plan: TranscodePlan): string[] {
  ...
  if (plan.videoAction === 'transcode') {
    args.push(
      ...
      '-row-mt',
      '1',
      // Регулярный интервал ключевых кадров (2с при 24fps) — не влияет на цельнофайловый режим,
      // но даёт предсказуемые границы фрагментов fMP4 для потокового старта воспроизведения
      '-g',
      '48',
      '-keyint_min',
      '48',
