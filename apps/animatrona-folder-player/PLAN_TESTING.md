...
- [x] `nx test animatrona-folder-player` — vitest по чистой логике в `shared/`
      (112 тестов: детекция неподдерживаемых кодеков/контейнеров, эскалация плана
      транскодирования, раскладка/WebVTT превью-спрайта). Таргет inferred от плагина
      `@nx/vitest`, конфиг — `vitest.config.mts`.
