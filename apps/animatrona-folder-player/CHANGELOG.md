...
- **Выбор внешней аудиодорожки** (`.mka`/`.flac`/`.opus`/... файлы в подпапках вида
  `Rus sound/`) — второй баг-репорт после релиза выбора аудиодорожки: «субтитры открываются, а
  вот аудиодорожки нет» на реальной раздаче с русской озвучкой отдельными файлами. Не
  регрессия — единственный встроенный поток корректно давал скрытую кнопку. Библиотечная
  инфраструктура (`external-audio-scanner.ts`, `useExternalAudio`, `player.externalTracks.audio`)
  уже существовала и использовалась в `animatrona`, но не была подключена к UI этого приложения.
  `AudioTrackSelector` теперь показывает embedded- и external-дорожки в одном списке
  (`embedded:`/`external:` префиксы), внешняя дорожка воспроизводится отдельным `Audio()`-
  объектом синхронно с видео (мьютит видео, синхронизация play/pause/seek/rate/volume).
  Проверено автоматизированно на реальной папке пользователя.

### Changed

- **Hi10P-видео теперь перекодируется в VP9 profile 2 (10-бит), а не в 8-битный H.264** —
  предыдущий фоллбэк деградировал цвет без необходимости: проблема была в кодеке, который
  Chromium не декодирует ни на какой системе, не в глубине цвета. VP9 profile 2 остаётся
  10-битным и декодируется Chromium софтверно (libvpx), без зависимости от GPU. Скорость
  кодирования сопоставима с прежним x264-фоллбэком (~6x реалтайма), проверено на реальном
  файле целиком, включая перемотку и звук.

### Fixed

- **«Пропустить опенинг» ставил видео на паузу** — кнопка не останавливала всплытие клика,
  и он доходил до кликабельного видеоконтейнера (клик = play/pause), выключая
  воспроизведение сразу после перемотки. Фикс — `event.stopPropagation()` в
  `ChapterSkipButton` (`@letar/video-player-react` 0.2.6 → 0.2.7), тот же паттерн, что уже
  стоит у `SharedPlayerControls`/`SharedProgressBar`.
...
- **Выбор дорожки субтитров** — кнопка на панели плеера (`SubtitleTrackSelector`, кастомный
  ...
  и встроенные в контейнер дорожки (`player.embeddedTracks.subtitles`, метаданные уже есть из
  пробы mediainfo.js — доп. чтения файла не требуют). Автовыбор по умолчанию не изменился: внешний
  субтитр эпизода, иначе первая встроенная дорожка — теперь просто ещё и переключаемо руками.
  Извлечение содержимого встроенной дорожки (`matroska-subtitles`, потоковый разбор всего файла)
  запускается лениво — только когда встроенная дорожка реально выбрана, не на каждый эпизод.
  Кнопка не показывается вовсе, если у эпизода нет ни одной альтернативы. Аудио — отдельная задача
  (риск неоднозначен: нужно проверить, отдаёт ли Shaka Player в режиме `src=` нативные
  `HTMLMediaElement.audioTracks` браузера как `variant`-треки).
...
- Отсутствующие иконки в `resources/` (`icon.ico`/`icon.png`/`icon-*.png`) — падал
  ...
  приложения.
- **Периодический спиннер посреди воспроизведения** (плеер играл пару секунд, показывал
  загрузку, продолжал с той же позиции — каждые ~5 сек весь сеанс). Причина — `startTime` в
  deps главного эффекта `useShakaPlayer` (`@letar/video-player-react`): фоновое автосохранение
  прогресса просмотра меняло значение `resumeTime`, что триггерило полную пересборку Shaka
  Player посреди штатного проигрывания. Фикс — снапшот через `startTimeRef`, эффект
  пересоздаёт плеер только при реальной смене `src`.
- **История открытых папок не сохранялась между перезапусками приложения.** SSR-гейт
  `mounted ? window.localStorage : noopStorage` в `page.tsx` подменяет хранилище уже ПОСЛЕ
  того, как эффект загрузки в `useFolderHistory` успевал отработать один раз с заглушкой
  (`deps: []`). Фикс — `deps: [storage]`, эффект перечитывает хранилище повторно при смене
  референса storage.
- Кнопки перемотки на ±10 сек (`LuSkipBack`/`LuSkipForward`) визуально путались с кнопками
  переключения серии — заменены на `LuRotateCcw`/`LuRotateCw` с числом секунд поверх иконки
  (`SharedPlayerControls`, `@letar/video-player-react`).
