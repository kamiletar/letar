...
- [x] **Аудиодорожки.** В отличие от `animatrona` (там «переключение аудио» на самом деле означает
      ...
      `audioTrack` браузера. Живого подтверждения владельцем на реальном сериале (изначальный
      баг-репорт «не вижу выбора аудиодорожки») — пока нет, записать сюда результат после теста.
