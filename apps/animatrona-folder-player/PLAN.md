...
- [x] **Аудиодорожки.** В отличие от `animatrona` (там «переключение аудио» на самом деле означает
      ...
      dropdown). Кнопка автоматически не рендерится, если `getVariantTracks()` вернула только
      один вариант — прежний риск («не проверено эмпирически, отдаёт ли Chromium несколько
      `audioTracks` для локального MKV») закрывается сам собой: если браузер не даёт несколько
      дорожек, кнопка просто не появляется, плеер не ломается. ⚠️ Живого подтверждения на
      реальном сериале с несколькими аудиодорожками ещё не было — записать сюда результат.
