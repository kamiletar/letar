...
- [x] **Выбора дорожек нет — субтитры закрыты, аудио отдельной задачей (см. §17).** Ни аудио-, ни
      сабо-селектора в UI не было вообще — `mediaInfo` (`audioTracks`/`videoTracks`) уже приходил
      из `probe:file`, но никуда не выводился, кроме `codecSupport`. Ближайший образец —
      ...
      самая заметная дыра из трёх исходных замечаний. Субтитры сделаны §17, аудио — новая
      территория для кодабазы (риск описан там же), сделано отдельным заходом.
...
- [x] **Кнопка выбора дорожки субтитров на панели плеера.** Новый `SubtitleTrackSelector`
      ...
      - Регресс-тестов не добавлено (UI-компонент с интерактивным dropdown, авто-выбор проверен
      глазами при следующем живом прогоне владельца — записать сюда результат после проверки).
- [ ] **Аудиодорожки — отдельная задача, не начата.** В отличие от `animatrona` (там «переключение
      аудио» на самом деле означает загрузку ДРУГОГО файла — другой IPFS CID на другую дорожку —
      не наш случай, у нас все аудиодорожки лежат в одном MKV), здесь нужно переключение дорожки
      внутри уже загруженного файла. Нигде в этой кодабазе такого переключения ещё нет. Кандидат —
      `getVariantTracks()`/`selectVariantTrack()` у Shaka Player: в режиме прямого `src=`
      (не DASH/HLS манифест — наш случай для локальных MKV) Shaka документированно проксирует
      нативные `HTMLMediaElement.audioTracks` браузера как `variant`-треки. `ShakaPlayerInstance` в
      `useShakaPlayer.ts` (`@letar/video-player-react`) сейчас минимальный интерфейс — этих методов
      в нём нет, добавлять аддитивно. Риск: не проверено эмпирически, отдаёт ли Chromium/Electron
      реально несколько `audioTracks` для локальных MKV с несколькими аудиопотоками — если нет,
      показывать в меню единственную дорожку без ошибки, не ломать плеер.
