...
export async function createTestUser(data: {
  ...
  // upsertCredentialAccount кодирует, чего требует Better Auth 1.7+ от credential-Account —
  // строгое совпадение providerId+issuer+accountId (accountId = userId, не email). См.
  // .claude/docs/better-auth-1.7-account-issuer-field.md и тот же фикс в
  // create-dev-session-route.ts (@letar/auth).
  if (existing) {
