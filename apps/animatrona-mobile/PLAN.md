...
- [ ] **Обновить react-native 0.85.0 → 0.87.0** (свой пин в `package.json`, отдельный от
  ...
  **Остаточный `TS2719` (дубль `ViewStyle`/`DimensionValue`) объясняется не одной причиной.**
  `@types/react-native` закрывал только часть — на 0.87 остаётся второй, более глубокий дубль:
  в дереве одновременно 4 разные версии `react` (19.2.3/19.2.6/19.2.7/19.2.8) — корневой
  `package.json` пинит `^19.2.8`, `animatrona-mobile`/`animatrona-tv` пинят точную `19.2.3`,
  плюс транзитивные peer у `react-native-video`/`vision-camera` и похожих резолвят ещё две.
  Ослабление пина mobile до `^19.2.3` не схлопнуло версии. Это репо-широкая унификация `react`,
  вне скоупа одного приложения (задевает ~20 веб-приложений через корневой `^19.2.8`) — решение
  за координатором.
  ...
  **Следующая попытка = сначала унификация `react` по монорепо, потом бамп версии** — код уже
  готов (все либы/hooks/экраны типизированы под 0.87), обязательно тестировать тач-хендлинг/
