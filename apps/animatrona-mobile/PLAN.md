...
- [x] ⚠️ **Унификация версии `react` по монорепо — решено 2026-08-19/20.** Владелец (Ками) решил
      через координатора GrayMill (сообщение #383, thread `cascade-rn-087-migration`): версии
      shared-зависимостей — только в корневом `package.json`, приложения не держат свой пин без
      технической причины. Убран `"react": "19.2.3"` из `apps/animatrona-mobile/package.json`
      (параллельная сессия `animatrona-tv-dev` независимо убрала такой же пин из
      `apps/animatrona-tv/package.json`, коммит `7e95b5ae`). После `bun install` в `node_modules/.bun`
      всё ещё физически лежали 4 версии — оказалось, это не резолюция `bun.lock` (там уже одна
      `react@19.2.8`), а непрочищенный кеш изолированных установок. `bun install --force`
...
- [ ] ⚠️ **Открытый вопрос: `react-native-gesture-handler` на nightly-версии.** Поставлена
      `3.3.0-nightly-20260824-5de6d2358` — временный обход, т.к. stable 3.2.1 не собирается под RN
      0.87 (удалён внутренний Android API `getZIndexMappedChildIndex`). Nightly не предназначена
      для прода. Следить за релизами gesture-handler и переключить на первый stable с поддержкой
      RN 0.87, когда выйдет — `bun add react-native-gesture-handler@latest` в корневом
      `package.json`, проверить typecheck + сборку.
- [ ] ⚠️ **Открытый вопрос: PiP и remote-control не проверены на RN 0.87.** Тест на устройстве
      2026-08-25 закрыл только базовый тач/навигацию (нет подключённого Desktop/Tracker-сервера
      для реального видеопотока в той сессии). Именно эти два сценария — заявленная известная
      категория регрессий на миграции RN 0.85→0.87 (см. `CLAUDE.md`) — нужен отдельный проход с
      реальным видео перед релизом.
  ...
  **2026-08-19: повторная попытка, до 0.87.0, синхронно и в мобильном, и в корне (включая
  ...
  `animatrona-tv` оба зелёные на откате.

  **2026-08-19, тем же вечером: миграция типов выполнена по задаче координатора GrayMill**
  (thread `cascade-rn-087-migration`), версия при этом осознанно НЕ поднята. Все правки
  обратно совместимы с текущей 0.85 (проверено — она и была основной причиной оставить их):

  - `libs/exoplayer-ass`/`libs/exoplayer-sync`: глубокие импорты `Libraries/Types/CodegenTypes`
    и `Libraries/Utilities/codegenNativeCommands` заменены на публичные `CodegenTypes`/
    `codegenNativeCommands` из корня `'react-native'` (доступны уже в 0.85, коммиты
  ...
  - `usePictureInPicture.ts`/`useRemoteControl.ts`: `NativeEventEmitter.addListener`
    перетипизирован под `(...args: readonly Object[]) => unknown`, приведение события внутри
    колбэка (коммит `e4264c23`).
  - Заодно всплыли и починены 2 доп. breaking change 0.87, не входившие в исходный список:
    `StatusBar` потерял `backgroundColor`/`translucent` (Android теперь всегда edge-to-edge,
    убраны из `App.tsx`); реф `Animated.View` в `SeekBar.tsx` больше не даёт типизированный
    доступ к `measureInWindow` через `View` — заменён на узкий локальный интерфейс + `as never`
    (паттерн уже использовался в `exoplayer-sync/index.tsx`).
  - Устаревшая devDependency `@types/react-native@^0.73.0` в обеих либах убрана — реальная
    причина части `TS2719` на 0.87 (RN несёт свои типы с 0.71+, community-пакет конфликтовал
    с bundled).

  **Остаточный `TS2719` (дубль `ViewStyle`/`DimensionValue`) объясняется не одной причиной.**
  `@types/react-native` закрывал только часть — на 0.87 оставался второй, более глубокий дубль:
  в дереве одновременно 4 разные версии `react` (19.2.3/19.2.6/19.2.7/19.2.8) — корневой
  `package.json` пинит `^19.2.8`, `animatrona-mobile`/`animatrona-tv` пинили точную `19.2.3`,
  плюс транзитивные peer у `react-native-video`/`vision-camera` и похожих резолвили ещё две.
  Ослабление пина mobile до `^19.2.3` не схлопнуло версии.

  **2026-08-19/20: снято решением владельца.** Оба приложения (`animatrona-mobile`,
  `animatrona-tv`, параллельной сессией) убрали свой пин `react` — версия только из корня.
  Оставшиеся 4 копии в `node_modules/.bun` оказались непрочищенным кешем, а не реальным
  разъездом резолюции: `bun.lock` уже держал одну `react@19.2.8`, `bun install --force`
  физически схлопнул кеш до одной копии. Открытый вопрос закрыт, детали — в задаче
  «Унификация версии `react`» выше.

  **Следующая попытка бампа `react-native` до 0.87** — код уже готов (все либы/hooks/экраны
  типизированы под 0.87), блокер версии `react` снят. Обязательно тестировать тач-хендлинг/
  жесты на реальном устройстве перед релизом (см. `CLAUDE.md`).
