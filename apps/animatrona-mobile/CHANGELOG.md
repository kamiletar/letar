...
- Четыре локальные копии `formatTime` заменены на `formatDuration` из `@letar/animatrona-shared`
  (`SeekBar.tsx`, `AnimeScreen.tsx`, `GestureOverlay.tsx`) — задача от координатора экосистемы
  Animatrona. Паттерн в приложении уже был: `ResumeOverlay.tsx` пользовался общей функцией,
  остальные точки с ней разошлись

### Fixed

- Таймкоды длиннее часа отображались как минуты: фильм на 1:35:30 показывался в `SeekBar` как
  `95:30`. Три из четырёх копий `formatTime` не умели формат `h:mm:ss` — общая `formatDuration`
  умеет. Затронуты полоса перемотки, прогресс на странице аниме
- Битая длительность (`NaN`/`Infinity`) больше не рендерится как `NaN:NaN` — общая функция
  отдаёт `--`

### Removed

- Мёртвый код: `formatTime` в `PlayerScreen.tsx` был объявлен и ни разу не вызывался
...
- `react-native` 0.85.0 → 0.87.0 (синхронно с корневым `package.json` монорепо), код был мигрирован под 0.87 API заранее (см. 0.7.4)
- ⚠️ Тест на реальном устройстве (тач-хендлинг/жесты, PiP, remote-control) не пройден — не было подключённого устройства в сессии бампа. Обязателен перед релизом.
