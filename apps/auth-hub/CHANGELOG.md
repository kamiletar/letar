...
- seed: localhost-redirect OIDC-клиента `animatrona-tracker-prod` указывал на порт 3010, а
  dev-порт приложения с 2026-09-09 — 3009 (`c52b2ce1a`). Локальный вход в трекер падал бы с
  `invalid redirect_uri`. Вступает в силу после re-seed боевого auth-hub (локальные приложения
  ходят в прод-Ключницу).

## [0.7.22] - 2026-09-15

### Added

- Сброс пароля кодом из письма (PLAN_EMAIL_CODE.md, Фаза A.4): `/forgot-password` — шаг 1
  (email → `requestPasswordReset`, всегда нейтральный ответ, не выдаёт наличие аккаунта), шаг 2
  (код + новый пароль → `resetPasswordWithCode`, разбор кодов ошибок Better Auth). Резолв
  привязанного email (Этап 8.5) на обоих шагах — код отправляется и проверяется по основному
  адресу владельца, даже если он вошёл по дополнительному. Ссылка «Забыли пароль?» на `/sign-in`.
- e2e-покрытие Фазы A (`apps/auth-hub-e2e`): регистрация → неверный код → ошибка; подтверждение
  email в БД (симуляция ссылки из письма на другом устройстве) → SSE переключает экран без
  перезагрузки; `/forgot-password` не выдаёт наличие аккаунта.

### Fixed

- Автосабмит кода при заполнении 6-й цифры не срабатывал: `Form.Field.Auto` с
  `meta.ui.fieldType` не прокидывает произвольные props (`onComplete`) в `renderFieldByType` —
  заменено на явный тег `Field.PinInput` в `verify-email-code.tsx` (баг библиотеки
  `@letar/forms`, задокументирован в `.claude/docs/letar-forms-field-auto-fieldtype-drops-extra-props.md`,
  не исправлен в самой библиотеке — вне scope этой задачи).
...
- `Table.Root` внутри `Card.Body` в `/admin/clients` и `/admin/users` вызывал горизонтальный
  скролл всей страницы на узких экранах вместо локального скролла таблицы — добавлен
  `overflowX="auto"` на `Card.Body`. Тот же класс бага, что закрыт в domwellbes.
...
- Подсказки валидации форм (`z.string().min/max`) показывались на английском —
  ...
  в `src/app/layout.tsx` (auth-hub не под next-intl, `<html lang="ru">` захардкожен). Разбор
  класса бага —
