...
- [x] Фаза 0 — общий слой (libs): подписанный ключ SSE-потока, плагин cookie, фабрика SSE-роута,
      сборка опций `emailOTP`, клиентский хук + `EmailCodePanel`. Детали — PLAN_EMAIL_CODE.md §0.
- [ ] Фаза A — auth-hub (после пилота во втором приложении): регистрация → код, SSE, OIDC-возврат
- [ ] ⚠️ Сброс пароля — **функции нет совсем** (ни страниц, ни отправки письма). Реализуется кодом
      из письма, пункт A.4 в [PLAN_EMAIL_CODE.md](./PLAN_EMAIL_CODE.md#a4-сброс-пароля-кодом-новая-функция);
      учесть вход по привязанному email и OAuth-аккаунты без пароля
- [ ] ⚠️ Открытый вопрос: подтвердить решение R3 — сброс пароля **только кодом**, без ссылки в
      письме. Ссылку и код одним письмом не собрать без повтора внутренностей Better Auth; решение
      принято при планировании (2026-09-14), владелец его явно не подтверждал. Нужно до Фазы A.
- [ ] ⚠️ Открытый вопрос: переводить ли driving-school на общие куски Фазы 0 (`emailOTP`, общий
      SSE-роут). Дыры там уже закрыты (в рабочем дереве submodule, на 2026-09-14 не закоммичено
      чужой сессией) — выгода только в меньшем объёме своего кода. Решает владелец.
...
- [x] ⚠️ Открытый вопрос: почему auth-hub не воспроизводит баг, хотя код выглядит так же —
      ...
      трогать прод-путь шифрования).
- [x] Попытка миграции на общий `createLazyPrismaAuthClient` (2026-08-31) — **отменена после
      анализа риска**, код не менялся. `createLazyPrismaAuthClient` возвращает голый нативный
      `PrismaClient` без какой-либо обёртки — у auth-hub шифрование секретов at-rest
      (`account.accessToken/refreshToken`, `oauthAccessToken.accessToken/refreshToken`,
      `oauthApplication.clientSecret`, см. `crypto-orm.ts`) живёт **только** внутри
      `wrapWithEncryption()`-прокси в `db.ts`, которым сейчас обёрнут объект, передаваемый в
      `prismaAdapter()`. Замена на `prismaAuth` из фабрики означает, что better-auth пишет
      `account`-записи (OAuth-привязка через VK/Google/GitHub/Yandex) и `oauthAccessToken`-записи
      (OIDC-выдача токенов восьми приложениям-клиентам) напрямую нативным Prisma-клиентом мимо
      прокси — секреты попадут в БД **в открытом виде**, без единой ошибки при этом (в отличие от
      500-бага, эта регрессия не самообнаруживается). Это не гипотетический риск, а прямое
      следствие устройства фабрики — она не создавалась с расчётом на дополнительный слой
      шифрования. Полный разбор и решение — `.claude/docs/better-auth-prismaadapter-zenstack-incompatibility.md`.
...
- [ ] ⚠️ **Открытый вопрос: два новых провала e2e на staging, не диагностированы.** Полный
      ...
      `2 failed` сверх уже описанной регрессии выше: (1) «OAuth-кнопки видны» — кнопка
      «Продолжить с Google» не найдена за 10с (проверено дважды подряд — не флак); правдоподобная
      причина — `getCountryCode()`/`isRussianIp` реально фильтрует google/github/facebook
      (149-ФЗ, см. `sign-in/page.tsx`), если сеть, с которой шёл прогон, резолвится как RU —
      тогда это не баг, а корректное поведение геофильтра под другим IP, не проверено, что
      именно так; (2) прямой `page.goto('/api/auth/oauth2/authorize?...')` даёт
      `net::ERR_CONNECTION_REFUSED` вместо ответа с кодом <500 — не установлено, временный ли
      сетевой сбой инфраструктуры или воспроизводимая проблема самого staging. Не решено, потому
      что вне скоупа исходной задачи (только локаторы `@letar/forms`) и требует прогона из
      другой сети/CI-раннера для отделения гео-фильтра от реального бага, а второй пункт — либо
      повторной проверки, либо логов Traefik/контейнера на момент прогона.
