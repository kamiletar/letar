...
- [ ] ⚠️ **Открытый вопрос: `/sign-in` не раскрывается из React-стриминга — вся страница
      мертва (не только формы).** Обнаружено при повторной визуальной проверке v0.7.11 после
      того, как баг `genericOAuthClient is not a function` (см. запись ниже) был закрыт другой
      сессией. После фикса SSR страницы `/sign-in` рендерится, `read_console_messages` чист, но
      весь контент правой/левой колонки (`AuthOAuthButtons`, `LoginForm`, `MagicLinkForm`) остаётся
      физически в DOM внутри `<div hidden id="S:0/S:1/S:2">` — это стандартные React 19
      Suspense-плейсхолдеры для стриминга, которые никогда не "раскрываются" (`offsetParent`
      всех `<input>` — `null` сколько угодно долго после hydration, HMR подключён, ошибок в
      консоли нет). Видна только шапка страницы и cookie-consent баннер.
      **Проверено A/B: баг НЕ связан с миграцией форм на `@letar/forms`** — временный откат
      `login-form.tsx`/`magic-link-form.tsx` на дореформенную версию (сырой `<form>`, коммит
      `29aaabdf`) дал тот же результат. `/sign-up` (тоже под `<Suspense>`, но одна граница вместо
      трёх) при этом рендерится нормально — значит дело не в Suspense как таковом, а в чём-то
      специфичном для `/sign-in` (три параллельных `<Suspense>` без fallback вокруг
      `AuthOAuthButtons`/`LoginForm`/`MagicLinkForm` в `page.tsx`, либо один из этих трёх
      компонентов синхронно бросает во время hydration и стопорит остальные reveal-скрипты).
      Не root-caused до конца — нужна отдельная сессия с полным вниманием именно на эту
      страницу (сравнить с `/sign-up`, попробовать раздельно закомментировать каждую из трёх
      `<Suspense>`-границ, проверить `PasskeySignInButton`/`OidcPendingCapture`, которые рендерятся
      вне `<Suspense>` на той же странице). Это тяжелее, чем баг v0.7.10 (там хотя бы клик не
      работал — здесь не виден ВЕСЬ блок входа по email, OAuth и passkey).
