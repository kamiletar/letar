...
- [ ] Фаза A — auth-hub (в работе с 2026-09-15): регистрация → код ✅, SSE ✅, сброс пароля ✅,
      тесты ✅ — A.1-A.6 закрыты (детали в PLAN_EMAIL_CODE.md). Осталось: A.7 (ручная проверка с
      реальным письмом, не закрыта автономно — код хранится хешем) и A.8 push/деплой (ждёт
      одобрения владельца).
- [x] Сброс пароля кодом реализован — A.4 в [PLAN_EMAIL_CODE.md](./PLAN_EMAIL_CODE.md#a4-сброс-пароля-кодом-2026-09-15)
      (2026-09-15): резолв привязанного email на обоих шагах, OAuth-аккаунты без пароля получат
      credential-аккаунт (поведение плагина — не проверено вручную, см. A.7).
- [x] Решение R3 подтверждено владельцем (2026-09-15): сброс пароля — только кодом, без ссылки.
