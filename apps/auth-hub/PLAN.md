...
- [x] Деплой v0.6.4 через BlackCove — ✅ выполнен (2026-07-16, msg #488, коммит `b7b8635`,
      ...
      до этого деплоя на проде НЕ была, весь Этап 8.5 уехал одной пачкой. ✅ E2e-предупреждение
      гейта закрыто: staging s3 передеплоен на `6935f11` (BlackCove, msg #490, миграция UserEmail
...
- [x] `infra/migrations/auth-hub-merge-accounts.ts` — параметризованный ручной инструмент merge
      двух уже существующих аккаунтов (`CANONICAL_EMAIL`/`DUPLICATE_EMAIL`/`DRY_RUN`, dry-run по
      умолчанию), проверен на локальной БД (edge-cases: конфликт `Account` providerId, конфликт
