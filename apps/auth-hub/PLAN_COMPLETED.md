...
сервере штатно, но клиентский reveal стримингового `<Suspense>` не срабатывает — весь контент
(OAuth-кнопки, email/пароль форма, magic-link форма) навсегда остаётся внутри
`<div hidden id="S:0/S:1/S:2">`, у всех `<input>` `offsetParent: null`. Помечено как более тяжёлый
баг, чем уже исправленный v0.7.10 (там хотя бы клик не срабатывал — здесь не виден весь блок входа).

**Root cause:** это не баг приложения, а известная ловушка инструментов браузерной автоматизации.
Вкладка Browser pane/`claude-in-chrome`, для которой браузер ни разу не выполнил реальный
композитинг кадра, живёт с `document.hidden === true` бессрочно. React's streaming reveal-скрипт
(`$RC`/`$RB`/`$RV`, вставляется Next.js после каждого разрешившегося `<Suspense>`-boundary) для
самого первого раскрытия на странице планирует финальный DOM-swap через `requestAnimationFrame` —
а `rAF` в таком состоянии не троттлится, а не срабатывает вообще, сколь угодно долго.

**Экспериментальное подтверждение (без единого изменения кода):** воспроизвёл зависание на
`/sign-in` через оба браузерных инструмента (Claude Browser pane и `claude-in-chrome`) —
идентичная картина в обоих. Проверил `document.hidden`/`requestAnimationFrame` напрямую:
`hidden: true`, ручной `requestAnimationFrame` не срабатывает за 500мс. Затем выполнил
`computer{action:"screenshot"}` (форсирует реальный композитинг кадра) — **сразу после этого**,
без перезагрузки и без изменений в коде, все три `<Suspense>`-boundary раскрылись:
`hiddenDivs` схлопнулся до одного постороннего div (cookie-consent баннер), все шесть `<input>`
получили `offsetParent !== null`. Это прямое доказательство: единственная причина «зависания» —
отсутствие реального рендер-кадра у тестовой вкладки, не логика `page.tsx`/`login-form.tsx`/
`magic-link-form.tsx`/`oauth-buttons.tsx`/`passkey-button.tsx`/`oidc-pending-capture.tsx`.

A/B-тест из v0.7.11 (откат форм на дореформенную версию давал тот же результат) был методологически
верным шагом, но привёл к неверному выводу «дело не в форме» — на самом деле дело было не в
приложении вовсе, а в среде проверки. `/sign-up` в тех же более ранних проверках «работал» не
потому, что там нет той же уязвимости (у него тоже один `$RC`-reveal на первом `rAF`), а по
случайности — какое-то предшествующее действие (скриншот, клик) уже успело форсировать кадр к
моменту проверки.

**Вывод для будущих сессий:** зависший стриминговый `<Suspense>` (`<div hidden id="S:N">`,
`offsetParent: null` у контента, консоль чиста) в браузерных инструментах — сначала проверить
`document.hidden`/`requestAnimationFrame` и получить хотя бы один реальный composited-кадр
(`screenshot`) **до** диагностической проверки, а не после. Тот же класс уже документирован в
`.claude/docs/raf-vs-timers-background-tab.md` и в приватной памяти
(`reference_browser_pane_hidden_raf.md`, там же случаи `libs/forms`/`form-example-e2e`) — это
третье по счёту подтверждение одного и того же механизма в разных приложениях. Никакой код не
менялся; PLAN.md обновлён, «открытый вопрос» снят.
