...
- [x] `typecheck:tsgo`/`oxlint` зелёные. `nx build` (`next build`) проверен отдельно после
      мержа в обычный чекаут (2026-08-13) — все 17 страниц (16 демо + список) собрались и
      пререндерились статически. В worktree-сессии сама правка была не верифицируема этой
