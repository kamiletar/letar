...
- [ ] Решить, где заводить фрагмент: отдельная `libs/animatrona-schema-fragments` или общий
      `libs/zenstack-fragments` вторым файлом. Склоняюсь ко второму — библиотека уже существует,
      её `src/*.zmodel` уже задуман как набор независимых фрагментов, плодить вторую либу ради
      одного файла незачем.
- [ ] Вынести `PinStatusFields`/`TrackerFields` из `apps/animatrona/schema/models/` во фрагмент и
      перевести саму Animatrona на него — **до** заведения схемы плеера, чтобы не создать вторую
      копию (тот же порядок, что и с `libs/ipfs-kubo-core` ниже). Правка `apps/animatrona/**` — не
      в scope этого приложения, идёт через `animatrona-coordinator-dev`.
