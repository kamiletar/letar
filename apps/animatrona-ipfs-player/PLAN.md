...
| Модель                          | Способ                                                                                                    |
| ------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `PinStatus` + `RemotePinStatus` | `type`-миксин — в Animatrona и в плеере набор полей идентичен, relation'ов на чужие модели нет            |
...
- [ ] **0.1 Место фрагмента.** Рекомендация — вторым файлом в существующую
      `libs/zenstack-fragments/src/` (например `animatrona.zmodel`), не заводить отдельную либу:
      библиотека уже задумана как набор независимых фрагментов, ради одного файла плодить вторую
      незачем. Требует подтверждения владельца.
- [ ] **0.2 Спроектировать минимальную схему плеера** — закрывает первый пункт «Открытых
      вопросов» ниже. Результат: список моделей плеера + для `PinStatus`/`Tracker` точное
      пересечение полей с Animatrona (что идёт в миксин, что остаётся сверху у каждого).
- [ ] **0.3 Завести фрагмент** — `PinStatusFields`, `TrackerFields`, enum `RemotePinStatus`.
      Политику `@@allow('all', true)` класть прямо в миксин: весь desktop-стек Animatrona
      работает без аутентификации, политика везде одна (в отличие от Better Auth-фрагмента, где
      её пришлось оставить приложениям).
...
- [ ] **0.7 Решить, что делать с багом form-плагина** — либо дождаться фикса от
      `forms-coordinator-dev` (тред `form-plugin-mixin-fields-lost`), либо сознательно не
      подключать `plugin formSchema` к моделям из миксинов в первой версии. Форма «добавить
      трекер» — ровно та, что попадает под баг, так что вопрос не отложить молча.
