 ...
 * Web Worker и `fetch` к соседним файлам. SubtitlesOctopus (рендер ASS-субтитров эпизода) — и то,
 * и другое разом. Привилегированная стандартная схема снимает оба ограничения, а заодно и хак
 * `assetPrefix: './'` — абсолютные пути `/_next/...` резолвятся от корня схемы, как в обычном
 * вебе. Портировано без изменений из `apps/animatrona-folder-player/main/protocols/app.protocol.ts`
 * (первого потребителя того же паттерна) — см. .claude/docs/electron-app-protocol.md.
 */
