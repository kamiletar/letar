...
import { LuArrowRight, LuBuilding, LuCalendar } from 'react-icons/lu'
...
export const metadata = {
  ...
  description: 'Управление расписанием теоретических занятий',
}
...
export default async function TheoryLessonsPage({ searchParams }: Props) {
  ...
  if (schoolId) {
    redirect(`/school/theory-lessons/${schoolId}`)
  }
  ...
  // Если только одна школа — редирект на её занятия
  if (schools.length === 1) {
    redirect(`/school/theory-lessons/${schools[0].id}`)
  }
  ...
  return (
    <VStack gap={6} align="stretch">
      ...
      <Text color="fg.muted">Выберите автошколу для управления расписанием занятий</Text>

      <SimpleGrid columns={{ base: 1, md: 2 }} gap={4}>
        {schools.map((school) => (
          <Link key={school.id} href={`/school/theory-lessons/${school.id}`}>
