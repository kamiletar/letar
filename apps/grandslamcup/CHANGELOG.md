...
Ссылки навигации футера (`public-footer.tsx`, колонки «Турнир» и «Ещё») были голым `next/link` с
вложенным `<Text fontSize="sm">` без `asChild` и без минимальной высоты — узкая строка текста не
даёт достаточной цели для тапа на мобильном (WCAG 2.5.5). Заменены на общий `TouchLink` из
`@letar/ui` (`libs/ui/src/lib/touch-link.tsx`) — минимум 44px высоты каждой ссылки, текстовые
стили (`fontSize`, `color`, `_hover`, `transition`) сохранены как пропы.
