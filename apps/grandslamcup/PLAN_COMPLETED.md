...
страницы — воркер, зарегистрированный в прошлой сессии браузера, туда не попадал, выключатель не
делал ничего. Найдено и заведено эталоном на `studio` (2026-09-03), собственная копия компонента
удалена в пользу общего `ServiceWorkerRegistration` (`@letar/ui`, снятие через
`getRegistrations()`, `unregister()` без `await`, очистка `caches`). Проверено на прод-сборке
(`next build --webpack`) Playwright-сценарием: включить/выключить с разными паузами — `regs: 0,
caches: 0` после выключения во всех случаях.
...
вручную со списком top-level директорий `src/app/**`. Забытое добавление нового top-level роута в
список превращало бы его в постоянный soft-404: `proxy.ts` трактовал бы каждый запрос к нему как
`citySlug` и делал rewrite на `/system-404`.

[src/proxy.spec.ts](/apps/grandslamcup/src/proxy.spec.ts) сверяет литерал с фактическими
директориями `src/app/**` (route groups вроде `(public)` разворачиваются на том же уровне,
приватные `_x` и динамический `[citySlug]` исключены). Проверено вручную — тест ловит
искусственно добавленный лишний сегмент. Паттерн сравнения текстом файла (без импорта `proxy.ts`,
который тянет `@/lib/db`/Prisma) — по образцу `apps/aboi/src/proxy.spec.ts`.

## 2026-08-28 — Вынос upload-валидации в `@letar/upload-validation`

Сквозной аудит безопасности нашёл `src/lib/upload/{validate-file,save-file}.ts` байт-в-байт
дублирующими одноимённые файлы в driving-school (собственный комментарий «паттерн аналогичен
driving-school» подтверждал осознанное копирование). Вынесены в общую библиотеку, оба
приложения переведены на неё, локальные копии удалены. `resize-image.ts` (sharp-ресайз) в
grandslamcup остался на месте — не часть дубля, своя логика этого приложения. Разбор и API —
[libs/upload-validation/README.md](/libs/upload-validation/README.md).
...
padding/minH, список в одну колонку на мобильном) переведены на `TouchLink` (`@letar/ui`). Найдено
доп. разведкой — первичный греп `Link asChild` этот паттерн не ловил.
