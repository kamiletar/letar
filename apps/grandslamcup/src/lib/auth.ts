...
/**
 * Better Auth конфигурация для Grand Slam Cup
 *
...
export const auth = betterAuth({
  ...
  trustedOrigins: ['http://localhost:3016', ...(process.env.BETTER_AUTH_URL ? [process.env.BETTER_AUTH_URL] : [])],
...
export async function getDbUser(session: Session) {
  ...
  return user ?? { id: session.user.id, roles: [] as ('USER' | 'ADMIN')[] }
}
