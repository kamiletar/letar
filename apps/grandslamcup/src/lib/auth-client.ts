 ...
 */
export async function signInWithLetarAuth(callbackURL?: string): Promise<string | null> {
  const target =
  ...
  try {
    ...
    if (result?.error) {
      console.error('[auth] OAuth2 error:', result.error.status, result.error.message)
