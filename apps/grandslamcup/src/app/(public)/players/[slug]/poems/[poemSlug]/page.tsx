/**
 * Публичная страница стихотворения (без привязки к городу).
 */
...
type Params = Promise<{ slug: string; poemSlug: string }>
...
export default async function PoemPage({ params }: { params: Params }) {
  const { slug, poemSlug } = await params
  ...
  return (
    <VStack gap={6} align="stretch" maxW="720px" mx="auto">
      ...
      {/* Автор */}
      <Link href={`/players/${poem.player.slug}`}>
