    ...
    - ⚠️ **Осталось 10/28, разбито по категориям:**
      - **7 — все `03-admin.spec.ts`.** ✅ **Три архитектурных бага найдены и закрыты системно
        ...
        3. Cookie ставилась под именем `better-auth.session_token` без `__Secure-` префикса —
           ...
           **Осталось:** BlackCove — передеплоить staging (подтянет 0.8.2), повторить `run_e2e`.
      ...
      - **1 — «Ближайшие матчи» не рендерится** на `/spb` — вероятно, в анонимизированном
        снепшоте нет матчей с датой в будущем относительно текущего времени сервера. Не
        разобрано глубже.
    - Подробности процесса — корневой `PLAN.md` §18 (Сессии №55/56), `apps/dashboard-agent/
PLAN_COMPLETED.md` v0.7.1, тред agent-mail `grandslamcup-staging-pilot`.
