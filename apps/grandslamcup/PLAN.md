...
37. ~~**Staging-пайплайн на s3, HTTPS-домен, deploy-mcp e2e-gate (§18 Сессия D)**~~ ✅ 2026-07-11
    ...
    - ⚠️ **E2E результат содержательно слабый: 3/28 passed.** Похоже, анонимизированный снепшот
      не содержит активного текущего сезона — домашняя страница (ближайшие матчи/таблица/
      результаты) рассчитана на его наличие, у нас только исторические данные. **Нужно решить:**
      либо синтетически пометить один из сезонов как активный при анонимизации/сидировании
      staging, либо переписать e2e так, чтобы не полагались на «текущий» сезон.
    - ⚠️ **Вероятный отдельный баг** (не связан со staging-данными): `getByAltText('Grand Slam
      Cup')` матчит 2 элемента в шапке — strict-mode violation, дублирующийся `alt` на логотипе
      (десктоп+мобильная навигация?) — стоит проверить и убрать дубликат или разные `alt`.
    - Подробности процесса и найденные инфраструктурные баги — корневой `PLAN.md` §18
      (Сессия №55), `apps/dashboard-agent/PLAN_COMPLETED.md` v0.7.1.
