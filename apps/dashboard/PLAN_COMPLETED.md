...
карточек CPU/Память/Диск), рядом с общей `formatBytes` в `src/lib/format.ts` (автоподбор
единицы B/KB/MB/GB/TB по логарифму). Найдено при аудите дублей `formatFileSize` по монорепо
(PLAN.md §65) — там `dashboard` был сознательно пропущен, так как сведение с
`@letar/format-utils` изменило бы видимый текст (либа не поддерживает принудительную единицу).

Это отдельная, более узкая задача — свести дубль _внутри_ приложения, не трогая
`@letar/format-utils`. Общая `formatBytes` расширена опциональным третьим параметром
`forceUnit?: 'B' | 'KB' | 'MB' | 'GB' | 'TB'`, локальная копия в `SystemOverview.tsx` удалена,
оба вызова (память, диск) переведены на `formatBytes(bytes, 2, 'GB')`. Видимый текст на экране
не изменился — новый параметр опциональный, ~15 остальных вызовов `formatBytes` по приложению
не затронуты (сигнатура обратно совместима).

`nx run-many -t format --projects=dashboard` → `nx lint dashboard` → `nx typecheck:tsgo dashboard`
зелёные (typecheck показал только предсуществующие ошибки Chakra-типизации в несвязанных файлах
— похоже, из-за незакоммиченных правок `libs/forms/src/lib/context.tsx` в рабочем дереве другой
сессии). 1.24.6 → 1.24.7, коммит `a3f07e90`.
