...
### 🔜 Голый счётчик посещаемости без cookie-consent gap (в дополнение к Umami)
...
**Цель метрики — только знать посещаемость (сколько раз открыли страницу), не кто и не сколько
уникальных пользователей.** Это важно для выбора реализации: как только в счётчике появляется
идентификатор (IP, хэш IP, соль, cookie) для дедупликации «уникальных» — снова всплывает вопрос
из обсуждения 2026-07-29 (IP по практике РКН/судов может быть ПДн; хэш IP не анонимизация, а
псевдонимизация — IPv4-пространство перебирается rainbow table за секунды, `ConsentLog.ipHash` в
[personal-data.md](/.claude/docs/personal-data.md) сам явно учтён как ПДн, а не «обезличенные
данные»). Раз цель — только грубое число просмотров, идентификатор не нужен вообще, и вопрос
снимается целиком, а не обходится.

**Задача:**

- [ ] Инкрементировать счётчик hits/day/domain при каждом запросе к странице приложений с
      `CookieBanner` — **не сохранять и не обрабатывать IP/UA/любой идентификатор посетителя** в
      этом счётчике, только факт запроса. Реализация — счётчик на уровне nginx-proxy-manager
      (map/counter в конфиге) либо простой middleware-инкремент в самом приложении, без БД-записи
      per-request.
- [ ] Хранить только агрегат (число за день/домен), не сырые события.
- [ ] Добавить агрегированные цифры (просмотры/день по домену) во вкладку `/analytics` дашборда
      как дополнение к Umami-графикам — с пометкой, что это грубый счётчик запросов (включая
      ботов и повторные заходы), а не уникальные пользователи/сессии.
- [ ] Если в будущем понадобится дедупликация «уникальных посетителей» — это отдельная задача с
      отдельным правовым основанием (см. обсуждение про соль с суточной ротацией и уничтожением
      после подсчёта), не расширять эту задачу молча.

**Зависимости:** нет, чисто инфраструктурная задача dashboard (+ опционально nginx-proxy-manager
на s2, если считать на уровне прокси).
