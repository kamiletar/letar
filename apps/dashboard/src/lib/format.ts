 ...
 * @param forceUnit принудительная единица измерения вместо автоподбора по логарифму
 */
export function formatBytes(bytes: number, decimals = 1, forceUnit?: 'B' | 'KB' | 'MB' | 'GB' | 'TB'): string {
  const k = 1024
  ...
  if (forceUnit) {
    ...
    return `${(bytes / Math.pow(k, unitIndex)).toFixed(dm)} ${forceUnit}`
  }
