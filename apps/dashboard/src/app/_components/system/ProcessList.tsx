...
export const ProcessList = memo(function ProcessList() {
  ...
  const memoryStats = useMemo(() => {
    ...
    const otherMemory =
      summary.used - summary.totalProcessMemory - (dockerMem?.total || 0) - summary.buffcache - summary.slab
                      ...
                      return (
                        <Box key={container.id}>
                          ...
                          <Progress.Root
                            ...
                            size="xs"
                            colorPalette={containerPercent > 20 ? 'red' : containerPercent > 10 ? 'yellow' : 'purple'}
                          >
                            <Progress.Track>
                              <Progress.Range />
                            </Progress.Track>
                  ...
                  {data?.memoryGroups.slice(0, 10).map((group) => (
                    <Box key={group.name}>
                      ...
                      <Progress.Root
                        ...
                        size="xs"
                        colorPalette={
                          group.totalMemPercent > 20 ? 'red' : group.totalMemPercent > 10 ? 'yellow' : 'green'
                        }
                      >
                        <Progress.Track>
                          <Progress.Range />
                      ...
                      {data?.topByMemory.map((proc) => (
                        <Table.Row key={proc.pid}>
                          ...
                          <Table.Cell textAlign="right">
                            <Text fontSize="sm" color={proc.cpu > 50 ? 'red.400' : proc.cpu > 20 ? 'yellow.400' : 'fg'}>
                              {proc.cpu.toFixed(1)}%
                            </Text>
                          </Table.Cell>
                          <Table.Cell textAlign="right">
