...
- `src/lib/ssl-monitor.ts`: проверка сроков сертификатов молча не работала. Она читала
  сертификаты только из API `nginx-proxy-manager-app-1:81`, а NPM с s2 снят (`ENOTFOUND`); ошибка
  возвращалась значением и нигде не обрабатывалась — ни алерта, ни строки в логе. Сертификат
  `mail.letar.best` (живёт в Maddy, к NPM отношения не имел) истёк 2026-09-20 незамеченным.
- `daysUntil`: сертификат, истёкший менее суток назад, давал `Math.ceil(-0.96) = -0`, то есть
  «истекает сегодня» с серьёзностью ERROR вместо «истёк» с CRITICAL. Теперь до срока округляем
  вверх, после срока — вниз.

### Added

- `src/lib/tls-cert-probe.ts`: прямая проверка срока сертификата TLS-подключением
  (`node:tls`, `rejectUnauthorized: false` — чтобы видеть и уже истёкший). Цели —
  `SSL_CHECK_TARGETS` (`host:port,host:port`), по умолчанию `mail.letar.best:993,mail.letar.best:465`.
- `ssl-monitor.ts`: NPM стал необязательным источником; недоступный NPM не ломает проверку.
  Активный алерт `SSL_EXPIRING` закрывается, только если проблемных сертификатов нет И все цели
  прямой проверки ответили — недоступность цели не должна выглядеть как «всё в порядке».
- `jobs/ssl-check.ts`: если цель не ответила, задача бросает ошибку, и прогон виден в журнале
  задач как неудачный (его подхватывает наблюдатель jobs → `CRON_FAILED`).
- `vitest.config.mts` (у dashboard не было ни конфига, ни таргета `test`, хотя
  `tsconfig.spec.json` на него ссылался) и 21 тест на проверку сертификатов.

## [1.26.6] — 2026-09-09

### Fixed

- `package.json`: `@letar/auth` не был объявлен ни в `dependencies`, ни в
