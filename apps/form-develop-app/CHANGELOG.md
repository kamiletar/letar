...
- `numeric-demo`: примеры `minorUnitScale` на `Form.Field.Currency`/`Form.Field.Percentage`
...
- `slug-demo`: демо `Form.Field.Slug` (v2.12.0) — сценарий создания (живое зеркалирование
  `slugify(name)` в слаг) и сценарий редактирования (синхронизация выключена по умолчанию,
  восстановление кнопкой «↺»). Ссылка добавлена на главную.
