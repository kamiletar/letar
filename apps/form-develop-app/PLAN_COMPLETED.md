...
`solid-js/web` у потребителей `@letar/query-provider`): `nx build form-develop-app` не задевает
баг — `build`-таргет запускает голый `next build` без `--webpack`, Next.js 16 по умолчанию
собирает прод-бандл через Turbopack (маркер `▲ Next.js 16.3.1 (Turbopack)`, `✓ Compiled
successfully` в логе). Сборка упала на несвязанной runtime-ошибке демо-страницы
`/controlled-state-demo` (`useFormContext` вызван вне `formComponent`, см. отдельную задачу),
но это произошло уже после успешной компиляции клиентского бандла Turbopack — до этой точки баг
с `solid-js/web` в принципе не может проявиться. Изменений в `next.config.js` не требуется.
