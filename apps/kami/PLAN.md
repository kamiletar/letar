...
- ⏳ Публичный `/links` с фильтром по категории/метке и поиском (первый слайс — только `/admin/links`)
- ⏳ Карточка ссылки: превью (favicon), полноценное редактирование метки/категории прямо из списка (сейчас только просмотр + переключение "прочитано" + удаление)
- ⏳ Admin CRUD для категорий/тегов (если нужно управлять списком меток отдельно от самих ссылок)

**Упрощения первого слайса (см. коммит):** нет мини-формы подтверждения после Share (сразу
редирект в список), нет фильтров/поиска, нет полноценного редактирования — только
просмотр/toggle-read/удаление. Живая проверка в браузере выполнена частично: подтверждены
routing-контракты (`GET /share` → 405 без locale-редиректа, `/admin/links` корректно редиректит
неавторизованного на вход, таблица `Link` создана в БД) — полный аутентифицированный проход через
реальный OAuth (Ключница) не пройден в этой сессии (нет тестовых учётных данных под рукой).
