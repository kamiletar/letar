...
## Страница «Обо мне»: hero и текст

- [ ] ⏳ Подключить hero на `/about`: адаптивный `<picture>` (WebP 16:9 для десктопа и 4:5 для
      телефона), файлы лежат в `.claude/artifacts/about-hero/` (вне git) — нужно положить в
      `public/`. Заголовок набирать HTML-текстом поверх тёмной области слева, а не оставлять в
      картинке: перевод, доступность, чёткость на любом экране.
- [ ] ⏳ Вставить утверждённую редакцию текста «Обо мне» (хранится в личных заметках владельца
      вне репозитория) и решить, чем заменить ключевые фразы на английской и японской версиях.
- [ ] ⚠️ Открытый вопрос: разрешение hero. Оригинал 1536 px по ширине, на Retina-мониторе на всю
      ширину будет мягче — перегенерировать в 2048 или апскейлить. Не решено: пока не ясно, будет
      ли картинка на всю ширину.
