...
- ⏳ **Статичная сонограмма до старта** — показывать спектрограмму файла до нажатия play:
  - `useEffect` при монтировании: `fetch(audioUrl)` → `arrayBuffer()` → `AudioContext.decodeAudioData()`
  - Офлайн FFT: разбить `AudioBuffer` на фреймы (размер окна ~2048), для каждого фрейма считать амплитуды через `OfflineAudioContext` или вручную (DFT/FFT)
  - Или использовать `AnalyserNode` на `OfflineAudioContext` с рендером через `startRendering()`
  - Результат: массив `Float32Array[]` (время × частота) → нарисовать статично на canvas
  - При старте воспроизведения — переключиться с офлайн-данных на live `AnalyserNode`
  - Прогрессивная отрисовка: показывать сонограмму по мере вычисления (не ждать весь трек)
  - Сложность: **средняя** (~2–3 часа). Основная проблема — производительность FFT на CPU для длинных треков
  - Вариант упрощения: использовать `wavesurfer.js` с `spectrogram` плагином (см. 9.5)

### 9.3 Waveform peaks на seekbar

- ⏳ **Отображение формы волны** на полосе прокрутки:
  - Тот же `decodeAudioData` (см. 9.2) → свести `AudioBuffer` до N пиков (N = ширина seekbar в px)
  - Алгоритм: разбить на N чанков, в каждом `Math.max(...samples)` для peak, `Math.min` для valley
  - Нарисовать на `<canvas>` поверх `<input type="range">` (absolute позиционирование)
  - Прогресс воспроизведения — цветом: сыгранная часть `green`, оставшаяся `gray`
  - При hover — показывать время в тултипе (позиция мыши → `time = x / width * duration`)
  - Сложность: **средняя** (~1–2 часа). Хорошо делать вместе с 9.2 (один decode → два вывода)
...
- Горизонтальный контейнер `overflow: hidden` на всю ширину экрана
- Обложка: `width = 100vh` (квадрат), `flex-shrink: 0`
- Сонограмма: `width = duration * pxPerSecond` (например 100px/сек для 5-мин трека = 30000px)
- При play: `requestAnimationFrame` обновляет `translateX = -(currentTime * pxPerSecond - centerOffset)`
- Центральная линия: псевдоэлемент или `Box` с `position: absolute`, `left: 50%`
- На перемотке (drag seekbar) — тот же translate обновляется по `currentTime`
- Сложность: **высокая** (~1 день). Ключевые проблемы:
  - Генерация статичной сонограммы нужной ширины (зависит от 9.2)
  - Синхронизация translate с audio time без дёрганья (rAF + lerp)
  - Touch/mouse drag для перемотки прямо по сонограмме
  - Производительность: canvas шириной 30000px — нужно виртуализировать (рисовать только видимую область)
...
- ⏳ **Установка:** `npm i hydra-synth` (standalone без iframe)
- ⏳ **Интеграция:**
  - Компонент `HydraVisualizer` — WebGL canvas + редактор кода (monaco-editor или textarea)
  - `hydra-synth` инициализируется с `{ canvas, detectAudio: false }`
  - Аудио пробрасывается вручную: FFT данные из `AnalyserNode` → `hydra.a.setBins(fftData)`
  - Пользователь пишет код прямо в редакторе, визуализация обновляется live (eval на Ctrl+Enter)
  - Несколько стартовых пресетов с реакцией на звук — чтобы не смотреть в пустой экран
- ⏳ **VJ-инструкция** (блок внутри режима, сворачиваемый) + отдельная страница на сайте:
  - Объяснение синтаксиса: `osc()`, `.color()`, `.modulate()`, `.out()`
  - Как подключить аудио: `a.fft[0]` (бас), `a.fft[1]` (середина), `a.fft[2]` (высокие)
  - 5–7 готовых примеров с реакцией на частоты
  - Ссылка на hydra.ojack.xyz (онлайн-среда) и toplap.org
  - **Автор инструкции:** VJ-друг (передать задачу отдельно)
- ⏳ **Публичная страница** `/audio/vj-guide` (или `/vj`) — полноценная инструкция на сайте:
  - Доступна без плеера, можно расшарить ссылку
  - Контент через Keystatic (блог-пост или отдельная коллекция) — VJ-друг пишет в CMS
  - Секции: «Что такое Hydra», «Первый патч», «Аудио в Hydra», «Примеры с треками», «Ресурсы»
  - Кнопка «Открыть в плеере» → переход на страницу трека с активированным Hydra-режимом
- ⏳ **Пресеты для старта:**
  ```js
  // Бас пульсирует размером
  osc(10, 0.1, () => a.fft[0] * 3)
    .color(
      () => a.fft[1],
      0.5,
      () => a.fft[2]
    ...
    .out()
  ```
- ⏳ **Сохранение кода:** `localStorage` — чтобы VJ не терял свои патчи между сессиями
- Сложность: **средняя** (~4–5 часов) + отдельная задача написания инструкции

### 9.8 Другие библиотеки (для оценки)

- ⏳ **wavesurfer.js v7** (`npm i wavesurfer.js`):
  - Waveform peaks, spectrogram плагин, timeline, zoom из коробки
  - Может заменить пп. 9.2 и 9.3 готовым решением
  - Минус: полный рефактор AudioPlayer

- ⏳ **audiomotion-analyzer** (`npm i audiomotion-analyzer`):
  - EQ bars / area / line с готовыми градиентами по высоте (для п. 9.1)
  - Много режимов, Canvas, быстро
...
- ⏳ Route `/share/` (POST, обрабатывает `share_target` из `manifest.ts` — уже настроен: `title`, `text`, `url`)
...
- ⏳ После сохранения — редирект на страницу подтверждения/редактирования метки (мини-форма: категория + метки) или сразу в список с возможностью отредактировать потом

### Модель данных

- ⏳ Модель `Link` в `schema.zmodel`: `url`, `title`, `description?`, `category` (enum или relation), `tags` (many-to-many), `createdAt`, `read: Boolean` (прочитано/не прочитано)
- ⏳ Модель `LinkTag` (метки, many-to-many с `Link`)
- ⏳ Категории: затравка — "Статьи" (Habr и т.п.), но категория должна быть расширяемой (не хардкод enum, а отдельная модель `LinkCategory` или просто свободные метки без отдельной категории)
- ⏳ Access control — приватный раздел (только владелец), не публичный

### UI

- ⏳ `/links` — список сохранённых ссылок (фильтр по категории/метке, поиск)
- ⏳ Карточка ссылки: превью (favicon + title + домен), метки, дата добавления, статус прочитано
- ⏳ Быстрое редактирование метки/категории прямо из списка
- ⏳ Admin CRUD для категорий/тегов (если нужно управлять списком меток)
...
- ⏳ Переиспользовать модель `UploadedFile` (уже есть, см. Фаза 6 `/admin/files`) — добавить к ней `category`/`tags` (те же `LinkTag`/категории, что у `Link`, либо общая модель меток на оба типа)
- ⏳ Объединённый раздел — `Link` (ссылки) и `UploadedFile` (файлы) показываются в одном списке `/links` с фильтром по типу (ссылка/файл) + категории/метке, либо общая модель `SavedItem` с полем `type: LINK | FILE`
