...
- ⏳ Объединённый публичный раздел — `Link` (ссылки) и `UploadedFile` (файлы, не аудио) в одном
  списке `/links` с фильтром по типу (ссылка/файл) + категории/метке. Не сделано в этом заходе —
  осознанно отложено: `/admin/files` и `/admin/files`-inline-редактирование категорий уже дают
  практическую пользу сами по себе, а объединение витрины — отдельная, не маленькая задача
  (два независимых источника с разной пагинацией, единая сортировка по `createdAt`). Модельное
  решение по-прежнему открыто (фильтр по типу над раздельными запросами vs. общая модель
  `SavedItem` с `type: LINK | FILE`) — решать в отдельном заходе
