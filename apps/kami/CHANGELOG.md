...
- Редизайн блога: список постов переведён с двухколоночной сетки на одну колонку карточек
  (`PostCard`) — по F-паттерну чтения (Nielsen Norman Group), тексту и заголовку отдана полная
  ширина, метаданные (дата/теги) вынесены вниз карточки.
- Добавлено поле `category` в Keystatic-схему постов (`article` | `dialogue`, по умолчанию
  `article`) — структурная замена free-text тегу «диалог с ИИ», который раньше был единственным
  (и ненадёжным) способом отличить экспорт диалога с нейросетью от обычной статьи.
- Посты с `category: dialogue` больше не попадают в главную ленту `/blog` — вынесены в отдельный
  раздел `/blog/dialogues` со своим фильтром по тегам и бейджем «Диалог с ИИ» на карточке.
- Фильтр по тегам и карточка-ссылка на раздел «Диалоги с ИИ» перенесены из горизонтального
  облака над списком в правый sticky-сайдбар (`BlogSidebar`) — по образцу разметки Хабра
  (контент на всю ширину слева, узкие виджеты справа), вместо загромождения верха страницы
  десятками чипов тегов.
- Навык `ai-dialog-to-post` (`.claude/commands/ai-dialog-to-post.md`) теперь всегда проставляет
  `category: dialogue` в генерируемом фронтматтере — посты, создаваемые этой командой, сразу
  маршрутизируются в `/blog/dialogues`.
