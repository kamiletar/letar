...
`overflowX="auto"` — на узких экранах скроллилась вся страница) проведён точечный аудит
`apps/kami`. Найдено 16 мест с `Table.Root`, из них только `admin/users/page.tsx` имело `Table.Root`
напрямую внутри `Card.Body p={0}` без обёртки скролла — исправлено добавлением
`overflowX="auto"` на сам `Card.Body`. Остальные 15 мест уже оборачивают таблицу в
`<Box overflowX="auto">`, ложных срабатываний не найдено.
