...
#   python scripts/generate-question-review-xlsx.py --batch adhd
#   python scripts/generate-question-review-xlsx.py --batch attachment
#   python scripts/generate-question-review-xlsx.py --batch rare
#
...
BATCH_SCALES = {
    ...
    "adhd": [
        ("INA", "Невнимательность"),
        ("HYI", "Гиперактивность/импульсивность"),
    ],
    "attachment": [
        ("AAX", "Тревога в привязанности"),
        ("AAV", "Избегание близости"),
    ],
    "rare": [
        ("SAD", "Садизм"),
        ("ASD", "Систематизация и спектр"),
...
INTRO_HH = [
    ...
    ("Срока нет (прежний дедлайн 02.08 снят вместе с фестивальным спринтом).", False),
    ("", False),
    ("Зачем шкала:", True),
...
INTRO_ADHD = [
    ("Ревью вопросов Архетеста — СДВГ-скрининг: 14 вопросов, две шкалы (INA 7 + HYI 7)", True),
    ("", False),
    ("⚠️ Этих вопросов ЕЩЁ НЕТ в тесте — ваши вердикты решают, попадут ли они туда. Срока нет.", False),
    ("", False),
    ("Зачем:", True),
    ("Пользователи и психологи спрашивают, «видно ли СДВГ». Сейчас — нет: в 22 шкалах ядра конструкта", False),
    ("внимания и гиперактивности нет. Прототип — ASRS / ASRS-5 (WHO; Kessler 2005; Ustun 2017),", False),
    ("только на уровне конструкта: пункты ASRS не переводятся и не перефразируются.", False),
    ("", False),
    ("Две шкалы вместо одной:", True),
    ("• INA «Невнимательность» — удержание внимания, доведение до конца, организация, забывчивость,", False),
    ("  потеря вещей, отвлекаемость, детали. Высокий INA при низком HYI — невнимательный тип («СДВ»).", False),
    ("• HYI «Гиперактивность/импульсивность» — двигательное беспокойство, трудность ждать, перебивание,", False),
    ("  импульсивные решения и реакции.", False),
    ("", False),
    ("Где это будет в продукте (предложение, решение за владельцем):", True),
    ("Экспериментальный блок «бета» — только психологу в кабинете. Пользователю в лучшем случае мягкая", False),
    ("рекомендация «если это мешает жить — обсудите со специалистом», никогда не «у вас СДВГ».", False),
    ("", False),
    ("Главный вопрос ревью — отличаем ли мы устойчивый паттерн от эпизода:", True),
    ("Концентрация падает при депрессии, беспокойство и скачка мыслей — при гипомании, рассеянность —", False),
    ("при тревоге и недосыпе. Это эпизоды. СДВГ — давний паттерн с детства, в разных сферах жизни.", False),
    ("Поэтому варианты с баллом описывают «как обычно у меня», «со мной это постоянно», а не", False),
    ("«в последнее время». Скажите, где формулировка всё-таки ловит эпизод, а не черту.", False),
    ("", False),
    ("Коротко по таблице:", True),
    ("• Колонка «Шкала» показывает подкластер симптома, «Код» — INA или HYI.", False),
    ("• Варианты скорят ТОЛЬКО INA/HYI — ни одного балла шкалам ядра. Пустые скобки = дистрактор.", False),
    ("• «Обр.» = да — балл набирается компенсаторным манёвром («пишу список — иначе половина выпадет»,", False),
    ("  «прикусываю язык — иначе закончу фразу за собеседника»). Проверьте, не поймают ли такие варианты", False),
    ("  просто аккуратных и собранных людей без СДВГ — оговорка «иначе…» должна это отсекать.", False),
    ("• «Вердикт»: ОК / Править / Удалить. «Комментарий / правка» — формулировку можно прямо в ячейке.", False),
    ("", False),
    ("Что ещё важно:", True),
    ("1. Импульсивные реакции (обида в переписке) пересекаются с эмоциональной дисрегуляцией — это", False),
    ("   спорная граница с пограничными чертами. Оставлять ли такой вопрос в HYI — ваше решение.", False),
    ("2. Возраст начала (до 12 лет) ситуационным вопросом не проверить — нужен ли отдельный вопрос", False),
    ("   о детстве или это за рамками самотестирования?", False),
    ("3. Язык и этика — те же критерии, что для 5.1 (INSTRUCTIONS.md). Без самообвинения: высокий балл", False),
    ("   должен звучать как честное описание, а не «ленюсь».", False),
]

INTRO_ATTACHMENT = [
    ("Ревью вопросов Архетеста — привязанность: 20 вопросов, две шкалы (AAX 10 + AAV 10)", True),
    ("", False),
    ("⚠️ Этих вопросов ЕЩЁ НЕТ в тесте — ваши вердикты решают, попадут ли они туда. Срока нет.", False),
    ("", False),
    ("Что строим:", True),
    ("Прототип — ECR-R (Fraley, Waller & Brennan, 2000) и ECR-RS (Fraley et al., 2011), только на", False),
    ("уровне конструкта: пункты ECR-R не переводятся и не перефразируются. Два независимых измерения:", False),
    ("• AAX «Тревога в привязанности» — страх быть покинутым, потребность в подтверждениях,", False),
    ("  гипербдительность к сигналам охлаждения.", False),
    ("• AAV «Избегание близости» — дискомфорт от откровенности и зависимости, дистанция при сближении.", False),
    ("Низкие оба — надёжный стиль; психолог в кабинете увидит квадрант, а не типологический ярлык.", False),
    ("", False),
    ("Главный вопрос ревью — не продублировали ли мы шкалы ядра:", True),
    ("• AAX ↔ BOR (страх покинутости) и DEP (потребность в опеке). Разводим: без импульсивности и", False),
    ("  качелей идеализации, не про беспомощность в решениях — только тревога о связи.", False),
    ("• AAV ↔ SZD (безразличие к связям) и AVD (страх оценки). Разводим: все сюжеты — внутри УЖЕ", False),
    ("  близких отношений (партнёр, близкий друг), не знакомства и не чужие люди.", False),
    ("Отметьте вопросы, которые, на ваш взгляд, меряют BOR/DEP/SZD/AVD, а не привязанность.", False),
    ("", False),
    ("Коротко по таблице:", True),
    ("• Колонка «Шкала» показывает грань измерения, «Код» — AAX или AAV.", False),
    ("• Варианты скорят ТОЛЬКО AAX/AAV (один вариант в AAX-вопросе даёт AAV 1 — это намеренно).", False),
    ("  Ни одного балла шкалам ядра. Пустые скобки = дистрактор.", False),
    ("• «Обр.» = да — балл набирается вариантом со спокойным, даже зрелым фасадом, за которым стратегия:", False),
    ("  «делаю вид, что мне всё равно, а сам(а) прислушиваюсь», «предлагаю каждому всё обдумать — и", False),
    ("  не возвращаюсь к разговору». Проверьте, честна ли инверсия и не наказывает ли она здоровую паузу.", False),
    ("• «Вердикт»: ОК / Править / Удалить. «Комментарий / правка» — формулировку можно прямо в ячейке.", False),
    ("", False),
    ("Что ещё важно:", True),
    ("1. Социальная желательность: «надёжный» вариант часто виден. Где он слишком очевиден — отметьте.", False),
    ("2. Гендер: в RU где нужно «он(а)», в EN — they. Нет ли сценариев, которые читаются только для пары", False),
    ("   одного типа?", False),
    ("3. Язык и этика — те же критерии, что для 5.1 (INSTRUCTIONS.md).", False),
]

INTRO_RARE = [
    ("Ревью вопросов Архетеста — добор редких шкал ядра: 30 новых вопросов (SAD 9, ASD 8, MAS 7, ALX 6)", True),
    ("", False),
    ("⚠️ Этих вопросов ЕЩЁ НЕТ в тесте. Срока нет.", False),
    ("", False),
    ("Зачем:", True),
    ("У четырёх шкал в банке меньше 30 вопросов — это порог высокой достоверности результата.", False),
    ("Садизм — 21 вопрос, систематизация — 22, мазохизм — 23, алекситимия — 24. Пользователь набирает", False),
    ("по ним единицы ответов, и высокая достоверность недостижима в принципе. Добор закрывает дефицит.", False),
    ("", False),
    ("Отличие от пакетов СДВГ и привязанности:", True),
    ("Это вопросы ТЕХ ЖЕ шкал ядра, стиль и логика — как в пакете 5.1 (основная шкала + правдоподобные", False),
    ("баллы других шкал в остальных вариантах). Критерии — те же, что для 5.1 (INSTRUCTIONS.md).", False),
    ("Их вливание меняет расчёт шкал, поэтому пройдёт одной пачкой после ваших вердиктов.", False),
    ("", False),
    ("Коротко по таблице:", True),
    ("• Один вопрос = одна строка, варианты А–Г с баллами в скобках.", False),
    ("• «Обр.» = да — основной балл набирается вариантом, который не выглядит «тёмным» или проблемным", False),
    ("  с первого взгляда («пишу безжалостный разбор — это же честная обратная связь»).", False),
    ("• «Вердикт»: ОК / Править / Удалить. «Комментарий / правка» — прямо в ячейке.", False),
    ("", False),
    ("Особое внимание:", True),
    ("1. Садизм: не пересекается ли вопрос с антисоциальностью или макиавеллизмом сильнее, чем нужно?", False),
    ("2. Мазохизм — авторский конструкт с меткой «бета»: узнаётся ли в вариантах самоотверженность", False),
    ("   через отказ себе, а не просто скромность или зависимость?", False),
    ("3. Социальная желательность: где «здоровый» вариант слишком очевиден — отметьте.", False),
]
