...
- **P7 Этап 6 — визуальный полиш, последний пункт overnight-чек-листа.**
  - `highlighted-code.tsx` — код внутри `SkinCodeFile` был всегда светлым в тёмной теме сайта:
    `highlight()` без `defaultColor: false` печатал цвета light-темы Shiki буквально в инлайн-стиль
    `<pre>`, `--shiki-dark(-bg)` оставались неиспользуемыми. Обычные ```-блоки MDX не были затронуты
    (fumadocs-mdx передаёт этот флаг сам).
  - `framework-switcher.tsx`/`skin-switcher.tsx` — `aria-label` навигации и текст "(скоро)" были
    захардкожены по-русски независимо от языка страницы; screen reader озвучивал русский accessible
    name на английских страницах. Исправлено через `useI18n()` (`fumadocs-ui/contexts/i18n`).
  - `skin-switcher.module.css` — ссылки переключателя были ~38px высотой на мобильном (375px),
    ниже рекомендованного минимума touch target (44px). Добавлен `min-height: 44px` +
    `flex-wrap: wrap` на контейнере.
  - Проверено: consistency admonition-блоков по всему `content/docs` — паттерн не используется
    нигде, унифицировать нечего. Мобильная адаптивность (375px) на 7 страницах разных типов —
    без горизонтального переполнения страницы.
