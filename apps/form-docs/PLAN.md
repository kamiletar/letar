...
**Этап 5 — сквозная вычитка RU (2026-08-14).** Задача от координатора `QuietRidge`, по прямой
просьбе Ками. Прочитаны все 56 файлов `content/docs/**/*.ru.mdx` (6 `fields/*`, 47 `guides/*`,
3 `api/*`) — орфография/грамматика, единообразие терминологии, соответствие примеров коду
текущему API (сверено через `mcp__form-mcp__list_fields`/`get_field_props`), битые внутренние
ссылки/якоря, единообразие структуры страниц, смысловое расхождение RU↔EN. Параллельно EN-вычитку
того же дерева вела другая сессия (список `.mdx` без `.ru`) — пересечений по файлам не было.

Найдено и исправлено:

- **Битые якоря — не найдены.** `grep "#form"` и `grep "](#"` по всем RU-файлам — ноль
  совпадений. Нейтрализация 87 заголовков (Этап 1, `## Form.Field.X` → `## X`) не оставила
  внутренних ссылок на старые якоря.
- **Грамматика.** Две сравнительные конструкции без запятой перед «чем» —
  `fields/select.ru.mdx` («богаче чем RadioGroup» → «богаче, чем RadioGroup»; «Легче чем Select»
  → «Легче, чем Select»).
- **Реальные расхождения RU↔EN** (RU отставал от EN, не наоборот — EN правился параллельно тем же
  заходом): `fields/number.ru.mdx` — отсутствовала целиком секция `Percentage` и подсекция
  «Автоматические ограничения» (обе есть в EN); `fields/date.ru.mdx` — отсутствовала секция
  `DateTimePicker`; `fields/select.ru.mdx` — отсутствовала подсекция «Асинхронный поиск» под
  Combobox; `fields/specialized.ru.mdx` — лишняя дублирующая секция `Tags` (уже описана в
  `fields/select.ru.mdx`, в EN `specialized.mdx` её нет вовсе — похоже на копипаст-артефакт при
  первичном переводе); `fields/index.ru.mdx` — обзорные таблицы не включали `Autocomplete`,
  `CascadingSelect`, `DateTimePicker`, `Address`, `City`, `OTPInput`, отсутствовала целиком секция
  «Логические поля» и «Пользовательские пропсы» (обе есть в EN `Custom Props`).
- **Битые названия API в примерах кода** — `guides/filters-state.ru.mdx` использовал
  `Form.Field.MultiSelect`/`SegmentedControl`/`CheckboxGroup`, ни один из которых не существует в
  библиотеке (проверено `get_field_props` — все три вернули "not found"). Заменены на реальные
  `Form.Field.Listbox ... multiple` и `Form.Field.SegmentedGroup`. **Тот же баг обнаружен и в EN
  `filters-state.mdx`** — идентичные несуществующие имена, не в скоупе этой сессии (правит
  параллельный EN-агент), зафиксировано здесь для видимости.
- **Устаревшее число полей** — `guides/porting-framework.ru.mdx` «6 из 56 полей» → «6 из 61 поля»
  (актуальное число по `list_fields` на 2026-08-13, см. предупреждение в конце Этапа 3 этого же
  PLAN.md). У EN `porting-framework.mdx` то же «56» — не тронуто, вне скоупа.

**Системная находка (не чинилась в этом заходе — не орфография, а пробел покрытия документации,
общий для RU и EN):** `list_fields` возвращает 5 полей, не описанных ни на одной категорийной
странице `fields/*` ни в одном языке — `YesNo` (multi-select; но описан отдельно в
`guides/survey-fields.ru.mdx`/`.mdx`), `ImageChoice` (select; тоже описан в `survey-fields`),
`Signature` (special; описан в `guides/signature.ru.mdx`/`.mdx`), `CreditCard` (special; описан в
`guides/credit-card.ru.mdx`/`.mdx`), `Auto` (special; упоминается как `Form.Field.Auto` в
`guides/auto-fields.ru.mdx`, но не как отдельная запись в обзорных таблицах). То есть контент
существует — он просто не продублирован в категорийных таблицах `fields/index`/`fields/specialized`
и т.д. Не расширяла эти таблицы дальше уже сделанного (Этап 5 выше) — решение, какие поля
считать «специальными гайдами» а какие «категорийными», архитектурное, не вычиточное.

Проверено: `nx lint form-docs` и `nx typecheck:tsgo form-docs` — зелёные,
`nx run-many -t format --projects=form-docs` не внёс изменений ни в один правленый RU-файл (уже
...
form-docs`, генератор `libs/generators`, обёртка над `@letar/theme-check`) пока не подключён. Уже
