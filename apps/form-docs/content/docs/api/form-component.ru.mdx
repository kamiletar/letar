...
Тексты (`dialogTitle`, `dialogDescription`, `confirmText`, `cancelText`, `message`) резолвятся так: проп →
...
```tsx
<Form.DirtyGuard dialogTitle="Уйти?" confirmText="Да, уйти" cancelText="Нет, остаться" />
```

Включите её сразу для всех форм инстанса, не расставляя компонент вручную (v2.18.0+):

```tsx
const AppForm = createForm({ dirtyGuard: true }) // или { dialogTitle, dialogDescription, confirmText, cancelText }

<AppForm dirtyGuard={false} onSubmit={login}>…</AppForm> // логин, фильтры: выключить
```

Проп формы перебивает опцию инстанса (`false` — выключить, `true` — оставить тексты инстанса, объект — дополнить
