...
> **Date filters:** `Form.Field.Date` is safe to use inside a `Form.UrlSync`-synced filter form as
> long as the field's schema does **not** declare `z.date()`/`z.coerce.date()` (a plain `z.string()`
> or no schema at all, as is typical for a URL filter). In that case the field commits a
> `YYYY-MM-DD` string, matching the string defaults `Form.UrlSync` expects. If the schema does
> require a real `Date` (used with `onSubmit`), the field keeps committing `Date` as before —
> that combination is unrelated to URL sync and isn't affected by this note. See the "Создано с"
> field in the full example above (`@letar/forms` 2.14.17+).

### Read from URL on mount

> **URL values are applied after mount.** `useFormUrlSync` (the companion of `Form.UrlSync`)
> always returns `defaults` on the first render — both the server one and the hydration one — and
> applies URL values in a `useEffect` (`@letar/forms` 2.14.20+). Otherwise the client markup
> diverges from SSR and a Select/Combobox stays on the wrong value. A full page load may show a
> brief (sub-frame) flash of the default. Don't read `window.location` during render next to a
> form — only inside `useEffect`.
...
It reuses the same diff `Form.UrlSync` computes internally before writing to the URL — no need
