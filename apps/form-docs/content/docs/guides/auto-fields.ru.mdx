...
`form-develop-app-shadcn` на сборке. Chakra-источник использует `Form.AutoFields`; shadcn-источник
использует `FieldAuto` — другой уровень иерархии контроля ниже, но та же идея автогенерации полей
из Zod-схемы. Аналога для Vue/Angular пока нет — эти вкладки показаны как disabled.
