...
| `schema`     | `ZodObject`                               | —        | Schema for labels via `.meta()`         |
| `compact`    | `boolean`                                 | `false`  | Compact layout (single line per field)  |
| `exclude`    | `string[]`                                | `[]`     | Fields to hide                          |
| `include`    | `string[]`                                | —        | Show only these fields (overrides `exclude`) |
| `labels`     | `Record<string, string>`                 | `{}`     | Manual label overrides                  |
| `formatters` | `Record<string, (value: unknown) => string>` | —    | Custom value formatters per field       |
