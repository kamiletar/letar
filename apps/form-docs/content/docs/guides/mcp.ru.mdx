...
Документация по директивам ZenStack `@meta("form.*", value)`. Результат включает синтаксис
field-атрибута `@meta` (`example`), плоский dot-path ключ `metaKey` и генерируемый вывод Zod
`.meta()`.

```
get_directives({ directive: "@form.fieldType" })
→ { name, metaKey, description, example, output }
```

**Директивы:** `@form.title`, `@form.placeholder`, `@form.description`, `@form.fieldType`, `@form.props`, `@form.relation`, `@form.exclude`

Поиск идёт по этому короткому имени (`@form.fieldType`) — это внутренний идентификатор MCP API,
не comment-синтаксис; синтаксис в самой схеме — `@meta("form.fieldType", ...)`.
...
| `form-docs://zenstack`          | @meta("form.\*", value) директивы, schema.zmodel |
