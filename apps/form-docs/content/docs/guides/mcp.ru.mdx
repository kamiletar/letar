...
Документация по директивам ZenStack `@meta("form.*", value)`. Результат включает актуальный
синтаксис field-атрибута `@meta` (`example`), плоский dot-path ключ `metaKey`, устаревший
comment-синтаксис для справки (`legacyExample`) и генерируемый вывод Zod `.meta()`.

```
get_directives({ directive: "@form.fieldType" })
→ { name, metaKey, description, example, legacyExample, output }
```

**Директивы:** `@form.title`, `@form.placeholder`, `@form.description`, `@form.fieldType`, `@form.props`, `@form.relation`, `@form.exclude`

Поиск идёт по legacy-имени (`@form.fieldType`) ради обратной совместимости — сам ответ показывает
актуальный синтаксис `@meta("form.fieldType", ...)` как основной `example`.
