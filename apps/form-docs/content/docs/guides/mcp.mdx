...
Get documentation for `@meta("form.*", value)` ZenStack directives. Each result includes the
current `@meta` field-attribute syntax (`example`), the flat `metaKey` dot-path, the deprecated
comment-based syntax for reference (`legacyExample`), and the generated Zod `.meta()` output.
...
Lookup is by the legacy short name (`@form.fieldType`) for backwards compatibility — the response
itself shows the current `@meta("form.fieldType", ...)` syntax as the primary `example`.
