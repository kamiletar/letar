...
`@meta` field-attribute syntax (`example`), the flat `metaKey` dot-path, and the generated Zod
`.meta()` output.
...
Lookup is by this short name (`@form.fieldType`) — it's an internal identifier for the MCP API,
not a comment-based syntax; the schema syntax itself is `@meta("form.fieldType", ...)`.
...
| `form-docs://zenstack`          | @meta("form.\*", value) directives, schema.zmodel |
