...
The old `///` doc-comment directives before a field still work — the plugin reads both syntaxes,
and `@meta` wins if the same key is set both ways on one field. `nx zenstack:generate` prints a
deprecation warning when it sees `@form.*` comments, but the build doesn't break:
...
New schemas should use `@meta`; there's no need to migrate existing `@form.*` comments by hand
unless you're already touching that model.
