...
When reading from `window.location`, the first render (server and hydration) returns `{}` and the URL values appear right after mount, so the client tree matches the server markup. With an explicit `searchParams` the values are computed synchronously.
