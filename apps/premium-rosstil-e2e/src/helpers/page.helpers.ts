 ...
 * Устраняет дублирование навигации, toast проверок и ожиданий загрузки.
 */
 ...
 *
 * @example
 * await gotoAndExpectHeading(page, '/catalog/', 'Каталог')
 * await gotoAndExpectHeading(page, '/profile/', /профиль/i)
 */
 ...
 *
 * @example
 * await expectHeading(page, 'Каталог')
 ...
 *
 * @example
 * const isEmpty = await checkEmptyState(page, /нет товаров|пока нет товаров/i)
...
/**
 * Проверка редиректа на signin (для защищённых страниц)
 *
 * @param page - Playwright Page
 * @param timeout - Таймаут в миллисекундах
 *
...
export async function expectSignInRedirect(page: Page, timeout = 10000): Promise<void> {
  await expect(page).toHaveURL(/\/auth\/signin/, { timeout })
}
 ...
 *
 * @example
 * skipWithLog(!hasFeature, 'Feature не реализована')
 */
