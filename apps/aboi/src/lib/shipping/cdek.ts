// CDEK API v2 — чистый fetch-клиент (без npm-пакетов)
...
// Тарифные коды: склад → ПВЗ (136), склад → дверь (137)
const TARIFF_POINT = 136
const TARIFF_DOOR = 137

// Кэш OAuth-токена на уровне модуля (в рамках Node.js процесса)
...
export async function getCdekToken(): Promise<string | null> {
  ...
  try {
    const resp = await fetch(`${getBaseUrl()}/v2/oauth/token`, {
      method: 'POST',
    ...
    tokenExpiresAt = Date.now() + (data.expires_in - 300) * 1000 // буфер 5 мин
    return cachedToken
  } catch {
    return null
  }
...
/**
 * Рассчитывает стоимость доставки по тарифам 136 и 137 одним запросом.
 * Возвращает копейки (null = тариф недоступен).
 */
export async function calculateShippingCosts(toPostalCode: string, totalMeters: number): Promise<CdekShippingCosts> {
  ...
  const pkg = estimatePackage(totalMeters)
...
// ---------------------------------------------------------------------------
// Моковые данные для CDEK_MOCK_MODE (локальная разработка без реального API)
// ---------------------------------------------------------------------------
...
/** Фильтрует мок-города по запросу (регистронезависимо). */
...
const MOCK_PVZ: CdekDeliveryPoint[] = [
  {
    code: 'MSK1',
...
export async function searchCdekCities(query: string): Promise<CdekCityItem[]> {
  ...
  try {
    const resp = await fetch(
      `${getBaseUrl()}/v2/location/cities?${new URLSearchParams({ city: query, country_codes: 'RU', size: '10' })}`,
      { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(8_000) },
    )
    if (!resp.ok) return []
    return ((await resp.json()) as CdekCityItem[]).slice(0, 10)
  } catch {
    return []
  }
}
...
export async function getDeliveryPoints(cityCode: number): Promise<CdekDeliveryPoint[]> {
  if (process.env.CDEK_MOCK_MODE === 'true') {
    ...
    // В мок-режиме возвращаем тестовые ПВЗ для любого города (достаточно для разработки)
    return MOCK_PVZ
  }
  ...
  try {
    ...
    if (!resp.ok) return []
    // CDEK возвращает все ПВЗ без пагинации — берём до 300 (для больших городов типа Москвы)
...
/** Создаёт заказ СДЭК. Возвращает uuid и трек-номер (если доступен). */
export async function createCdekOrder(
  request: CdekOrderRequest,
): Promise<{ uuid: string; trackNumber?: string } | null> {
  const token = await getCdekToken()
  if (!token) return null

  try {
    const resp = await fetch(`${getBaseUrl()}/v2/orders`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
    ...
    const data = (await resp.json()) as CdekOrderResponse
    if (!data.entity?.uuid) {
      const errors = data.requests?.[0]?.errors
      console.warn('[cdek] createOrder error', errors)
      return null
    }

    return {
      uuid: data.entity.uuid,
      trackNumber: data.entity.track_number ?? undefined,
    }
  } catch (err) {
...
/** Регистрирует вебхук трекинга в CDEK. Вызывается один раз при настройке. */
export async function registerCdekWebhook(url: string): Promise<boolean> {
  const token = await getCdekToken()
