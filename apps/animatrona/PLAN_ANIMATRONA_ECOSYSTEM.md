# Экосистема Animatrona — локализация, метаданные, лендинг, IPFS-плеер

> Продолжение [PLAN.md](./PLAN.md), пункт «Animatrona Player — отдельное приложение...». Раньше
> этот файл назывался `PLAN_ANIMATRONA_PLAYER.md` и содержал единый план для отдельного
> папочного плеера. При приведении в порядок 2026-09-08 обнаружено: разделы §0–§13 были
> исключительно про папочный плеер (перенесены в
> [apps/animatrona-folder-player/PLAN.md](/apps/animatrona-folder-player/PLAN.md), когда
> приложение стало реальным отдельным nx-проектом), а разделы §14 и далее — про темы,
> которые с папочным плеером связаны лишь частично или не связаны вовсе: локализация всей
> экосистемы (§14 — затрагивает и папочный плеер тоже), провайдер метаданных Shikimori/AniList
> (§15 — только Animatrona, у папочного плеера нет БД), мультиязычный лендинг (§16), статьи про
> трекер (§17), UI/UX плеера в общих либах `@letar/video-player-core`/`react` (§18–19 — общий код,
> но §19 содержит и Animatrona-специфичный раздел про библиотечный режим), расследование двух
> IPFS-веб-плееров и чек-лист перезалива библиотеки (§20–21 — только Animatrona). Нумерация
> секций (§14…) сохранена как есть, не пересчитана с 1 — меньше риска сломать перекрёстные ссылки
...
2. **animatrona-folder-player целиком** — UI новый и небольшой, заодно проверка стека на живом продукте
