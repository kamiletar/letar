...
- [ ] **Схема** (`schema/models/import.zmodel`): `BulkImportItemStatus` enum (`PENDING`,
      ...
      проверка размера» (см. «Адаптивный VMAF-таргет» ниже): какой таргет из лесенки реально
      выбрали и фактические размеры исходника/результата, не только оценка по сэмплам, один
...
- [ ] **Адаптивный VMAF-таргет для старых/шумных источников** (2026-09-06, по решению
      пользователя — фиксированный VMAF 95 у старых зашумлённых раздач (аналоговый мастер,
      сильное зерно) требует непропорционально много бит на «сохранение шума», который VMAF
      как метрика ценит наравне с полезной деталью — итоговый AV1-файл может выйти **больше**
      источника, хотя перцептивно почти неотличим от версии с VMAF 91).
  ...
  - **Что уже есть и переиспользуется.** `findOptimalCQ()`
    ...
    **бросает исключение**, если оценочный размер закодированного файла (по 4 сэмплам) не меньше
    оригинала (`estimatedSavings <= 0`, [vmaf.ts:423-429](main/src/ffmpeg/vmaf.ts)) — требование
    «не должно стать больше» частично уже обеспечено на уровне оценки по сэмплам. Проблема батча в
    другом: ручной флоу на этот throw реагирует диалогом пользователю (`VmafAutoDialog.tsx`), а
    батч работает без присмотра — просто упадёт в ERROR на каждой шумной старой раздаче, вместо
    того чтобы сама подобрать более разумный таргет.
  - **Лесенка таргетов вместо одного фиксированного значения.** В `rutracker-batch-import.ts`
    (не в самом `findOptimalCQ` — сигнатуру не меняем) — при подготовке эпизода к транскоду
    пробовать таргеты по убыванию, например `[95, 93, 91]` (нижняя граница 91 — не опускаться
    ниже, эта раздача тогда просто не сожмётся сильно, но перцептивное качество ниже 91 уже
    самому пользователю не нравится, см. постановку задачи), останавливаясь на первом таргете,
    для которого `estimatedSavings` достигает минимального порога (например 0.10 — хотя бы 10%
    экономии; порог, а не просто «не отрицательно», иначе будем годами хранить файлы с экономией
    в 1%, которая не стоит потери качества). Если таргет бросил исключение (`estimatedSavings <=
    0`) — не проваливать элемент, а перейти к следующему, более низкому таргету в лесенке; каждая
    пройденная попытка — это только 4 коротких сэмпла (`extractSamples`), не полный эпизод, так
    что лесенка из 2-3 шагов не удорожает пайплайн ощутимо.
  - **С какого таргета стартовать — эвристика по году выхода, не жёсткое правило.** Для раздач
    старше условного порога (например `airedOn.year < 2010` — предшествует повсеместному переходу
    на BD-мастера) начинать лесенку сразу с 91, а не с 95 — экономит одну заведомо провальную
    попытку на большинстве по-настоящему старых шумных тайтлов. Год — только оптимизация порядка
    попыток, не гейт: если у относительно новой раздачи всё равно плохое сжатие на 95 (плохой рип,
    любительский апскейл, шумная плёнка независимо от года выхода) — лесенка всё равно сама
    дойдёт до 91 через `estimatedSavings`, год тут ничего не блокирует.
  - **Реальная проверка размера — не только оценка по сэмплам.** Оценка `estimatedSize` в
    ...
    ошиблись) — **не удалять исходник**, элемент уходит в `ERROR` с сообщением о фактическом
    перерасходе размера вместо тихого перехода в `ARCHIVING`/`DONE` с раздутым файлом и удалённым
    оригиналом, который было бы уже не восстановить. Оба размера (исходный/итоговый, вместе с
    выбранным итоговым `targetVmaf` из лесенки) пишутся в `diskCleanupJson` рядом с остальными
    таймстампами — видно в таблице статусов, насколько реально сжалась каждая раздача, а не
    только по факту «сжалась/не сжалась».
- [ ] **Дисковая гигиена батча** (2026-09-06, встроено по решению пользователя — место на диске не
  ...
  1. **Удаление исходников.** Сразу после того, как транскод и IPFS-заливка эпизода(ов) успешно
     ...
     ⚠️ **Почему этот шаг вообще нужно писать явно:** в ручном флоу импорта
     (`import-service.ts`) такого вызова нет вообще ни на одном успешном пути — только
     `import-failure-cleanup.ts` трогает файлы, и то при ошибке/отмене. Успешный ручной импорт
     сегодня оставляет исходник висеть в `Downloads/Animatrona/` и раздачу — сидировать в
     qBittorrent бессрочно. Для одиночного импорта это терпимо (пользователь решает вручную), но
     для батча на десятки раздач — гарантированная нехватка места за несколько прогонов. Чинить
     это в батче, не трогая ручной флоу (не в скоупе этой фазы; если понадобится и там — отдельная
     задача, вынести в общую функцию наподобие `finalizeSuccessfulDownload(infoHash)`, вызываемую
     из обоих мест).
...
6. **Лесенка VMAF реально понижает таргет на шумном источнике:** прогнать через батч старую
   (условно `airedOn.year < 2010`) грязную/зашумлённую раздачу — лесенка стартует сразу с 91 (не
   с 95), `diskCleanupJson.targetVmafUsed` в итоге меньше 95, `estimatedSavings` на выбранном
   таргете положителен. Отдельно прогнать современную чистую раздачу — лесенка останавливается
   на 95 с первой попытки, лишних шагов вниз нет.
7. **Реальный перерасход ловится до удаления исходника:** искусственно занизить сообщаемый
   ...
   оценку) так, чтобы факт оказался больше исходника — элемент уходит в `ERROR` с обоими
   размерами в сообщении, `Downloads/Animatrona/<torrent>/` **не** удалена, торрент **не** снят с
   раздачи в qBittorrent — оригинал остаётся физически цел для повторной попытки с другим
   таргетом/профилем.
