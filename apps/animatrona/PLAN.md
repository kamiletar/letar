...
- [ ] **Автоопределение и обрезка чёрных полос при импорте** — сейчас крадущий эти полосы кроп
      ...
      воспроизведение.
      **2026-09-06: backend-инфраструктура готова, живой пайплайн импорта и предпоказ — ещё нет
      (см. подпункты).** Сделано: `main/ffmpeg/cropdetect.ts` (`detectCropFilter` — сэмплирование
      + мода + пороги, покрыт unit-тестами `__tests__/cropdetect.spec.ts`, 8/8 зелёных);
      `EncoderStrategy.buildDebandFilter()` → `buildVideoFilterChain({ deband, cropFilter })` в
      `encoder-strategies.ts` (обе стратегии, GPU-ветка с hwdownload/hwupload вокруг crop+deband);
      `transcodeVideoWithProfile()` в `transcode.ts` принимает опциональный `cropFilter` и
      прокидывает в фильтр-граф; `encodeSample()` (сэмпл для оценки качества профиля) намеренно
      **не** получает `cropFilter` — сэмпл не финальный транскод. lint/typecheck:tsgo animatrona —
      зелёные. Осознанно не тронуто в этой сессии — риск тихого необратимого применения без
      предпоказа (см. подпункт про необратимость ниже) перевешивает пользу от «на всякий случай
      подключил».
  ...
  - [ ] **Необратимость:** как и 10-bit→8-bit решение в §21.4, обрезка при транскоде — решение
        ...
        применение. **Не реализовано** — блокирует включение `detectCropFilter` в реальный импорт.
  - [ ] Подключить `detectCropFilter` к живому пайплайну импорта (вызов перед
        `transcodeVideoWithProfile` в оркестрации `transcode-runner.ts`/`ParallelTranscodeManager`
        — call site ещё не найден/не прочитан) — заблокировано предыдущим пунктом.
  - [x] Записать применённый `crop=` в `ManifestEncodingInfo`/`ffmpegCommand` — как и остальные
        ...
        Поле `cropFilter?: string` добавлено в `ManifestEncodingInfo`
        (`renderer/src/types/electron.d.ts`); фактическая запись значения — вместе с подключением
        к пайплайну выше (пока нечего записывать).
