...
- [x] **Расписать разделение `settings/_settings/*` по карточкам явно, по факту использования**
  ...
  - **Расхождения с черновой классификацией по имени файла:** `P2PSharingCard` целиком считался
    ...
    `FederationCard`+`use-federation.ts` — **решено (2026-09-08, владелец): переносится целиком,
    полная федерация со своими ключами**, не только `sync`/`discover`. IPFS Player сам генерирует
    RSA-пару (`generateKeys`) и сам управляет доверием входящих трекеров (`setTrust`/`blockTracker`/
    `unblockTracker`) — но **только как приёмник**, не издатель: у него нет своей библиотеки на
    публикацию (не строит `directoryCid`), уточнение владельца — «плеер только чужое смотрит».
    Ключи и доверие здесь нужны, чтобы трекеры аутентифицировали и доверяли именно этому инстансу
    при синхронизации/discovery чужого контента, а не чтобы анонсировать собственный каталог. Не
    сокращать при переносе. `use-p2p-sharing.ts` (композитный хук) и `use-settings.ts` тянут creator-only
