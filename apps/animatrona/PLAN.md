...
- [ ] **Infinite scroll / пагинация для списка аниме**
  ...
  **Реализация (infinite scroll + виртуализация):**
  - [ ] Добавить `@tanstack/react-virtual` в deps
  - [ ] Переключить `use-library-page.ts` с `findMany` на `findMany + skip/take` (cursor pagination)
  - [ ] Обернуть список в `useVirtualizer` — рендерить только видимые строки
  - [ ] Sentinel-элемент внизу → `useInfiniteQuery` подгружает следующую страницу
