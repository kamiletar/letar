...
## Текущая версия: 0.56.0

## ⚠️ Приоритет 2026-09-07 переехал в отдельное приложение (обновлено 2026-09-08)

Этот раздел раньше указывал «взять в работу `PLAN_ANIMATRONA_PLAYER.md`» как приоритет сессии
**Animatrona**. С 2026-09-07 папочный плеер — реальный отдельный nx-проект
(`apps/animatrona-folder-player`), с 2026-09-08 у него своя команда
[`/animatrona-folder-player`](/.claude/commands/animatrona-folder-player.md) и своя identity в
Agent Mail (`animatrona-folder-player-dev`) — задача «довести до конца и опубликовать» (релиз в
`kamiletar/letar`, тег `animatrona-folder-player-v*`, живой инсталлятор на сайте) относится к
**той** сессии, не к этой. План — [apps/animatrona-folder-player/PLAN.md](/apps/animatrona-folder-player/PLAN.md).

Публикация плеера по-прежнему затрагивает `animatrona-landing` (см. §9 того плана «Фаза 5 —
сайт» и §16 [PLAN_ANIMATRONA_ECOSYSTEM.md](./PLAN_ANIMATRONA_ECOSYSTEM.md) «Мультиязычный
лендинг») — но это координируется из сессии `/animatrona-folder-player`, а не отсюда.
...
- [ ] **Аудит содержимого `directoryCid` перед массовым перезаливом библиотеки** (план от
  ...
  **билдер положит в `directoryCid` только то, что импорт записал в БД.** Аудит выше проверял
  ...
  - [x] **`detectIntros` при импорте работал вхолостую** — исправлено 2026-09-08. Шаг определения
        OP/ED сохранял главы через `updateChaptersInManifest`, а та первым делом читает
        `Episode.manifestCid` ([chapter-creator.ts](main/services/import/chapter-creator.ts) —
        файл лежит в `import/`, прежний путь в этом плане был неверный), которого на той фазе ещё
        нет: ранний `return`, потеря без ошибки. ⚠️ **Уточнение по ходу фикса: главы из контейнера
        НЕ терялись** — вопреки прежней формулировке, их заново собирает
        `generateManifestFromDemux` из `demuxResult.metadata.chapters`
        ([manifest-generator.ts](main/services/manifest-generator.ts)); вхолостую работал только
        путь автоопределения, чьи главы существуют лишь в памяти и генератору неизвестны.
        Фикс: `GenerateManifestOptions.detectedChapters` + `PostProcessData.detectedChapters` —
        найденные OP/ED кладутся в `postProcessDataMap` и попадают в манифест при его сборке,
        где главы контейнера имеют приоритет (детектор и так запускается только для эпизодов без
        них). Тесты — `main/services/__tests__/manifest-generator-chapters.spec.ts` (3 теста:
        автоопределённые применяются, контейнерные побеждают, при отсутствии обоих
        `ChaptersDocument` не создаётся). Pre-pass в билдере остаётся страховкой, но больше не
        единственным путём для фильмов/одиночных серий.
  - [x] **`needsReupload` снимался, когда не должен** — исправлено 2026-09-08. Условие считало
        `failedCount` от файлов **текущего** entry, поэтому долив 2 серий из 12 снимал флаг со
        всего аниме, хотя 10 серий остались на старой раздаче. Теперь флаг снимается, только если
        в прогоне не осталось ни одного эпизода аниме, которого не было в этом entry (сверка
        номеров эпизодов из БД с `selectedFiles`), иначе — остаётся с записью в лог.
  - [x] **Осиротевшие эпизоды** — закрыто отчётом, а не удалением (2026-09-08). Эпизоды, не
        вошедшие в прогон, теперь перечисляются номерами в `warning` результата импорта
        («Эп. 3, 4 не входили в этот прогон — остались со старыми CID»). ⚠️ Автоудаление
        намеренно не делается: `prisma.anime`-каскад сносит `WatchProgress`/историю просмотра, а
        отличить «раздача сократилась» от «долили часть серий» изнутри импорта нельзя — решение
        за пользователем. Если понадобится реальная чистка — заводить отдельным пунктом с UI
        подтверждения.
  - [x] **`updateAnimeManifest` целиком под `log.warn`** — оказалось уже исправленным (проверено
        2026-09-08): `generateAndPublishAnimeManifest`
        ([relations-and-manifest.ts](main/services/import/relations-and-manifest.ts)) возвращает
        `{ success, error }`, и `import-service` добавляет отдельное предупреждение
        «AnimeManifest не опубликован (directoryCid не обновлён)». Импорт по-прежнему возвращает
        `success: true` — осознанно, это частичный успех, видимый в `warning`.
  ...
  - [x] **`Franchise.graphCid` не обновлялся** — закрыто фиксом от 2026-09-06 (см. выше в этом же
        разделе), буллет оставался незачёркнутым по недосмотру, сверено с кодом 2026-09-08:
        `isFranchiseGraphStale()`/`FRANCHISE_GRAPH_TTL_MS` в
        [anime-manifest-generator.ts](main/services/anime-manifest-generator.ts) реально вызываются
        в ветке кеша (строка ~428), тесты на границы TTL есть. Действует и при реимпорте в
        существующую БД, ради чего пункт и заводился.
...
- [ ] **Animatrona Folder Player — отдельное приложение для папочного просмотра** (план от
      2026-07-30, приложение сгенерировано и в основном реализовано 2026-09-07/08, переименовано
      из `animatrona-player` 2026-09-08, но публикация/релиз ещё не сделаны) — раздел «Плеер»
      выделен в самостоятельный лёгкий продукт,
      общий код уехал в `libs/folder-player-react` + `libs/folder-scan`, Animatrona перешла на них
      (Фаза 1 из плана плеера). Собственный план приложения —
      [apps/animatrona-folder-player/PLAN.md](/apps/animatrona-folder-player/PLAN.md) (был
      `PLAN_ANIMATRONA_PLAYER.md` в этом каталоге, физически перенесён в каталог приложения
      2026-09-08 — планы уже не в одном месте с этим файлом). Сквозные для экосистемы темы,
      которые попутно накопились в старом файле (локализация, метаданные, лендинг, статьи трекера,
...
оставлено только в историческом контексте таблицы решений.
**Решено с владельцем (2026-09-07):**

| Вопрос                               | Решение                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Отношение к «Animatrona Player» выше | **Отдельное третье приложение**, не расширение папочного плеера. Три продукта по аналогии с K-Lite Lite/Full/Mega: `animatrona-folder-player` (папки, без IPFS; переименован из `animatrona-player` 2026-09-08) — Lite; `animatrona-ipfs-player` (IPFS-просмотр, без импорта; переименован из `animatrona-viewer` 2026-09-08) — Standard; `animatrona` (всё) — Mega |
...
| Имя nx-проекта                       | `apps/animatrona-ipfs-player` — по аналогии с `animatrona-folder-player`, не путается с `animatrona-tracker` (это отдельный веб-каталог/модерация, не desktop-клиент)                                                                                                                                                                                               |
...
- Пересечение почти полное — БД у viewer'а нужна практически такая же тяжёлая (SQLite+Prisma+
  ZenStack+миграции), в отличие от папочного `animatrona-folder-player`, у которого её нет вовсе
...
- [x] **Решено (2026-09-08, владелец): `libs/ipfs-kubo-core` — только Animatrona + IPFS Player
      на старте.** `animatrona-tracker` — другой рантайм (веб-бэкенд, не Electron main), его код
      скорее всего вообще не пересекается напрямую; добавлять его в общую библиотеку только при
      реальной обнаруженной необходимости, не заранее.
...
- [x] **Расписать разделение `settings/_settings/*` по карточкам явно, по факту использования**
  ...
  - **Расхождения с черновой классификацией по имени файла:** `P2PSharingCard` целиком считался
    ...
    `FederationCard`+`use-federation.ts` — **решено окончательно (2026-09-08, владелец, отменяет
    предыдущую формулировку «полная федерация со своими ключами» из того же дня): IPFS Player —
    ТОЛЬКО просмотр.** `generateKeys`/`setTrust`/`blockTracker`/`unblockTracker` (роль узла
    федерации, издательское/трастовое администрирование) **не переносятся вообще** — этого нет ни
    в каком виде в IPFS Player. Из всей карточки федерации нужны только две вещи: (1) добавить
    трекер в список (откуда брать раздачи для discovery — без криптографии и без доверия к нему
    как к пиру, просто источник списка), (2) посмотреть конкретную раздачу по её `directoryCid`.
    `sync`/`discover`/`getGlobalSeeders`/`getTrustScore` тоже НЕ переносятся — это уже про
    полноценное участие в сети федерации, не про «добавил трекер → посмотрел по CID». Реальный
    объём переноса из `FederationCard` кода почти нулевой — старая функциональность карточки целиком
    creator/federation-hub-only, а нужные IPFS Player два действия (список трекеров, просмотр по
    CID) скорее ближе к простому UI-минимуму, который стоит писать заново компактно, а не резать
    по кускам из существующей 24КБ-карточки. `use-p2p-sharing.ts` (композитный хук) и `use-settings.ts` тянут creator-only
...
## NVENC: временной фильтр Blackwell, реальный lookahead, 10 бит [ЗАКРЫТО 2026-09-16, v0.56.0]

- [x] **`-tf_level 4` для профилей с `temporalFilter`** («Blackwell UHQ», «Архив»). Прежний
      комментарий винил драйвер 572.90, но отказ шёл от обёртки ffmpeg: `libavcodec/nvenc.c`
      принимает только уровни 0 и 4, а старый код передавал `-tf_level 1`. Фильтр включается,
      только если `getGpuCapability()` видит Blackwell. Если ffmpeg всё равно отказал
      («Temporal filtering not supported / needs at least 4 B-Frames / Invalid temporal filtering
      level»), задача перезапускается без фильтра и не падает. Так устроены все три пути: VideoPool,
      `transcodeVideoWithProfile` и VMAF-сэмплы.
- [x] **`-bf 4` передаётся явно**, если у пресета меньше 4 B-кадров. Иначе фильтр не
      инициализируется. Замерено на ffmpeg N-124496: AV1 HQ — 5 B-кадров, AV1 UHQ — 7,
      HEVC HQ — 3, HEVC UHQ — 5, H.264 HQ — 3.
- [x] **VMAF-сэмплы кодируются теми же аргументами, что и финальный файл.** Раньше `sample.ts`
      держал свой набор: tune hq, aq-strength 15, GOP 120, без lookahead и без фильтра. HEVC и
      H.264 на GPU вообще уходили на CPU. Общий источник теперь — `main/ffmpeg/nvenc-args.ts`.
- [x] **Lookahead 250 → 51.** ffmpeg режет глубину до `min(surfaces, delay) − (B+1) − 4`, а
      surfaces ограничены 64 (`MAX_REGISTERED_FRAMES`). `-surfaces 64` предел не поднимает:
      ffmpeg пишет «increasing used surfaces 64 -> 263» и тут же обрезает обратно. Предел
      51 для AV1 UHQ, 53 для AV1 HQ, 55 для HEVC HQ. В форме профиля максимум — 58 (при 0
      B-кадров), рядом подсказка. Встроенные профили пересеиваются при старте, так что новое
      значение доезжает и до старых баз.
- [x] **`-highbitdepth 1` вместо `-pix_fmt` для `force10Bit` на CUDA-пути** (`encoder-strategies.ts`).
      Проверено с `-hwaccel cuda -hwaccel_output_format cuda`: `-pix_fmt p010le` там падает, а
      `-highbitdepth 1` даёт 10-битный AV1/HEVC из 8-битного источника. Для H.264 10 бит не
      включается: High 10 не играет в Chromium. VideoPool декодирует без
      `hwaccel_output_format cuda`, поэтому там остался `-pix_fmt p010le`.
- [x] Попутные фиксы: `lookaheadLevel` не доходил до финального кодирования
      (`video-options-builder.ts`); цепочка deband на 10-битном источнике падала на
      `hwdownload,format=nv12`; снимок профиля в очереди импорта терял `force10Bit` и
      `temporalFilter`, поэтому VMAF-поиск шёл без них.

### Замер: `-tf_level 0` против `4`

Условия замера:

- Кодирование: av1_nvenc p7, UHQ, multipass fullres, VBR по CQ, AQ 8, lookahead 51, level 3,
  `b_ref_mode middle`, p010le.
- Сэмплы: 4 × 20 с в FFV1, как у VMAF-поиска (20/40/60/80% длительности).
- Метрика: `libvmaf vmaf_v0.6.1`.
- Железо: RTX 5080 Laptop, драйвер 616.92, 2026-09-16.
- Скрипт: `-tf_level` — единственное отличие между прогонами.

**Источник 1 — BDRip HEVC 10 бит 1080p, заметное зерно** (ТВ-эпизод, ~8 Мбит/с):

| CQ | Размер tf0, МиБ | Размер tf4, МиБ | Δ размер | VMAF tf0 | VMAF tf4 | Δ VMAF | Δ время |
| -- | --------------- | --------------- | -------- | -------- | -------- | ------ | ------- |
| 20 | 27.06           | 23.55           | −13.0%   | 97.261   | 97.254   | −0.007 | +14.4%  |
| 24 | 17.06           | 15.02           | −11.9%   | 97.071   | 97.067   | −0.004 | +16.1%  |
| 28 | 11.17           | 10.01           | −10.3%   | 96.829   | 96.820   | −0.009 | +7.2%   |
| 32 | 7.35            | 6.86            | −6.6%    | 96.450   | 96.427   | −0.023 | +4.8%   |

**Источник 2 — WEB-DL H.264 8 бит 1080p, чистый** (OVA-эпизод, ~3.5 Мбит/с, `force10Bit`):

| CQ | Размер tf0, МиБ | Размер tf4, МиБ | Δ размер | VMAF tf0 | VMAF tf4 | Δ VMAF | Δ время |
| -- | --------------- | --------------- | -------- | -------- | -------- | ------ | ------- |
| 20 | 51.63           | 47.52           | −8.0%    | 98.434   | 98.418   | −0.015 | +11.5%  |
| 24 | 33.31           | 30.94           | −7.1%    | 98.331   | 98.316   | −0.015 | +22.7%  |
| 28 | 21.87           | 20.52           | −6.2%    | 98.159   | 98.141   | −0.018 | +10.1%  |
| 32 | 14.48           | 13.68           | −5.5%    | 97.859   | 97.844   | −0.015 | +54.9%  |

Выводы:

- При том же CQ фильтр экономит 5–13% размера. Больше всего он даёт на зернистом источнике и
  на низких CQ.
- VMAF падает не больше чем на 0.023, то есть в пределах шума метрики.
- Время у источника 2 шумное: параллельно шли `nx lint`/`typecheck`, а FFV1 декодируется на
  CPU. На спокойной машине (источник 1) прибавка — +5…16%.
- ⚠️ **VMAF почти не наказывает за потерю зерна.** На источнике 1 разница между CQ 20 и CQ 32 —
  всего 0.8 пункта. Поэтому «VMAF не изменился» не доказывает, что зерно сохранилось.

### Что осталось

- [ ] ⚠️ **Открытый вопрос: оставлять ли фильтр в профиле «Архив».** VMAF не отличает сглаженное
      зерно от сохранённого, поэтому замер ответа не даёт. Нужен взгляд владельца на зернистом
      рипе: кадр 1:1, tf0 против tf4. Если фильтр съедает зерно, которое хочется сохранить, — в
      «Архиве» его выключить, в «Blackwell UHQ» оставить.
...
- [ ] **VMAF-фоллбэк на шумных источниках, без предсказаний** (2026-09-06, по решению
      ...
      (SVT-AV1 `film-grain-denoise=1`, уже частично используется — [sample.ts:53-54](main/src/ffmpeg/sample.ts),
      [video-pool.ts:1027-1028](main/services/pools/video-pool.ts)) как более «правильный» по сути
