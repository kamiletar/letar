...
- [ ] **Два standalone-плеера** — подробный чек-лист расследования уже расписан в §20. Начинать с
      него: пока плееров два, всё остальное в плеерной части делается дважды
- [ ] **Классификатор «надписи/песни» работает только для внешних файлов.**
      [external-subtitle-scanner.ts:345](main/services/external-subtitle-scanner.ts) содержит
      `SubtitleType = 'full' | 'signs' | 'songs'` и `SIGNS_PATTERNS`, определяет тип по суффиксу
      файла и имени папки. Для **встроенных** дорожек та же логика не применяется, хотя `title` у
      probe уже есть. Свести к одной функции при выносе в `libs/folder-scan` (§3)
...
- [ ] **Клик по видео не ставит паузу** — [VideoPlayer.tsx:528](renderer/src/components/player/VideoPlayer.tsx)
      глушит событие через `stopPropagation` (подробно — §18.1). Самая дешёвая правка из всего
      плана, и её видно при каждом просмотре
- [ ] **`disposition` не запрашивается у ffprobe** ни для аудио
      ([probe.ts:116](main/ffmpeg/probe.ts)), ни для субтитров ([probe.ts:256](main/ffmpeg/probe.ts))
      → forced-дорожки не видны вообще (подробно — §19.4). Правка на две строки

**Документация:**

- [ ] Заметка в `.claude/docs/` про **`app://` вместо `file://`** в Electron со статическим
      экспортом: Worker и WASM под `file://` не запускаются (origin `null`), ловится только на живом
      запуске. Дополнить [.claude/rules/electron.md](/.claude/rules/electron.md)
- [ ] Скилл **`/create:new-electron-app`** — сейчас в `/create:new-app` про Electron только
      предупреждение-заглушка, а Electron-приложений в монорепо становится четыре (§6, §13)
- [ ] Зафиксировать в скилле [`i18n-multilingual`](/.claude/skills/i18n-multilingual/SKILL.md), что
      стеков **два и это осознанно**: i18next для Electron-стека, next-intl для веба (§14.1).
      Иначе следующий агент попытается «унифицировать» и сломает либы
