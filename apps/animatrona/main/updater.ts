 ...
 * `autoUpdater.checkForUpdates()` без предварительной установки feed URL) — `kamiletar/letar`
...
function pointOwnFeed(): Promise<boolean> {
  return pointFeedAtOwnRelease(autoUpdater, {
    ...
    userAgent: 'Animatrona-Update-Client',
    onNotFound: (message) => log.warn(message),
  })
}
