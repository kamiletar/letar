...
export function registerHistoryHandlers(): void {
  ...
  ipcMain.handle(
    'history:get',
    createValidatedHandler(historyFilterSchema.optional(), async (filter?: ImportHistoryFilter) => getHistory(filter)),
  )

  // Получить запись по ID
  ipcMain.handle(
    'history:getById',
    createValidatedHandler(idSchema, async (id: string) => getHistoryById(id)),
  )

  // Добавить запись
  ipcMain.handle(
    'history:add',
    createValidatedHandler(historyCreateSchema, async (data: ImportHistoryCreateData) => addHistoryEntry(data)),
  )

  // Удалить запись
  ipcMain.handle(
    'history:delete',
    createValidatedHandler(idSchema, async (id: string) => deleteHistoryEntry(id)),
  )
