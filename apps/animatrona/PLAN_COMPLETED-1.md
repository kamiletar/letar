...
## Удалён мёртвый код `_hooks/useAudioSync.ts` (2026-09-08)

**Контекст:** отдельная проверка побочной находки из аудита `useShakaPlayer.ts` выше. Греп по
`useAudioSync(` во всём `apps/animatrona` дал только определение функции и реэкспорты
(`_hooks/index.ts`, `player/index.ts`) — ни одного реального вызова. `GlobalVideoProvider.tsx`
(строки ~180-246) синхронизирует раздельные аудиодорожки инлайновым `useEffect`, тот же паттерн
persistent-video архитектуры, что и с Shaka Player. Хук и его экспорты удалены.
