...
- **Временной фильтр NVENC (`-tf_level 4`) на RTX 50** для профилей с `temporalFilter`
  («Blackwell UHQ», «Архив»). На других поколениях GPU фильтр не передаётся. Если ffmpeg
  отказал, кодирование перезапускается без фильтра и не падает. Замер на двух источниках: при
  том же CQ файл меньше на 5–13%, VMAF меняется не больше чем на 0.023 (таблицы — в `PLAN.md`).
  Уровень 1 из старого закомментированного кода ffmpeg не принимает — только 0 и 4. Если у
  пресета меньше 4 B-кадров, добавляется `-bf 4`.
- `main/ffmpeg/nvenc-args.ts` — единый набор NVENC-аргументов для VideoPool и VMAF-сэмплов.
  `shared/nvenc-limits.ts` — пределы lookahead и подсказки к форме профиля.
- 4 новых spec-файла, 54 теста: аргументы NVENC, стратегии кодирования, пределы lookahead,
  аргументы VMAF-сэмплов.

### Changed

- **Lookahead встроенных GPU-профилей: 250 → 51.** ffmpeg всё равно обрезал 250 до 51–55
  («Clipping lookahead depth…»): предел зависит от числа B-кадров, а поверхностей не больше 64.
  В форме профиля максимум 58, рядом подсказка.
- `force10Bit` на CUDA-пути (`encoder-strategies.ts`) даёт 10 бит через `-highbitdepth 1`
  вместо `-pix_fmt`, который конфликтует с `-hwaccel_output_format cuda`. Для H.264 10 бит не
  включается: High 10 не декодируется в Chromium.

### Fixed

- VMAF-сэмплы кодировались своим набором аргументов (tune hq, aq-strength 15, GOP 120, без
  lookahead), поэтому подобранный CQ не соответствовал финальному файлу. HEVC и H.264 на GPU
  сэмплились на CPU.
- `lookaheadLevel` из профиля не доходил до финального кодирования.
- Deband на 10-битном источнике падал на `hwdownload,format=nv12`: теперь для 10 бит берётся
  `p010le`.
- Снимок профиля в очереди импорта терял `force10Bit` и `temporalFilter`. Элементы, поставленные
  в очередь до этой версии, по-прежнему идут без них.
...
- **Автоопределённые OP/ED больше не теряются при импорте.** Шаг `detectIntros` сохранял главы
  через `updateChaptersInManifest`, а та читает `Episode.manifestCid`, которого на той фазе ещё
  не существует — ранний `return` без ошибки, главы пропадали молча. Теперь найденные главы
  кладутся в `PostProcessData.detectedChapters` и попадают в манифест при его сборке в
  пост-обработке (`GenerateManifestOptions.detectedChapters`). Главы из контейнера сохраняют
  приоритет. Затронуты в первую очередь фильмы и одиночные серии: страховочный pre-pass в
  `anime-directory-builder` требует ≥2 эпизодов и для них не срабатывал никогда.
- **`needsReupload` больше не снимается при доливе части серий.** Условие считало ошибки только
  по файлам текущего entry, поэтому долив 2 серий из 12 снимал метку со всего аниме, хотя
  остальные 10 оставались на старой раздаче. Теперь метка снимается, только если в аниме не
  осталось эпизодов вне этого прогона.
- **Эпизоды вне прогона видны в результате импорта.** Их номера попадают в `warning`
  («Эп. 3, 4 не входили в этот прогон — остались со старыми CID») — раньше о них не сообщалось
  никак, и они молча утягивали `contentHealth` в `broken`. Автоудаление намеренно не делается:
  каскад `Anime` сносит историю просмотра.
