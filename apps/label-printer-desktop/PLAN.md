...
- [ ] ⚠️ **Открытый вопрос: переход на `installAndRelaunchViaScheduler` (2026-09-17, v0.5.17) не
      ...
      Заодно подтверждён отдельный пробел: `main/` не покрыт `typecheck:tsgo` (`tsconfig.json`
      его исключает) — правки там проверяются только `lint`, не полноценным typecheck.
