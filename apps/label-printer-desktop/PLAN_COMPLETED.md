...
## `next/font/google` → `next/font/local` (2026-09-22, v0.5.20)

Репо-широкая миграция от 2026-09-22 (`PLAN-INFRA-6.md` §195) изначально не включала это
приложение — Electron не входил в исходный список кандидатов, собранный грепом на более раннюю
дату, найден попутно финальной проверкой. Отдельная сессия оценила применимость и решила
мигрировать: хотя цикл сборки Electron-приложения отличается от веб-приложений на сервере деплоя
(`build`/`build:win`/`build:linux` на машине релиза, не при каждом деплое), `next build`
renderer'а — обязательный шаг во всех трёх, а найденный на `domwellbes` баг был про хрупкость
самого парсера ответа загрузчика Google, не про недоступность сети — риск применим независимо от
частоты сборки.

**Что сделано:**

- `Source Sans 3` (variable, ось `wght` 400–700 — покрывает прежний дискретный
  `weight: ['400','600','700']`) и `Cousine` (в google/fonts нет variable-файла — два статических
  начертания Regular/Bold) вырезаны субсеттером под `latin`+`cyrillic`, те же диапазоны, что были
  в `subsets: ['latin', 'cyrillic']`.
- Сканирование исходников (`scripts/fonts/scan-extra-glyphs.py`) нашло один символ вне диапазона —
  `→` (U+2192) в `app/api/model/[...path]/route.ts:66`, внутри `console.error(...)` (серверный
  лог, не рендерится в UI) — субсет расширять не потребовалось.
- Покрытие кириллицы (А–я, Ёё) проверено разбором `cmap` (`fontTools.getBestCmap()`) — полное в
  обеих гарнитурах.
- `renderer/app/_fonts/index.ts` переведён на `localFont()`, добавлен
  `renderer/app/_fonts/fonts/README.md` с рецептом пересборки.
- Полный гейт зелёный: format → lint → `typecheck:tsgo` → `nx build` (production `next build` +
  main webpack) — оба прошли, `.next/static/media/` содержит захешированные локальные `.woff2`.
- Живая проверка в браузере (`getComputedStyle`) не выполнена — приложение рассчитано только на
  запуск внутри Electron IPC-контекста, голый `next dev` вне Electron отдаёт 404 на всех роутах
  (не связано со шрифтами, известное ограничение — `.claude/rules/electron.md`). Подтверждение
  миграции — эмитированные локальные файлы шрифтов в production-сборке, без единого сетевого
  обращения к Google Fonts.

**Файлы:** `renderer/app/_fonts/index.ts`, `renderer/app/_fonts/fonts/` (новые `.woff2` + `OFL-*.txt`
