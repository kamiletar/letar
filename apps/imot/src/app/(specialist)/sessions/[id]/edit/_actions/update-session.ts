...
type UpdateSessionResult = { success: true } | { success: false; error: string; field?: string }

/**
 * Server action для обновления существующей сессии.
 */
export async function updateSession(sessionId: string, data: SessionFormData): Promise<UpdateSessionResult> {
  ...
  // 4. Обновление сессии
  try {
    // Проверяем, что сессия существует и принадлежит специалисту
    const existingSession = await db.therapySession.findFirst({
      where: {
        id: sessionId,
    ...
    if (!existingSession) {
      return { success: false, error: 'Сессия не найдена или не принадлежит вам' }
    }
    ...
    return { success: false, error: 'Не удалось обновить сессию. Попробуйте еще раз.' }
  }

  // 5. Редирект на страницу детального просмотра сессии
