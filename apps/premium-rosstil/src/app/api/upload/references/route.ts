...
/**
 * API endpoint для загрузки фото-ориентиров заказов.
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Проверка размера файла (максимум 10MB для фото-ориентиров)
    ...
    const uploadsDir = join(process.cwd(), 'uploads', 'references')
...
/**
 * API endpoint для удаления фото-ориентиров.
 * Доступен для всех авторизованных пользователей.
 */
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!url || !url.startsWith('/api/files/references/')) {
    ...
    const filepath = join(process.cwd(), 'uploads', 'references', filename)
    const path = `references/${filename}`
