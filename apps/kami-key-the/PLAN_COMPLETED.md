...
детерминированной схемой и нашла, почему прежние живые тесты давали противоречивые результаты.

- **Диагноз 1.9.25:** табличный `tasklist` обрезает имя образа до 25 символов, поэтому
  `KamiKeyThe-Setup-1.9.26.exe` не находился никогда, и двухфазная проверка ничего не делала.
  Воспроизведено на копии `ping.exe` с тем же именем.
- **Новая схема (`main/updater.ts`, `installAndRelaunchViaScheduler`):** `.bat` через `schtasks`
  ждёт выхода приложения по PID, синхронно запускает `<installer> --updated /S`, логирует
  `errorlevel` и только потом запускает приложение. `quitAndInstall` не вызывается,
  `autoInstallOnAppQuit` отключается перед `app.quit()`. Если планировщик недоступен — откат на
  `quitAndInstall(true, false)`. В начале `.bat` стоит `chcp 65001` для кириллических путей.
- **Проверка без релиза:** bun-тест вызывает настоящий `updater.ts` с заглушками electron и
  настоящим `schtasks`, ставит скачанный инсталлятор поверх установленной версии — зелёный.
- **Живой цикл 1.9.27 → 1.9.28:** установка 31 с, `errorlevel=0`, запуск после неё, баннер
  v1.9.28, процесс стабилен.
- **MSIX-контейнер Claude Desktop:** шеллы агента виртуализируют HKCU и новые каталоги
  `%LOCALAPPDATA%`. Этим объясняются «1.9.27 в реестре» и, скорее всего, первая неудачная попытка
  1.9.25 → 1.9.26. Разбор — `.claude/docs/claude-desktop-msix-container-virtualization.md`.
- Отвергнутые гипотезы: фоновая задача агента убивает потомков; `KILL_ON_JOB_CLOSE` в job шелла.

## Иконка Ḱ² + заметное warning-уведомление + релиз v1.9.6 (2026-09-16)

Косметическая сессия: редизайн иконки приложения по запросу пользователя и заметность
предупреждения о несовместимой раскладке, оба пункта уточнялись итеративно по живым скриншотам.

- **Иконка Ḱ² (v1.9.3–1.9.6):** `scripts/build-icons.mjs` — буква K дополнена акутом (отсылка к
  функции AltGr+Enter, ударение U+0301) и надстрочной «²» (сочетание клавиш). На размерах <48px
  «²» опущена, остаётся только Ḱ. Две итерации правок по обратной связи: акут сначала «уехал»
  (отдельный полигон вместо привязки к стеблю — заменён на stroke-путь), затем вся композиция
  оказалась смещена вправо и обрезалась мутной каймой на скруглённых углах 16/32px — причина
  каймы: `build-icons.mjs` растеризовал 256px и уменьшал через `sharp.resize()`, ресемплинг
  полупрозрачных пикселей скругления давал грязный ободок. Фикс — рендерить SVG сразу под
  целевой пиксельный размер (`width`/`height` = size в самой SVG-строке), без промежуточного
  растрового ресайза; плюс сдвиг всего кластера на 34px влево для баланса полей.
- **Warning-стиль уведомления (v1.9.4–1.9.5):** `src/notification.ts` — у «AltGr не работает —
  раскладка …» теперь отдельный `variant: 'warning'` (кэшированные вторые brush/font), крупнее
  шрифт, значок ⚠, время показа 2.5с→3.5с. Цвет менялся дважды по правке пользователя: красный →
  оранжевый (`#D35400`). Добавление горизонтального отступа текста (`TEXT_PADDING_X=20px`)
  вскрыло, что `DrawTextW` по умолчанию клипает текст по границам `rect` (без `DT_NOCLIP`) — без
  расширения окна 480→530px «English» обрезалось до «Engli».
- **Живая проверка на реальном железе:** GDI-оверлей и warning-notification не рендерятся в
  браузере — верификация каждой итерации через PowerShell (`user32.dll keybd_event` для
  физического AltGr + `CopyFromScreen` для скриншота), см.
  [native-win32-ui-verification-screenshot.md](/.claude/docs/native-win32-ui-verification-screenshot.md).
  Продакшн-копия останавливалась/переустанавливалась/перезапускалась перед каждой проверкой.
- **Релиз v1.9.6:** `gh release create kami-key-the-v1.9.6` (аккумулировал v1.9.3–1.9.6, предыдущий
  тег — v1.9.2). `git push` на общий `main` наткнулся на непушнутый бамп `domwellbes`/
  `domwellbes-e2e` от параллельной сессии (pre-push хук блокировал бы деплой) — запушен только
  диапазон до своего коммита (`git push origin <sha>:main`), чужой более поздний коммит остался
  локальным, не тронут.

## GDI-оверлей под новую тему + релиз v1.9.1 (2026-09-16)

Три backlog-пункта, оставленных редизайном v1.8.0, закрыты в одной сессии, плюс первый публичный
релиз после v1.7.4.

- **GDI-оверлей (v1.9.0):** `src/overlay.ts` переведён на палитру редизайна — цвета сняты живым
  `getComputedStyle()` с отрендеренного `key-button.tsx` и вручную пересчитаны в COLORREF
  (GDI не умеет альфа-смешение кистей/перьев, полупрозрачные токены заранее сведены в плотный
  цвет). Квадранты синхронизированы с редактором: EN верх-лево, RU низ-лево, AltGr низ-право,
  AltGr+Shift верх-право, один шрифт на оба AltGr-символа.
- **Красноватый RU (v1.9.0):** новый семантический токен `fg.ru` (`theme.ts`) — RU-подпись клавиши
  и в редакторе, и на GDI-подсказке теперь тёплого красноватого оттенка, отличима от EN на взгляд.
- **`stats:getTop` (v1.9.0):** отсутствующий IPC-обработчик — добавлен `main/ipc/stats.handlers.ts`
  по образцу `system.handlers.ts`, устранена ошибка `Error: No handler registered` в консоли
  рендерера.
- **Размер GDI-оверлея (v1.9.1):** по итогам живой проверки (было мелко и плохо видно) —
  клавиатура оверлея увеличена на ~25% (`56px`→`70px` клавиша, окно `936×336`→`1170×420`).
- **Релиз v1.9.1:** первый GitHub Release после v1.7.4 (тег `kami-key-the-v1.9.1`), накопил весь
  путь v1.8.0–v1.9.1. Процесс — вручную по документированной схеме (`gh release create` +
  `gh release upload` с переименованием ассетов под дефисы), `kami-key-the-landing` подхватывает
  новую версию/changelog из GitHub API сам, без ручных правок.
- Верификация GDI-оверлея (нерендерится в браузере) делалась PowerShell-симуляцией физического
  `AltGr` + полноэкранным скриншотом — техника задокументирована как переиспользуемый паттерн в
  [native-win32-ui-verification-screenshot.md](/.claude/docs/native-win32-ui-verification-screenshot.md).

## Редизайн окна приложения (v1.8.0, 2026-09-15)

Полный UI/UX редизайн по плану `.claude/plans/vast-growing-hinton.md` (аудит нашёл: окно не
прокручивалось на минимальном размере, редактор и настройки не были связаны навигацией, тулбар
«радуга» кнопок разного веса, `div onClick` вместо доступных `<button>`, 150+ hex-литералов
цвета мимо темы Chakra).

- **Окно и шапка:** `titleBarStyle: 'hidden'` + `titleBarOverlay` (не `frame: false`) — сохраняет
  нативные Snap Layouts Windows 11, разбор — `.claude/docs/electron-window-controls-overlay-pattern.md`.
  Своя HTML-шапка (`shell/title-bar.tsx`) с навигацией Редактор/Настройки, переключателем
  перехвата AltGr, безопасной зоной под системные кнопки через `env(titlebar-area-*)`.
- **Тема** (`theme.ts`): неоновый зелёный `#39ff14` + циан `#22d3ee` по образцу лендинга,
  тёмная/светлая по `prefers-color-scheme` (Electron транслирует системную тему Windows),
  семантические токены вместо хардкода.
- **Редактор:** вкладки раскладок получили меню действий по явному индексу (раньше удаление
  всегда било по активной вкладке — баг), клавиатура масштабируется под ширину окна
  (`use-element-width.ts` на `ResizeObserver`), нижняя `ActionBar` (статус + undo/redo +
  сохранить) вместо ряда разноцветных кнопок.
- **Страница клавиши:** карточки слотов AltGr/AltGr+Shift (`key-slot-card.tsx`), поиск символов
  растягивается на доступную высоту (`flex="1" minH="0"` вместо `maxH="300px"`), пустое
  состояние с кликабельными подсказками.
- **Настройки:** карточки (`settings-card.tsx`), «Шпаргалка раскладки» — `Table.Root` вместо
  моно-текста с hex-кодами.
- **Иконка** перерисована под зелёную палитру (`scripts/build-icons.mjs`, буква «K» полигонами —
  шрифт в librsvg на Windows непредсказуем).
- **Кнопка «Назад»** (аппаратная/мышиная, `main/background.ts` `app-command`) привязана к
  реальному стеку истории — выбор клавиши и категория пикера пишутся `pushState`
  (`editor-route.ts` `pushRouteToLocation`), `popstate` переигрывает route обратно; live-проверено
  в Browser pane (клавиша → категория → «назад» ×2 → клавиатура, Настройки → «назад» → Редактор).
- Найден и обойдён (не исправлен в источнике) баг Vite dev: импорт из barrel `@letar/ui` тянет
  `next/*` в пребандл, `process is not defined` в Electron renderer — разбор и временный шим в
  `.claude/docs/vite-dev-letar-ui-barrel-process-undefined.md`. Корневой фикс (подпути-экспорты
  `@letar/ui`) — вне объёма, передано `ui-coordinator-dev`.
- Backlog из находок по ходу редизайна (не задача сессии) — GDI-оверлей не следует новой
  палитре, drag-and-drop символа на клавиатуру стал недостижим после разделения экранов,
  отсутствующий IPC-обработчик `stats:getTop` — все три занесены в `PLAN.md` § Бэклог.

## Критичный краш прод-сборки + раскладка «Индексы» (v1.7.5, 2026-09-15)

- **Критичный фикс:** production-инсталлятор падал сразу после установки —
  `TypeError: Cannot set properties of undefined (setting 'options')`. Причина — webpack
  `optimization.concatenateModules` (scope hoisting, включён по умолчанию только в
  `--mode production`) ломал циклическую CJS-загрузку `js-yaml` (транзитивная зависимость
  `electron-updater` из `main/updater.ts`, добавленного в v1.7.4): конструктор `Type`
  вызывался без `new`. В dev-сборке webpack concatenation не включает, поэтому баг не был
  виден до этого релиза. Фикс — `concatenateModules: false` в `main/webpack.config.js`.
- **Уточнение предыдущего решения по категории «Индексы»** (см. запись v1.7.3 ниже): решение
  «¹²³ не затронуты — уже были в Latin-1» оказалось неполным с точки зрения UX — пользователь
  ожидал видеть их именно во вкладке «Индексы» вместе с остальными над-/подстрочными знаками.
  `U+00B9/00B2/00B3` добавлены в диапазоны категории `super-sub`
  (`renderer/src/editor/symbol-categories.ts`) — они теперь состоят в обеих категориях
  одновременно (пересечение категорий уже было нормой для других символов, например ¢ в
  «Валюты» и «Latin-1»).
- **Новая встроенная раскладка «Индексы».** Цифры 0-9 и операторы `+ - = ( )` — без Shift
  подстрочный вариант (`₀₋...`), с Shift надстрочный (`⁰⁻...`), для формул вида `xⁿ⁺¹`
  (`src/config.ts`). Добавлена мягкая миграция `migrateBuiltInLayouts()` — новая встроенная
  раскладка дописывается в уже существующий `keymap.json` при следующем запуске, не трогая
  раскладки, которые пользователь завёл сам под тем же именем.
- Отладка попутно вскрыла, что `%APPDATA%\kami-key-the\` (нижний регистр, из `name` в
  `package.json`) — реальный путь к конфигу, не `%APPDATA%\KamiKeyThe\` (`productName`,
  который в `name` пакета electron-builder не переносит). Не проблема кода, просто зафиксировано
  для будущей отладки — см. `PLAN.md`.

## Автообновление через electron-updater + фикс категории «Индексы» (v1.7.3–1.7.4, 2026-09-12…13)

- **Фикс категории пикера символов «Индексы».** Диапазон U+2070–U+209F (верхние/нижние подцифры
  ⁴⁵⁶⁷⁸⁹/₀₁…₉, знаки ⁺⁻⁼⁽⁾ⁿ/₊₋₌₍₎) не попадал ни в одну категорию `symbol-categories.ts` —
  дыра между «Пунктуация» (до U+206F) и «Буквенные» (с U+2100), видна была только на вкладке
  «Все». ¹²³ (U+00B9/B2/B3) не затронуты — уже были в «Latin-1». Добавлена отдельная категория
  (v1.7.3).
- **Автообновление** (`main/updater.ts`, `electron-updater`, v1.7.4) — детали и обе найденные
  грабли (repo-wide `GithubProvider` в общем с animatrona репозитории; несовпадение имён
  ассетов с `latest.yml`) описаны в `PLAN.md` § «Технический долг → Автообновление» и
  `CHANGELOG.md`. Общая логика поиска своего релиза вынесена в
  `@letar/electron-monorepo-updater`, использована также в `animatrona` — устранён дубль.
  Стандарт публикации Electron-релизов в монорепо задокументирован в
  `.claude/docs/electron-monorepo-shared-releases.md`.
- Синхронизировано со скачиванием на `kami-key-the-landing` (автогенерация версии/URL из
  GitHub Releases — см. её `PLAN.md`/`PLAN_COMPLETED.md`).
...
- [x] Исследование архитектуры: keysender vs Koffi (см. «Архитектурные решения» в PLAN.md)
...
> ~~keysender~~ — удалён. См. «Архитектурные решения» в PLAN.md.
...
**Дефолтный конфиг:** одна раскладка «Типографика» с текущими 25 маппингами (число росло по мере
задач из бэклога — см. PLAN.md, «Аудит свободных слотов и категорий символов»).
