...
- Автообновление через `electron-updater` (`main/updater.ts`) — источник GitHub Releases, тот же
  `publish.provider: github`, что уже настроен в `electron-builder.yml`. Тихая проверка при
  старте (задержка 10с, без диалога «обновлений нет»), плюс пункт меню трея «Проверить
  обновления» для ручной проверки с явным результатом. Скачивание и установка — только по
  подтверждению пользователя в диалоге (`autoDownload: false`), по образцу
  `label-printer-desktop` (`main/services/updater.service.ts`) — не toast/renderer-интеграция, как
  у animatrona: приложение живёт в трее без постоянно открытого окна, стримить статус в renderer
  было бы некуда.
