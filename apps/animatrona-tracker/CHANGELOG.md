...
- Фикс `:active` на iOS (пустой пассивный `touchstart` на `document`) вынесен в `useIosActiveFix` из `@letar/chakra-provider`. Локальная копия удалена, хук вызывается явно (провайдер на голом `ChakraProvider`); листенер теперь снимается при размонтировании (в StrictMode dev он навешивался дважды).
...
- **`prisma/seed.ts` — идемпотентный сид демо-данных для локальной разработки.** Раньше dev-БД
  оставалась пустой после `prisma migrate reset`, а аниме попадали в трекер только реальной
  публикацией из Animatrona Desktop по IPFS — визуальные регрессии ловились только на проде.
  Сид создаёт: dev-пользователя со взрослой датой рождения (иначе `getAllowedRatings()` режет
  каталог до g/pg/pg_13); франшизу «Сумеречный Ветер» из 6 тайтлов + отдельное «Городские
  Легенды» со всеми шестью `RelationKind` (SEQUEL/PREQUEL/SIDE_STORY/SPIN_OFF/CHARACTER/
  ALTERNATIVE_VERSION); 42 эпизода, включая фильм 1:45:12 (часовая ветка `formatDuration`);
  библиотеку dev-пользователя во всех 6 `WatchStatus`; прогресс просмотра на 2 эпизодах (питает
  блок «Продолжить просмотр»); 3 комментария. Проверено живьём (`nx dev` + dev-session bypass):
  каталог, главная, страница аниме. Повторный `nx db:seed` не плодит дублей (проверено `psql`).
