...
- [ ] ⚠️ Открытый вопрос (2026-08-28, сквозной аудит монорепо): `notFound()` из кода страницы
      отдаёт **HTTP 200**, а не 404 — подтверждено живым замером на dev-сервере. Причина —
      корневой `src/app/loading.tsx` (Suspense-граница) оборачивает всё приложение целиком, в том
      числе вложенные `anime/loading.tsx` и т.п. Механизм и разбор —
      [nextjs-streaming-soft-404-loading-boundary](/.claude/docs/nextjs-streaming-soft-404-loading-boundary.md)
      (найдено изначально в domwellbes, тот же паттерн).

      Замер (`/anime/[id]/page.tsx` — публичная страница, доступна без сессии):

      | Путь                                       | Код | `<meta robots>` |
      | -------------------------------------------- | --- | ---------------- |
      | `/anime/no-such-anime-probe-404`              | 200 | `noindex`         |
      | `/nope-nope-nope-route` (несуществующий маршрут) | 404 | `noindex`     |

      Индексации сейчас не вредит — тег `noindex` уже стоит. Три варианта решения — как в
      domwellbes (принять как есть / проверка в `proxy.ts` / убрать `loading.tsx`) — решение за
      владельцем, этой сессией не принималось.
