    ...
    <VStack align="stretch" gap={6}>
      ...
      <Box>
        ...
        {pinnedOn ? (
          <Box p={4} bg="bg.subtle" borderRadius="lg" borderWidth="1px">
            <Grid templateColumns={{ base: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)' }} gap={4}>
              <VStack align="start" gap={1}>
                ...
                <Badge colorPalette="purple" variant="outline" size="sm">{pinnedOn.role}</Badge>
              </VStack>
              ...
              <VStack align="start" gap={1}>
                ...
                <Text fontSize="sm" fontFamily="mono" wordBreak="break-all">{pinnedOn.apiUrl}</Text>
                {pinnedOn.peerId && (
                  <Text fontSize="xs" color="fg.muted" fontFamily="mono" isTruncated maxW="200px">
                    {pinnedOn.peerId}
                  </Text>
                )}
              </VStack>
              ...
              <VStack align="start" gap={1}>
                ...
                <Text fontSize="sm">
                  ...
                  {pinnedOn.capacityBytes > 0 && (
                    <Text as="span" color="fg.muted"> / {formatBytes(pinnedOn.capacityBytes)}</Text>
                  )}
                </Text>
              </VStack>
            </Grid>
          ...
          <Box p={4} bg="bg.subtle" borderRadius="lg" borderWidth="1px" borderStyle="dashed">
            <Text color="fg.muted" fontSize="sm">Аниме не запинено ни на один сервер</Text>
          </Box>
        )}
      </Box>
      ...
      <Box>
        <HStack mb={3}>
          ...
          <Badge colorPalette="purple" variant="subtle" size="sm">{viewCount} уникальных</Badge>
        </HStack>

        {viewers.length === 0 ? (
          <Box p={4} bg="bg.subtle" borderRadius="lg" borderWidth="1px" borderStyle="dashed">
            <Text color="fg.muted" fontSize="sm">Никто не добавил в библиотеку</Text>
          </Box>
        ) : (
                ...
                {viewers.map((v) => {
                  ...
                  return (
                    <Table.Row key={v.userId}>
                      ...
                      <Table.Cell>
                        <Badge colorPalette={ws.color} size="sm">{ws.label}</Badge>
                      </Table.Cell>
                      ...
                      <Table.Cell>
                        {v.pinnedLocally ? (
                          <Badge colorPalette="green" size="sm" variant="subtle">Да</Badge>
                        ) : (
