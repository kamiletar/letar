...
- `src/app/_components/use-toc-scroll.ts` (новый файл): вынесена общая логика `toc.tsx` и
  `mobile-toc.tsx` — сбор заголовков из DOM и throttled scroll-хэндлер (прогресс чтения + активный
  пункт), которая была продублирована дословно и уже дважды приводила к рассинхронизации фикса
  (`setTimeout` вместо `requestAnimationFrame` применили сперва в `toc.tsx` в 1.9.9, `mobile-toc.tsx`
  унаследовал старый баг и был исправлен только в 1.9.10). Оба компонента используют
  `useTocScroll()`; уникальная часть (desktop sticky nav с автоскроллом к активному пункту vs
  mobile Drawer/FAB) осталась на месте.

## [1.9.10] - 2026-09-22

### Fixed

- `src/app/_components/mobile-toc.tsx`: тот же фикс, что в 1.9.9 для `toc.tsx` — throttle
  scroll-хэндлера (прогресс чтения + активный пункт TOC) `requestAnimationFrame` → `setTimeout(50мс)`.
  Мобильная версия была скопирована из десктопной до фикса 1.9.9 и унаследовала тот же баг: `rAF`
  замирает без фокуса окна, throttle на его основе может не срабатывать в реальных условиях.

## [1.9.9] - 2026-09-22

### Fixed

- `src/app/_components/toc.tsx`: throttle scroll-хэндлера (прогресс чтения + активный пункт TOC) —
  `requestAnimationFrame` → `setTimeout(50мс)`. Тот же класс бага, что уже дважды чинился в этом
  файле (см. запись 1.9.8 выше): `rAF` полностью замирает без фокуса окна
  (`.claude/docs/raf-vs-timers-background-tab.md`), а Playwright-браузеры в CI обычно без реального
  фокуса. На staging это давало стабильное (не флейковое) падение в webkit и флейковое — в firefox:
  прогресс-бар/активный пункт застревали на начальном значении при скролле и не сбрасывались при
  клиентской навигации. `setTimeout` не замирает без фокуса, визуальную плавность по-прежнему даёт
  CSS `transition` на самой полосе.
...
- **Короткие текстовые ссылки без достаточного touch target** (WCAG 2.5.5) — навигационные
  ссылки-списки (футер, главная страница, ссылка «На главную» в сайдбаре, «Вернуться к
  документам»/«Вернуться назад») заменены на `TouchLink` из `@letar/ui` (`libs/ui/src/lib/touch-link.tsx`,
  паттерн из `domwellbes`) — минимальная высота цели тапа 44px. Инлайновые ссылки внутри MDX-контента
  (`cross-ref.tsx`) и ссылки-карточки с уже достаточной областью клика (документы на главной,
  списки кодексов/уставов/регламентов) не трогали — попадают под essential/inline-исключение WCAG 2.5.5.
