...
(pg-boss) по паттерну `PLAN-INFRA-4.md §75`, тираж после `dashboard`/`driving-school`/
`dsperevod`/`aboi`.

- **У приложения нет роли ADMIN и admin-панели вообще** (проверено — ни `role`/`roles` в модели
  `User`, ни один `@@allow` со ссылкой на роль во всей `schema.zmodel`, ни одной admin-директории
  в `src/app`). Это единственное из пяти приложений тиража без привилегированной зоны — поэтому
  здесь **нет `/admin/jobs` и нет ручного runNow через UI**, только автотик по расписанию и
  read-only `/api/jobs/status` для наблюдателя `dashboard-agent`. Отклонение от паттерна
  остальных приложений намеренное, не недосмотр.
- `schema.zmodel` — модель `JobOverride` всё равно обязательна (`createAppJobsModule` читает её
  при старте через `prisma.jobOverride.findMany()` независимо от наличия UI), но с
  `@@deny('all', true)` — правится только напрямую через raw `prisma`, как `AppSettings` в aboi;
  миграция `20260903160436_add_job_override`.
- `src/jobs/notifications.ts` — логика **скопирована из `/api/cron/notifications` без изменений**
  (не вынесена в общий lib-хелпер, чтобы не трогать код живой ручки, которая намеренно не
  удаляется до подтверждённого прод-прогона). Как и оригинал, НЕ бросает на частичных ошибках
  отправки отдельным подписчикам — они пишутся в `NotificationLog.error`, задача считается
  успешной в целом (иначе задача с 300+ подписчиками падала бы от одного невалидного email).
- `instrumentation.ts` — добавлен `startTimeJobs()`.
- `docker-compose.production.yml` уже использует `env_file: .env.docker` — явной строки
  `JOBS_ENABLED` не потребовалось. `JOBS_ENABLED=true` выставлен в `.env.docker.enc`.
- Живая проверка: dev-сервер поднят, лог подтвердил регистрацию задачи (`autoSchedule` off без
  `JOBS_ENABLED` — ожидаемо для dev), `GET /api/jobs/status` с `X-Cron-Secret` вернул корректный
  статус задачи `notifications` (`schedule: "* * * * *"`, `enabled: true`) — конвейер
  регистрации/статуса подтверждён. Ручной `runNow` не проверялся отдельно — эндпоинта для него
  нет намеренно (см. выше), а автотик проверять локально бессмысленно (задача триггерится раз в
  минуту по реальному времени, не по клику).
...
физически существует — в аудите его не заметили. Next.js metadata route convention отдаёт
`icon.*` (любое расширение файла-источника, включая `.svg`) на URL `/icon` БЕЗ расширения
