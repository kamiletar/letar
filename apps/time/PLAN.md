...
- [x] Проверка SEO: hreflang для каждой локали — 2026-08-06, `sitemap.ts` заведён с
      `alternates.languages` на все 40 локалей (`alternates` в `generateMetadata` уже был). Заодно
      найден и починен реальный баг: `robots: { index: true, follow: true }` было захардкожено
      безусловно — staging индексировался наравне с продом. Детали — `PLAN_COMPLETED.md`
      и `PLAN-INFRA.md` §33
