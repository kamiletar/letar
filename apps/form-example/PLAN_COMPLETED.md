...
Грепом по всем 46 `page.tsx` найдено 16 файлов без `.meta(` вызова — на первый взгляд похоже на
расхождение конвенции.

Прочитано всё 16 файлов построчно, разложено на 3 категории:

- **14 страниц вообще не используют `schema=`** (`async-validation`, `autosave`, `calculated`,
  `captcha`, `conversational`, `credit-card`, `data-grid`, `documents`, `matrix-choice`,
  `server-errors`, `survey-fields`, `table-editor`, `templates`, `testing-utilities`) — `Form`
  без Zod-схемы, заголовки полей заданы через `label=` проп напрямую. `.meta({ui})` физически
  неприменим без схемы — это не расхождение, а другой, тоже документированный и рабочий, режим
  `@letar/forms` (schema-less форма). Форсировать схему только ради `.meta()` уменьшило бы
  дидактическую ценность этих страниц (часть из них специально показывает работу без схемы).
- **`edit-intent`** — схема есть (`editIntentValueSchema(...)`), но обёрнутый EditIntent-тип не
  ложится на `.meta({ui:{title}})` естественно; заголовок внутреннего `Form.Field.Password`
  задан через `label=` — стандартный паттерн для составных полей.
- **`zenstack`** — схема сгенерирована `@letar/zenstack-form-plugin` из `schema.zmodel`, и весь
  смысл страницы — «ни строчки написанного вручную Zod» (дословно в тексте страницы). Добавление
  `.meta()` вручную было бы прямым нарушением демонстрируемого паттерна.

Отдельно проверены inline-комментарии на предмет языка (репозиторное правило — комментарии на
русском): единственные англоязычные комментарии найдены внутри `Code`-блоков (`captcha`,
`autosave`) — это строки примеров кода, показываемые читателю страницы на английском (сам текст
страницы англоязычный), не реальные комментарии разработчика в TS-коде. Не нарушение.

**Итог: правок не потребовалось** — все найденные расхождения оказались намеренными паттернами,
а не дефектами. Задача закрыта аудитом, как и предыдущие пункты бэклога, где сначала
проверялось реальное состояние кода, а не факт существования файла.
