...
- [x] Groups — sortable drag&drop + вложенные массивы (Course → Modules → Lessons)
...
| 2026-09-14 | ✅ `/examples/server-errors` дополнена секцией `useFormServerAction` (`@letar/forms-react` v0.9.0/`@letar/forms` v2.14.13) — живая форма рядом с существующим explorer `mapServerErrors()`, см. PLAN_COMPLETED.md                                   |
| 2026-09-24 | ✅ Новая страница `/examples/create-option` — `onCreate` у Select/Combobox (`@letar/forms`), ссылка в сайдбаре                                                                                                                                      |
