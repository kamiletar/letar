...
**Что реально несёт роль «основной storage-узел сети» — `infra/letar-ipfs/` на s3**
(на 2026-09-08 это был старый s3 `188.127.235.141`, PeerId
`12D3KooWM7KtRLjqRmJzva7Qy5KZzfaLES4Fk8GgnjabbWoo8A52`, проверено `docker exec kubo ipfs id`
там же; ⚠️ с 2026-09-19 старый s3 отключён, роль у нового s3 `185.130.251.234` — совпадение
PeerId после переноса сверь `docker exec kubo ipfs id`, здесь не проверялось). Это ДРУГОЙ узел с другим PeerId, не переезд этого
