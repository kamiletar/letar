...
⛔ **Поправка 2026-08-08: это касается s3, а не s2, и упирается в M2, а не в M3.** Здесь раньше
стояло «касается s2 и всплывёт в M3» — неверно. Слепок proxy host'ов s3 показал `gateway.letar.best`
именно там, вместе с `ipfs.letar.best`, `ipfsstor4.letar.best` и `media.letar.best`. Ни у одного из
четырёх нет compose в репозитории, то есть **label'ами они не закрываются в принципе** — а
остановка NPM на s3 их снесёт.

Урок общий, не про IPFS: перед остановкой прокси список его host'ов надо снимать **с сервера**, а не
брать из документации. Дока описывала намерение, сервер — факт, и разошлись они на целый сервер.
