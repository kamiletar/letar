...
> Hosts на s2», — теперь **чистая история**, не текущее состояние ни одного сервера.
> **Проверять живым сервером (`ss -tlnp`, `docker ps`), не этим файлом** — ровно эта ошибка
> (спланировать новый сервис через NPM по устаревшей доке) уже случилась 2026-08-10 с s3.
> Актуальная конфигурация прокси — [infra/traefik/README.md](/infra/traefik/README.md).
...
| ----------------------------------- | ----------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------ |
| **s2.letar.best**                   | все production-приложения (см. deploy-agent.md) | ⛔ **остановлен** (2026-08-31) | Маршрутизация — Traefik, см. [infra/traefik/README.md](/infra/traefik/README.md)           |
| **s3.letar.best**                   | staging-домены приложений + e2e-раннер          | ⛔ **выведен** (2026-08-08)    | Маршрутизация — Traefik, см. [infra/traefik/README.md](/infra/traefik/README.md)           |
...
**Docker сеть:** `kami-network` — Traefik и приложения на s2 (NPM в ней остался только как
остановленный контейнер, для отката). Маршрутизация на обоих серверах — через
[infra/traefik/README.md](/infra/traefik/README.md); host-gateway NPM ниже описан только для
истории.
...
### ⛔ NPM на s2 (npm.s2.letar.best) — ОСТАНОВЛЕН 2026-08-31, раздел ниже — история

**С 2026-08-31 контейнер NPM на s2 остановлен** (не удалён — данные в бэкапе на сервере,
`/home/deploy/backups/npm-s2-*.tar.gz`), весь трафик 80/443/443-udp держит Traefik (см.
предупреждение в начале файла и [infra/traefik/README.md](/infra/traefik/README.md)). Таблица
ниже — снимок конфигурации на момент актуальности, те же домены теперь маршрутизируются
Traefik-labels/`dynamic/s2/*.yml`.
...
### ⛔ NPM на s3 — ВЫВЕДЕН ИЗ ЭКСПЛУАТАЦИИ 2026-08-08 (было: обнаружен и задокументирован 2026-07-11)

**Раздел ниже — история, не текущее состояние.** С 2026-08-08 контейнера NPM на s3 не существует
вообще, весь трафик 80/443 держит Traefik (см. предупреждение в начале файла и
[infra/traefik/README.md](/infra/traefik/README.md)). Таблица доменов оставлена как снимок
конфигурации на момент актуальности — те же домены теперь маршрутизируются Traefik-labels в
`docker-compose.staging.yml` каждого приложения, порты и хост-гейтвей `172.17.0.1` больше не
задействованы.
