...
> ⚠️ **Замена NPM на Traefik — [PLAN-INFRA-2.md §48](/PLAN-INFRA-2.md).** На **s3 уже выполнено**
> (2026-08-08): NPM там **выведен из эксплуатации полностью**, контейнера не существует, весь
> трафик 80/443 держит Traefik (labels в `docker-compose.staging.yml` каждого приложения, см.
> [infra/traefik/README.md](/infra/traefik/README.md#как-подключить-приложение)). Таблица ниже и
> раздел «Proxy Hosts на s3» описывают **прод s2** и историю — не текущее состояние s3.
> **Проверять живым сервером (`ss -tlnp`, `docker ps`), не этим файлом** — ровно эта ошибка
> (спланировать новый сервис через NPM на s3 по устаревшей доке) уже случилась 2026-08-10.
...
| ----------------------------------- | ----------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------ |
| **s2.letar.best**                   | все production-приложения (см. deploy-agent.md) | npm.s2.letar.best           | Единственный prod-сервер, NPM активен                                                      |
...
**Docker сеть:** `kami-network` (s2, NPM активен). На s3 маршрутизация через `traefik-network` —
раздел «Как подключить приложение» в [infra/traefik/README.md](/infra/traefik/README.md), не
через host-gateway NPM (устаревший способ, описан ниже только для истории/s2).
