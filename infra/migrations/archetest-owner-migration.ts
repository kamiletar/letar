 ...
 *
 * Dry-run:
 *   DRY_RUN=1 DATABASE_URL=... OLD_EMAILS=... bun run infra/migrations/archetest-owner-migration.ts
 *
    ...
    for (const oldUser of oldUsers) {
      ...
      for (const ach of achievements) {
        ...
        if (exists.length > 0) {
          ...
          await client.query('UPDATE "UserQuizAchievement" SET "userId" = $1 WHERE id = $2', [newUser.id, ach.id])
      ...
      if (oldUser.leaderboard_id) {
        ...
        if (newLb.length > 0) {
          ...
          await client.query('UPDATE "QuizLeaderboardEntry" SET "userId" = $1 WHERE id = $2', [
            newUser.id,
            oldUser.leaderboard_id,
          ])
