...
⚠️ **Осталось два хвоста, намеренно не сделанных в этой сессии.**

1. **`.gitignore` 11 submodule держат точное `.next/`.** Корневой на них не действует. На граф
   Nx это не влияет (`.nxignore` в корне воркспейса покрывает всё дерево) — только на untracked-
   шум в `git status` самого submodule. Правка на строку в каждом, но это 11 отдельных репозиториев
   (коммит + пуш + бамп SHA), а в одном из них на момент находки шла живая сессия. Делать пачкой,
   когда submodule не заняты.
2. **Структурно правильнее — не класть временный distDir внутрь `apps/` вообще.** Тогда Nx его
   не обойдёт по построению, независимо от имени, и оба слоя `.nxignore` станут страховкой, а не
   единственной защитой. Кандидат — каталог под уже игнорируемым `.nx/` (`.nx/tmp/prodcheck-<app>`).
   Требует согласованной правки во всех местах, где сборка запускается с `NEXT_DIST_DIR`.
