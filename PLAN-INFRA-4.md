...
## §70 — Трекинг ошибок: GlitchTip на s3 🟡 ЧАСТИЧНО ЗАКРЫТО (сервер+23 приложения — 2026-09-04/08) — Electron/RN и коммерческие/ПДн-приложения не начаты
...
6. ✅ **Загрузка sourcemaps в CI — сделано (2026-09-04, `d9b2ef86`).** Классический Sentry-эндпоинт
   ...
   - [x] ✅ `letar` запушен (2026-09-08, `repo-dev`) — оба блокировавших submodule давно
         подтянуты, `check-submodule-push-state.sh` зелёный 14/14.
   - [x] ✅ **Закрыто (коммит `3ac94223`, подтверждено предком `origin/main`).**
         `dashboard`/`dashboard-agent` деплоятся тем же `deploy-affected.sh` (self-deploy
         ветка через `systemd-run`), не отдельным SSH-путём — `export GLITCHTIP_RELEASE`/
         `NEXT_PUBLIC_GLITCHTIP_RELEASE` прописан прямо внутри heredoc `RESTART_SCRIPT`
         (`deploy-affected.sh:1351-1352`), т.к. detached-процесс не наследует `export`
         родительского bash. `release:` в событиях GlitchTip для этих двух приложений больше
         не `undefined`.
