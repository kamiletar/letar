...
## §145 — аудит утверждений-абсолютов в `.claude/docs`: 7 устаревших фактов из ~20 проверяемых ✅ ЗАКРЫТО (2026-09-03)
...
**Открытый вопрос:** фикс общий, но поштучно на остальных шести потребителях
(`dashboard`, `mandala`, `grandslamcup`, `animatrona`, `animatrona-tracker`,
`label-printer-desktop`, `form-develop-app`) не перепроверялся — прогнан только
`typecheck:tsgo dashboard`. Изменение строго в сторону стабильности, но если где-то на нём
держался нежелательный сброс кеша между навигациями, это всплывёт как «данные стали
залипать». Deploy studio с этим фиксом прошёл на прод s2 (`0af4c030`).
