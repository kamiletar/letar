...
- [ ] ⚠️ **Открытый вопрос: гейт нигде не запускается автоматически.**
      `check-patched-deps.mjs` сработает, только если человек или агент дойдёт до чеклиста
      `/infra:deps-update`. Ровно та же слабость у `check-peer-deps.mjs`. Настоящий барьер —
      pre-commit-хук на изменение `bun.lock`/`package.json` (или шаг в CI). Не сделано: решение
      владельца, стоит ли утяжелять pre-commit ещё одной проверкой. Вопрос задан 2026-08-28,
      ответа пока нет.
...
## §120 — перечисление путей в `@nx/vitest include` прятало 88 нерабочих тестов ✅ ЗАКРЫТО (2026-08-28)
