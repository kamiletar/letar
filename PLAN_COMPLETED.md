...
### Этап 0 — Доставка писем (первопричина) ✅ ЗАКРЫТО (2026-09-03, проверено живьём; перенесено в архив 2026-09-08)
...
## Предложения архитектора — поверхностный анализ, все пункты закрыты (перенесено 2026-09-08)

> Исходный раздел «13. Предложения архитектора» из `PLAN.md` — разовый поверхностный аудит, по
> каждому пункту которого решение уже принято и отражено в основном плане (§9 «Точки принятия
> решения», соответствующие этапы roadmap). Оставлен здесь как история анализа, не как активная
> задача.

### Вопросы для уточнения — ЗАКРЫТЫ
...
4. **Tier 2 / динамика:** ✅ **Отложено** до заключительных этапов — D8 в §9 `PLAN.md`, spike перед реализацией.
5. **Ключница OIDC / refresh:** проверено при работе над Этапом 6 (kami).
6. **Rate-limit NAT:** ✅ NAT не актуален (пользователи из разных мест) — IP-based достаточен.

### Уязвимость: SSE endpoint с email в URL — включено в Этап 1

**Проблема.** Реализация SSE `/api/auth/verification-stream/${email}` — email в URL, позволяет
подписаться на поток чужого email (enumeration юзеров). **Рекомендация** — одноразовый
`streamToken` (UUID) вместо email-параметра, инвалидируется при верификации/истечении PIN.

### Timing-атака на PIN — включено в Этап 1

`pin-validator.ts`: строковое сравнение PIN уязвимо к timing-атаке. Рекомендация —
`crypto.timingSafeEqual` вместо `!==`.

### Rate-limit: два уровня (IP + email) ✅ уточнено

NAT не актуален. Итог: IP-уровень `{ window: 60, max: 10 }`, email-уровень
`{ window: 3600, max: 5 }`, ключ = `ip + email` (`rateLimit.customRules` Better Auth).

### SMTP graceful degradation ✅ включено в план

Cooldown не применяется при `success === false`; пользователю — нейтральное сообщение;
Telegram — 3 подряд failure → webhook; Umami — event на каждый failure.

### Динамика OAuth-провайдеров Better Auth ✅ отложено → D8

Для существующих коммерческих приложений и Tier 2 (ключи из БД при старте/reload) динамика не
нужна. Актуально только для гипотетической «SaaS Ключницы» — D8 в `PLAN.md` §9, spike перед
реализацией.

### Passkeys / WebAuthn ✅ сделано → Этап 6.5

Реализовано, см. Этап 6.5 выше в этом файле.

### Ключница OIDC: refresh-token handling — проверено в Этапе 6

Sub-task Этапа 6 (kami): `accessTokenExpiration`, реакция на 401, `offline_access` scope.

### Авто-логин токен: гарантия single-use ✅ включено → Этап 1

Адаптер `updateTokenForAutoLogin` — delete + create, не update.

### Наблюдаемость: KPI верификации ✅ включено → Этап 0 + Этап 2

Umami events (отправка письма, успешная верификация, resend) + Telegram alerting (Этап 0) —
вместе дают картину % доставки + % верификации.

### Nx module-boundary tags ✅ включено → Этап 0.5

Описание — Этап 0.5 выше в этом файле.
