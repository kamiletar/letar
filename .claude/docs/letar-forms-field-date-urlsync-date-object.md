...
Известное ограничение, фикс в библиотеку не внесён. Правка `FieldDate`, коммитящая строку вместо
`Date` при отсутствии `schema`/`z.date()`-типа, рискует сломать существующих потребителей,
рассчитывающих на `Date` в `onSubmit` (в т.ч. сценарий, ради которого сделан комментарий в самом
компоненте) — компонент не различает «внутри формы со схемой» и «внутри `Form.UrlSync`-фильтра
без схемы» на уровне пропсов. Если понадобится URL-персистентный date-range фильтр в другом
приложении — делегировать через `.claude/rules/form-delegation.md`, не копировать workaround
studio молча.
