...
### ⚠️ Версия vite резолвится per-package по peer-dep хешу — «обязательность» файла не универсальна

Проверено эмпирически (2026-08-05): когда у `grandslamcup` появился первый реальный тест
(`album.action.spec.ts`), `nx test grandslamcup` прошёл **без** `tsconfig.spec.json` — TSCONFIG_ERROR
не воспроизвёлся, хотя `vitest.setup.tsx` так же не покрыт `include` корневого `tsconfig.json`, как
и у всех остальных приложений с этим багом.

Причина — в общем bun-сторе одновременно живёт больше десятка версий vite
(`node_modules/.bun/vite@X.Y.Z+<hash>`), и то, какая именно резолвится конкретному приложению,
зависит от peer-dep хеша его собственного дерева зависимостей, а не от версии в корневом
`package.json` (там `"vite": "^8.2.0"` одна на всех). Временный снос `tsconfig.spec.json` показал:
`archetest` резолвит `vite@8.1.3` — TSCONFIG_ERROR воспроизводится; `grandslamcup` на тот момент
резолвил `vite@8.2.0` — не воспроизводится ни разу за прогон.

**Вывод:** отсутствие ошибки на конкретном приложении сейчас **не значит**, что баг пофикшен
апстримом или что `tsconfig.spec.json` этому приложению не нужен в принципе — это значит, что
именно ему сейчас достался «удачный» хеш vite. Любой будущий `bun install`/обновление зависимостей
может перехешировать peer-deps и вернуть баг без единого изменения в коде приложения. Поэтому
**правило «tsconfig.spec.json обязателен при появлении первых тестов» остаётся в силе для всех
приложений** независимо от того, воспроизводится ли TSCONFIG_ERROR прямо сейчас — не полагаться на
результат одного зелёного прогона теста как на доказательство, что фикс не нужен. Именно поэтому
`grandslamcup` получил `tsconfig.spec.json` проактивно (v3.38.4,
[apps/grandslamcup/PLAN_COMPLETED.md](/apps/grandslamcup/PLAN_COMPLETED.md)), а не был оставлен
«как есть, раз тесты и так зелёные».
