...
## Грабля с порядком `GET /releases` — не доверяй ответу API как «свежие первыми»

`findOwnLatestTag` изначально брал первый тег с нужным префиксом из ответа `GET /releases`,
опираясь на документированное поведение GitHub («сортировка по дате публикации, свежие
первыми»). На практике (живой тест `kami-key-the` 1.9.9 → 1.9.10, 2026-09-16) свежесозданный
релиз **не поднимался на первую позицию 30+ минут** при `per_page=100` и всего 9 релизах в
ответе — не лаг на секунды/минуту, а стабильно неверный порядок, не менявшийся при повторных
запросах с cache-busting параметром и `Cache-Control: no-cache` (то есть не клиентское
кеширование). Возможная причина — эвентуальная консистентность индекса списка релизов на
стороне GitHub под высокой частотой релизов в этом монорепо (десятки приложений публикуются
почти непрерывно), но точный механизм не подтверждён — важен эмпирический факт, что полагаться
на порядок ответа нельзя.

**Фикс** (`find-own-release.ts`): среди всех тегов с нужным префиксом выбирать максимальный по
