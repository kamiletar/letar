...
> риска для источника. **Вторая offsite-точка взамен `pinner2`** ([PLAN-INFRA-5.md §90](/PLAN-INFRA-5.md),
...
⚠️ **Уточнено 2026-09-08 (§82 PLAN-INFRA-5.md):** формулировка ниже описывала желаемый эффект, а
не механизм. Resilio синхронизирует **весь** `dir` (корень репозитория) за вычетом deny-list
`IgnoreList` — не только `uploads`/`backups`. На практике это означает, что **любой** новый
bind-mount/каталог с приватными файлами (не только `uploads`) синхронизируется автоматически без
отдельной регистрации, если его имя не совпадает с одним из паттернов ниже. Подтверждено
эмпирически на `apps/aboi/print-sources/` (первый такой bind-mount, §82) — каталог есть в оффсайт
Windows-копии `C:\BackupSync\letar\s2\...`, хотя в `IgnoreList` для него отдельной записи никогда
не заводилось. Действие нужно, только если новый каталог **должен** быть исключён (секрет,
пересобираемый артефакт) — тогда добавить его имя в `IgnoreList` явно, как остальные записи ниже.
...
### Оффсайт-копия (добавлено 2026-09-02, PLAN-INFRA-5.md §90)
