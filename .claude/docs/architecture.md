...
инфраструктурный сервер s1 (`185.56.162.213`; до 2026-09-19 эту роль нёс старый s3, теперь
отключён), работает по ночному cron, изолирован от production серверов (s2) и от деплоя:
`deploy-affected.sh` статус e2e не проверяет.

- Инфраструктура, порты, настройка нового приложения для e2e — [e2e-testing.md](/.claude/docs/e2e-testing.md#e2e-ранер-на-s3-188127235141) (заголовок раздела исторический, содержимое — про s1)
