...
⚠️ Это качественная оценка по чтению тем, а не грепаемый факт: «замкнут полностью» значит, что
приложение переопределило все ключи из §1, а не просто упомянуло часть из них. Аудит 2026-09-03
проверил только счётчик приложений с каталогом `src/theme` (было 15, стало 16) — **кто из них
закрыл контракт после 2026-08-19, не перепроверялось**. Не опирайся на «единственное» как на
текущий факт: прежде чем делать из этого вывод (например «мы первые, образца нет»), перечитай
`semanticTokens` конкретных кандидатов.
