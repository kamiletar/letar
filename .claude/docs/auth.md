...
**«Сервер авторизации недоступен» при OIDC-входе — не всегда виновата Ключница**

Клиентские приложения, подключённые через OIDC (`genericOAuth`, см. выше), часто маппят
любой 5xx-ответ от своего же `/api/auth/oauth2/*` в generic-сообщение вида
«сервер авторизации недоступен» — хотя реальной причиной может быть сбой **локальной БД/окружения
самого клиентского приложения**, а не Ключницы (`auth.letar.best`). Сообщение об ошибке
указывает на внешний сервис, даже если проблема локальная — на практике так уже случалось
минимум в одном приложении монорепо.

Если пользователь видит «сервер авторизации недоступен» (или аналог) при входе через Ключницу —
**сначала** проверь логи/консоль самого клиентского приложения на предмет ошибок БД/окружения,
и только потом считай проблему внешней (сбоем auth-hub).
