...
## Скрипт `scripts/add-lib-tsconfig-path.mjs` — массовая правка `paths` в apps/ и libs/

Заводишь новый subpath у библиотеки с несколькими точками входа (см. чек-лист выше, п.3) —
строку `paths` нужно добавить в **каждый** `tsconfig.json`-потребитель библиотеки-якоря — и в
`apps/*/tsconfig.json` (`@letar/forms` даёт ~19 приложений, включая `apps/animatrona/renderer` и
`apps/label-printer-desktop/renderer`, которые на уровень глубже и поэтому получают
`../../../` вместо `../../`), и в `libs/*/tsconfig.json|tsconfig.lib.json|tsconfig.spec.json`
(библиотеки тоже потребляют подпути друг друга — см. `scripts/check-lib-subpath-paths.mjs` и
`.claude/rules/libs.md` § «Потребителю нужны paths и на транзитивные `@letar/*`, и на все их
подпути»). Делать это руками одноразовым скриптом под каждый новый subpath — источник ошибок:
сессия расслоения `libs/forms` → `libs/forms-core` (Фаза 7.1, Этапы 1/3а,
[libs/forms/PLAN.md](/libs/forms/PLAN.md)) проходила через это четыре раза подряд, каждый раз с
ручным подбором relative-prefix.

⚠️ **2026-09-14: область правки расширена внутрь `libs/`** (до этой даты скрипт трогал только
`apps/*/tsconfig.json`, а 8 расхождений внутри `libs/`, найденные при расширении
`check-lib-subpath-paths.mjs` тем же днём, чинились вручную). Заодно исправлен relative-prefix:
скрипт раньше считал кратчайший путь (`path.relative`), что между соседними библиотеками внутри
`libs/` давало более короткую, но стилистически отличную от остальных записей строку
(`"../forms-core/..."` вместо принятого повсюду `"../../libs/forms-core/..."`) — теперь путь
всегда выражается как «подняться до корня репозитория, затем спуститься», как и в существующих
записях `apps/*`.
...
Потребители находятся автоматически — и в `apps/*/tsconfig.json` с вложенными
`apps/*/*/tsconfig.json` (`renderer/`, `main/` и т.п.), и в `libs/*/tsconfig.json|
tsconfig.lib.json|tsconfig.spec.json` — без явного списка. Библиотека-владелец правки (каталог,
вычисленный из `--package`, например `forms-core` для `@letar/forms-core/security`) внутри
`libs/` пропускается сама — самоссылка на собственные подпути не нужна. Идемпотентен:
...
⚠️ **До 2026-09-24 скрипт молча пропускал tsconfig с комментариями (JSONC).** Файл, который не
разбирается `JSON.parse`, считался «не потребителем», даже если anchor-ключ в нём был. Отчёт при
этом выглядел полным: «Добавлено в 28 файлов». Так без подпути `@letar/chakra-provider/next`
остался `apps/animatrona-folder-player/renderer/tsconfig.json`, и поймал это только
`check-lib-subpath-paths`. Теперь такие файлы перечисляются отдельным блоком «добавь строку
руками», и скрипт выходит с кодом 1. Вывод: после скрипта всегда гоняй
`bun scripts/check-all.mjs --only=lib-subpath-paths`, отчёт скрипта полноту не доказывает.
...
## `check-lib-subpath-paths.mjs` — область сканирования расширена внутрь `libs/` (2026-09-14)

До этой даты сторож (см. `.claude/rules/libs.md` § «Потребителю нужны `paths` и на
транзитивные…») видел только `apps/*/tsconfig.json` — собственные потребители библиотек друг у
друга внутри `libs/` оставались невидимы проверке. Расширение сразу вскрыло реальный пример того
же класса бага, что описан в этом документе (Фаза 7.3, 2026-08-09), но теперь между библиотеками:
`libs/forms/tsconfig.spec.json` держал `paths` только на 9 подпутей `@letar/forms-core/*` из 18
(найдено и починено вручную 2026-09-14, коммит `41f2eb537`), а следом скрипт нашёл ещё 7 похожих
потребителей внутри `libs/` (`forms`, `forms-react`, `forms-shadcn`, `folder-player-react` →
`@letar/folder-scan`). Долг разгребён тем же днём — недостающие строки `paths` добавлены во все 7
файлов, `typecheck:tsgo` зелёный на всех четырёх затронутых библиотеках, `severity` вернулась в
`gate`.
