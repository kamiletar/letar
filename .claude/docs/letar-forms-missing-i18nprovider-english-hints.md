...
## Третий механизм: статичные UI-строки самих полей (2026-09-22)

Кроме constraint hints и сообщений Zod есть третья группа текстов — статичные надписи, которые
компоненты печатают сами: заголовок сводки `Form.Errors`, кнопки диалога черновика
(`useFormPersistence`), пустое состояние выпадающего списка. Они не проходят ни через
`generateConstraintHint`, ни через `createFormErrorMap`, поэтому ни `locale="ru"`, ни
`setupZodErrorMap` их не переводят — каждую надо локализовать отдельно, по месту.

Последний случай (`@letar/forms` 2.16.3): `Form.Field.Combobox`/`Form.Field.Autocomplete` печатали
литерал `Enter at least {minChars} characters` прямо в JSX. В русском интерфейсе это выглядело как
необъяснимо пустой список с английской подсказкой под полем, причём на дефолтном `minChars: 1` ещё
и с ошибкой числа (`1 characters`).

**Канонический порядок резолва для такой строки** (`resolveDefaultErrorsTitle` в `form-errors.tsx`,
`resolveMinCharsHint` в `selection/min-chars-hint.ts`):

1. перевод приложения по стабильному ключу (`formErrors.title`, `formSelection.minCharsHint`) —
   только если `FormI18nProvider` получил `t`;
2. встроенный словарь ru/en по `locale` из провайдера;
3. английский текст — поведение при полном отсутствии провайдера в дереве.

Третья ступень обязательна и ломать её не нужно: `FormI18nProvider` опционален по контракту
библиотеки, его отсутствие — не ошибка конфигурации.

⚠️ Хук локализации в поле, собранном через `createField`, вызывается **в `useFieldState`**, а не в
`render`: `render` — колбэк внутри `form.Field`, а не компонент, хуки там небезопасны. Готовая
строка приходит в `render` через `fieldState`.

⚠️ Числительные в такой строке склоняются через `Intl.PluralRules` (формы слов — те же, что в
`CHAR_PLURALS`/`ITEM_PLURALS` в `constraint-hints.ts`), иначе русский текст даёт «минимум 3
символов», а английский — «1 characters».
