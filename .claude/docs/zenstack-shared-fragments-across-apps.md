...
⚠️ **Но фикс не подхватывается сам по себе: приложения грузят плагин из `dist/`, а `dist/` не
коммитится.** В `schema.zmodel` он подключён путём к артефакту сборки:

```zmodel
plugin formSchema { provider = "../../libs/zenstack-form-plugin/dist/index.js" }
```

После правки исходников плагина `dist/*.js` остаётся старым, пока не выполнен
`nx build zenstack-form-plugin` — при этом `tsc` обновляет `.d.ts` отдельно от `.js`, так что
свежие `.d.ts` рядом со старыми `.js` выглядят как «собрано» и вводят в заблуждение (поймано
ровно так 2026-09-08: `.d.ts` от 12:04, `.js` от позавчерашних 21:56, фикс в `src` уже был).
...
То же касается свежего клона и `git worktree` — там `dist/` отсутствует вовсе, и генерация падает
