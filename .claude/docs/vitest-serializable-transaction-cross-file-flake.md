...
**✅ 2026-09-22: частота выросла — вернулись.** Масштаб флака по всему приложению (не только
`schedule/`) достиг 4-11 из 392 файлов почти на каждом полном прогоне — принято именно то
решение, от которого отказались 2026-08-27: `fileParallelism: false` в `vitest.config.mts`
глобально. Подтверждено эмпирически: 0/2723 падений при полной сериализации (было 129s/4 файла
при дефолтном параллелизме). Полный разбор, включая рассмотренные и отклонённые альтернативы —
