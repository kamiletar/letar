...
Гейт ловит два разных по тяжести класса и печатает их раздельно, но роняет прогон в обоих
случаях одинаково (`gate` целиком, не по классам):

- **Класс 1 (build-breaking): ключ `transpilePackages` отсутствует ЦЕЛИКОМ**, а приложение
  реально импортирует внешние `@letar/*`. Это ровно случай, ломающий сборку, описанный выше
  (`Module parse failed`) — не соглашение, а настоящий будущий 500/красный `nx build`.
- **Класс 2 (соглашение, не поломка): ключ есть, но конкретный пакет в списке не перечислен.**
  Гейт проверяет единообразие литерала, а не работоспособность — красный именно этого класса
  означает «список разъехался с tsconfig», не «прод-сборка сломана». В §140(4) и §141
  `PLAN-INFRA-4.md` красным был именно класс 2 (шаг `Integrity checks`), а не `nx build` —
  формулировка «регрессия `transpile-packages`» там про расхождение списка, и её легко прочитать
  как «сборка падала».

⚠️ **До 2026-09-15 класс 1 не был покрыт вовсе.** Цикл проверки безусловно пропускал приложение,
если `extractTranspilePackages` возвращал `null` (ключа нет) — `if (!transpiled) { continue }` —
считая любое отсутствие ключа «вне охвата этой проверки». Именно так `apps/form-example` тихо
накопил падающий прод-билд: у него не было `transpilePackages` вовсе, но был реальный внешний
импорт (`libs/glitchtip` через `src/instrumentation-client.ts`), а гейт молчал зелёным, потому что
про этот единственный build-breaking случай он не проверял ничего. Обнаружено и закрыто
2026-09-15 (forms-coordinator-dev) — скрипт теперь различает `transpiled === null` (класс 1,
критично) от пустого/неполного `Set` (класс 2, соглашение) и репортит оба явно.

Побочное следствие охвата гейта (не изменилось): пакет, импортируемый **без** записи в `paths`
(резолвится через симлинк bun — см. `.claude/rules/libs.md`, «paths — вспомогательные, не
обязательные»), гейт не видит вовсе — ни по классу 1, ни по классу 2. Так `@letar/query-provider`
в `studio` не попадает ни в отчёт гейта, ни в реальную проблему.
