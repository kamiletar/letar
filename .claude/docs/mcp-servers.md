...
| ----------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| studio-time | `libs/studio-time-mcp`                | `time_*` — тайм-трекер, см. [time-tracking.md](/.claude/rules/time-tracking.md); `time_log` — см. ниже                                             |
...
в агенте). Проявилось 2026-09-21 на деплое letar-landing после консолидации 2026-09-14 (до неё
каждая часть была отдельным процессом без внутреннего hop'а).

Фикс — `INTERNAL_CALL_OPTIONS` в `.claude/mcp/letar.ts` (`timeout` 10 мин) на всех четырёх
вызовах. **Новая часть/long-poll инструмент:** собственный лимит ставь в самой части (как
`timeoutMs` в `agentRequest`), а не в агрегаторе; агрегатор лишь страхует от вечного зависания.
⚠️ Проверка — реальным вызовом дольше 60с: короткие тесты через `connectedClient` эту ловушку не
видят. Сообщение вида «Не удалось достучаться до агента … Unable to connect» — отдельная история
(fetch к SSH-туннелю не дотянулся), с этим таймаутом не связано.

### `time_log`: реальный интервал, а не «последние N минут» {#time-log-interval}

До 2026-09-23 `time_log` принимал только `minutes` и ставил запись на `[сейчас − minutes, сейчас]`.
Созвон 21:06–22:29 МСК, записанный в 22:42, встал на 21:19–22:42 и перекрылся с идущим с 22:30
таймером той же сессии — двойной учёт.

Теперь интервал задаётся любыми двумя из `startedAt`/`endedAt`/`minutes` (ISO-8601; **без зоны —
московское время**, с `Z`/`+03:00` — как есть); один `minutes` — прежнее поведение, один
`startedAt` — до текущего момента. studio (`resolveLogInterval` в `apps/studio/src/lib/time-mcp.ts`)
отвергает конец в будущем (допуск 60 с), длительность ≤ 0 или > 1440 мин и все три параметра
сразу. Пересечение с записями того же исполнителя, включая идущий таймер, запись **не блокирует**
— приходит строкой `⚠️` в ответе. Ответ показывает, куда запись встала на самом деле
(`23.09, 21:06–23.09, 22:29 МСК (83 мин)`).

Знаешь время события — передавай `startedAt`/`endedAt`, не `minutes`. Подробнее —
[README studio-time-mcp](/libs/studio-time-mcp/README.md).
...
| `deploy_status`       | Статус деплоя + логи по курсору `sinceLine`; `phases[]`/`stalled`; `grep`/`regex`/`context`/`routeTable` — поиск по логу и таблица маршрутов Next.js (агент ≥ 0.18.1 хранит её отдельно от капнутого лога), ответ ≤ ~30 тыс. символов ([README](/libs/deploy-mcp/README.md#поиск-по-большому-логу-деплоя)) |
...
Блок «Route (app)» … легенда «(Static)/(SSG)/(Dynamic)» из лога деплоя распознают две независимые
копии трёх констант — `ROUTE_HEADER_RE`, `LEGEND_RE`, `BLANK_RE`:
`apps/dashboard-agent/src/lib/deploy-route-table.ts` (агент вынимает блок из потока лога в
`DeployStatus.routeTables`) и `libs/deploy-mcp/src/log-tools.ts` (разбор лога и запасной путь для агента
старше 0.18.1). Агент собран изолированно от монорепо (`Dockerfile.production`) и не может
импортировать из `libs/`, поэтому дубль **намеренный** — в общую библиотеку его не выносят.

⚠️ Цена дубля: Next.js изменит формат вывода, кто-то поправит одну копию — агент и `deploy-mcp` начнут
молча искать разные блоки. Ни typecheck, ни lint, ни тесты каждой стороны этого не видят: у каждой свои
фикстуры, зелёные по построению. Поэтому в общем раннере есть проверка `route-table-regex-sync`
(`scripts/check-route-table-regex-sync.mjs`, уровень **gate**, в CI видна целиком):

```bash
bun scripts/check-all.mjs --only=route-table-regex-sync
```

- Сравнивается **исходный текст** литералов (тело и флаги), а не поведение: эквивалентная по смыслу
  регулярка в другой записи — тоже расхождение, правку нельзя сделать «наполовину».
- Потерянная константа (переименована в одной копии), объявление дважды, незакрытый или не
  компилирующийся литерал и отсутствующий файл — тоже красная, а не молчаливая зелень. Переименовал
  константу — правь список `CONSTANTS` в скрипте.
- Меняешь формат распознавания таблицы — правь **обе** копии в одном коммите. Остальные регулярки
  `log-tools.ts` (`TOP_ROUTE_RE`, `CHILD_PATH_RE`) агент не дублирует, сторож их не касается.
- Тесты сторожа (`scripts/check-route-table-regex-sync.test.mjs`) запускаются шагом `Scripts tests` в CI.

### `BUILD_ON_S1_APPS`, `E2E_GATED_APPS` перечитываются при каждом деплое {#build-on-s1-fresh}
...
Тот же механизм у e2e-гейта, и последствие тяжелее: приложение, добавленное в `E2E_GATED_APPS` после
старта процесса, деплоилось вообще без проверки (fail-open) — вместо блокировки. Список гейта перечитывается
так же, как `BUILD_ON_S1_APPS` (2026-09-22, `deploy-mcp` 0.7.2).

⚠️ До 2026-09-22 гейтов было два — warn-only `E2E_GATED_APPS` и fail-closed `HARD_GATED_APPS`
(строгое подмножество, инвариант закреплялся тестом). Владелец их схлопнул в один список, всегда
fail-closed — держать отдельный warn-only список для приложений, которые всё равно не должны там
годами оставаться, было только риском рассинхрона (история — комментарий у константы в
`libs/infra-config/src/index.ts`).

Решение принимается в `deploy_app` (`resolveDeployServer`, `evaluateE2eGate`), поэтому там же списки и перечитываются:

- `build-on-s1.ts` (`readDeployLists`) читает `libs/infra-config/src/index.ts` **текстом** один раз на
  `production`-вызов и разбирает два массива как литералы строк. Не `import()` с cache-busting: у Bun
  (рантайм `letar.ts`) и у vite-node (тесты) разная семантика кеша модулей по query-строке, чтение файла
  одинаково везде. Имя файла историческое — сначала в нём читался один `BUILD_ON_S1_APPS`.
- Читается файл рабочего дерева (тот же, что импортирован), а не `origin/main`: правка списка в общем
  чекауте видна сразу после коммита, без `git fetch` на каждый деплой. Ограничение — если список изменили
  только удалённо и `git pull` в чекауте deploy-mcp не сделан, инструмент увидит старый файл; на этот случай
  `deploy-affected.sh --remote-release` на s1 сам отказывает приложению, которого нет в списке на s1.
- ⛔ **Нет молчаливого отката на s2 и нет молчаливого «гейта нет».** Файл не прочитан, объявления нет или
  массив записан не литералом (spread, константа, вычисление) — `deploy_app` возвращает `isError` с причиной
  и не обращается к агенту. Формат записи — контракт: комментарий у `BUILD_ON_S1_APPS` в `infra-config` его
  называет, охранный тест `build-on-s1.spec.ts` роняет CI, если разбор реального файла разошёлся с двумя
  импортированными константами.
- `staging` списки не читает (всегда s1, без гейта) и битым файлом не блокируется.
- Расхождение с тем, что процесс запомнил при старте, видно в ответе строкой «<СПИСОК> перечитан из файла…»
  — объясняет, почему маршрут или гейт не те, что дал бы устаревший список; строка есть и в отказе гейта.
- Описание инструмента `deploy_app` больше не интерполирует `E2E_GATED_APPS`: описание регистрируется один
  раз при старте, перечисленный в нём список устаревал бы (и вводил бы в заблуждение) так же, как массив.

Что **не** покрыто: остальные константы `infra-config` (`SERVERS`, `SERVER_APPS`) по-прежнему фиксируются
при старте. Сам код `deploy-mcp` (не списки) по-прежнему подхватывается только перезапуском. Процесс,
стартовавший **до** изменения (0.7.0 — списки маршрута; 0.7.2 — списки гейтов), нового кода не содержит — ему
нужен один последний перезапуск; после него правки списков рестарта не требуют.

Тест — `server.tools.spec.ts` («BUILD_ON_S1_APPS перечитывается при каждом вызове» и «E2E_GATED_APPS
перечитывается при каждом вызове»): сервер создаётся один раз, файл правится между вызовами
того же настоящего `Client` (подход — [mcp-tool-handler-testing-pattern](/.claude/docs/mcp-tool-handler-testing-pattern.md));
путь к файлу подменяется опцией `createDeployMcpServer({ infraConfigPath })`.
