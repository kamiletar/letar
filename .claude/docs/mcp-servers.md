...
Решение принимается в `deploy_app` (`resolveDeployServer`), поэтому там же список и перечитывается:

- `build-on-s1.ts` читает `libs/infra-config/src/index.ts` **текстом** при каждом `production`-вызове и
  разбирает массив как литерал строк. Не `import()` с cache-busting: у Bun (рантайм `letar.ts`) и у
  vite-node (тесты) разная семантика кеша модулей по query-строке, чтение файла одинаково везде.
...
- ⛔ **Нет молчаливого отката на s2.** Файл не прочитан или массив записан не литералом (spread, константа,
  вычисление) — `deploy_app` возвращает `isError` с причиной и не обращается к агенту. Формат записи —
  контракт: комментарий у `BUILD_ON_S1_APPS` в `infra-config` его называет, охранный тест
  `build-on-s1.spec.ts` роняет CI, если разбор реального файла разошёлся с импортированной константой.
- `staging` список не читает (всегда s1) и битым файлом не блокируется.
- Расхождение с тем, что процесс запомнил при старте, видно в ответе строкой «BUILD_ON_S1_APPS перечитан
  из файла…» — объясняет, почему маршрут не тот, что дал бы устаревший список.

Что **не** покрыто: перечитывается только `BUILD_ON_S1_APPS`. Остальные константы `infra-config`
(`SERVERS`, `SERVER_APPS`, `E2E_GATED_APPS`, `HARD_GATED_APPS`) по-прежнему фиксируются при старте — включая
список hard-gated приложений, для которого устаревший процесс означает «гейта нет» (остаётся кандидатом на
такое же решение). И сам код `deploy-mcp` (не список) по-прежнему подхватывается только перезапуском.
Процесс, стартовавший **до** этого изменения (0.7.0), нового кода не содержит — ему нужен один последний
перезапуск; после него правки списка рестарта не требуют.

Тест — `server.tools.spec.ts` («BUILD_ON_S1_APPS перечитывается при каждом вызове»): сервер создаётся один
раз, файл правится между вызовами того же настоящего `Client` (подход —
[mcp-tool-handler-testing-pattern](/.claude/docs/mcp-tool-handler-testing-pattern.md)); путь к файлу
подменяется опцией `createDeployMcpServer({ infraConfigPath })`.
