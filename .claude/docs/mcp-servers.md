...
### `BUILD_ON_S1_APPS`, `E2E_GATED_APPS`, `HARD_GATED_APPS` перечитываются при каждом деплое {#build-on-s1-fresh}
...
Тот же механизм у e2e-гейтов, и последствие тяжелее: приложение, добавленное в `HARD_GATED_APPS` после
старта процесса, деплоилось вообще без проверки (fail-open) — вместо блокировки. Списки гейтов перечитываются
...
- `build-on-s1.ts` (`readDeployLists`) читает `libs/infra-config/src/index.ts` **текстом** один раз на
  `production`-вызов и разбирает три массива как литералы строк. Не `import()` с cache-busting: у Bun
...
- ⛔ **Нет молчаливого отката на s2 и нет молчаливого «гейта нет».** Файл не прочитан, объявления нет или
  ...
  называет, охранный тест `build-on-s1.spec.ts` роняет CI, если разбор реального файла разошёлся с тремя
  импортированными константами.
- Приложение из `HARD_GATED_APPS` гейтится и без записи в `E2E_GATED_APPS` (`gated = hard || e2e`): инвариант
  «HARD ⊂ E2E» закреплён тестом в `libs/infra-config/src/index.spec.ts`, но правка файла не должна
  превращать нарушение инварианта в отключённую блокировку.
...
- Описание инструмента `deploy_app` больше не интерполирует `HARD_GATED_APPS`: описание регистрируется один
...
Тест — `server.tools.spec.ts` («BUILD_ON_S1_APPS перечитывается при каждом вызове» и «HARD_GATED_APPS и
E2E_GATED_APPS перечитываются при каждом вызове»): сервер создаётся один раз, файл правится между вызовами
