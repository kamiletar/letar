...
## 8. ⚠️ `seedFullHouse`/`seedHouseOptionExtras` пишут только в `HouseVersion.version === 1`

**Симптом:** правка `optionGroups`/`effects` в `FULL_HOUSES` (`houses.ts`) для дома, у которого в
БД уже есть версия новее `version: 1`, не появляется в конфигураторе после `nx db:seed` — при
этом сид отрабатывает без единой ошибки и печатает «Готово!». Классическая ловушка, которая
выглядит как успех.

**Причина:** обе функции ищут `HouseVersion` жёстко по `where: { houseId, version: 1 }`
(`seedFullHouse` — houses.ts:816-818, `seedHouseOptionExtras` — houses.ts:1025-1027), а не по
`isCurrent: true`. Если для дома вручную заведена новая версия через админку (тот же паттерн, что
у витринной карточки «Ивовая» — см. комментарий в шапке `seed.ts`), `version: 1` остаётся в БД, но
перестаёт быть текущей (`isCurrent: false`). Пользователи конфигуратора видят `isCurrent: true`
версию, а сид продолжает писать в неактуальную v1 — `HouseOption`/`HouseOptionEffect` создаются,
но не там, где их видно.

**Найдено:** `domwellbes`, `dom-sosnovyy` в dev-БД — рядом с `version: 1` от сида вручную заведена
`version: 2` (id `cmtsab9ox0000e0mxeyck40u4`, `isCurrent: true`). Правка опции «Тёплый пол на
первом этаже» (сессия 2026-09-23, коммит submodule `97e35208`) обошла ловушку одноразовым tsx-
скриптом, писавшим напрямую в v2, — `nx db:seed` эту версию не увидел бы вовсе.

**Не починено системно** — риск задеть все 7+ домов сида без явного запроса на этот рефакторинг не
оправдан для точечной правки одной опции. Возможные направления на будущее (в `PLAN.md`
domwellbes, не сделаны):

- искать `HouseVersion` по `isCurrent: true` вместо `version: 1` (осторожно: `seedFullHouse`
  создаёт версию, если её нет вовсе — эта ветка не должна измениться для домов без ручных правок);
- либо минимально: если у дома есть `HouseVersion` с `version !== 1`, печатать предупреждение в
  консоль вместо молчаливой записи в неактуальную v1 — дешевле, не меняет поведение по умолчанию.
