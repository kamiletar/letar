...
вернуть баг. Замену на отдельный `prismaAuth` (по образцу остальных пяти) можно сделать
проактивно ради единообразия, но это не срочный фикс — только если менять `db.ts`/`prisma.ts`
auth-hub по другому поводу, стоит заодно завести `createLazyPrismaAuthClient` из `@letar/auth` и
повторно прогнать тот же живой тест до и после.
