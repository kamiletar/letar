...
### Ещё два ручных паттерна, найденных при роллауте на `animatrona`/`driving-school`

- **`icon: React.ElementType` в пропах компонента + `color=` строкой-токеном.** Частый паттерн
  для карточек статистики (`StatsCard`/`ReputationCard`/`BonusPointsCard` в animatrona,
  `TRANSACTION_CONFIG`-подобные объекты) — компонент принимает `{ icon, color }` и рендерит
  `<Icon as={icon} color={color} boxSize={4} />`. Фикс: деструктурировать проп сразу в
  capitalized-имя (`{ icon: IconComponent }`), а `color` (строка вида `"green.400"`) перевести в
  CSS-переменную тем же способом, что и статический токен: `` `var(--chakra-colors-${color.replaceAll('.', '-')})` ``.
- **`_dark={{ color: ... }}` на самой иконке.** react-icons — обычный SVG-компонент, не понимает
  Chakra-псевдопропы (`_dark`, `_hover` и т.п.) в принципе — код после наивной замены
  компилируется (TS не ловит несуществующий React-проп как ошибку, `_dark` просто осядет в DOM
  как invalid attribute) но не работает: тёмная тема не переключает цвет иконки. Правильный
  перенос — **не** на иконку, а на её обычно уже существующий Chakra-родитель (`<Box>`), которому
  назначить `color`/`_dark={{ color: ... }}` как обычно, а иконке — `color="currentColor"`, чтобы
  она унаследовала цвет от родителя (см. `today-section.tsx` в `driving-school`, три карточки с
  паттерном «цветной квадрат-подложка + иконка внутри»). Кодмод такие узлы не трогает
  (`_dark` не входит в список безопасных пропов) — это осознанно ручной случай.
