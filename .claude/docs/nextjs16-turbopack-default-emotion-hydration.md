...
> 🔴 **Разбор ниже верен по фактам, но его итоговый вывод («дев-режимный шум, фикс не нужен») —
> ОШИБОЧЕН.** Причина найдена 2026-08-28, баг воспроизведён на прод-сборке, фикс сделан.
> Актуальный разбор —
> [chakra-css-memo-prop-order-hydration](/.claude/docs/chakra-css-memo-prop-order-hydration.md).
> Текст ниже оставлен как история проверки: он показывает, **почему** правильно поставленный
> прод-эксперимент дал ложноотрицательный результат (см. поправку в конце раздела).
...
### ❌ Статус (2026-08-12), опровергнут 2026-08-28: «не воспроизводится на prod-сборке»
...
### 🔴 Поправка 2026-08-28: вывод выше неверен, причина найдена

Те же самые классы (`css-ap60qa` / `css-1rb706t`, та же пара `fontSize="sm"` + `color="fg.muted"`)
всплыли снова. Разбор:
[chakra-css-memo-prop-order-hydration](/.claude/docs/chakra-css-memo-prop-order-hydration.md).

Причина — не бандлер и не dev-режим, а **баг в самом `@chakra-ui/react`** (регрессия 3.29.0):
LRU-кеш `css()` считает ключ по **отсортированным** ключам объекта (`Object.keys(value).sort()` в
`utils/memo.js`), тогда как результат от порядка зависит. Кто первым вызвал `css()` с этим
набором свойств — тот и задал порядок объявлений для всего процесса, а значит и имя класса
emotion. Серверный процесс живёт часами и обслуживает все маршруты, клиентский кеш свежий на
каждую загрузку страницы — отсюда расхождение.

**Почему прод-проверка 2026-08-12 дала «чисто»:** результат зависит от того, какой вызов попал в
кеш серверного процесса первым. Холодный `next start`, которому сразу открывают проверяемую
страницу, кеширует ровно тот порядок, который потом посчитает и клиент, — мисматча нет. Это
свойство эксперимента, а не свойство прод-сборки. Воспроизведение на проде 2026-08-28: тому же
прод-серверу сначала отдали маршрут с обратным порядком пропов, затем проверяемый — мисматч
появился, идентичный дев-режимному.

Отсюда общее правило для проверок этого класса: **«на холодном прод-сервере не воспроизвелось»
не доказывает отсутствие бага**, если подозреваемое состояние живёт в процессе, а не в бандле.
Прогревать процесс надо так, как это делает реальный трафик, — несколькими разными маршрутами
до проверяемого. Смежная запись —
...
3. **Реальный, воспроизведённый мисматч** на этой же странице/сервере — тот же паттерн, что в
   ...
   🔴 **Поправка 2026-08-28:** механизм тот же, но вердикт про «дев-режимный шум» неверен —
   причина в кеше `css()` самого Chakra, баг живёт и на проде, см.
   [chakra-css-memo-prop-order-hydration](/.claude/docs/chakra-css-memo-prop-order-hydration.md).
   Совпадение с `Fast Refresh` объясняется тем, что пересборка меняет порядок первых вызовов
   `css()` в процессе, а не тем, что дефект существует только в dev.
