...
> ⚠️ **С 2026-09-19 «s3» ниже — это старый сервер `188.127.235.141`, он отключён.** Сборочную роль
> принял новый s1 (`185.56.162.213`), хранилище — новый s3 (`185.130.251.234`). Доступность
> `dl-cdn.alpinelinux.org` с новых серверов не проверялась — зеркало в Dockerfile оставлено как
> есть и работает везде. Проверять `dl-cdn` с s1 стоит, только если решишь снять зеркало.

**Кратко:** с s3 (`188.127.235.141`, старый) нет сетевого пути до `dl-cdn.alpinelinux.org` — ни по IPv4
