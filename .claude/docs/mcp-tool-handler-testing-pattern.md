...
## ⚠️ Лишний ключ в аргументах молча отбрасывается — если схема не строгая

Тест `expectValidationError` на «отсутствует обязательное поле» строгость схемы **не проверяет**:
`z.object` отвергает пропуск, но проглатывает лишний ключ. Для инструментов с побочным эффектом или
фильтрами объявляй `z.strictObject` и добавляй пару тестов — валидный вызов проходит, тот же вызов
с `unknownArg` даёт `expectValidationError` и **не доходит до мока внешнего вызова**
(`expect(mock).not.toHaveBeenCalled()`). Без второй проверки тест пропустит регресс, при котором
ошибка есть, а запрос всё равно ушёл. Какие инструменты строгие и почему —
[mcp-servers § строгие входные схемы](/.claude/docs/mcp-servers.md#strict-input-schemas).
