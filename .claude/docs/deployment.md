...
> ⚠️ **Раздел ниже описывает однохостовую модель (ручной SSH + `deploy-affected.sh` на самом
> сервере назначения).** Она остаётся верной как есть только для приложений **вне**
> `BUILD_ON_S1_APPS` ([libs/infra-config/src/index.ts](/libs/infra-config/src/index.ts)). Для
> приложений из этого списка (с 2026-09-22 — большинство: все с собственным
> `Dockerfile.production`) прод-деплой идёт через `deploy_app({ app, target: "production" })` —
> сборка на s1, релиз на s2, см. [«Сборка на s1, релиз на s2»](#сборка-на-s1-релиз-на-s2-plan-infra-6md-157)
> ниже. Правило «не запускать локально» и весь троттлшутинг из этого раздела актуальны в обеих
> моделях одинаково — расходится только то, **на каком сервере** реально исполняется
> `deploy-affected.sh`.
...
**Гейт fail-closed для всех приложений из списка** (`E2E_GATED_APPS` в
`libs/infra-config/src/index.ts` — актуальный состав смотри там, здесь не дублируется) —
**ОТКАЗЫВАЕТ**, если данных нет / прогон упал / коммит другой / старше 24ч (включая ошибку самой
проверки статуса — «не могу проверить» тоже блокирует, не пропускает). `deploy_app` в этом случае
возвращает `isError` до вызова `/api/deploy/app` — деплой на прод не запускается вообще, ни на шаг.

⚠️ До 2026-09-22 существовало два режима одновременно (2026-07-28, инцидент archetest v0.25.5):
warn-only (только предупреждал) для всех gated-приложений вне отдельного списка `HARD_GATED_APPS`
(fail-closed, строгое подмножество). Владелец их схлопнул в один список, всегда fail-closed: «мы
же договорились, что у нас теперь всё HARD_GATED_APPS без исключений» — держать warn-only для
приложений, которые всё равно не должны там годами оставаться, было только риском рассинхрона.
...
задача deploy-agent-dev/владельца, deploy-mcp и dashboard-agent её не автоматизируют.
...
deploy-agent-dev при первичной настройке пайплайна на новом приложении, кандидат на автоматизацию
...
**Fail-closed гейт реализован 2026-07-28** (см. выше). Состав списка с тех пор рос (`studio` —
2026-08-06, `auth-hub` — 2026-08-28, всё оставшееся — 2026-09-22 разом); актуальный смотри в
`libs/infra-config/src/index.ts` (`E2E_GATED_APPS`).
...
> всех контейнеров) выполняется отдельным заходом через deploy-agent-dev — правки в коде (docker-compose,
...
### Текущие хосты (актуально на 2026-06-20 — устарело, см. предупреждение)

⚠️ **Таблицы ниже не перепроверялись с 2026-06-20 и минимум частично неверны (найдено 2026-08-12,
не исправлено — нужна живая сверка, не косметическая правка):**

- **`s1.letar.best` выведен из эксплуатации 2026-06-20** — тем же днём, которым помечена
  «актуальность» этой таблицы. Раздел `#### s1.letar.best` ниже описывает сервер, которого
  больше нет — оставлен как исторический снимок, не как рабочая инструкция.
- Таблица s2 содержит `premium-rosstil-app`/`imot-app` — оба приложения удалены из монорепо
  2026-07-05 (`project_premium_rosstil_imot_removed`), порты 3000/3001 либо освобождены, либо
  переиспользованы — таблица не говорит, что из двух.
- Таблица не содержит приложений, заведённых после 2026-06-20 (как минимум `domwellbes`,
  `dsperevod`, `studio`, `synth`, `aprel8008`, `aboi`, `poster-microtext-desktop`,
  `form-develop-app` — по данным `apps/*` на 2026-08-12), то есть заведомо неполна независимо от
  вопроса актуальности старых строк.

Источник истины на сегодня — сама конфигурация NPM (`http://<host>:81`, креды в KeePassXC), не эта
таблица. Живая пересверка (снять реальный список Proxy Hosts с обоих активных NPM-инстансов —
s2 и mail — и сравнить построчно) в этой сессии не делалась: это отдельная по объёму задача, не
косметическая правка чисел.

#### s1.letar.best (исторический снимок, сервер выведен из эксплуатации 2026-06-20)
