...
## E2E-ранер и деплой — разделены

⚠️ **E2E-тесты сегодня НЕ блокируют и НЕ гейтят деплой.** Это два независимых процесса:

| Процесс        | Где                                      | Триггер                                                   | Влияет на деплой? |
| -------------- | ---------------------------------------- | --------------------------------------------------------- | ----------------- |
| **Деплой**     | s2 (или свежий сервер приложения)        | `deploy-affected.sh`, только вручную/через BlackCove      | —                 |
| **E2E-прогон** | выделенный сервер s3 (`188.127.235.141`) | cron `0 2 * * *` (ежедневно ночью) + ручной запуск по SSH | **нет**           |

`deploy-affected.sh` не читает статус e2e и не содержит проверки — упавший ночной прогон на s3 не
остановит и не пометит ближайший деплой. Уведомление о падении уходит только в Telegram, читать его
и решать нужно вручную.

**Если e2e упал ночью** — это сигнал разобраться до следующего деплоя вручную, автоматической защиты нет.
...
**Планируется, но не реализовано:** pre-deploy gate (`check_e2e_gate()` в `deploy-affected.sh`,
блокировка деплоя при красном e2e) — см. `PLAN.md` §15.3.1. Решено делать в два этапа: сначала лёгкий
gate поверх уже работающего ночного прогона (без новой инфры), потом отдельным решением — прод-снепшот
с анонимизацией. Пока оба этапа — план с открытым DoD, а не работающий код — не полагайся на это как на
существующую защиту.
