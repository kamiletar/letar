...
## Второй случай: подпуть, чей путь на диске не совпадает с ключом `exports` (2026-09-21)

Алиас на весь пакет (`'@letar/forms' → libs/forms/src`) молча «переводит» каждый подпуть в
одноимённый каталог. У `@letar/forms/server-errors` файл физически лежит в `src/server/server-errors/`
(граница `no-restricted-imports` держит серверный код отдельно), поэтому запрос уходил в
несуществующий `src/server-errors` и vitest падал с «Failed to resolve import». `typecheck:tsgo`
и `lint` при этом зелёные — они читают tsconfig `paths`, а не алиасы vitest, так что ловится
только запуском тестов. Лечение то же: отдельная запись на подпуть **перед** общей, с прямым путём
к `index.ts`. Проверять при первом импорте подпути из нового места, а не «на всякий случай».
