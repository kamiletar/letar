...
Повторно всплыло третий раз 2026-09-16 (`domwellbes`) — 2 файла в исходном отчёте
(`src/app/houses/[slug]/page.tsx:156`, `src/app/materials/item/[sku]/page.tsx:350`), но починка
вскрыла ещё 3 маскированных ошибки в том же `houses/[slug]/page.tsx` (`house.images.filter` ×2,
`extras.map`) — снова тот же эффект «следующая ошибка не видна, пока не убрана предыдущая» из
записи 2026-09-15 выше. На всех пяти местах лёгкий вариант `(typeof x)[number]` на параметре
callback не сработал ни разу — сразу понадобился ручной узкий тип на промежуточной переменной.
