...
Если полей с этой проблемой у одной модели несколько и точек чтения тоже несколько — инлайновый
`Number(...)` на каждую точку легко расползается почти идентичным блоком по всему приложению
(так было с `FinancingProgram` — 6 мест). Вынести один раз в общий helper, конвертирующий только
те поля, что реально присутствуют в объекте (частичный `select` может брать не все) — образец:
`apps/domwellbes/src/lib/financing/to-number-financing-program.ts`.
