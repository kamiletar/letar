...
## Динамический сегмент верхнего уровня ловит вообще любой опечатанный URL

Найдено в grandslamcup (2026-08-28, сквозной аудит монорепо на этот паттерн): если под
Suspense-границей лежит **динамический сегмент верхнего уровня** (`(public)/[citySlug]/page.tsx`,
вызывающий `notFound()` для неизвестного значения), soft-404 перестаёт быть проблемой только
«глубоких» страниц с явным `notFound()`. Next матчит **любой** несуществующий путь верхнего
уровня в этот динамический сегмент раньше, чем дошёл бы до настоящего 404 — то есть у такого
приложения вообще не остаётся путей с честным 404 на верхнем уровне: замер `/nope-nope-nope-route`
(случайный несуществующий URL, не только «похожий на маршрут») тоже дал 200 с `noindex`. Это
не отдельный баг — тот же механизм, просто наложенный на структуру роутинга, где перехватывающий
всё динамический сегмент не редкость, а обычный паттерн («страница города/тенанта/организации по
слагу в корне»).
...
### Реализованный пример — grandslamcup (2026-08-28)

Владелец выбрал вариант с `proxy.ts` именно для этого приложения (динамический сегмент верхнего
уровня `[citySlug]` ловил вообще любой опечатанный URL сайта — цена бездействия была выше, чем
у трёх других приложений с тем же паттерном, которые приняли soft-404 как есть). Рабочая пара
файлов:

- `src/proxy.ts` — для первого сегмента пути, не входящего в список известных top-level роутов
  приложения (`RESERVED_SEGMENTS`), проверяет существование сущности по in-memory кэшу с TTL
  (обновляется из БД раз в несколько минут, не на каждый запрос). Неизвестный слаг →
  `NextResponse.rewrite(new URL('/system-404', request.url))`.
- `src/app/system-404/page.tsx` — top-level роут-цель rewrite, **намеренно вне** сегмента с
  `loading.tsx` и без собственной Suspense-границы. `notFound()`, вызванный здесь, отдаёт
  настоящий HTTP 404 — ровно потому, что над ним нет ни одной Suspense-границы, гасящей статус
  (см. причину бага выше).

Ключевая идея переносима на любое приложение с тем же паттерном: rewrite на «голый» роут без
`loading.tsx`-предков — дешёвый способ получить настоящий 404 через `proxy.ts`, не трогая
существующее дерево `(public)` и не жертвуя стриминговым скелетоном на нём.
