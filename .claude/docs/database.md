...
**Диагностика прод-проблем с БД:** помни, что `letar-db` без явного имени `<app>-prod` смотрит на
**dev-базу** приложения (`db: "<app>"`), не на прод. Вывод «колонки на месте» с dev-базы ничего не
говорит о проде — при том инциденте именно это увело первую диагностику в ложное «drift
безобиден». Для проверки прод-состояния используй только `db: "<app>-prod"`.
...
`letar-db` (`sql({ db, sql: "EXPLAIN ..." })`) — точечно, не как постоянный мониторинг.
...
диагностике через `letar-db`, когда под рукой нет полного checkout приложения).
...
## ✅ postgres-* MCP искажал вывод timestamp без зоны — закрыто 2026-09-14

**Было:** запрос `SELECT column FROM table` к колонке типа `timestamp without time zone` через
старые `postgres-*` MCP (`@modelcontextprotocol/server-postgres`/Postgres MCP Pro) возвращал
значение с суффиксом `Z` (как будто это UTC), хотя физически в БД лежит "наивное" время без
указания зоны — сам инструмент пропускал значение через свою сессионную `TimeZone` и дописывал
смещение на лету, только в момент показа. Данные в БД не были тронуты.

**Закрыто** заменой этих серверов на `letar-db` (.claude/mcp/letar-db.ts) — там
`pg.types.setTypeParser` для `timestamp`/`timestamptz`/`date` (oid 1114/1184/1082) отдаёт
значение сырым текстом без какой-либо реинтерпретации. `SELECT column::text` больше не должен
расходиться с обычным выводом `sql` — если расхождение всё же наблюдается, это уже не старый
баг, разбираться заново.
