...
Найдено разом шесть workspace: `apps/aboi` (0.83.17 против 0.83.20), `apps/svoichuzhie`
(0.10.60 против 0.10.68), `apps/poster-microtext-desktop`, `apps/dashboard-agent`,
...
2. **Бамп внутри приватного submodule.** `bun.lock` лежит в корне letar, submodule — отдельный
   ...
   Так разошлись `aboi`, `svoichuzhie`, `poster-microtext-desktop`.
...
- **pre-commit** (`pre-commit-deps-integrity.sh`): коммит, содержащий сам `bun.lock`, при
  ...
  **предупреждение**: lock и `package.json` едут разными коммитами (scope-guard режет `bun.lock`
  - `apps/<x>` как multi-scope), и в момент бампа расхождение неизбежно — но хук напоминает про
