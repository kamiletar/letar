...
**Остальные ~14 приложений по-прежнему уязвимы** — у них дефолтный префикс, и любой из них
падает в 500, если рядом на `localhost` поработали с dashboard. Альтернатива, закрывающая всех
разом одной строкой, — убрать `strategy: 'jwt'` из `apps/dashboard/src/lib/auth.ts` (источник
точек), но она оставляет ловушку латентной: следующий, кто добавит `jwt`/`jwe`, вернёт всё
обратно, а перетирание общего `session_token` между приложениями останется в любом случае.
