...
comment-директива. Ставится прямо на поле, без doc-комментария:

```zmodel
title String @meta("form.title", "Название")
```

⚠️ **Legacy-синтаксис** (`///`-комментарий **ДО** поля) продолжает работать — плагин читает
**оба**, `@meta` побеждает при конфликте на одном ключе метаданных. При обнаружении `@form.*`
`nx zenstack:generate` печатает deprecation-warning с готовой заменой, но **не ломает сборку**.
Новый код пиши на `@meta`; существующий переводит кодмод
`scripts/codemods/codemod-form-directives.mjs` (см. `libs/forms/PLAN.md`, Фаза 3), не переписывай
вручную.
...
### Legacy-синтаксис (deprecated, но рабочий) — для справки при чтении старого кода
...
⚠️ ZenStack связывает doc-комментарии `///` со **следующим** за ними элементом — та же ловушка,
что и раньше:

```zmodel
// ❌ Неправильно — привяжется к следующему полю!
title String
/// @form.title("Название")
```

Это ограничение относится только к legacy-синтаксису — `@meta` как field-атрибут стоит прямо на
строке поля, порядок/привязка неоднозначности не создают.
