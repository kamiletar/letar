...
- Deploy Agent сразу отвечает в тот же `thread_id` коротким подтверждением приёма
  (`🔄 Принято в работу: ...`) — это не финальный статус, а сигнал «запрос увиден, не потерян».
  Молчание сразу после отправки, до этого первого ответа, — норма (секунды), не повод считать
  Deploy Agent недоступным.
- Финальный результат (успех/ошибка) придёт отдельным `reply_message` в тот же тред, когда
  деплой действительно завершится — это может занять минуты (сборка, миграции, health-check).
...
- `target: "staging"` резолвится на s1 (образ `<app>:staging`; до 2026-09-19 — на старом s3).
