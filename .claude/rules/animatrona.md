...
- См. [electron-monorepo-shared-releases.md](/.claude/docs/electron-monorepo-shared-releases.md) — релизы публикуются напрямую в `kamiletar/letar` (тег `animatrona-v<semver>`, workflow `.github/workflows/release-animatrona.yml`), не в отдельный `kamiletar/animatrona` (устаревшая схема, снята 2026-09-13); рантайм-поиск своего релиза — `@letar/electron-monorepo-updater` в `main/updater.ts`
- См. [nvenc-web-video-codec-ladder.md](/.claude/docs/nvenc-web-video-codec-ladder.md) — ловушки `ffmpeg` + NVENC: `-tf_level` только 0/4 и ≥ 4 B-кадров, предел `-rc-lookahead` (~51), `-highbitdepth 1` вместо `-pix_fmt` на CUDA-пути; единый набор NVENC-аргументов для VideoPool и VMAF-сэмплов — `main/ffmpeg/nvenc-args.ts`
