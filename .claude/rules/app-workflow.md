...
⚠️ Перед регистрацией под `<app>-dev` проверь `whois`: если имя держит живая сессия (свежий
`last_active_ts`), заводи временную identity — иначе две сессии молча станут одним агентом и их
резервации перестанут конфликтовать
([agent-mail.md § «Вторая сессия того же приложения»](/.claude/rules/agent-mail.md)).
