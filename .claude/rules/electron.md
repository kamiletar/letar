...
## Паттерны: `shared/` для main+renderer, единый объект настроек

Два повторяющихся паттерна в Electron-приложениях монорепо — рантайм-код (не только типы) в
`shared/`, доступный обоим процессам без настройки сборки, и единый объект настроек вместо
россыпи `useState` (пропущенный элемент зависимостей — частый источник бага «настройка
сохраняется, но не участвует в пересчёте»). Оба с кодом и разбором — в
[electron-shared-runtime-and-settings-patterns](/.claude/docs/electron-shared-runtime-and-settings-patterns.md).
