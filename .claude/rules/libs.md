...
| @letar/forms-query        | Формы — интеграция с TanStack Query (npm, тег `forms-query-v*`): `fromSearchQuery`, `useQueryOptions`, `useLoaderQuery`, `useInvalidateAfter`; подпуть `/zenstack`. Скины не импортирует                                                                                                                          |
...
| @letar/undo-toast         | Заповедь студии №20 — `triggerUndoableAction` (немедленный commit + реальная отмена) и `triggerDeferredUndoableAction` (отложенный commit + pagehide-safety), `./client` — одноимённые хуки. Тонкая обёртка над `createAppToaster()` из `@letar/ui`. Первая интеграция — 11 удалений в `domwellbes` (2026-09-23). |
