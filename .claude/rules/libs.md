...
⚠️ **2026-09-14: область сканирования расширена внутрь `libs/`.** До этой даты сторож видел
только `apps/*/tsconfig.json` — собственные потребители библиотек друг у друга оставались
невидимы проверке. Расширение сразу вскрыло реальный пример того же класса бага, что описан
выше (Фаза 7.3, 2026-08-09), но теперь между библиотеками: `libs/forms/tsconfig.spec.json`
держал `paths` только на 9 подпутей `@letar/forms-core/*` из 18 (найдено и починено вручную
2026-09-14, коммит 41f2eb537), а следом скрипт нашёл ещё 7 похожих потребителей внутри `libs/`
(`forms`, `forms-react`, `forms-shadcn`, `folder-player-react` → `@letar/folder-scan`). Долг
разгребён тем же днём — недостающие строки `paths` добавлены во все 7 файлов, `typecheck:tsgo`
зелёный на всех четырёх затронутых библиотеках (`folder-player-react`, `forms-react`,
`forms-shadcn`, `forms`), прогон снова чистый, `severity` вернулась в `gate`.
