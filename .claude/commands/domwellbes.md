...
Если задача достаточно крупная, чтобы иметь смысл отдельным пунктом в «Этапах» проекта (не
разовая правка) — передавай её в параметр `stage` у `time_start`/`time_switch`. Studio сама
заведёт/переиспользует `ProjectStage` с этим названием и привяжет к нему запись времени — карточка
