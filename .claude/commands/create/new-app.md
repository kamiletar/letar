...
Для Electron есть отдельный скилл — **`/create:new-electron-app`**: генератор, выбор `app://` против
`file://`, ассоциации файлов, single instance lock, грабли платформы и способы проверки без GUI.
Раздел «Приватные приложения» ниже (submodule) всё равно применим — сначала сгенерируй приложение,
потом заведи submodule.
