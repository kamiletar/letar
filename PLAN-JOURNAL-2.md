...
## §82 — скилл `transcribe-call`: локальная расшифровка звонков вынесена из `.claude/artifacts/` (2026-09-11)

Рабочий скрипт faster-whisper (large-v3, RTX 5080, см. `reference_local_audio_transcription_whisper.md`
в памяти) жил в `.claude/artifacts/transcribe-whisper.py` — каталог в `.gitignore`, скрипт нигде не
версионирован и не переживал перенос на другую машину. Перенесён в
`.claude/skills/transcribe-call/` (`SKILL.md` + `transcribe.py`) — скиллы видны любой сессии этого
репозитория автоматически, что важно для сценария «Kami скинул путь к аудио» без явного упоминания
инструмента.

Три отличия от старого скрипта: (1) словарь `HOTWORDS` (был зашит под domwellbes — строительные
термины) вынесен в параметр `--hotwords`/`--hotwords-file`, в коде скилла клиентских терминов нет
(`public-repo-hygiene.md`); (2) докрутка после падения процесса (exit code 9 без трассировки, известная
особенность CTranslate2 на этой машине) автоматизирована — `transcribe.py` стал супервизором:
запускает распознавание подпроцессом, при падении сам находит последний записанный таймкод и
перезапускает с этого места, в конце склеивает части; раньше это делалось вручную через `start_sec`;
(3) известные галлюцинации Whisper («Продолжение следует…», «Субтитры делал…») помечаются префиксом
`[ВОЗМОЖНАЯ ГАЛЛЮЦИНАЦИЯ WHISPER, ПРОВЕРИТЬ]`, не удаляются молча.
