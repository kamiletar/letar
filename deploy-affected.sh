...
if [ -f "bun.lock" ]; then
    ...
    if [ "$CLEAN_INSTALL" = true ]; then
      ...
      if ! run_bun_install --frozen-lockfile; then
        ...
        # печатаем РЕАЛЬНОЕ отличие, а не общую жёлтую строку. Решение владельца
        # (2026-08-12): непустой diff ОСТАНАВЛИВАЕТ деплой — молчаливая установка не тех
        # версий хуже, чем незадеплоенное приложение. lockfile правится локально
        # (bun install без --frozen-lockfile), коммитится и приезжает готовым — деплой
        # не место для решения, что именно ставить.
        if ! git diff --quiet -- bun.lock 2>/dev/null; then
          echo -e "${RED}❌ bun.lock изменился после фолбэка — версии пакетов разошлись с закоммиченными (см. PLAN-INFRA.md §50):${NC}"
          git diff --stat -- bun.lock
          echo -e "${RED}❌ Деплой остановлен: обнови bun.lock локально (bun install), закоммить и запушь.${NC}"
          exit 1
        else
