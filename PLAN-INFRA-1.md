...
## §16 — Конвенция: фото-галереи через `PhotoGallery` из `@letar/ui` ✅ ЗАКРЫТО (конвенция принята, сессия №42)
...
## §18.7 — Тираж E2E-гейта на все приложения ✅ ЗАКРЫТО (2026-08-25)
...
- `HARD_GATED_APPS` (`archetest`, `dsperevod`, `svoichuzhie`, `aboi`, `aprel8008`, `studio` —
  добавлен 2026-08-06, `auth-hub` — 2026-08-28) в `libs/infra-config/src/index.ts`.
  `auth-hub` выпадает из логики «коммерческое приложение»: это Ключница, SSO для всех остальных —
  сломанный деплой закрывает вход везде сразу, поэтому цена ошибки выше, чем у любого приложения
  из списка, хотя выручки за ним нет. К моменту включения был в `E2E_GATED_APPS` (warn-only) с
  18 тестами в четырёх spec-файлах и зелёным последним прогоном.
  У `studio` staging-инфраструктуры не
...
**✓ DoD этого трека:** код — ✅ готово (см. выше). **Живая цепочка для archetest подтверждена
2026-08-24** — первый прогон `deploy_app(archetest, staging)` → `run_e2e`: deployId
`0a70d778-ae99-44cd-ac29-4ff473e4b14b` (healthy за 9с), e2e `passed:true`, runId
...
**✅ M4, вторая половина (`driving-school`) — закрыто 2026-08-25. §18.7 M4 и весь тираж M
закрыты целиком.** Деплой ✅. Первый прогон полного набора (17 projects) упёрся в 15-минутный
таймаут e2e-раннера на s3 без единого результата — оказалось, полный набор в принципе не
рассчитан на staging (ролевые projects логинятся через `storageState`, записываемый напрямую в
БД, недоступную staging-раннеру). Правильная цель — уже существовавший `staging-smoke` project
(`src/staging/*.spec.ts`, dev-session), но он находил 0 тестов: `testMatch`/`testIgnore` с
якорем `^staging\/` матчились Playwright против **абсолютного** пути файла (так и задокументировано
поведение `RegExp` в конфиге), якорь никогда не совпадал — и заодно `staging/00-public-smoke.spec.ts`
случайно протекал в `shard-core` по совпадению хвоста `00-`. Фикс — regex → glob-строки
(`'staging/**'`), Playwright резолвит их относительно `testDir` корректно (submodule-коммит
`fa18aa3`, bump SHA в letar `5bc80697`). Повторный прогон — `expected: 3, skipped: 1
(SOCKET_BASE_URL не настроен на раннере, ожидаемо), unexpected: 0`. `driving-school` добавлен в
`E2E_GATED_APPS`. Детали — `apps/driving-school/PLAN.md`.

**Побочная находка обоих деплоев M4:** непушнутые коммиты в приватных submodule (`aboi`,
`domwellbes`, `dsperevod`, `studio`, `svoichuzhie`) дважды блокировали `git submodule update` на
s3 (`not our ref`) — deploy-agent-dev с одобрения пользователя запушил только существующие
локальные коммиты (без force, без новых коммитов) в оба раза, не трогая грязные незакоммиченные
файлы чужих сессий. Не связано с M4-фиксами по существу, но стоит иметь в виду при следующих
staging-деплоях этих пяти приложений — если непушнутые коммиты появляются регулярно, возможно,
стоит завести привычку пушить сразу после коммита в submodule, а не откладывать.
...
**✅ `mandala` (M1) — закрыт 2026-09-01.** Root cause найден и починен (три независимые причины:
`Account.accountId` в seed, ненадёжный сид согласия баннеров, гонка гидратации при клике по
SSR-ссылкам) — разбор в `apps/mandala/PLAN_COMPLETED.md` § Раунд 7—8. Полный прогон 123/123,
добавлен в `E2E_GATED_APPS`.

**⏸️ `pravda` (M1) — намеренно НЕ закрыт, решение владельца (2026-09-01).** RSC-навигация
(Firefox/WebKit) блокирована неисправленным багом апстрима Next.js
([vercel/next.js#85374](https://github.com/vercel/next.js/issues/85374)), Command Palette Escape
(webkit) — нестабильный флейк без репро, кластер нестабильности scroll-position (8 тестов) не
диагностирован. Владелец решил: pravda не критичное приложение, тратить время на эти баги не
нужно. Два детерминированных кластера помечены `test.skip` с обоснованием, скролл-кластер
оставлен как есть. `pravda` **не добавлена** в `E2E_GATED_APPS` — закрытый вопрос, не открытый
TODO. Детали — `apps/pravda/PLAN.md`.

---

## §18.8 — `.env.staging` не шифруется и не трекается: завести `.env.staging.enc` по образцу `.env.docker.enc` ✅ ЗАКРЫТО (2026-08-25)
...
- [x] **Проверено живым прогоном (2026-08-06):** первый реальный `deploy_app(domwellbes, staging)`
      после заведения `.env.staging.enc` прошёл штатно — `decrypt_sops_env()` действительно
      резолвит `.env.staging.enc` без изменений кода, как и предполагалось по чтению исходника.
...
- [x] **Проверено сверкой по журналу, без повторных деплоев (2026-08-25):** все 11 приложений
      этой группы уже имели подтверждённый живой `deploy_app(staging)` на момент сверки —
      `grandslamcup`/`time` (Сессия D/C, 2026-07-11, до появления самого §18.8), `archetest`
      (§18.7 M3, 2026-08-24), `auth-hub`/`driving-school` (§18.7 M4, 2026-08-25), остальные шесть
      (`aboi`, `aprel8008`, `dsperevod`, `mandala`, `pravda`, `svoichuzhie`) — тираж §18.7 M1/M2,
      2026-08-12. Расшифровка `.env.staging.enc` — обязательный шаг любого `deploy_app(staging)`:
      провал `decrypt_sops_env()` уронил бы весь деплой, а не только e2e, поэтому зелёный деплой в
      каждом из этих случаев сам по себе и есть подтверждение. Отдельных повторных деплоев не
      потребовалось — §18.7 и §18.8 закрывают одно и то же требование с двух сторон.

**✅ Оба открытых пункта DoD закрыты сверкой с журналом §18.7, без повторных деплоев (2026-08-25).
§18.8 закрыт целиком.**

---

## §18.8.1 — Секреты инфра-сервисов вне конвейера `.enc` вообще ✅ ЗАКРЫТО (2026-08-06, скрипт+конвейер — 2026-08-12)
...
### §19.1 — Гейт проверки типов в деплое 🟡 ЧАСТИЧНО (Трек 1 реализован 2026-09-06)
...
**⚠️ Блокер снят (2026-09-06).** Приоритет ниже §18.7 Тираж M был установлен 2026-07-18 —
Тираж M закрыт 2026-08-25 (см. §18.7 в этом файле), но задача провисела нетронутой ещё почти две
недели, пока не была подхвачена при живой проверке статуса секций монорепо.

### Что сделано (Трек 1, 2026-09-06)

- [x] `deploy-affected.sh` — сразу после сборки приложения (перед проверкой build output) добавлен
      безусловный блокирующий `nx typecheck:tsgo $app` (fallback на `nx typecheck $app`, если
      `typecheck:tsgo` не объявлен в `project.json` — на 2026-09-06 единственное такое исключение
      среди деплоящихся приложений — `dashboard-agent`). Провал → `FAILED_APPS`, деплой этого
      приложения прерывается, остальные продолжаются (тот же паттерн, что у провала сборки).
      Работает одинаково для `--staging` и прода, для Node.js-приложений (сборка внутри Docker) и
      Next.js — сам typecheck всегда идёт на хосте через `nx`, к Docker-сборке не привязан.
- [x] **Live-прогон подтверждён (2026-09-06, `deploy_app(time, staging)`, коммит `41b52a5d`).**
      Лог: `🔨 Building time... ✅ Build completed → 🔎 Typecheck (typecheck:tsgo) for time...
      ✅ Typecheck passed for time` (29.3с) — шаг встал ровно между сборкой и Docker-образом,
      не сломал остальной пайплайн (sourcemaps → GlitchTip, rollout, health-check). Деплой
      завершён `exitCode: 0`. Broadcast разослан всем `<app>-dev` — приложения с известными
      непройденными ошибками типов теперь будут блокироваться при следующем деплое.

### Что сделано (Трек 1b, 2026-09-06)

- [x] `deploy-affected.sh` — для production-деплоя (`$STAGING != true`) приложения из
      `HARD_GATED_APPS` (`@letar/infra-config`) пропускают повторный typecheck: гарантия —
      `evaluateE2eGate` (`libs/deploy-mcp`) уже отказывает в проде без свежего зелёного e2e на
      staging для текущего коммита (не affected с прогона, `isAffectedSince`), а сам
      staging-деплой того же коммита не мог пройти без успешного Трека 1 (typecheck там
      безусловен и блокирует раньше, чем приложение доходит до Docker-образа) — значит к моменту
      прод-деплоя typecheck для этого кода уже подтверждён. Список читается динамически через
      `bun -e "import('./libs/infra-config/src/index.ts')..."` (bun импортирует `.ts` без
      сборки, проверено живьём) — не дублирующий bash-список, который бы расходился с каноном.
      Безопасный дефолт при сбое резолва — **typecheck не пропускается** (`SKIP_TYPECHECK`
      остаётся `false`, если `bun -e` не вернул ровно `"true"`).
      ⚠️ Путь к `libs/infra-config` — **относительный** (`./libs/...`), не через `$WORKSPACE_ROOT`:
      абсолютный POSIX-style путь на Windows-машине (`/c/web/letar/...`) сбивал резолвер bun
      (`Cannot find module`) — на самом Linux-сервере абсолютный путь работал бы, но
      относительный проще и переносим, а `cd "$WORKSPACE_ROOT"` перед этим блоком гарантирован
      кодом (нет `cd` между строками ~1025 и ~1150). Live-прогон на реальном деплое hard-gated
      приложения ещё не выполнялся — следующий прод-деплой любого из семи (`archetest`,
      `dsperevod`, `svoichuzhie`, `aboi`, `aprel8008`, `studio`, `auth-hub`) станет проверкой.
- [ ] DoD §19.1 в целом — не закрыт, оба трека (1 и 1b) реализованы, живьём подтверждён пока
      только Трек 1.
- [ ] ⚠️ Открытый вопрос: живая проверка Трека 1b на `archetest` начата 2026-09-06 (проверен
      `e2e_status` — staging зелёный, 21/21, коммит `c494d4fa`), но не доведена до конца — сессия
      переключилась на планирование §157 раньше отправки deploy-запроса на прод-деплой archetest.
      К моменту следующей проверки коммит staging-прогона (`c494d4fa`) уже не совпадёт с
      `origin/main` (ушёл вперёд на `eb416a9c` и позже) — понадобится свежий e2e-прогон на
      staging для актуального коммита, старый результат уже не годится как доказательство.
...
- **Риск подтвердился повторно (2026-09-06):** в `node_modules/.bun/` найдены сразу два
  осиротевших слепка (`dprint@0.55.2`, `dprint@0.56.1`) при актуальном пине `^0.57.1` —
  ни `bun.lock`, ни один `package.json` монорепо на них не ссылались. Прочищены (`rm -rf`),
  `node_modules/.bin/dprint --version` после чистки — по-прежнему `0.57.1`, главное дерево не
  задето. Подтверждает вывод 2026-08-12: bun-стор не самоочищается при бампе версии, накопление
  неизбежно повторяется — периодическая ручная сверка (`ls node_modules/.bun | grep '^dprint@'`
  против пина в `package.json`) остаётся единственным практическим действием, автоматической
  защиты по-прежнему нет.
...
## §23 — Логика СДЭК и расчёт упаковки дублируются, `@letar/cdek` не хостит оценку габаритов ✅ ЗАКРЫТО (клиент — 2026-08-12, тонкая часть упаковки — 2026-09-08)
...
**Не сделано (на тот момент):** п.2 (поднять тонкую часть расчёта упаковки — `CdekPackageDims` +
конвертация см — в `@letar/cdek`) и п.3 — за пределами решения владельца в этой сессии (запрошена
только миграция клиента, не расчёта упаковки).

### Что сделано (2026-09-08, `repo-dev`) — п.2/п.3

Поднята ровно та тонкая часть, что описана в выводе выше: `mmToCdekPackageDims()` в новом файле
`libs/cdek/src/server/package-dims.ts` — конвертация `{lengthMm, widthMm, heightMm, weightG}` →
`CdekPackageDims` (см, округление вверх), без единой строчки товароспецифичной логики. Формула
намотки листов в рулон (`estimatePackage()`) осталась в `apps/aboi/src/lib/shipping/
package-estimator.ts` — она знает про `ProductKind` и свёртку через короткую сторону, в общую
либу не просится (см. вывод 2026-08-12 выше, актуальность подтверждена повторным чтением кода).

`aboi`'s `toCdekPackage()` теперь — однострочная обёртка над `mmToCdekPackageDims()` (сохраняет
свою сигнатуру с необязательным `number` — используется в тесте и потенциально вызывающим кодом).

`svoichuzhie` не тронут (п.3) — `merch-package.ts` считает в сантиметрах сразу (суммой объёмов
коробок), конвертация мм→см ему не нужна, общего с `mmToCdekPackageDims()` там нет.

Проверено: `nx run-many -t typecheck:tsgo lint test --projects=cdek,aboi` — оба зелёные
(`aboi:test` — 181/182 прошли, единственный красный файл `checkout.test.ts` падает по
несвязанной причине — `server-only` в клиентском модуле, не затронуто этой правкой).
...
- [x] Решено, что именно из расчёта упаковки поднимать в `@letar/cdek` — тонкая конвертация
      мм→см (`mmToCdekPackageDims`), формула намотки остаётся в приложении (2026-09-08)
