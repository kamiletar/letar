...
**Перед коммитом:** `nx run-many -t dprint` → `nx lint` → `nx typecheck:tsgo`

⚠️ Не `nx format` — это встроенная команда Nx, запускающая Prettier, конфликтующий с dprint
(правки друг друга откатывают, ломает markdown в плановых файлах). dprint — единственный
форматтер репозитория (PLAN-INFRA.md §32, закрыт 2026-08-06).
