...
3. **Одно нерискованное staging-подобное приложение с `letar.best`-доменом, без rollout-label.**
   ⏳ **В работе (2026-08-25).** По аналогии с `time` на s3 (M1b) — первый прод-домен на Traefik,
   но без совмещения с zero-downtime rollout, чтобы не проверять два новых механизма одновременно.
   Выбран `aira.letar.best` (`aira-web`) — не в списке `letar.rollout: 'true'`, низкий трафик
   (лендинг релизов Aira), без сессий/платежей. Роутер заведён
   (`infra/traefik/dynamic/s2/pilot-aira.yml`), использует уже готовый wildcard-сертификат зоны
   `letar.best` из шага 1 — отдельного acme-dns-аккаунта не требуется. DNS всё ещё указывает на
   NPM, трафик не переключён. Задача передана deploy-agent-dev: задеплоить, проверить `curl`
   напрямую на `https://s2:8443` с `Host: aira.letar.best`, дать постоять день-два перед
   переключением DNS/порта и переходом к шагу 4.
