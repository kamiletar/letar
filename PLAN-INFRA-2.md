...
2. **Домены вне зоны `letar.best` — отдельно и первыми, до общего переключения.** ⏳ **В работе
   ...
   **собственного** продления сертификатов тех же 6 доменов. Добавленный поверх location либо
   ...
   ⏳ **Отложено: `dsperevod.ru`/`www.dsperevod.ru`** — нет доступа к регистратору этого домена.
   acme-dns аккаунты под них уже зарегистрированы и ждут (см. таблицу CNAME выше) — когда доступ
   найдётся, добавить оставшиеся 2 CNAME, дальше Traefik подхватит сертификат сам. Шаг 2 M3
   считается закрытым, когда готов и этот последний домен.
