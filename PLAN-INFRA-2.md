...
1. **Подготовка на s2, без переключения трафика.**
   ...
   - Явно прописать `compress` middleware (gzip/brotli/zstd) на entrypoint — это пункт 3 чек-листа
     выше, делать сразу на этапе поднятия, а не потом отдельным заходом.
   - Traefik слушает **не** 80/443 на этом этапе (свободный порт по аналогии с пилотным `8090` на
     s3), NPM продолжает держать боевые порты.
