...
### Дозачистка 2026-09-03 (letar-dev): «README доведён до полноты» снова отстало

Тот же скрипт-сверка (`for name in ...; do grep -q "$name" README.md || echo MISSING; done` по
полному списку именованных экспортов `src/index.ts`) на этот раз нашёл **пять** экспортов,
добавленных в библиотеку уже после 2026-08-11/12 и ни разу не попавших в README:
`PriorityNav` (навигация), `RouteAnnouncer` (доступность), `chakraColorVar` (утилита конверсии
Chakra-токена в CSS var), `RatingDistribution` (добавлен рядом с `RatingStars`/`RatingDisplay`,
но не в тот же раздел README), `useConfirmDialog` вместе с `DeleteConfirmDialog`/
`StopConfirmDialog`/`TriggerConfirmDialog` (варианты `ConfirmDialog`, в README была описана
только базовая controlled-форма). Все пять — не новые компоненты этой сессии, они уже были в
коде, просто не задокументированы задним числом при добавлении.

**Вывод: пункт 0 (DoD) — не разовое действие, а требующий периодической ре-сверки инвариант.**
Разрыв растёт тем же механизмом, что описан в «Дополнение 2026-07-28» выше — новый экспорт
попадает в `index.ts` за одну строку кода, а README требует отдельного (и вечно не приоритетного)
усилия. Разовая полная сверка не гарантирует, что через месяц счётчик снова не разъедется — нужен
либо линт-скрипт по образцу `scripts/check-lib-subpath-paths.mjs` (сверяет экспорты `index.ts`
против упоминаний в README построчным `grep`), либо принятие того, что ре-аудит README придётся
периодически повторять руками. Решение о том, заводить ли автоматический сторож, не принято —
задача штучная (5 разделов на всю библиотеку), делать отдельный скрипт ради этого объёма пока не
обосновано, но если разрыв повторится в третий раз — стоит его завести.

### Дозачистка 2026-09-05: разрыв повторился в четвёртый раз, но не весь он реальный

Автоматическая сверка (тот же принцип — построчный `grep` имени экспорта по README) нашла
10 «пропавших» из 68 value-экспортов `index.ts`: `HeaderActions`, `HeaderLogo`,
`HeaderMobileActions`, `HeaderMobileMenu`, `HeaderMobileProvider`, `HeaderNav`, `HeaderRoot`,
`RippleEl`, `useHeaderMobile`, `useRipple`.

**5 из 10 — ложное срабатывание построчного grep, не реальный пробел.** `HeaderActions`/
`HeaderLogo`/`HeaderMobileActions`/`HeaderMobileMenu`/`HeaderNav` уже описаны в разделе `###
Header` — но через compound dot-нотацию примера (`<Header.Logo>`, `<Header.Nav>` и т.п.), а не
литеральным именем экспорта. Ровно тот способ документирования, что уже разрешён DoD §28 для
`useRipple`/`RippleEl`/`pressableConfig` — просто наивный grep по имени идентификатора его не
распознаёт (`HeaderLogo` ≠ подстрока `Header.Logo`). Автоматический сторож по образцу
`check-lib-subpath-paths.mjs`, если его когда-нибудь заводить, обязан это учитывать — иначе
будет закрывать разработчикам глаза ложными «пропусками» на каждом compound-компоненте
библиотеки (сейчас их два: `Header`, и по тому же паттерну могут появиться другие).

**Остальные 5 — реальный, ранее не задокументированный пробел, закрыт этой правкой.**
`HeaderRoot`/`HeaderMobileProvider`/`useHeaderMobile` (низкоуровневые части `Header` — сборка
своей разметки вместо готового `Flex`, доступ к состоянию мобильного меню напрямую) и
`RippleEl`/`useRipple` (примитивы, на которых построен `Pressable`) не были упомянуты в README
вообще, даже сгруппированно — добавлены короткими абзацами в разделы `### Header` и `### Pressable
/ PressableButton` соответственно, без отдельных подзаголовков (тот же принцип, что и остальные
низкоуровневые API библиотеки).

**Решение по автоматическому сторожу — по-прежнему не принято.** Порог «третий раз» из
предыдущей записи формально пройден (это уже четвёртая сверка подряд), но половина находок в
этот раз оказалась артефактом самого метода проверки (наивный grep не понимает dot-нотацию), не
реальным дрейфом документации. Заводить скрипт, который сам первым делом даст ложноположительный
результат на два (Header) из ~30 компонентов библиотеки, не имеет смысла без отдельного
проектирования (нужен разбор compound-экспортов, а не построчный grep) — вынесено отдельной
задачей, не блокирует закрытие этой сверки.
...
- [x] **Закрыто (2026-09-07, letar-dev):** аудит `NODE_ENV`-ловушки индексации по всему
      монорепо. Грепом `NODE_ENV` и `isProductionDomain` по всем 17 `apps/*/src/app/robots.ts` —
      единственное совпадение с `NODE_ENV` (`archetest`) оказалось комментарием, объясняющим
      _почему_ гейт НЕ через `NODE_ENV` (ложное срабатывание грепа, код корректен). Остальные 16
      уже используют `isProductionDomain()`. Дополнительно проверены 6 приложений без
      `robots.ts` в исходном списке §33 Части B: `animatrona-landing`/`letar-landing` — свой
      статический домен без `docker-compose.staging.yml` (staging-цель физически не существует,
      класс бага не применим, безусловный `allow: '/'` безопасен); `form-develop-app-shadcn`/
      `scaffold-test-app` — без `Dockerfile.production` вовсе, не деплоятся публично (тот же
      случай, что уже описан для `form-develop-app`); `domwellbes` (приватный submodule) — свой
      `SITE_LAUNCHED = false` в `lib/seo.ts`, жёсткая константа, явно не `NODE_ENV`-производная
      (комментарий в файле прямо ссылается на этот класс бага и объясняет, почему решение другое).
      Новых пробелов не найдено — весь монорепо чист от этого класса ошибки.
...
## §47 — Разгрёб «разошедшихся» агентов: коммит накопленных изменений по всему репо ✅ ЗАКРЫТО (2026-08-06)
...
## §48 — Traefik + wildcard-TLS вместо Nginx Proxy Manager ✅ ЗАКРЫТО ЦЕЛИКОМ (шаг 6 — 2026-08-31)
...
1. **Подготовка на s2, без переключения трафика. ✅ ЗАКРЫТО (2026-08-25).** Traefik поднят
   ...
   - Явно прописать `compress` middleware (gzip/brotli/zstd) на entrypoint — это пункт 3 чек-листа
     ...
     ⚠️ **Найдено 2026-09-04, живой замер прод:** `compress: {}` без явного списка `encodings`
     живёт на дефолтном порядке Traefik, где `gzip` идёт раньше `br` — обычный браузерный
     заголовок `Accept-Encoding: br, gzip` (порядок в самом заголовке без q-значений ничего не
     значит) всегда получал `gzip`, хотя brotli был технически доступен и честно отдавался при
     запросе с одним только `br` в заголовке. Фикс — явный `encodings: [br, zstd, gzip]` в
     `dynamic/s2/compress.yml` (коммит `33229056`). Динамический конфиг (`watch: true`) —
     подхватывается без рестарта контейнера.
...
2. **Домены вне зоны `letar.best` — отдельно и первыми, до общего переключения.** ⏳ **В работе
   ...
   **собственного** продления сертификатов тех же 6 доменов. Добавленный поверх location либо
   ...
   fulldomain-цели для всех 6 доменов и вернуть список.

   ⚠️ **Второй барьер по пути:** регистрация новых acme-dns аккаунтов на s2 закрыта
   (`disable_registration=true`, решение от 2026-08-06 — защита от чужой регистрации через
   `acme-api.letar.best`). Плюс файл `/home/deploy/lego/acme-dns-accounts.json` смонтирован в
   Traefik `:ro` (та самая защита от инцидента шага 1). Оба флага пришлось открыть синхронно на
   ~15 минут (с одобрения владельца, с обязательным ручным бэкапом файла перед окном) и закрыть
   сразу после регистрации — коммиты `767225c9`/`11970d15` (acme-dns), `a135be6d`/`a4aa1f4b`
   (`:rw`→`:ro` в `docker-compose.s2.yml`). Подтверждено закрытым с обеих сторон.

   ✅ **Готово (2026-08-25): все 9 acme-dns аккаунтов зарегистрированы** (6 доменов, apex+`www`
...
3. **Одно нерискованное staging-подобное приложение с `letar.best`-доменом, без rollout-label.**
   ✅ **Закрыт целиком, реальный трафик переключён (2026-08-25).** По аналогии с `time` на s3
   (M1b) — первый прод-домен на Traefik, но без совмещения с zero-downtime rollout. Выбран
   `aira.letar.best` (`aira-web`) — не в списке `letar.rollout: 'true'`, низкий трафик, без
   сессий/платежей. Роутер заведён (`infra/traefik/dynamic/s2/pilot-aira.yml`, коммит `50c380c3`),
   задеплоен, сперва проверен прямым `curl --resolve` на 8443 (TLS/ответ приложения корректны),
   владелец решил не ждать «день-два» — сразу перешли к реальному cutover.

   **Механизм cutover (не через DNS/A-запись):** `aira.letar.best` — не wildcard, а отдельный
   `proxy_host` в NPM (id=19) со своим выделенным Let's Encrypt-сертификатом (не общим с другими
   25 доменами) — значит правка задевает ровно одну строку в БД NPM. NPM продолжает публично
   держать 443 и отдавать клиенту свой прежний сертификат (id=32, без изменений) — только
   `forward_host/port/scheme` этой одной записи сменены с `aira-web-app:3017` (http) на `traefik`
   (https). Двойной TLS-хендшейк NPM→Traefik остаётся (накладные расходы одного внутреннего хопа
   в той же docker-сети, не архитектурная проблема). Реальные пользователи `aira.letar.best`
   теперь идут NPM → Traefik-роутер `pilot-aira` → `aira-web-app`, подтверждено access-log
   Traefik (`pilot-aira@file`, `200`, 25мс) на публичном запросе, не только через `--resolve`.

   ⚠️ **Грабля первой попытки:** `forward_port: 8443` дал `502` — `8090:80`/`8443:443` в
   `docker-compose.s2.yml` это host-публикация портов, она не участвует в container-to-container
   трафике внутри `kami-network`. Контейнер-контейнер идёт напрямую на внутренний порт Traefik
   (`443`), не на опубликованный наружу `8443`. Исправлено `forward_port: 443`, дальше сразу ок.
   Откат (если понадобится) — тот же PUT с `forward_host: aira-web-app, forward_port: 3017,
   forward_scheme: http`, секунды.

   Этот же прогон заодно доказал совместимость Traefik-cutover с обычным force-recreate деплоем
   приложения (без `letar.rollout`) — сразу следом задеплоен несвязанный фикс `aira-web`
   (`ec94cc20`/`0e231256`, GlitchTip AIRA-WEB-1, `ContextError` от Turbopack по умолчанию),
   deploy-agent-dev отдельно перепроверил `https://aira.letar.best/` после recreate — цепочка
   NPM→Traefik→новый контейнер отработала без сбоя.

   **Паттерн cutover пригоден и для 5 доменов шага 2** (не только для `letar.best`-поддоменов) —
   там тоже отдельные `proxy_host` с собственными сертификатами, тот же механизм применим без
   ожидания шага 5/6. Не форсируется сейчас — решение когда именно переводить эти домены остаётся
   за владельцем, сама возможность зафиксирована здесь для справки.

   ✅ **Второй домен переключён тем же механизмом (2026-08-25): `animatrona.letar.best`.**
   `proxy_host id=24`: `forward_host: traefik`, `forward_port: 443` (грабля с портом уже
   известна, не повторилась), `certificate_id` (37, собственный) не менялся. Нюанс NPM API:
   `locations: null` в PUT-теле роняет валидацию (`must be array`), нужен `locations: []`.
   Подтверждено публичным `curl` (200, cert CN=animatrona.letar.best) и логом Traefik —
   `animatrona-landing@docker` обслужил запрос, то есть это одновременно первое боевое
   подтверждение шага 5 (docker-provider, не file-provider). Откат — тот же PUT с
   `forward_host: animatrona-landing-app, forward_port: 3008, forward_scheme: http`.

4. **Переписать `rollout.ts` под Traefik-ветку** (см. «ключевая находка» выше).
   ✅ **Код готов (2026-08-25, коммит `189f5ba7`).** `RolloutOptions.proxyKind: 'npm' | 'traefik'`
   (по умолчанию `'npm'` — обратная совместимость для всех текущих rollout-приложений, ничего не
   переключилось само). При `'traefik'` шаги `nginx-reload-1`/`nginx-reload-2` становятся no-op
   (`ok: true`, без `docker exec`) — id сохранены теми же намеренно, чтобы не плодить два набора
   id в одном `RolloutResult.steps[]`. `npmContainerName` стал опциональным, обязателен только
   при `proxyKind: 'npm'`. CLI: `--proxy-kind npm|traefik`. Тесты — 2 новых кейса в
   `rollout.spec.ts` (traefik no-op, npm без `npmContainerName` — явная ошибка на первом
   reload-шаге, не силентный краш).

   ⚠️ **Отклонение от «ключевой находки» выше:** там предполагался автодетект через
   `docker ps --format '{{.Names}}' | grep -qx traefik` внутри `rollout.ts`, по аналогии с
   `deploy-affected.sh`. Реализовано иначе — явный параметр `proxyKind`, который прокидывает
   вызывающая сторона (CLI-флаг/будущий deploy-mcp), а не рантайм-детект внутри `rollout.ts`.
   Причина: явный флаг детерминирован и тестируется без мока `docker ps`, а автодетект добавил бы
   ещё один `docker` вызов и скрытую зависимость от состояния сервера в каждый прогон. Ветвление
   по серверу (s2 vs s3, NPM vs Traefik) остаётся на стороне вызывающего кода
   (`deploy-affected.sh`/будущий deploy-mcp), не внутри самого движка.

   ✅ **Обкатка на реальном контейнере — сделана (2026-08-25), но неявно.** Первый rollout-деплой
   шага 5 (`animatrona-landing`, коммит `97d4ff4b`) прошёл **без** `--proxy-kind traefik` —
   `deploy-affected.sh` ещё не передаёт флаг никому (см. следующий пункт), поэтому rollout ушёл в
   npm-ветку и выполнил реальный `nginx -s reload` дважды. Сработало вхолостую-безвредно: label
   Traefik добавлен впервые именно на новом контейнере, который создал этот же rollout, так что
   docker-provider увидел его независимо от того, что делала npm-ветка reload'а. Настоящая
   `traefik`-ветка (no-op reload) остаётся не обкатанной живым прогоном — обкатается на первом
   приложении, задеплоенном ПОСЛЕ того, как будет решён следующий пункт про передачу флага.

   ✅ **Решение по передаче `--proxy-kind` в пайплайн (2026-08-25):** label в compose,
   `letar.proxy-kind: traefik` рядом с `letar.rollout: 'true'` — симметрично уже работающему
   паттерну, источник истины у приложения, не в отдельном списке. `deploy-affected.sh` уже
   парсит один такой label через grep (строка ~1156) — второй такого же вида добавляется тем же
   способом. Альтернативы (bash-массив `TRAEFIK_APPS=(...)` в `deploy-affected.sh`, поле в
   `libs/infra-config` с bash-мостом) рассмотрены и отклонены — источник истины не должен уезжать
   от приложения, а bash→node мост на раннем этапе деплоя добавлять рано. Реализация — отдельная
...
### ✅ Шаг 6 — NPM снят с s2 (2026-08-31)

Порядок: живая сверка всех доменов через пилотный `8443` (25/25 без ошибок) → бэкап
`data/`+`letsencrypt/` NPM (`/home/deploy/backups/npm-s2-*.tar.gz` на сервере, ~21МБ) → правка
трёх портов в `docker-compose.s2.yml` (`8090:80`→`80:80`, `8443:443/{tcp,udp}`→`443:443/{tcp,udp}`,
`traefik.s2.yml` без `advertisedPort` — трогать нечего, в отличие от s3) → `docker compose stop`
NPM → `docker compose -f docker-compose.s2.yml up -d` Traefik → сквозная проверка тех же 25
доменов на реальных `80`/`443` (25/25) + `HTTP→HTTPS 308` + `alt-svc: h3` + UDP 443 слушает.

⚠️ **Найдено по ходу: `acme-api.letar.best` на пару минут остался без всякой защиты.** NPM был
единственным периметром этого маршрута (Access List). После его остановки заготовленный заранее
`ipAllowList` на Traefik ещё не был задеплоен (первый `scp` оборвался по сети, файл не долетел —
не хватило проверки после доставки). Закрыто тем же приёмом, что и всё остальное здесь: не
предполагать, что команда прошла, проверять содержимое файла на сервере после каждой доставки.
Финальная защита — `ipAllowList` по `RemoteAddr` (без `ipStrategy`, он был нужен только пока NPM
стоял между клиентом и Traefik) — [infra/traefik/dynamic/s2/acme-api.yml](/infra/traefik/dynamic/s2/acme-api.yml).

⛔ **Побочная находка на пути к этому шагу — не про NPM.** `DOCKER-USER` не защищал IPv4 так,
как считалось (userland-proxy у Docker включён по умолчанию для обеих версий IP, не только v6) —
два PostgreSQL были реально открыты всему интернету в обход firewall. Разбор, фикс и то, почему
это НЕ относится к самим пилотным портам Traefik (те защищены иначе, через совпадение
контейнерного порта) — [§49 в PLAN-INFRA-3.md](/PLAN-INFRA-3.md), дополнение 2026-08-31.

**NPM не удалён** — контейнер остановлен (`docker compose stop`, не `down`/`rm`), данные и
`docker-compose.yml` целы на сервере. Откат при проблеме: `docker compose up -d` в
`infra/nginx-proxy-manager/` + вернуть три порта в `docker-compose.s2.yml` на `8090`/`8443` +
пересоздать Traefik.

**Не сделано этим шагом (не входило в scope):** чистка мёртвых строк в БД NPM
(`gateway.letar.best` disabled, `anime.letar.best` без backend) — не блокировали cutover, NPM
всё равно остановлен, смысла чистить прямо сейчас нет.

### §48 M3 — ЗАКРЫТО (2026-08-27)

Все 20 оставшихся приложений переведены пачками (3–6 за проход), router-only регистрация →
пилот-верификация на `:8443` → cutover NPM→Traefik только после отдельного одобрения владельца
на каждую волну доменов. Итог:
...
- **archetest** был заблокирован собственным hard e2e-gate (3/21 тестов, firefox/webkit,
  ...
  только через bottom-отступ по CSS-переменной, работает лишь пока панель уже «застряла»
  (`position: sticky`); на короткой странице без скролла панель могла ещё не «застрять» и
  оказаться в потоке у нижнего края — ровно там же, где всегда сидит `position: fixed`
  `CookieBanner` с зачительно более высоким zIndex по умолчанию. Фикс — `zIndex="docked"` →
  `zIndex="sticky"` (коммит `33feb329`), e2e зелёный, деплой прошёл, cutover выполнен.
- Технический механизм cutover для будущих ссылок: `PUT /api/nginx/proxy-hosts/{id}` с
  `forward_host: traefik, forward_port: 443, forward_scheme: https, locations: []`,
  `certificate_id` не трогается — NPM продолжает подавать свой сертификат, форвардит дальше на
  Traefik вместо приложения напрямую. Откат — обратный PUT на `<app>-app`/родной порт/`http`.
