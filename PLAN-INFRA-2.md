...
1. **Подготовка на s2, без переключения трафика.** ⛔ **Первый пилотный запуск (2026-08-25,
   deploy-agent-dev) провален с инцидентом, повторно не запускался.**
   (`infra/traefik/docker-compose.s2.yml`, `traefik.s2.yml`, `dynamic/s2/`,
   `secrets/deploy.s2.conf` заведены и корректны, но `scripts/deploy-infra.sh`
   **фактически не выбирает** server-specific override — при вызове
   `deploy_infra({service:"traefik", server:"s2"})` в логе оказалось
   «Расшифровываю секреты traefik по `.../deploy.conf`» (общий, s3-манифест) вместо
   ожидаемого `deploy.s2.conf`. Итог: `acme-dns-accounts.json.enc` (урезанная s3-копия)
   расшифровался поверх боевого `/home/deploy/lego/acme-dns-accounts.json` с ключом всей
   зоны `letar.best` — запись `letar.best` из файла пропала.
   - **Восстановлено без потерь:** traefik-контейнер ни разу не поднимался (только
     тянулись образы — `docker ps -a`/`network ls`/`volume ls` по traefik пусты), поэтому
     попытки выпуска сертификата с испорченным ключом не было. Штатный автобэкап
     `backups/acme-dns/acme-dns_auto_2026-08-25-03-30-00.tar.gz` (снят cron'ом в 03:30, за
     ~4.5 часа до инцидента в 08:13) содержал корректный файл — восстановлен, `chown
     root:root`, `chmod 600`, проверено содержимое (`letar.best` + `s3.letar.best` на
     месте). Испорченная версия сохранена рядом как
     `acme-dns-accounts.json.corrupted-20260825-0813` для разбора. NPM на s2 не тронут,
     прод не пострадал.
   - **Попутно найдено:** `scripts/deploy-infra.sh` лежал в git с режимом `100644` (без
     `+x`) — первый вызов падал `Permission denied` (exit 126) ещё до основной проблемы.
     На сервере временно выставлен `chmod +x` (слетит при следующем `git pull`).
   - ⚠️ **Блокер шага 1: логика выбора `deploy.s2.conf` в `deploy-infra.sh` требует
     фактического фикса и повторной проверки на реальном вызове** (не только на
     чтении кода) — предыдущая уверенность «научен выбирать override» была неверной.
     Деплой traefik на s2 не повторять, пока это не исправлено — иначе гарантированно
     тот же инцидент. Диагностика и цепочка восстановления — сообщение deploy-agent-dev →
     repo-dev в Agent Mail (thread `traefik-s2-m3-step1`, 2026-08-25).
   - Порты 8090/8443 на s2 проверены живьём и свободны (`ss -tlnp`/`docker ps`) — этот
     пункт закрыт, не требует повторной проверки при следующей попытке (если промежуток
     между попытками небольшой).
