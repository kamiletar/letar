...
2. **Домены вне зоны `letar.best` — отдельно и первыми, до общего переключения.**
   Причина порядка: у них резолвер `http` (HTTP-01), а не `dns` — механизм независим от wildcard
   и от общего переезда, поэтому это самый дешёвый и обратимый первый шаг. Список доменов взять с
   **сервера** (`docker ps -a` + текущие proxy host'ы NPM), не из документации — тот же урок,
   что уже трижды сработал на s3 (§48 M2, «список host'ов и портов — только со слепка сервера»).
