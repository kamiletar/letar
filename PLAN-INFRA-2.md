...
2. **Домены вне зоны `letar.best` — отдельно и первыми, до общего переключения.** ⏳ **В работе
   ...
   **собственного** продления сертификатов тех же 6 доменов. Добавленный поверх location либо
   ...
   отдельного деплоя не требуется, если не нужно форсировать раньше renewal-цикла. Шаг 2 M3
   закрыт после того, как владелец добавит CNAME и сертификаты подтвердятся живьём
   (`docker logs traefik | grep 'Server responded with a certificate'`).
