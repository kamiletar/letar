...
2. **Домены вне зоны `letar.best` — отдельно и первыми, до общего переключения.** ⏳ **В работе
   ...
   **собственного** продления сертификатов тех же 6 доменов. Добавленный поверх location либо
   ...
   ⚠️ **Частично заблокировано (2026-08-25): нет доступа к регистратору `dsperevod.ru`.**
   Владелец добавляет CNAME для 4 из 6 доменов (направа.рф, aprel8008.ru, svoichuzhie.ru,
   domwellbes.ru) сразу; `dsperevod.ru`/`www.dsperevod.ru` откладываются до появления доступа —
   acme-dns аккаунты под них уже зарегистрированы и ждут, ничего переделывать не нужно, когда
   доступ найдётся. Шаг 2 M3 закрывается по частям: домен считается готовым, когда сертификат
   подтвердится живьём (`docker logs traefik | grep 'Server responded with a certificate'`), весь
   шаг — когда готовы все 6.
