...
3. **Одно нерискованное staging-подобное приложение с `letar.best`-доменом, без rollout-label.**
   ✅ **Роутер проверен, стоит (2026-08-25).** По аналогии с `time` на s3 (M1b) — первый прод-домен
   на Traefik, но без совмещения с zero-downtime rollout. Выбран `aira.letar.best` (`aira-web`) —
   не в списке `letar.rollout: 'true'`, низкий трафик, без сессий/платежей. Роутер заведён
   (`infra/traefik/dynamic/s2/pilot-aira.yml`, коммит `50c380c3`), задеплоен, проверен прямым
   `curl --resolve` на 8443 (deploy-agent-dev): TLS-хендшейк валидный (сертификат `*.letar.best`,
   Let's Encrypt), ответ приложения корректный (Next.js, RSC-заголовки, `x-nextjs-cache: HIT`).
   DNS `aira.letar.best` по-прежнему указывает на NPM, трафик не переключён.

   ⏳ **Стоит день-два перед переключением** — не форсировать раньше срока. Дальше: перевести
   `aira.letar.best` реально на Traefik (смена A-записи или порта NPM ↔ Traefik) и перейти к
   шагу 4 (переписать `rollout.ts`).
