...
2. **Домены вне зоны `letar.best` — отдельно и первыми, до общего переключения.** ⏳ **В работе
   ...
   **Решение владельца (2026-08-25):** NPM проксирует `/.well-known/acme-challenge/` на
   `http://traefik:80` (внутри `kami-network`, в обход опубликованных портов) для всех 6
   host'ов — боевой трафик остаётся на NPM как есть, ни один домен физически не
   переключается. Временный шим на время пилота; после реального переключения домена в
   шаге 5/6 location в NPM снимается вместе с самим host'ом. Задача передана
   deploy-agent-dev (Agent Mail, thread `traefik-s2-m3-step1`), ждёт исполнения.
