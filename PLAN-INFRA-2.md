...
1. **Подготовка на s2, без переключения трафика.** ⏳ **Конфигурация готова 2026-08-25**
   (`infra/traefik/docker-compose.s2.yml`, `traefik.s2.yml`, `dynamic/s2/`,
   `secrets/deploy.s2.conf`, `scripts/deploy-infra.sh` научен выбирать server-specific
   override), на сервере ещё не развёрнуто — ждёт deploy-agent-dev.
