...
2. **Домены вне зоны `letar.best` — отдельно и первыми, до общего переключения.** ⏳ **В работе
   ...
   **собственного** продления сертификатов тех же 6 доменов. Добавленный поверх location либо
   ...
   ⏳ **Отложено на неопределённый срок: `dsperevod.ru`/`www.dsperevod.ru`** — нет доступа к
   регистратору этого домена (решение владельца 2026-08-25 — не искать доступ специально, не
   форсировать). acme-dns аккаунты под них уже зарегистрированы и ждут (см. таблицу CNAME выше) —
   если доступ появится, добавить оставшиеся 2 CNAME, дальше Traefik подхватит сертификат сам, без
   отдельного деплоя. NPM продолжает обслуживать `dsperevod.ru` как сейчас — ничего не сломано,
   продление его собственного сертификата этим доменом идёт своим чередом.

   **Не блокирует остальной M3.** Шаг 2 в широком смысле закрыт — 5 из 6 доменов готовы к
   переключению. Шаги 3–6 идут по остальным доменам/приложениям независимо от статуса
   `dsperevod.ru`; при шаге 6 (снятие NPM с s2) `dsperevod.ru` — единственный домен, который
   потребует либо доступа к регистратору к этому моменту, либо NPM/аналог придётся оставить живым
   персонально под этот один домен (решать при подходе к шагу 6, не сейчас).
