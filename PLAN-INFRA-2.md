...
2. **Домены вне зоны `letar.best` — отдельно и первыми, до общего переключения.** ⏳ **В работе
   ...
   **собственного** продления сертификатов тех же 6 доменов. Добавленный поверх location либо
   ...
   Владелец добавляет CNAME для 5 из 6 доменов (направа.рф, aprel8008.ru, svoichuzhie.ru,
   domwellbes.ru, neyroaboi.ru) сразу; `dsperevod.ru`/`www.dsperevod.ru` откладываются до
   появления доступа —
