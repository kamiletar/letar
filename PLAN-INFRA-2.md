...
4. **Переписать `rollout.ts` под Traefik-ветку** (см. «ключевая находка» выше).
   ...
   ⏳ **Обкатка на реальном контейнере — не сделана.** `aira-web` (пилот шага 3) намеренно без
   `letar.rollout: 'true'` — единственный механизм проверялся отдельно (см. шаг 3). Живой прогон
   `rollout --proxy-kind traefik` — либо на `aira-web` после того, как ему добавят label обратно
   (по плану ниже), либо на первом приложении пачки шага 5, смотря что реалистичнее к моменту
   перехода к шагу 5.
