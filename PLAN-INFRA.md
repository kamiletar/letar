...
- [ ] `dprint check` зелёный по всему репозиторию — не проверено
- [ ] Массовое переформатирование **отложено**, см. инцидент ниже
- [ ] Плановые файлы перестали ломаться форматтером — актуально автоматически после отказа от
      Prettier, но не подтверждено прогоном
...
**M1b — переключение на wildcard DNS-01** (когда доступ к админке регистратора есть)

- [ ] API-ключ+секрет Dynadot получены, разложены по `.env.docker`/`.enc`, IP s3 разрешён
- [ ] Резолвер `dns` выпустил `*.letar.best` + `letar.best`; слепок зоны до/после сверен
