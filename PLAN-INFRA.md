...
### ✅ Конфиг nginx-кеша подготовлен (2026-08-09) — не задеплоен

`infra/gateway-cache/` (`nginx.conf` + `docker-compose.yml` + `README.md` с пошаговым чеклистом
деплоя) реализует решение из блока выше: фильтр по Content-Type (`application/json`, `image/*`,
`text/vtt`), видео/аудио мимо кеша, `proxy_cache_valid 200 365d` (CID неизменяем), origin
адресуется по сети (`https://ipfs.letar.best`, не `127.0.0.1`) — прокси и Kubo на разных
серверах. Порт публикации `8098` (плейсхолдер, менять при конфликте — README расписывает три
места, которые надо поправить синхронно).

Старая дока `infra/nginx-proxy-manager/README.md` (раздел «IPFS Gateway (proxy_cache)») —
та самая «несуществующая схема» из заголовка §57 — помечена устаревшей, оставлена зачёркнутой для
истории.

**Не сделано:** сам деплой на mail (`docker compose up -d`), Proxy Host в NPM на mail, порт 8098
в `infra/firewall/ports.mail.env`, выпуск сертификата, переключение DNS. Порядок — в README,
нарушать нельзя (см. «Инцидент 2026-08-07» ниже — ровно так уже ловили поломку).
...
## §65 — На s2 не было swap вообще: сборка на прод-сервере может убить чужой продакшен 🆕 (2026-08-08)
...
## §66 — Два дефекта, вскрытых деплоем агента на s3 🆕 (2026-08-08)
...
Скрипт различает только `--staging` → `docker-compose.staging.yml`, иначе берёт
`docker-compose.production.yml`. Файла `docker-compose.s3.yml` для него не существует, поэтому
...
**Это опаснее дефекта №1:** тот чинится кодом и уже починен, а этот повторится у любого, кто
запустит штатную команду. Причём симптом (`port is already allocated`) не подсказывает
первопричину — выглядит как занятый порт, а не как выбор не того файла.

**Пока не починено — `dashboard-agent` на s3 деплоится только вручную:**

```bash
docker compose -f docker-compose.s3.yml --env-file .env.docker up -d
```

- [ ] Научить `deploy-affected.sh` выбирать compose по серверу (`SERVER_NAME`/hostname) либо
      добавить явный флаг. ⛔ Не делать в конце сессии и не без прогона: через этот скрипт идут
      все деплои на всех серверах.
