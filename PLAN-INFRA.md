...
- [x] `CLAUDE.md` называет ту же команду, что реально форматирует репозиторий
      (`nx run-many -t dprint` вместо `nx format`)
...
⚠️ **DNS-01 намеренно вынесен из M1.** Он требует доступа к админке регистратора, а это внешняя
зависимость вне нашего контроля. Весь стек Traefik — label'ы, роутеры, healthcheck, дашборд,
HTTP/3 — проверяется на HTTP-01, который не требует ни API, ни правки зоны: DNS-записи staging уже
существуют, порт 80 на s3 публичен. Wildcard включается отдельным шагом M1b и меняет ровно одну
строку в конфиге резолвера. Так пилот не стоит на месте, если доступ к регистратору задерживается.

**M1a — Traefik на s3 на HTTP-01 (без DNS-01, без Dynadot)**

- [ ] Traefik поднят на s3 рядом с работающим NPM (пин версии ≥ 3.6.25), на непубличных портах
- [ ] Резолвер `http` (HTTP-01) выпустил сертификат на один staging-домен
...
**M1b — переключение на wildcard DNS-01**

Два взаимоисключающих пути к одному результату. Выбор — по тому, что доступно в панели
регистратора: включение API и правка DNS-записей могут быть закрыты разными уровнями доступа.

_Путь A — прямой API Dynadot_ (предпочтителен, если API включается):

- [ ] API-ключ+секрет Dynadot получены, разложены по `.env.docker`/`.enc`, IP s3 разрешён
- [ ] Резолвер `dns` (провайдер `dynadot`) выпустил `*.letar.best` + `letar.best`
- [ ] Слепок зоны до/после сверен — проверить риск «перезапись набора записей целиком»

_Путь B — делегирование через собственный acme-dns_ (если API регистратора недоступен, но правка
записей — да):

- [ ] `acme-dns` ([joohoi/acme-dns](https://github.com/joohoi/acme-dns)) поднят на нашем сервере,
      порт 53 UDP/TCP, доступ ограничен (`ACME_DNS_ALLOWLIST` по CIDR)
- [ ] В зоне `letar.best` **однократно руками** созданы: `A` на хост acme-dns, `NS`-делегирование
      его подзоны, и `CNAME` `_acme-challenge.letar.best` → выданный acme-dns fulldomain
- [ ] Резолвер `dns` (провайдер `acme-dns`, переменные `ACME_DNS_API_BASE` +
      `ACME_DNS_STORAGE_PATH`) выпустил `*.letar.best` + `letar.best`
- [ ] acme-dns добавлен в бэкапы (файл аккаунтов lego + база acme-dns) и в мониторинг

> Путь B ценен и сам по себе: после однократной ручной настройки ACME **никогда** больше не
> обращается к регистратору — ни при выпуске, ни при продлении, ни при новом домене. Зона остаётся
> у Dynadot. Провайдер `acmedns` есть в lego **с v1.1.0**, поэтому требование «Traefik ≥ 3.6.25»
> для этого пути **не действует** — оно нужно только Пути A.
>
...
- [ ] Роутеры staging-доменов переведены с `http` на `dns`; выпуск сертификата на новый домен
      больше не происходит вовсе
