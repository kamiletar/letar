...
## §21 — `libs/ui` разросся: плоский `index.ts` на 30 экспортов 🆕
...
- [ ] Функция оценки упаковки принимает материал параметром, а не хардкодит константы
      флизелина/бумаги внутри

---

## §24 — Степпер количества («−1 +») будет написан трижды 🆕
...
(`add-to-cart.tsx`, `cart-lines.tsx`) — оба заменяются степпером по задачам §P3.3/§P3.5.
`apps/svoichuzhie` уже написал свой quantity-UI в `merch/cart/_components/cart-view.tsx`.
В `libs/ui` (см. §21 выше — 30 экспортов) готового степпера нет.
...
- [ ] `QuantityStepper` существует в `@letar/ui` (min/max, шаг, disabled-состояние загрузки)
- [ ] `aboi` использует его в §P3.3/§P3.5 вместо собственной реализации
