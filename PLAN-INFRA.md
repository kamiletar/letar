...
- [x] `curly` закрыт целиком: `bunx dprint fmt` из корня (первый repo-wide прогон с 2026-08-09,
      ...
      `git checkout`, в коммит не попали. **Вывод:** repo-wide `dprint fmt` из корня — безопасно
      для `apps/`/`libs/`/`scripts/`, но не гони его вслепую по `.claude/docs`/`.claude/skills` —
      там нужен ручной просмотр каждого затронутого файла на предмет испорченных обучающих
      примеров.
