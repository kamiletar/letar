...
- [ ] ⏳ **Блокер: полный список proxy host'ов NPM на s3.** Запрошен у BlackCove дважды, ответа
      пока нет. Без него NPM останавливать нельзя: label'ы покрывают только приложения монорепо,
      а через NPM могут ходить вещи без compose в репозитории (медиа, IPFS-шлюз, служебное)
