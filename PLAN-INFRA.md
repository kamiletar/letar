...
- [ ] Уровень 1 применён ко всем приложениям на s2 (разбор `DB_PORT` в `deploy-affected.sh`
      обе формы записи уже переваривает — проверено, править нечего; но тест зафиксировать)
...
- [ ] Админка NPM (:81, plain HTTP) — либо на loopback + SSH-туннель, либо за самим прокси
      с TLS. Сейчас пароль ходит по сети открытым текстом
