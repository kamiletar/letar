...
- [ ] External-нога ищет письмо **во всех папках**, а не только в INBOX (либо канарейка перестаёт
      выглядеть как спам: уникальная тема, получатель в `To:` вместо скрытой копии)
- [ ] Подтверждена причина подвеса на пути контейнер s2 → mail:993 и восстановлена internal-нога
- [ ] Алерт **повторяется**, пока проблема жива (например, с нарастающим интервалом), а не гаснет
      после первого раза
- [ ] Факт доставки алерта фиксируется в состоянии отдельно от факта его отправки — чтобы
      «отправили и потеряли» не выглядело как «отправили»
- [ ] `messageFlagsAdd` вынесен из активного `fetch`-цикла; проверка — счётчик непрочитанных
      уменьшается после успешного прогона
- [ ] Ящик канарейки чистится по расписанию (или письма удаляются сразу после нахождения)
