...
- [ ] `enableOfflineQueue: false` в `libs/redis-client` — корневой фикс: сейчас обещание
      «работает без Redis» ложно для любого, кто дождётся команды до подключения. ⛔ Не хотфикс:
      меняет поведение всех потребителей, включая хранилище сессий `@letar/auth`, где команды во
      время короткого разрыва сейчас дожидаются переподключения, а начнут падать.
