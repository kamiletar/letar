...
⛔ **Чего решение НЕ даёт:** запись в чужое рабочее дерево осталась. `nx run-many -t format` без
`--projects` по-прежнему запускает dprint с `cwd` внутри семи submodule — просто теперь без
изменений. Риск смешения с чужой незакоммиченной работой снят (диффа не будет), но привычка
ограничивать прогон своими проектами остаётся правильной, и `CLAUDE.md` продолжает предписывать
`--projects`. Полностью закрыть запись мог бы вариант 1, он не применялся.
