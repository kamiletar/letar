...
- [x] Страница `/deps` (баннер устаревания + карточки риска + таблица с фильтрами/поиском/
      раскрытием advisory) и пункт «Зависимости» в `Sidebar`. **Не проверено вживую в браузере** —
      авторизация дашборда идёт только через OIDC Ключницы, headless-обхода нет; вместо этого
      компонент прогнан через `tsgo --noEmit` (типизация/JSX) и подтверждён компиляцией без
      ошибок при живых HTTP-запросах к API-роутам.
