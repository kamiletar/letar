...
- [x] **`*:42080` на s3 опознан и закрыт снаружи** (2026-08-08). Это `animatrona-pin-queue` в
      ...
      ⚠️ **Loopback-бинд здесь не подходит, и это неочевидно.** У сервиса есть легитимный
      потребитель снаружи — `ipfsstor4.letar.best` через NPM, форвард на `172.19.0.1:42080`
      (`PLAN-INFRA.md:4212`). Первая версия правила фильтровала по источнику (`! -s 127.0.0.1`) и
      зарезала этот маршрут — `ipfsstor4` отдавал `000`. Починено сужением по интерфейсу.
      Заготовленная пересборка с `BIND_ADDR=127.0.0.1` сломала бы то же самое ещё раз, поэтому
      отменена: слушатель на loopback не принимает обращения на адрес docker-бриджа.
      ...
      **Урок:** перед закрытием порта проверять не только код потребителей, но и конфиги прокси.
      ...
      Остаётся: правило не переживёт ребут (`iptables-persistent` на s3 нет). Согласовано занести
      третьей секцией в `docker-user-firewall.sh` (файл root-weaver) — по интерфейсу, не по
      источнику, с проверкой через `systemctl restart`, а не чтением файла.
- [ ] **Уровень 2 (`DOCKER-USER`) на mail-сервере не применён.** Аудит 2026-08-07 показал там
      `DOCKER-USER` пустым при 8 портах наружу (`80/81/443`, `25/465/587/993`, `4001`, `41080`).
      `ufw` там единственный из трёх серверов реально активен (`-P INPUT DROP`) — и ровно поэтому
      опаснее: создаёт впечатление защищённости, а docker-порты идут мимо него так же, как везде.
      Сервер не входил в исходный периметр §49, всплыл при подготовке §57
...
- [ ] **Перепись:** какие ещё сервисы на s2/s3/mail запущены из каталогов вне git. Известен один
      (`/opt/pin-queue`), но искали не специально — надо пройтись по
...
- [ ] **Решение по способу.** Варианты: `git init` + sparse-checkout нужного пути; симлинк на
      каталог в клоне `letar`; перенос сервиса в обычный деплой-конвейер. Выбор зависит от
      результата переписи — для одного сервиса и для десяти ответы разные.
