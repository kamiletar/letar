...
- [x] Первые потребители размечены (2026-08-12): `infra/traefik/secrets/` с манифестом на два
      ...
      для деплоя не несёт — секрет (файл аккаунтов lego) потребляет Traefik, не сам acme-dns.
      **Разовая миграция не выполнена** — `*.enc` файлов ещё нет, оба секрета по-прежнему заведены
      на s3 вручную по старой инструкции в `infra/traefik/README.md`; ручной `scp` пока не убран,
      README помечено явно. Требует SSH-доступа к серверу и решения владельца о разовом запуске
      миграции — оставлено следующей сессии/BlackCove.
