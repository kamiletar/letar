...
- [ ] 2–3 staging-домена переведены на Traefik через label'ы в compose приложения. Кандидаты —
      `pravda` и `aira-web`: оба публичные (не submodule) и без БД/auth, то есть при поломке
      маршрута ломается только страница
...
- [ ] ⏳ **Факт согласования h3 клиентом — не подтверждён, отложено до M2.** Проверка с рабочей
      ...
      из [electron-net-fetch-tun-vpn.md](/.claude/docs/electron-net-fetch-tun-vpn.md). `curl` на
      серверах собран без QUIC, искать QUIC-сборку ради пилотного порта не стали.
...
## §53 — `e2e-postgres` на s3: не ротация пароля, а вывод из эксплуатации 🆕 (2026-08-07)
...
- [ ] `docker update --restart=no` + `docker stop`, сервис закомментирован в compose
- [ ] Имя volume зафиксировано в этой секции — иначе через две недели удалять будет нечего найти
- [ ] Две строки `DATABASE_URL` на 5499 закомментированы в `.env.local` на s3 (с `.bak`)
- [ ] Порт 5499 не слушается: `ss -tulnp` **и** `ps aux | grep docker-proxy` (v6 обслуживает
      userland-процесс, отдельная проверка — урок §49)
- [ ] `run_e2e auth-hub` зелёный после остановки
- [ ] ~2026-08-21: удалить контейнер и volume, снять закомментированный блок
- [ ] Убран беспарольный fallback `REDIS_URL` в пяти compose-файлах (перед этим — сверить, что
      `REDIS_URL` реально присутствует в окружении всех четырёх прод-контейнеров)
