...
| ---------------------- | ------------------ | --------------------------------- |
| `gateway.letar.best`   | `kubo:8080`        | IPFS-шлюз с `proxy_cache` на 2 ГБ |
...
⛔ **`gateway.letar.best` — не «ещё один host», а блокер другого рода.** У Traefik HTTP-кэша нет
вовсе, заменить нечем. То есть остановка NPM на s3 не просто оставит домен без маршрута — она
уничтожит кэш раздачи IPFS, и заметно это станет по счетам за трафик, а не по ошибке в мониторинге.

⚠️ **Дока утверждала, что это проблема s2 и всплывёт в M3.** В `infra/traefik/README.md` стояло
ровно так. Слепок показал `gateway.letar.best` на **s3** — то есть зависимость упирается в M2, в
шаг, который делался следующим. Ошибка не поймалась бы ничем, кроме этого слепка: снаружи всё
отвечало, в репозитории было написано «s2», и расхождение вскрылось бы уже после остановки NPM.

**Урок шире IPFS:** перед остановкой прокси список его host'ов снимается **с сервера**, а не
берётся из документации. Дока описывает намерение, сервер — факт, и здесь они разошлись на целый
сервер. Тот же принцип, что в
[verification-pitfalls](/.claude/docs/verification-pitfalls.md) — проверять тем путём, которым
ходит настоящий потребитель.

**Следствие для порядка работ:** §57 (переезд раздачи IPFS на почтовый сервер) из смежного трека
превращается в **зависимость** M2. Пока у этих четырёх нет дома, NPM на s3 не останавливается —
и, значит, Traefik не встаёт на боевые `80`/`443`.

- [ ] Запрошены четыре `.conf` целиком: `proxy_cache*`, дополнительные `location`, access-list,
      websocket/SSE, сертификат и форс-редирект. Именно эти «специальные конфигурации» теряются
      при переносе молча — снаружи `200`, а кэша больше нет
...
- [ ] Опознать **s3 `*:42080` tcp** — единственный слушатель на обоих серверах, про который не
      известно ничего. Требует root (на s3 у `deploy` нет `sudo`) → привилегированный контейнер
      с `nsenter`. `127.0.0.1:5001`/`127.0.0.1:323` намеренно не опознавали: они на loopback,
      для firewall неинтересны
