...
- [ ] `acme-dns` ([joohoi/acme-dns](https://github.com/joohoi/acme-dns)) поднят на нашем сервере
      (для пилота — s3), доступ ограничен `ACME_DNS_ALLOWLIST` по CIDR
...
- [ ] В зоне `letar.best` **однократно руками** созданы три записи (self-delegation по README
      acme-dns): `A` на хост acme-dns, `NS` его подзоны с указанием на этот же хост, и
      `CNAME` `_acme-challenge` → выданный acme-dns fulldomain
- [ ] Резолвер `dns` (провайдер `acme-dns`, переменные `ACME_DNS_API_BASE` +
      `ACME_DNS_STORAGE_PATH`) выпустил `*.letar.best` + `letar.best` одним сертификатом
