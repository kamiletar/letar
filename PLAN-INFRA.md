...
- [ ] **§57 — `gateway.letar.best` на mail.** Кеширующий прокси там не поднят. Пока не поднят и не
      проверен снаружи — NPM на s3 не выключается. Порядок из §57 обязателен и уже нарушался:
      прокси → проверка снаружи → DNS → удаление host'а.
- [ ] **`media.letar.best`** — аккаунт acme-dns + `CNAME` + ключ на s3 (см. «Выбран вариант 1»).
- [ ] **`ipfs.letar.best`** — то же самое + `dynamic/ipfs.yml` в git.
- [ ] **`aboi`** — единственное приложение без меток: деплой упал на `P3018` (`CartItem.quantity`
      содержит `NULL`) ещё до rollout'а. Сейчас живёт через NPM и **отвалится молча** при его
      остановке. Чинить данные → передеплоить → проверить маршрут.
