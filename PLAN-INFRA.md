...
## §54 — `auth-hub` и `driving-school`: staging-контейнеры живы три недели, но снаружи недоступны 🆕 (2026-08-07)
...
> ## ⛔ ДВА ОТКРЫТЫХ ВОПРОСА — читать первыми
>
> **1. Возможно сломана раздача видео прямо сейчас.** Владелец 2026-08-07 сделал
> `ipfs.letar.best` CNAME на `mail.letar.best` под будущий кеш. **Прокси там ещё нет.** А этот
> домен — основной источник видео в плеере `svoichuzhie`
> (`src/app/video/[slug]/_components/upload-player.tsx:19`, плюс CSP `media-src`/`connect-src`
> в `next.config.mjs:26,28`).
>
> Проверка (снаружи, **не с рабочей машины** — под TUN-VPN резолвер врёт, см.
> [electron-net-fetch-tun-vpn.md](/.claude/docs/electron-net-fetch-tun-vpn.md)):
> резолв `ipfs.letar.best` → запрос `https://ipfs.letar.best/ipfs/<любой известный cid>` →
> проигрывание видео в `svoichuzhie`. Запрошено у BlackCove (тред `firewall-audit-s2`,
> сообщение 1172), ответа на момент записи нет.
>
> Если сломано — решение владельца: откатить CNAME или срочно поднять фронт. **Не чинить молча.**
>
...
**Размер: 10 ГБ из 35 ГБ свободных** (решение владельца 2026-08-07). Запас для почты — 25 ГБ.
...
| Вариант                     | Что кешируется                                     | Когда уместен                                            |
| --------------------------- | -------------------------------------------------- | -------------------------------------------------------- |
| порог «< 5 МБ», без `slice` | постеры, превью, мелкие ответы; видео **мимо**     | если цель — снять с Kubo мелочёвку, а видео и так редкое |
| `slice 1m` + кеш кусков     | горячие участки видео, независимо от размера файла | если цель — ускорить именно проигрывание                 |

Без этого решения конфиг написать нельзя: это не деталь настройки, а разные назначения кеша.
...
- [ ] Решено, что делать с `*:42080` — привязка ко всем интерфейсам (`fmt.Sprintf(":%d", …)` в
      `cmd/pin-queue/main.go:172`), в `network_mode: host` это мимо `DOCKER-USER` (§49)
...
- [ ] ⚠️ **Решено, что именно кешируем** — порог «< 5 МБ» без `slice` (тогда видео идёт мимо
      кеша) или `slice 1m` + кеш кусков (тогда кешируется горячее видео). Разные назначения, без
      выбора конфиг не написать
