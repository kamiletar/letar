...
Два объяснения, и они требуют разных действий:

1. **Никто ничего не пинил** — контент не добавляли, очередь простаивает законно. Тогда пять
   зависших заданий просто мусор, их надо закрыть.
2. **Пиннинг сломан с июня и об этом никто не узнал** — тогда сегодняшние 20 минут были не
   инцидентом, а первым разом, когда на эту подсистему вообще посмотрели.

Отличить одно от другого можно только со стороны трекера: добавляли ли контент после 22 июня.
⛔ Не инфраструктурная задача — вынести владельцу как вопрос по `animatrona-tracker`.
