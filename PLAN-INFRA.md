...
- [ ] Прогнать `nx run-many -t lint typecheck:tsgo test` по всему публичному репо (не `affected`,
      чтобы увидеть полную картину техдолга разом), собрать список пакетов с нарушениями.
- [ ] Разобрать по пакетам: часть — тривиальные автофиксы (`curly`, часть `no-empty-function`
      через `--fix`), часть требует решения (`preserve-caught-error` — добавлять `cause` в
      перевыброшенные ошибки; `@nx/enforce-module-boundaries` — нарушение границ либ, требует
      разбора импорта, не автофикса).
- [ ] Решить про `react-hooks/exhaustive-deps`: правило не находится (`Definition for rule
      ... was not found`) — либо не хватает плагина `eslint-plugin-react-hooks` в зависимостях
      прогона, либо правило указано в конфиге, но пакет не установлен/версия не та. Прежде чем
      чистить нарушения — сначала почему правило вообще не резолвится.
- [ ] Не чистить втихую большим коммитом — по пакетам/владельцам, с проверкой, что автофикс не
      меняет поведение (особенно `no-empty-function` в тестовых моках).
