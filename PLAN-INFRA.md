...
- [ ] **`media-nginx:3101` на s2 — не «порт, который надо закрыть», а контейнер, которого там
      ...
      **✅ Сделано 2026-08-07:** владелец согласовал, BlackCove остановил `media-nginx` на s2
      ...
      данных нет, стек можно удалять целиком. Остаток — `docker rm` после выдержки
...
- [ ] Уровень 3 — `ufw` включён со списком правил выше, SSH не потерян. **Приоритет упал**: для
      IPv6 хостовой фильтр уже стоит (`ip6tables INPUT`), для IPv4 остаётся только то, что слушает
      сам хост мимо Docker. Делать по остаточному принципу, вместе с ротацией e2e-credentials
...
- [ ] ⚠️ **Ротация credentials `e2e-postgres` (дефолтные `e2e`/`e2e`) — не сделана.** Тянется
      с §37. Firewall её не заменяет: правило можно снять по ошибке, пароль от этого не
      изменится. **Самый приоритетный остаток этого трека**
...
## §52 — Cron-задачи агента к приложениям падают с `HTTP 401` 🆕 (2026-08-07)
...
- [ ] Снять полный список падающих задач и код ответа по каждой (`/api/cron/jobs` + логи выполнения),
      а не только те две, что попались на глаза
- [ ] Определить, когда начались отказы — по логам выполнения в Redis (`cron:logs:*`); если история
      не хранится достаточно долго, зафиксировать это как отдельный пробел
- [ ] Сверить секрет: значение у агента против `.env.docker` каждого падающего приложения. Не
      генерировать новый вслепую — сначала понять, какая сторона разъехалась
- [ ] Проверить, не сузилась ли проверка на стороне приложений (правка `verifyCronSecret()` или
      middleware авторизации), — 401 может приходить и от общего auth-слоя, а не от cron-проверки
- [ ] Завести алерт на **неуспешные** прогоны cron-задач: сейчас `CRON_FAILED` существует, но эти
      401 никого не разбудили — выяснить, не шлётся алерт вовсе или шлётся, но теряется
...
⚠️ **Главная зацепка: каталог `terminalOutputs` пуст.** Nx открывает файл, которого нет, при
нормальных правах — значит либо он не был записан, либо **был удалён между записью и чтением**.
Второе стоит проверить первым: на s3 один и тот же workspace обслуживает и e2e-прогоны, и деплои,
а `.nx/cache` у них общий. Параллельный `deploy_app` (со своим `nx build`, сбросом или вытеснением
кэша) — самый правдоподобный кандидат в источники гонки, и он же объясняет, почему сбой разовый, а
не постоянный. Это ровно та же почва, что и §51: деплои и прогоны на s3 идут вперемешку.

### Что предлагается

1. Проверить гипотезу общего `.nx/cache`: воспроизводится ли `ENOENT`, если e2e-прогон и деплой
   идут одновременно, и исчезает ли, если развести их по времени. Если да — вопрос не в записи
   вывода, а в том, что два процесса делят кэш.
2. Развести источники истины: вердикт о тестах брать из отчёта Playwright (JSON-репортёр), а код
   возврата `nx` использовать только как признак «прогон вообще не состоялся». Сейчас это один
   сигнал на два разных события — и именно поэтому сбой кэша стал «провалом тестов».

П.2 нужен независимо от того, подтвердится ли п.1: даже когда причину гонки устранят, вердикт о
тестах не должен зависеть от кода возврата сборочного инструмента.
...
- [x] Известный класс (`EACCES`, root-owned артефакты) исключён — права `deploy:deploy`, каталог
      на месте и пуст
- [ ] Проверена гипотеза общего `.nx/cache` между параллельным деплоем и e2e-прогоном
- [ ] Понятно, воспроизводится ли `ENOENT` или это разовая гонка
