...
### Открытый вопрос: запущена ли она сейчас

Из репозитория это не проверить — cron стоит (если стоит) на сервере, не в git (тот же класс
проблемы, что §60). Отправлен запрос BlackCove (Deploy Agent, thread `infra-canary-crontab-check`)
проверить `crontab -l` на s2 и логи `docker compose run` для контейнера `canary` — ответ на момент
записи не получен.

**Статус: не удалено, ждёт ответа BlackCove и решения владельца.** Если окажется, что `infra/canary/`
не запущена нигде — каталог мёртвый код, кандидат на удаление со ссылкой на §62 как на актуальную
замену. Если запущена — это независимый третий источник алертов о доставке почты, дублирующий
нагрузку на тот же внешний ящик, и его придётся либо чинить тем же способом, что и §62 (все её
пункты Definition of Done применимы один в один), либо выключать как избыточный.
