...
Relay, Symbolicator) и ~16 ГБ RAM: на наших VPS это займёт сервер целиком. GlitchTip — четыре
контейнера (Django-приложение, Celery-воркер, Postgres, Redis), работает в 512 МБ, комфортно в 2 ГБ.
Принимает **те же самые** Sentry SDK и тот же формат DSN, поэтому переезд на настоящий Sentry, если
понадобится, — это смена одной строки в конфиге, без правок в коде.
...
1. **Проверить ресурсы s3** — свободные RAM/диск, не конфликтует ли ещё один Postgres с уже
   работающими. Это первый шаг: если на s3 тесно, вопрос переносится на другой сервер.
2. **⚠️ Проверить юрисдикцию s3 перед боевым включением.** Если сервер физически за пределами РФ,
   отправка в него ошибок с ПДн — трансграничная передача, и одна проблема (SaaS) меняется на ту
   же самую. В этом случае либо размещать GlitchTip на сервере в РФ, либо резать ПДн на стороне
   SDK (`beforeSend`), оставляя только техническую часть. Решение принимать до подключения первого
   боевого приложения, а не после.
3. `docker-compose` для четырёх сервисов + роут через Traefik (на s3 уже пилот, см. §48) +
   поддомен через acme-dns (wildcard уже выпускается без API регистратора).
4. **SDK — в общую библиотеку `libs/`, не копипастой по приложениям** (shared-first). Обёртка над
   ...
   тег окружения prod/staging.
5. `SENTRY_DSN` — в `.env.docker.enc` **и** в `environment:` compose-файла каждого приложения.
   Пропуск второго места — типовая ошибка, переменная просто не доедет до контейнера
