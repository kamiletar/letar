...
Контрпример в том же репозитории — `apps/poster-microtext-desktop`: его таргет запускается из
корня воркспейса с явным путём в аргументе (`dprint fmt apps/poster-microtext-desktop/**/*...`),
а не через `cwd`, и корневые `excludes` для него работают штатно (0 файлов).

**Не чинилось — требуется решение владельца**, вариантов три:

1. Переписать семь таргетов на форму `poster-microtext-desktop` (запуск из корня с явным путём) —
   тогда excludes сработают, но собственный `nx format` этих приложений станет no-op.
2. Завести каждому submodule собственный `dprint.json` — форматирование внутри работает, из
   родителя не течёт.
3. Признать форматирование submodule из родителя допустимым и снять их из `excludes`, чтобы
   поведение хотя бы стало единообразным.

Правка в любом случае уходит в семь чужих репозиториев (коммит + пуш + бамп SHA в `letar`),
поэтому в этот заход не делалась. Хазард живой; до решения — **всегда запускать `format` с
`--projects`**. Документировано в
...
- [ ] Тест на разбор `DB_PORT` в обеих формах записи зафиксирован, чтобы разбор не переписали
      обратно на наивный
- [ ] **App-порты 3002 / 3017 / 3024 / 3101 не тронуты — нужна одна проверка.** NPM на s2
      форвардит по именам контейнеров (в таблице [nginx-proxy-manager/README.md](/infra/nginx-proxy-manager/README.md)
      это видно для `dashboard-app:3002`), значит хостовая публикация им не нужна вовсе. Но та
      же таблица честно признаётся неполной («их конкретные proxy hosts сюда ещё не сведены»),
      и если для `aira-web`/`studio` proxy host заведён через `172.17.0.1:<порт>` (как это
      сделано для всего staging на s3), привязка к loopback уронит публичный сайт при
      следующем деплое. Порядок: посмотреть proxy hosts в NPM → перевести на имена контейнеров,
      где не так → и только потом `127.0.0.1`. Приоритет ниже, чем у баз: за этими портами
      всё равно публичный HTTP-контент, а не данные
- [ ] `media-nginx:3101` — разобраться, каким compose он поднят на s2 (в
      `infra/media-server/docker-compose.production.yml` вместе с ним публикуется 3100, а он на
      s2 занят `dashboard-agent`, то есть running-конфигурация расходится с репозиторием)
