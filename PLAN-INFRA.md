...
- [ ] ⛔ **Решение по TLS для `media.letar.best`** — label'ы задают маршрут, но не сертификат
...
Приятная часть: **у NPM для `media` нет никакой «специальной конфигурации» и переносить нечего.**
Всё содержательное — `Accept-Ranges` для перемотки, иммутабельный кэш на год, `CORP`/`CORS` против
блокировки cross-origin видео браузером, защита от хотлинкинга по `Referer` (с отдельной строкой
под кириллический домен в punycode) — живёт в собственном `nginx.conf` **внутри** контейнера
`media-nginx`. Прокси здесь тупой пробрасыватель на `:3101`, смена прокси эту логику не задевает.
Загрузка видео через прокси не ходит вовсе — она идёт в `media-api` на другой порт, поэтому
`client_max_body_size` и длинные таймауты на прокси не нужны и никогда не были нужны.
...
- [ ] Маршруты для `ipfs` и `ipfsstor4` — их compose в репозитории есть (`infra/animatrona-gateway`,
      `infra/animatrona-pinner*`), но у `ipfsstor4` запущенное расходится с git (`/opt/pin-queue`,
      находка BlackCove) — метки туда класть бессмысленно, пока расхождение не закрыто
...
## §51 — Hard e2e-gate сверяет HEAD всего монорепо, а не то, от чего зависит приложение 🆕 (2026-08-07)
...
- [ ] Найдено место сравнения (предположительно `dashboard-agent`, роут прод-деплоя) и
      подтверждено, что сравнивается именно HEAD корня — сейчас это вывод из наблюдаемого
      поведения, а не из кода
- [ ] Решено, осознанный ли это консерватизм при проектировании гейта или недосмотр
- [ ] Замена на affected-замыкание, с явным списком «корневых файлов, инвалидирующих всё»
      (изменение `deploy-affected.sh` или `bun.lock` обязано инвалидировать гейт всем)
- [ ] Проверено на живом сценарии: коммит в доки не сбрасывает гейт, коммит в `libs/*`
      сбрасывает у зависимых приложений
- [ ] Пока не сделано — знать про обходной путь: запускать e2e и прод-деплой подряд без
      зазора, либо просить остальных агентов не коммитить в окно деплоя (плохо масштабируется,
      поэтому и заведён этот пункт)
...
## §52 — Cron-задачи агента к приложениям падают с `HTTP 401` 🔧 ДИАГНОЗ НАЙДЕН, ФИКС ЖДЁТ ДЕПЛОЯ (2026-08-07)
...
- [ ] Найти, где путь «HTTP-ответ не 2xx» расходится с путём «исключение/abort» в `executeJob`, и
      почему первый не порождает `CRON_FAILED`

#### Что чинить

- [ ] Ключ дедупликации: `type` + `metadata.jobId` вместо одного `type`
- [ ] `sendNotification` — логировать причину, когда отправить некуда; вызывающий код должен
      проверять результат
- [ ] Решить, что делать при `enabled: false` — сейчас это тихий режим «копим и молчим»
- [ ] **Сторож для сторожа.** У почты есть `email-canary`, проверяющий сквозную доставку. У
      алертов такого нет: неработающая сигнализация не подаёт признаков неработы. Пока её нет,
      любой из трёх пунктов выше починится — и сломается снова незаметно
...
## §55 — `run_e2e` не отличает «тесты упали» от «упал тулинг вокруг тестов» 🆕 (2026-08-07)
...
- [ ] Вердикт `run_e2e` берётся из отчёта Playwright, а не только из кода возврата `nx`
- [ ] Гонка устранена: блокировка e2e на время деплоя **или** раздельный `NX_CACHE_DIRECTORY`
      (при втором варианте — проверить и `.nx/workspace-data`, он тоже общий)
- [ ] Проверено на живом сценарии: сбой тулинга при зелёных тестах не блокирует гейт, а падение
      теста — блокирует

## §56 — Список cron-задач на проде расходится с кодом, и код почти ничего не может с этим сделать 🆕 (2026-08-07)
...
- [ ] Решить судьбу `schedule`: либо синхронизировать из кода наравне с `app`/`endpoint`/`server`,
      ...
      описано нигде, и код вводит в заблуждение
- [ ] Придумать, как выводить задачу из эксплуатации через репозиторий (список `RETIRED_JOB_IDS`,
      вычищаемый при merge, — самый дешёвый вариант)
- [ ] Проверить остальные задачи на «зомби»: выполняется ли на проде что-то, чего в коде нет

---

## §57 — Раздача IPFS: у Traefik нет кеширования, а дока пиннеров описывает несуществующую схему 🆕 (2026-08-07)
...
- [ ] **Перепись:** какие ещё сервисы на s2/s3/mail запущены из каталогов вне git. Известно
      **два**, оба найдены случайно, а не поиском — надо пройтись по
      `docker inspect ... com.docker.compose.project.working_dir` для **всех** контейнеров и
      сверить с путями репозитория. Без этой цифры непонятен масштаб.

      | Сервис                  | Каталог на сервере      | Как всплыл                                   |
      | ----------------------- | ----------------------- | -------------------------------------------- |
      | `animatrona-pin-queue`  | `/opt/pin-queue` (s3)   | фикс `BIND_ADDR` не доезжал                  |
      | `animatrona-relay`      | `/root/animatrona-relay` (mail) | сверка портов перед allow-list §49   |
...
## §61 — Общий серверный секрет внутри распространяемого desktop-приложения 🆕 (2026-08-08)
...
- [ ] Токен `pin-queue` ротирован (старый недействителен)
- [ ] Выбрана конструкция авторизации, где клиент не держит серверный секрет
- [ ] Проверено, что **никакой** общий секрет не попадает в установщик — грепом по собранному
      артефакту, а не по исходникам. Положительный контроль обязателен: убедиться, что греп
      **находит** заведомо присутствующую строку, иначе «ничего не нашлось» ничего не доказывает
- [ ] Перепись остальных распространяемых приложений (`poster-microtext-desktop`,
      `label-printer-desktop`) на тот же класс: искали здесь, а не везде
...
### Internal-нога: Maddy здоров, причина осталась неустановленной

Сломалась 2026-08-02 около 07:40 UTC (596 × 15 минут) после 11 дней зелёного — настоящая смена
состояния, в отличие от external. Ошибка `IMAP-операция не завершилась за 105000мс (зависший
сокет)` при `clientError == null` — это подвес, а не отказ.

Проба с рабочей машины: connect к `mail.letar.best:993` — 608мс, полный проход
`fetch({seen:false})` по всем 1695 письмам — **426мс**. IMAP Maddy исправен.

**Две гипотезы проверены и обе опровергнуты — зафиксировано намеренно, чтобы их не выдвигали заново:**

- **«Накопился backlog непрочитанных, обход перестал укладываться в таймаут».** Красиво совпадала с
  тем, что нога прожила ровно 11 дней. Замер: 426мс против таймаута 90000мс. Совпадение формы
  графика с формой гипотезы её не подтверждает.
- **«DROP пакетов на маршруте контейнер s2 → mail:993».** Проба из контейнера (BlackCove):
  `nc -z` на 993 и 587 — оба `rc=0` мгновенно. При дропе соединение висело бы.

⚠️ При этом `nc -z` доказывает только приём TCP-соединения, а канарейка делает поверх него
TLS-handshake, IMAP-логин и `getMailboxLock('INBOX')` с ожиданием до 90 секунд. Подвес мог жить на
любом слое выше TCP, и эта проба его не исключает — тот же класс, что «`ufw status: active` без
фильтрации».

Живая версия, не проверенная на момент записи: подвес держался **состоянием самого процесса**
(зависшая IMAP-сессия или неотпущенный лок от прошлого прогона, из-за которого каждый следующий
ждал свои 90 секунд и упирался в жёсткий дедлайн 105). Если так — рестарт `dashboard-agent`
2026-08-08 ~12:04 UTC, сделанный по другому поводу, снял её побочно. Проверяется тем, сбросился ли
`internal.consecutiveFailures` в 0 после этого времени.

⚠️ Прямых логов за 22 июля и 2 августа больше нет: тот же рестарт обрезал `docker logs`
`dashboard-agent`. Диагностика по логам этого инцидента невозможна, дальше — только по состоянию и
БД.
...
- [ ] **Деплой `dashboard-agent` и проверка на живом прогоне** — счётчики обеих ног сбрасываются
      в 0, `lastFolder` заполнен, число непрочитанных в служебном ящике убывает
- [ ] Подтверждена причина подвеса internal-ноги (см. выше — возможно, снята рестартом 12:04 UTC)
- [ ] Разовая уборка накопленного: ~1695 писем во внутреннем ящике, ~1684 в спаме внешнего
- [ ] Правило в Gmail «от `canary@letar.best` → не отправлять в спам». Не обязательное: код теперь
      не слепнет от спама, но без правила сигнал `deliveredToSpam` будет гореть постоянно
...
## §63 — `animatrona-pin-queue` задокументирован в трёх местах, один из них с 2026-08-08 под вопросом 🆕 (2026-08-08)
...
### Открытый вопрос: жив ли standalone-путь (1)?

`infra/animatrona-pin-queue/README.md` описывает установку «рядом с pinner1»
(`mail.letar.best`) как отдельный самостоятельный деплой — не через build-контекст (2). Но §57
(владелец подтвердил 2026-08-07): **пиннер сейчас один, на s3** — pinner1 на mail-сервере не
существует. Из этого следует, что документированный standalone-путь либо никогда не
разворачивался отдельно, либо разворачивался исторически на pinner1 и умер вместе с ним при
переходе на единственный пиннер.

Отправлен запрос BlackCove (2026-08-08, тред `pin-queue-dedup-consolidation`): видел ли он
когда-либо на любом сервере контейнер `animatrona-pin-queue`, поднятый не из `/opt/pin-queue` и
не как часть pinner3-compose. Ответ определяет дальнейшее:

- **Если standalone никогда не использовался** — пометить `docker-compose.yml`/`setup.sh` в
  `animatrona-pin-queue/` как исторические (не удалять без отдельного подтверждения), README
  переписать без раздела «Установка (отдельный деплой)».
- **Если использовался** — оставить оба варианта, но свести дублирующиеся `environment:`-блоки к
  одному `.env`-шаблону, документированному в одном месте, вместо копий значений в двух YAML.

### Definition of Done

- [ ] Ответ BlackCove получен и записан здесь
- [ ] По итогу — либо пометка standalone-варианта как исторического, либо консолидация env-блоков
- [ ] `infra/animatrona-pinner` (pinner1, не существует физически с §57) — удалён из репозитория
      или явно помечен как история; это уже входит в DoD §57, здесь не дублируется отдельным пунктом
- [ ] `bootstrap-all.sh` (`animatrona-pinner3/`) приведён в соответствие с реальной топологией —
      тоже входит в DoD §57, здесь только напоминание о связи
...
## §64 — `docker-user-firewall.sh` (§49) версионирован в git — тот же класс проблемы, что §60 🆕 (2026-08-08)
...
### ⚠️ Открытый вопрос — содержимое реконструировано, не сверено с серверами

Файлы собраны **по документации** (`PLAN-INFRA.md` §49/§57, `.claude/docs/firewall.md`), а не
скопированы побайтово с живых серверов — эта сессия на серверы по SSH не ходит
(`.claude/rules/deploy-coordination.md`). Запрошена сверка у BlackCove через Agent Mail
(`project_key: c-web-letar`, `thread_id: firewall-versioning`, 2026-08-08) — читать реальные
`docker-user-firewall.sh`/`.service` на s2/s3/mail и внешний интерфейс mail-сервера (в §49
задокументированы только s2→`eth0`, s3→`ens3`). Ответ ещё не получен на момент записи.

**До подтверждения эти файлы — черновик, не источник истины.** Любая правка, которую нужно
задеплоить прямо сейчас, должна сначала свериться с реальным содержимым на сервере, а не
применяться из этого черновика вслепую.
...
- [ ] Ответ BlackCove получен — реальное содержимое трёх скриптов/юнитов сверено с
      `infra/firewall/` построчно, расхождения исправлены
- [ ] Внешний интерфейс mail-сервера подтверждён и вписан в `ports.mail.env`
- [ ] Пометка «черновик» в README снята после сверки
