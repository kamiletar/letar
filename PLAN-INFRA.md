...
- [ ] Занести все три в `DEFAULT_CRON_JOBS` с прод-значениями `schedule`/`enabled` (ждём полный
      JSON с сервера — переносить по догадке нельзя, особенно расписания)
...
- [ ] Замерено свободное место на mail-сервере (**полный `df -h`**, не только корень) — от него
      считается `max_size` кеша. И отдельно: на каком разделе лежит почта
- [ ] Кеширующий nginx на **mail-сервере**: файлы < 5 МБ через кеш, крупные мимо
- [ ] Бэкенд (`pin-queue`/Kubo на s3) адресуется по сети, а не через `127.0.0.1` — фронт и пиннер
