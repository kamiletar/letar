...
**🆕 Инцидент 2026-07-28 (archetest) продвигает archetest вне очереди батча M.** `CookieBanner`
рендерился вне `ChakraProvider` (сиблинг вместо потомка в `[locale]/layout.tsx`) — падал на КАЖДОЙ
странице сайта для реальных посетителей, деплой прошёл незамеченным (v0.25.0 → 0.25.4) потому что
верификация шла через HTTP `fetch` (статус-коды), а не через реальный браузерный рендер. Владелец
решил: **hard gate сразу** (не warn-only минимум неделю, как в DoD выше) — прогон e2e на staging
обязателен перед КАЖДЫМ `deploy_app` на прод для archetest, начиная немедленно, а не после
своей очереди в батче M3. Пилот на archetest (а не сразу на все 14) — по предложению BlackCove:
у части приложений e2e не настроен или флакует по причинам, не связанным с самим приложением,
раскатка без пилота дала бы ложные отказы деплоя сразу везде.

Технические требования BlackCove (реализация на его стороне — deploy-mcp/`deploy-affected.sh`):

- Подтвердить, что `archetest-e2e` гоняется против настоящего staging-инстанса, а не отдельной
...
Статус: решение принято, реализация не начата. Не путать с М3 в таблице выше — там archetest
ждала после M2 «специфика психометрии, снепшот» для ПОЛНОГО staging-гейта (анонимизация данных
и т.п.); этот трек — только hard e2e-gate перед деплоем, конкретный ответ на конкретный инцидент.
...
## §29 — Нет документации по e-commerce-паттернам, а магазинов уже три 🆕
...
- [ ] Выбран вариант
- [ ] Если пишется док — на него есть ссылка в разделе «Документация» корневого `CLAUDE.md`
- [ ] Skill `ecommerce-patterns` либо перепривязан к живому приложению, либо помечен устаревшим
...
- [ ] `docs/database.md` и `rules/database.md` не противоречат друг другу по `db:push`/миграциям
...
## §32 — Два форматтера с конфликтующими правилами: `nx format` против `dprint` 🆕
...
- [ ] Выбран единственный форматтер, второй конфиг удалён или приведён в соответствие
- [ ] `CLAUDE.md` называет ту же команду, что реально форматирует репозиторий
- [ ] `dprint check` (или аналог выбранного инструмента) зелёный по всему репозиторию
- [ ] Массовое переформатирование лежит отдельным коммитом, не смешанным с логикой

---

## §33 — SEO-фундамент: гейт индексации по домену не зашарен, у трети приложений нет `robots.ts` 🆕
...
- [ ] `libs/seo` существует, `isProductionDomain()` не продублирована ни в одном приложении
- [ ] Ни одно приложение не решает «прод или нет» через `NODE_ENV`
- [ ] У каждого приложения с публичными страницами есть `robots.ts`, `sitemap.ts`, `metadataBase`
- [ ] Закрытые панели отдают `Disallow: /` и `noindex`
- [ ] Staging-домены не индексируются — проверено запросом `/robots.txt` на staging
