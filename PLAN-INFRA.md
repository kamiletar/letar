...
- [ ] **`aboi`** — единственное приложение без применённых меток: деплой упал на миграции ещё до
      rollout'а. Сейчас живёт через NPM и **отвалится молча** при его остановке. Причина и фикс
      разобраны в плане самого приложения, задача у BlackCove
      (тред `aboi-staging-p3018-cartitem-quantity`). ⛔ **Сам файл миграции не править** — прод её
      уже применил, правка разъедет checksum в `_prisma_migrations`. Проблема только в
      staging-данных. От нас нужно: дождаться фикса → передеплоить → проверить маршрут на `:8443`.
...
2. Остановить NPM — два процесса на `443` не уживутся.
3. Три файла с номером порта, **все три**:
   | Где                                                  | Сейчас                        | Станет                           |
   | ---------------------------------------------------- | ----------------------------- | -------------------------------- |
   | `infra/traefik/docker-compose.yml` → `ports`         | `8090:80`, `8443:443/tcp+udp` | `80:80`, `443:443/tcp+udp`       |
   | `infra/traefik/traefik.yml` → `http3.advertisedPort` | `8443`                        | убрать                           |
   | `/usr/local/sbin/docker-user-firewall.sh` на s3      | `udp --dport 8443`            | `udp --dport 443` — **заменить** |
