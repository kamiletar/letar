...
- [ ] Маршруты для `ipfs`, `ipfsstor4`, `media` описаны в `infra/traefik/dynamic/` — file-провайдер
      Traefik умеет статические upstream'ы по адресу, docker-label'ы для этого не требуются
