...
- [ ] Задеплоить фикс и убедиться, что задачи `studio-*` и `driving-school-cleanup-api-logs`
      перешли в `success`
- [ ] Выяснить у логов агента, работала ли `driving-school-cleanup-api-logs` **когда-нибудь**
      (задача заведена ещё в initial commit — есть шанс, что она не отработала ни разу)
