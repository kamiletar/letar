...
- [ ] ⏳ Задеплоено на s3 — запрос у BlackCove (тред `infra-48-traefik-m2`). Порядок задан:
      сначала одно приложение (`time`), проверка роутера в логе Traefik и `curl` через `:8443`,
      и только потом остальные одиннадцать. Ошибка в шаблоне label'ов была бы одинаковой на всех
      двенадцати — нет смысла тиражировать её двенадцать раз, прежде чем убедиться на одном
...
>
> 1. **Сначала вернуть `ipfs.letar.best` на s3.** Сейчас он ошибочно указывает на
>    `mail.letar.best`, где ничего нет. Это чинит поломку из пункта ниже — сразу, без ожидания
>    прокси.
...
>
> **1. Возможно сломана раздача видео прямо сейчас** (лечится пунктом 1 выше). Владелец 2026-08-07
> сделал `ipfs.letar.best` CNAME на `mail.letar.best` под будущий кеш. **Прокси там ещё нет.** А
> этот домен — основной источник видео в плеере `svoichuzhie`
