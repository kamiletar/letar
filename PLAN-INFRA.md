...
### Internal-нога: Maddy здоров, вопрос к сетевому пути
...
сокет)` при `clientError == null` — это подвес, а не отказ: так выглядит DROP пакетов, а не REJECT.

Проба с рабочей машины: connect к `mail.letar.best:993` — 608мс, полный проход
`fetch({seen:false})` по всем 1695 письмам — **426мс**. IMAP Maddy исправен, значит подвес живёт на
маршруте **контейнер `dashboard-agent` на s2 → mail:993**. Подтверждение пробой из контейнера — за
BlackCove (тред `email-canary-diagnostics`).

⚠️ **Опровергнутая гипотеза, зафиксирована намеренно.** Напрашивалось объяснение «в ящике накопился
backlog непрочитанных, обход перестал укладываться в таймаут» — оно красиво совпадало с тем, что
нога прожила ровно 11 дней. Замер даёт 426мс против таймаута 90000мс — версия неверна, совпадение
формы графика с формой гипотезы её не подтверждает.
...
Не потому, что алерты уходили в Telegram и их не читали. **Алерт был отправлен один раз** — на
третьей неудаче, 22 июля, — и больше не повторялся никогда: флаг `alerted` взводится при первом
пересечении порога и сбрасывается только успехом, которого не было. Дальше канарейка молчала 17
дней, продолжая падать каждые 15 минут.

Хуже, что `alerted: true` вообще не означает «алерт доставлен» — только «вызов был сделан».
`postDashboardAlert` глотает любую ошибку в `console.error` и ничего не возвращает наверх. Дошёл ли
тот единственный алерт до dashboard и превратился ли в сообщение — по состоянию не проверяется
никак.

Это тот же класс, что §52: канал, который сообщает о проблеме один раз и не умеет отличить
«доставлено» от «провалилось», каналом оповещения не является.
