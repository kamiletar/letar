...
- [ ] Плановые файлы перестали ломаться форматтером (либо через `.prettierignore`, либо
      автоматически — как следствие отказа от Prettier)
...
**Что делать вместо этого.** Дублирование остаётся осознанным; в обоих файлах стоит перекрёстный
комментарий «при правке регулярок меняй оба файла». Способ снять риск дрейфа, не трогая резолв, —
guard-тест, сверяющий `source` регулярок между копиями (тот же приём, которым `infra-config` уже
сторожит локальную копию `server-config.ts` в `dashboard-agent`). Не сделан — отдельная задача.
