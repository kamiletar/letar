...
**Остаточное (не блокирует §32):** по `apps/*` та же дыра — таргет есть у **25 из 48**, у 23 его
нет. Состав: 16 e2e-наборов (из них 4 — submodule: `aboi-e2e`, `domwellbes-e2e`,
`driving-school-e2e`, `studio-e2e`) и 7 обычных приложений — `animatrona-mobile`,
`animatrona-tv`, `dashboard-agent`, `form-docs`, `form-example`, `kami-key-the`, `umami`.
Не чинилось: заход был по `libs/`. Для e2e-наборов сначала стоит решить, нужен ли им таргет
вообще, — у них своя специфика (`form-docs/.source` уже потребовал отдельного `ignores`,
см. [lib-entry-points.md](/.claude/docs/lib-entry-points.md)).
