...
- [ ] Уровень 3 — `ufw` (или `iptables INPUT`) для хостовых служб, SSH не потерян.
      ...
      Перед применением — снять актуальный список хостовых слушателей (`ss -tulnp` без
      `docker-proxy`) и **не забыть, что SSH к `s2.letar.best` идёт по IPv6**, а по IPv4 доступ
      только по явному адресу
...
- [ ] Завести алерт на **неуспешные** прогоны cron-задач: `CRON_FAILED` шлётся, но эти 401 никого
      не разбудили — выяснить, теряется алерт или приходит и остаётся непрочитанным.
      ⚠️ Это теперь **главный незакрытый пункт трека**: сам 401 починен, но механизм, который был
      обязан о нём сообщить, восемь дней подряд молчал — и это не проверено до сих пор
...
В ответе BlackCove по §52 фигурируют `studio-send-reminders` и `studio-recurring-invoices`.
Обоих **нет в `DEFAULT_CRON_JOBS`**, и `git log -S` по всей истории `cron.ts` не находит их
никогда. При этом `studio-close-stale-timers`, который в коде есть, в ответе не назван.

Создать задачу через API нельзя — такой ручки нет. Значит либо файл на сервере правился руками,
либо это неточность в самом ответе. Гадать не буду: нужен фактический список с прода.

### Что сделать

- [ ] Снять фактическое состояние: `GET /api/cron/jobs` и содержимое `cron-jobs.json` с s2 —
      сверить с `DEFAULT_CRON_JOBS` построчно
- [ ] Выяснить происхождение `studio-send-reminders` / `studio-recurring-invoices`: реальные
      задачи вне git или неточность в отчёте
