...
1. Выяснить, что реально записалось в `.last-e2e-status/time.json` — успех или провал. От этого
   зависит, задет ли гейт вообще (возможно, статус пишется до падения и тогда проблема только в
   выводе для человека).
2. Проверить, не тот ли это известный класс, что уже описан в комментариях
   `apps/dashboard-agent/src/routes/e2e.ts`: root-owned артефакты в `.nx/` от прогонов без
   переключения на пользователя `deploy`. Там `EACCES`, здесь `ENOENT` — семейство может быть то
   же (владелец/содержимое `.nx/cache/terminalOutputs`).
3. Развести источники истины: вердикт о тестах брать из отчёта Playwright (JSON-репортёр), а код
   возврата `nx` использовать только как признак «прогон вообще не состоялся». Сейчас это один
   сигнал на два разных события.

### Definition of Done

- [ ] Установлено содержимое `.last-e2e-status/time.json` после этого прогона
