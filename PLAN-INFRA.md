...
### 🆕 Смежная находка (не починено, только замечено 2026-08-09): тот же формат вывода `nx`, вероятно, ломает собственное affected-обнаружение `deploy-affected.sh`
...
Живым вызовом подтверждено (см. DoD выше): текущий `nx` (22.6) на этой машине печатает вывод
**JSON-массивом одной строкой** (`["app1","app2",...]`), не по имени на строку. Такой
`grep -E "^...$"` с якорями начала/конца строки никогда не совпадёт со строкой вида
`["@letar/source","dashboard-agent",...]` — значит `APP_AFFECTED` всегда пуст, и ветка
«приложение не менялось, пропускаю» срабатывает для КАЖДОГО приложения при каждом запуске
`./deploy-affected.sh` без `--app`.

⚠️ **Не проверено само по себе** (проверен только вызов из `deploy-mcp`, не bash-путь в
`deploy-affected.sh`) — возможно другая версия `nx` на конкретном сервере даёт другой формат,
возможно `2>/dev/null` или окружение сервера как-то влияет. Но раз симптом воспроизводится
детерминированно на этой машине с тем же `nx`, стоит перепроверить и на s2/s3 до следующего
использования bare-режима (`./deploy-affected.sh` без `--app`). ⚠️ **Практический эффект пока
неясен**: по `.claude/rules/deployment.md` штатный путь деплоя — `deploy_app` через deploy-mcp
(всегда с явным `--app`), а bare-режим — резервный. Если он и правда сейчас всегда «нечего
деплоить», это тихий регресс резервного канала, а не прод-инцидент — но и не проверено.

- [ ] Проверить на живом сервере (s2/s3): `./deploy-affected.sh` без `--app` реально находит
      affected-приложения, или всегда молчит «Nothing to deploy»
- [ ] Если баг подтверждён — тот же фикс, что в `isAffectedSince` (`config.ts`): парсить
      `nx show projects --affected ...` как JSON, не построчным `grep`
