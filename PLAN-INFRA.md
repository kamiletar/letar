...
## §51 — Hard e2e-gate сверяет HEAD всего монорепо, а не то, от чего зависит приложение 🟡 КОД ГОТОВ, НЕ ПРОВЕРЕНО ЖИВЫМ ПРОГОНОМ (2026-08-07, код 2026-08-09)
...
- [ ] ⚠️ **Не проверено на живом сценарии.** Юнит-тесты покрывают логику `evaluateE2eGate` на
      инжектированных моках, но НЕ реальный `git diff`/`nx show projects --affected` на
      настоящем репозитории — тот путь (`isAffectedSince` без моков) не имеет отдельного теста
      и не запускался вживую ни разу. Перед тем как доверять этому в проде: (1) коммит в
      `.claude/docs/*` не должен сбрасывать гейт гейтованного приложения; (2) коммит в
      `libs/forms` должен сбрасывать гейт у зависящих от него гейтованных приложений;
      (3) коммит в `deploy-affected.sh` должен сбрасывать гейт всем.
- [ ] Пока живой сценарий не пройден — знать про обходной путь: запускать e2e и прод-деплой
      подряд без зазора, либо просить остальных агентов не коммитить в окно деплоя (плохо
      масштабируется, поэтому и заведён этот пункт).
...
### 2. ✅ ПОЧИНЕНО (2026-08-09, код): `deploy-affected.sh` не знал про `docker-compose.s3.yml`
...
- [ ] ⚠️ **Не проверено живым прогоном** — `bash -n` (синтаксис) чист, но реального деплоя
      `--app dashboard-agent` на s3 после правки не было. Перед тем как доверять штатному
      деплою на s3 — прогнать вручную (или через deploy-mcp) и свериться, что контейнер
      поднимается на порту из `docker-compose.s3.yml`, а не продового.
