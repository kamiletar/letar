...
- [ ] `libs/jobs`: `defineJob`, синхронизатор расписаний, наложение `JobOverride`, тесты
- [ ] `JobOverride` + `/api/jobs/status` как конвенция (шаблон zmodel + роут)
- [ ] Компонент админки задач в `libs/admin-ui`
- [ ] Пилот на приложении с наибольшим числом задач (6 задач, коммерческое) — миграция и снятие
      его задач с агента
