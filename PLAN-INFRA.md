...
**Остаточный техдолг (не блокирует §32):** `targets.format` объявлен только у 9 библиотек из 45
и у 13 приложений. У остальных проектов таргета нет, значит `nx run-many -t format` их не
трогает вовсе — форматирование там держится на PostToolUse-хуке `.claude/hooks/auto-format.js` и
на прогонах `dprint fmt` из корня. Это не расхождение стилей (файлы зелёные), но и не гейт.
